...
use buoyant_kernel::resource::{
    JsonResourceLimits, NativeResourceScope, NativeResourceThreadGuard, NativeResourceThreadPolicy,
};
use std::sync::Arc;
    ...
    Stop: Fn() + Send + Sync + 'static,
{
    profile()
        .build(
            native_tokio_resource::NativeTokioAdmission::try_new(scope).unwrap(),
            start,
            stop,
        )
        .unwrap()
...
pub fn kernel_runtime(scope: Arc<NativeResourceScope>, limits: JsonResourceLimits) -> Runtime {
    ...
    runtime(
        scope,
        move || KERNEL.with(|slot| *slot.borrow_mut() = Some(policy.enter_thread())),
        || {
            KERNEL.with(|slot| {
