...
use codefabric::gate_b_candidate::{
    generate_functional_candidate_bundle, verify_candidate_bundle,
    verify_functional_candidate_bundle, write_candidate_bundle,
};
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match arguments.next().as_deref() {
        Some("emit-functional") => {
            ...
            let bundle = generate_functional_candidate_bundle(&repository_root, &scratch_root)?;
            write_candidate_bundle(&repository_root, &output, &bundle)?;
            verify_functional_candidate_bundle(&repository_root.join(&output))?;
            println!(
                "Functional Gate B review candidate emitted at {}",
                output.display()
            );
        }
        Some("verify") => {
        ...
        Some("verify-functional") => {
            let candidate_root = path(arguments.next());
            if arguments.next().is_some() {
                usage();
            }
            verify_functional_candidate_bundle(Path::new(&candidate_root))?;
            println!("Functional Gate B review candidate digest chain is valid");
        }
        Some("accept") => {
            let repository_root = repository_root(arguments.next())?;
