...
root-test-rust: extractor-identity
    ...
    CODEFABRIC_PYREFLY_SIDECAR_BIN="$CF_ROOT/target/debug/codefabric-pyrefly-sidecar" CODEFABRIC_RUSTC_EXTRACTOR_BIN="$CF_ROOT/target/extractor/debug/codefabric-rustc-extractor" cargo nextest run --locked
...
rust-provider-test: extractor-identity
    cargo build --locked --manifest-path pyrefly-sidecar/Cargo.toml --bin codefabric-pyrefly-sidecar
    CODEFABRIC_RUSTC_EXTRACTOR_BIN="$CF_ROOT/target/extractor/debug/codefabric-rustc-extractor" ./scripts/cargo-check-mode.sh cargo nextest run --locked -E 'test(contained_cargo_extracts_real_selected_rust_call) | test(pragmatic_rust_semantics_publish_real_call_targets)' --no-tests=fail
...
[doc("Prove plan-derived schemas, catalog observations, child views, and shared logical reuse")]
[group('test')]
...
datafusion-cache-resource-operations-check:
    cargo nextest run --locked --lib -E 'test(/(datafusion_stream_cache_resource_operations|object_list_ttl_is_a_refresh_bound_not_validity_or_authority|actual_transformation_reads_enforce_row_and_memory_bounds_without_preexecution|relational_program_executes_only_through_authorized_child_inputs|logical_plan_cache_bypasses_oversized_entries_without_changing_semantics)/)' --no-tests=fail

[doc("Prove Delta history creation, transaction identity, exact rows, and activation readback integrity")]
[group('test')]
...
[doc("Reconstruct exact semantic, CDF, and checkpoint state from pinned Delta versions")]
[group('test')]
delta-exact-reconstruction-v3-check:
    cargo nextest run --locked --lib -E 'test(/(snapshot_recipe_reconstructs_the_exact_pin_without_a_version_selector|exact_provider_read_retains_full_stats_and_marks_missing_values_unknown|exact_cdf_range_has_an_inclusive_end_and_excludes_a_newer_head|reconstructed_checkpoint_restarts_at_the_next_exact_version)/)' --no-tests=fail
