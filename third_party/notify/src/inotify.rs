...
impl INotifyWatcher {
    fn from_event_handler(
        ...
        let worker = Some(event_loop.run()?);
        Ok(INotifyWatcher {
            channel,
            waker,
            worker,
...
impl Drop for INotifyWatcher {
    fn drop(&mut self) {
        // The event loop may already have failed. Always retain ownership through the join;
        // a closed channel must not cause a second panic that abandons the thread handle.
        let _ = self.channel.send(EventLoopMsg::Shutdown);
        let _ = self.waker.wake();
        if self.join_on_drop {
            if let Some(worker) = self.worker.take() {
                if worker.join().is_err() {
                    log::error!("notify inotify worker panicked before its owned join");
                }
            }
        }
    }
