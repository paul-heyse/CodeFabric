...
impl Config {
    /// Join the polling or Linux inotify backend thread when its watcher is dropped.
    ///
    /// CodeFabric extension: defaults to false, preserving upstream nonblocking Drop.
    /// Construction-only. Drop must run on an owning thread, never its own event callback.
    /// A running scan or callback must finish before the join returns. Other backends ignore it.
    pub fn with_join_on_drop(mut self, join: bool) -> Self {
        self.join_on_drop = join;
        self
    }

    /// Whether construction selected joined polling/Linux backend cleanup.
    pub fn join_on_drop(&self) -> bool {
        self.join_on_drop
    }
