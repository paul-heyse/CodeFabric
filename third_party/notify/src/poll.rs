...
impl PollWatcher {
    ...
    fn with_opt<F: EventHandler, G: ScanEventHandler>(
        ...
        poll_watcher.worker = Some(poll_watcher.run(rx)?);

        Ok(poll_watcher)
    }

    fn run(&self, rx: Receiver<()>) -> crate::Result<thread::JoinHandle<()>> {
...
impl Drop for PollWatcher {
    fn drop(&mut self) {
        self.want_to_stop.store(true, Ordering::Release);
        // Wake both timed and manual polling. A stop flag alone leaves manual polling asleep.
        let _ = self.message_channel.send(());
        if self.join_on_drop {
            if let Some(worker) = self.worker.take() {
                if worker.join().is_err() {
                    log::error!("notify polling worker panicked before its owned join");
                }
            }
        }
    }
