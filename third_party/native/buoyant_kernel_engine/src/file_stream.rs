...
use delta_kernel::engine::arrow_data::NativeOwnedRecordBatch;
use delta_kernel::{DeltaResult, OwnedFileMeta, OwnedFileMetas};
use futures::FutureExt;
use futures::future::BoxFuture;
use futures::stream::{BoxStream, Stream, StreamExt};

/// A fallible future that resolves to a stream of [`NativeOwnedRecordBatch`]
/// cbindgen:ignore
pub type FileOpenFuture =
    BoxFuture<'static, DeltaResult<BoxStream<'static, DeltaResult<NativeOwnedRecordBatch>>>>;

/// Generic API for opening a file using an [`ObjectStore`] and resolving to a
/// stream of [`NativeOwnedRecordBatch`]
///
/// [`ObjectStore`]: delta_kernel::object_store::ObjectStore
pub trait FileOpener: Send + Unpin {
    /// Asynchronously open the specified file and return a stream
    /// of [`NativeOwnedRecordBatch`]
    fn open(&self, file_meta: OwnedFileMeta, range: Option<Range<i64>>) -> DeltaResult<FileOpenFuture>;
}
...
enum NextOpen {
    ...
    Ready(DeltaResult<BoxStream<'static, DeltaResult<NativeOwnedRecordBatch>>>),
}

enum FileStreamState {
    /// The idle state, no file is currently being read
    Idle,
    /// Currently performing asynchronous IO to obtain a stream of NativeOwnedRecordBatch
    ...
    Scan {
        /// The reader instance
        reader: BoxStream<'static, DeltaResult<NativeOwnedRecordBatch>>,
...
pub struct FileStream {
    ...
    file_iter: delta_kernel::path::resource::OwnedFileMetaIterator,
...
impl FileStream {
    ...
    fn start_next_file(&mut self) -> Option<DeltaResult<FileOpenFuture>> {
        let file_meta = self.file_iter.next()?;
    ...
    fn poll_inner(
        &mut self,
        cx: &mut Context<'_>,
    ) -> Poll<Option<DeltaResult<NativeOwnedRecordBatch>>> {
        loop {
...
impl Stream for FileStream {
    type Item = DeltaResult<NativeOwnedRecordBatch>;
