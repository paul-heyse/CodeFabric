...
use delta_kernel::{DeltaResult, Error, FileMeta, OwnedFileMeta, FileSlice, StorageHandler};
use delta_kernel::resource::{AllocationRequest, NativeResourceScope};
...
pub struct ObjectStoreStorageHandler<E: TaskExecutor> {
    ...
    task_executor: Arc<E>,
    readahead: usize,
    pub(crate) json_resource_limits: Option<delta_kernel::resource::JsonResourceLimits>,
    pub(crate) resource_scope: Option<Arc<NativeResourceScope>>,
}
...
impl<E: TaskExecutor> ObjectStoreStorageHandler<E> {
    pub fn new(store: Arc<DynObjectStore>, task_executor: Arc<E>) -> Self {
        Self {
            ...
            task_executor,
            readahead: 10,
            json_resource_limits: None,
            resource_scope: None,
        }
    }

    /// Configure finite native JSON ingress geometry before parser construction.
    pub fn with_json_resource_limits(mut self, limits: delta_kernel::resource::JsonResourceLimits) -> DeltaResult<Self> {
        limits.validate()?;
        self.json_resource_limits = Some(limits);
        Ok(self)
    }
...
async fn list_from_impl(
...
) -> DeltaResult<BoxStream<'static, DeltaResult<OwnedFileMeta>>> {
    ...
    // original URL.
    super::resource::admit_listing_prefix(&path, scope.as_ref())?;
    ...
    let item_scope = scope.clone();
    let stream = store
        .list_with_offset(Some(&prefix), &offset)
        .map(move |meta| {
            let meta = meta?;
            OwnedFileMeta::try_from_object_path_admitted(&path, meta.location.as_ref(),
                meta.last_modified.timestamp_millis(), meta.size, item_scope.clone())
        });

    if !has_ordered_listing {
        // Local filesystem doesn't return sorted list - need to collect and sort
        let mut stream = super::owned_box_stream(stream, scope.clone())?;
        let mut items: Vec<OwnedFileMeta> = Vec::new();
        while let Some(item) = stream.next().await {
            let item = item?;
            if items.len() == items.capacity() {
                let capacity = items.capacity().checked_mul(2).unwrap_or(usize::MAX).max(4);
                let bytes = capacity.checked_mul(std::mem::size_of::<OwnedFileMeta>())
                    .filter(|bytes| *bytes <= isize::MAX as usize)
                    .ok_or(delta_kernel::resource::ResourceExhausted { kind: "native_listing_descriptors", requested: usize::MAX, limit: isize::MAX as usize })?;
                if let Some(scope) = &scope { scope.reserve(AllocationRequest { kind: "native_listing_descriptors", bytes })?; }
                items.reserve_exact(capacity - items.len());
            }
            items.push(item);
        }
        items.sort_unstable();
        // The iterator retains the original Vec charge even after its final
        // value is yielded; each yielded item independently owns its URL charge.
        super::owned_box_stream(stream::iter(items.into_iter().map(Ok)), scope)
    } else {
...
impl<E: TaskExecutor> StorageHandler for ObjectStoreStorageHandler<E> {
    fn json_resource_limits(&self) -> Option<delta_kernel::resource::JsonResourceLimits> {
        self.json_resource_limits
            .or_else(delta_kernel::resource::current_json_resource_limits)
    }

    fn list_from(
        &self,
        path: &Url,
    ) -> DeltaResult<Box<dyn Iterator<Item = DeltaResult<OwnedFileMeta>>>> {
        let current = delta_kernel::resource::current_resource_scope();
        if let (Some(current), Some(explicit)) = (&current, &self.resource_scope) {
            if !Arc::ptr_eq(current, explicit) {
                return Err(delta_kernel::resource::ResourceExhausted { kind: "native_foreign_scope", requested: 1, limit: 0 }.into());
            }
        }
        let scope = current.or_else(|| self.resource_scope.clone());
        if let Some(scope) = &scope {
            scope.reserve(AllocationRequest { kind: "native_listing_base_url", bytes: path.as_str().len() })?;
        }
        let future = list_from_impl(self.inner.clone(), path.clone(), scope);
    ...
    fn put(&self, path: &Url, data: Bytes, overwrite: bool) -> DeltaResult<()> {
        ...
        self.task_executor
            .try_block_on(put_impl(self.inner.clone(), path, data, overwrite))?
    }
    ...
    fn delete(&self, path: &Url) -> DeltaResult<()> {
        let path = Path::from_url_path(path.path())?;
        self.task_executor
            .try_block_on(delete_impl(self.inner.clone(), path))?
    }
}
...
mod tests {
    ...
    async fn test_file_meta_is_correct() {
        ...
        let engine = DefaultEngineBuilder::new(store).build().unwrap();
