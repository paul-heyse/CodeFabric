...
pub trait TaskExecutor: Send + Sync + 'static {
    ...
    /// Governed execution admits native channel/task layouts before enqueue.
    /// Unknown executor implementations must supply their own allocation proof.
    fn try_block_on<T>(&self, task: T) -> DeltaResult<T::Output>
    where T: Future + Send + 'static, T::Output: Send + 'static,
    {
        if delta_kernel::resource::current_resource_scope().is_some() {
            return Err(delta_kernel::resource::ResourceExhausted { kind: "native_executor_admission_unavailable", requested: 1, limit: 0 }.into());
        }
        Ok(self.block_on(task))
    }

    /// Validate that governed work stays on the caller-owned runtime.
    /// Implementors outside the selected Tokio seam must supply their own proof.
    fn validate_current_runtime(&self) -> DeltaResult<()> {
        if delta_kernel::resource::current_resource_scope().is_some() {
            return Err(delta_kernel::resource::ResourceExhausted { kind: "native_executor_admission_unavailable", requested: 1, limit: 0 }.into());
        }
        Ok(())
...
pub mod tokio {
    ...
    fn admitted_block_on<T>(task: T, spawn: impl FnOnce(BoxFuture<'static, ()>)) -> DeltaResult<T::Output>
    where T: Future + Send + 'static, T::Output: Send + 'static,
    {
        let scope = delta_kernel::resource::current_resource_scope();
        crate::resource::admit_executor_channel::<DeltaResult<T::Output>>(scope.as_ref())?;
        crate::resource::admit_executor_send::<T::Output>(scope.as_ref())?;
        let (sender, receiver) = channel::<DeltaResult<T::Output>>();
        let task_scope = scope.clone();
        let future = async move {
            if let Some(expected) = &task_scope {
                let current = delta_kernel::resource::current_resource_scope();
                if current.as_ref().is_none_or(|current| !std::sync::Arc::ptr_eq(current, expected)) {
                    let error: Error = delta_kernel::resource::ResourceExhausted { kind: "native_executor_worker_scope", requested: 1, limit: 0 }.into();
                    expected.record_failure(delta_kernel::resource::ResourceExhausted { kind: "native_executor_worker_scope", requested: 1, limit: 0 });
                    sender.send(Err(error)).ok();
                    return;
                }
                if let Err(error) = expected.check_available() {
                    sender.send(Err(error)).ok();
                    return;
                }
            }
            ...
            if let Some(scope) = &task_scope {
                if let Err(error) = scope.check_available() {
                    sender.send(Err(error)).ok();
                    return;
                }
            }
            // Required runtime admission latches pressure in the original
            // scope before dropping a denied relay closure. Receiver maps that
            // exact failure instead of turning the inline JoinError into panic.
            let _ = tokio::task::spawn_blocking(move || { sender.send(Ok(output)).ok(); }).await;
        };
        crate::resource::admit_executor_relay(&future, scope.as_ref())?;
        spawn(Box::pin(future));
        let recv = || match receiver.recv() {
            Ok(value) => value,
            Err(_) => {
                if let Some(error)=scope.as_ref().and_then(|scope|scope.failure()) {return Err(error.into());}
    ...
    impl TokioBackgroundExecutor {
        pub fn new() -> Self {
            Self::try_new().unwrap_or_else(|_| panic!("governed background executor requires an explicitly owned runtime"))
        }

        /// Compatibility background runtime creation. A required scope cannot
        /// create workers without the owner's complete thread policies and join.
    ...
    impl TaskExecutor for TokioBackgroundExecutor {
        ...
        where T: Future + Send + 'static, T::Output: Send + 'static,
        {
            self.validate_current_runtime()?;
            admitted_block_on(task, |future| self.send_future(future))
        }
    ...
    impl TokioMultiThreadExecutor {
        ...
        /// Construct only the executor descriptor, borrowing the current owned
        /// runtime. The caller owns runtime construction, policies and shutdown.
        pub fn try_new_current(handle: tokio::runtime::Handle) -> DeltaResult<Self> {
            if handle.runtime_flavor() != RuntimeFlavor::MultiThread {
                return Err(delta_kernel::resource::ResourceExhausted { kind: "native_executor_multithread_required", requested: 1, limit: 0 }.into());
            }
            let value = Self { handle, _runtime: None };
            value.validate_current_runtime()?;
            Ok(value)
        }

        /// Admit the original Arc descriptor before allocation.
        pub fn try_new_current_shared(handle: tokio::runtime::Handle) -> DeltaResult<std::sync::Arc<Self>> {
            let value = Self::try_new_current(handle)?;
            crate::resource::admit_arc::<Self>("native_executor_descriptor", delta_kernel::resource::current_resource_scope().as_ref())?;
        ...
        pub fn new_owned_runtime(
            ...
            if delta_kernel::resource::current_resource_scope().is_some() {
                return Err(delta_kernel::resource::ResourceExhausted { kind: "native_executor_runtime_unowned", requested: 1, limit: 0 }.into());
            }
    ...
    impl TaskExecutor for TokioMultiThreadExecutor {
        fn validate_current_runtime(&self) -> DeltaResult<()> {
            if let Some(scope) = delta_kernel::resource::current_resource_scope() {
                ...
                if self._runtime.is_some() || Handle::try_current().ok().is_none_or(|current| current.id() != self.handle.id()) {
                    return Err(delta_kernel::resource::ResourceExhausted { kind: "native_executor_foreign_runtime", requested: 1, limit: 0 }.into());
                }
            }
            Ok(())
        }
        ...
        {
            self.validate_current_runtime()?;
            admitted_block_on(task, |future| { self.handle.spawn(future); })
        }
