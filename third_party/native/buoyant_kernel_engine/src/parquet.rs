...
use delta_kernel::engine::arrow_data::{ArrowEngineData, NativeDataOwners, NativeOwnedRecordBatch};
use delta_kernel::engine::arrow_utils::{
    RowIndexBuilder, fixup_parquet_read, generate_mask, get_requested_indices,
...
use delta_kernel::parquet::resource::OwnedWriteBuffer;
...
pub struct DefaultParquetHandler<E: TaskExecutor> {
    ...
    // Original descriptor allocation stays owned if a handler Arc escapes its engine.
...
impl DataFileMetadata {
    ...
    pub(crate) fn as_record_batch(
        ...
        Ok(Box::new(
            ...
            .try_inherit_owners(self.native_owners.clone())?,
        ))
    }
}
...
impl<E: TaskExecutor> DefaultParquetHandler<E> {
    pub fn new(store: Arc<DynObjectStore>, task_executor: Arc<E>) -> Self {
        Self {
            ...
            resource_scope: delta_kernel::resource::current_resource_scope(),
        }
    }
    ...
    async fn write_parquet(
        ...
        let mut buffer = OwnedWriteBuffer::new();
        ...
        self.store
            .put(&Path::from_url_path(path.path())?, buffer.into_bytes()?.into())
            .await?;
        ...
        let mut metadata = DataFileMetadata::new(file_meta, stats);
        metadata.native_owners = metadata
            .native_owners
            .try_merge(batch.native_owners().clone())?;
        Ok(metadata)
    }
...
async fn read_parquet_files_impl(
    store: Arc<DynObjectStore>,
    files: delta_kernel::OwnedFileMetas,
    physical_schema: SchemaRef,
    predicate: Option<PredicateRef>,
    buffer_size: usize,
    batch_size: usize,
) -> DeltaResult<BoxStream<'static, DeltaResult<Box<dyn EngineData>>>> {
    let scope = files.resource_scope().cloned();
    let total_files = files.len();
    if files.is_empty() {
        return super::owned_box_stream(stream::empty(), scope);
    }
    ...
    if files[0].location.is_presigned() {
        ...
        let stream = FileStream::new(files, arrow_schema, file_opener)?.map_ok(
            |record_batch| -> Box<dyn EngineData> { Box::new(record_batch.into_engine_data()) },
        );
        return super::owned_box_stream(stream, scope);
    }
    ...
    let result_stream = super::resource::owned_buffered(
        stream::iter(file_futures), buffer_size, total_files, scope.as_ref(),
    )?.try_flatten()
        .map_ok(|record_batch| -> Box<dyn EngineData> {
            Box::new(record_batch.into_engine_data())
        });

    super::owned_box_stream(result_stream, scope)
}
...
impl<E: TaskExecutor> ParquetHandler for DefaultParquetHandler<E> {
    fn read_parquet_files(
        ...
        let future = read_parquet_files_impl(
            self.store.clone(),
            delta_kernel::OwnedFileMetas::try_copy(files)?,
            physical_schema,
            predicate,
    ...
    fn write_parquet_file(
        ...
        let first_record_batch = first_arrow.into_owned_record_batch();
        // The writer retains the first schema through finish, beyond its first write.
        let _schema_owners = first_record_batch.owners().clone();

        let object_writer = ParquetObjectWriter::try_new(store, path)?;
        let schema = first_record_batch.record_batch().schema();
        ...
        let w = writer.clone();
        self.task_executor.try_block_on(async move {
            // Write the first batch
            let mut writer = w.lock().await;
            writer.write(first_record_batch.record_batch()).await
        })??;
        ...
        for result in data {
            ...
            let batch = arrow_data.into_owned_record_batch();
            let w = writer.clone();
            self.task_executor.try_block_on(async move {
                let mut writer = w.lock().await;
    ...
    fn read_parquet_footer(&self, file: &FileMeta) -> DeltaResult<ParquetFooter> {
        let store = self.store.clone();
        let file = delta_kernel::OwnedFileMeta::try_copy_url(&file.location, file.last_modified, file.size)?;

        self.task_executor.try_block_on(async move {
            let metadata = if file.location.is_presigned() {
                let client = reqwest::Client::new();
                let response =
                    client.get(file.location.as_str()).send().await.map_err(|e| {
                ...
                super::resource::admit_object_path(file.location.path(), file.resource_scope())?;
...
async fn open_parquet_file(
    ...
    limit: Option<usize>,
    batch_size: usize,
    file_meta: delta_kernel::OwnedFileMeta,
) -> DeltaResult<BoxStream<'static, DeltaResult<NativeOwnedRecordBatch>>> {
    super::resource::admit_object_path(file_meta.location.path(), file_meta.resource_scope())?;
    ...
    let scope = file_meta.resource_scope().cloned();
    let stream = stream.map(move |rbr| {
        fixup_parquet_read(
            rbr?,
            &requested_ordering,
            row_indexes.as_mut(),
            Some(file_meta.location.as_str()),
            Some(&table_schema),
        )
        .map(ArrowEngineData::into_owned_record_batch)
    });
    super::owned_box_stream(stream, scope)
}
...
impl FileOpener for PresignedUrlOpener {
    fn open(&self, file_meta: delta_kernel::OwnedFileMeta, _range: Option<Range<i64>>) -> DeltaResult<FileOpenFuture> {
        ...
        Ok(Box::pin(async move {
            ...
            let reader = client.get(file_meta.location.as_str()).send().await?.bytes().await?;
...
mod tests {
    ...
    use delta_kernel::engine::arrow_data::{
        ArrowEngineData, NativeDataOwners, NativeOwnedRecordBatch,
    };
