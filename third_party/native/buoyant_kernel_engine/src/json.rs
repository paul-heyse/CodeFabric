...
use delta_kernel::engine::arrow_data::{ArrowEngineData, NativeOwnedRecordBatch};
...
use delta_kernel::{
    ...
    JsonHandler, PredicateRef, OwnedFileMetas,
};
use futures::stream::{self, BoxStream};
use futures::{StreamExt, TryStreamExt, ready};
use url::Url;
...
async fn read_json_files_impl(
    ...
    let file_futures = files.into_iter().map(move |file| {
        ...
        async move {
            ...
            let scope = file.resource_scope().cloned();
            let tagged = batch_stream.map(move |result| {
                    result?.try_map_engine(|batch| {
                        fixup_json_read(batch, &reorder_indices, file.location.as_str())
                    })
                });
            super::owned_box_stream(tagged, scope)
        }
    });
...
impl<E: TaskExecutor> JsonHandler for DefaultJsonHandler<E> {
    ...
    fn write_json_file(
        ...
        self.task_executor.try_block_on(write_json_file_impl(
            self.store.clone(),
            path.clone(),
            to_json_bytes(data)?,
            overwrite,
...
async fn open_json_file(
    ...
    match result.payload {
        GetResultPayload::File(file, _) => {
            ...
            let reader = futures::stream::iter(reader).map(|batch| {
                batch
                    .map(|batch| ArrowEngineData::new(batch).into_owned_record_batch())
                    .map_err(Error::from)
            });
            ...
            let s = futures::stream::poll_fn(move |cx| {
                ...
                Poll::Ready(
                    decoder
                        .flush()
                        .map(|batch| {
                            batch.map(|batch| ArrowEngineData::new(batch).into_owned_record_batch())
                        })
                        .map_err(Error::from)
                        .transpose(),
                )
...
mod tests {
    ...
    fn test_read_invalid_json() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let default_engine = DefaultEngineBuilder::new(Arc::new(LocalFileSystem::new())).build().unwrap();
