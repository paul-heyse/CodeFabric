...
pub(crate) trait Wake: Send + Sync + Sized + 'static {
    ...
    /// Release one original Arc. Resource-owning native nodes can preserve their
    /// external allocation receipt through the actual Arc deallocation.
    fn drop_arc(arc_self: Arc<Self>) {
        drop(arc_self);
    }
}
...
unsafe fn drop_arc_raw<T: Wake>(data: *const ()) {
    // Safety: `data` was created from an `Arc::as_ptr` in function `waker_ref`.
    T::drop_arc(unsafe { Arc::<T>::from_raw(data.cast()) });
}
