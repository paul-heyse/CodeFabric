...
struct ListEntry<T> {
    ...
    _pin: PhantomPinned,
    #[cfg(all(feature = "rt-multi-thread", feature = "time"))]
    resource: Option<crate::runtime::resource::ResourceAnchor>,
}

/// Inline wrapper around the unchanged original node Arc. The receipt is cloned
/// before releasing a reference, so the last Arc allocation is freed before the
/// receipt even if the payload destructor unwinds. No Weak node handles escape.
struct EntryArc<T>(ManuallyDrop<Arc<ListEntry<T>>>);
impl<T> Clone for EntryArc<T> {
    fn clone(&self) -> Self {
        Self(ManuallyDrop::new(Arc::clone(&self.0)))
    }
}
impl<T> std::ops::Deref for EntryArc<T> {
    type Target = Arc<ListEntry<T>>;
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl<T> Drop for EntryArc<T> {
    fn drop(&mut self) {
        #[cfg(all(feature = "rt-multi-thread", feature = "time"))]
        let _receipt = self.0.resource.clone();
        // SAFETY: this wrapper exclusively owns this one Arc reference and drops it once.
        unsafe {
...
impl<T> IdleNotifiedSet<T> {
    ...
    pub(crate) fn new() -> Self {
        ...
        #[cfg(all(feature = "rt-multi-thread", feature = "time"))]
        let resource = crate::runtime::resource::current()
            .as_ref()
    ...
    pub(crate) fn insert_idle(&mut self, value: T) -> EntryInOneOfTheLists<'_, T> {
        ...
        let entry = EntryArc(ManuallyDrop::new(Arc::new(ListEntry {
...
impl<T: 'static> Wake for ListEntry<T> {
    ...
    fn drop_arc(me: Arc<Self>) {
        #[cfg(all(feature = "rt-multi-thread", feature = "time"))]
        let _receipt = me.resource.clone();
        drop(me);
    }
}
...
unsafe impl<T> linked_list::Link for ListEntry<T> {
    ...
    unsafe fn from_raw(ptr: NonNull<ListEntry<T>>) -> EntryArc<T> {
        EntryArc(ManuallyDrop::new(unsafe { Arc::from_raw(ptr.as_ptr()) }))
    }
...
#[cfg(all(feature = "rt-multi-thread", feature = "time"))]
pub(crate) fn resource_new_bytes<T>() -> Result<usize, crate::runtime::resource::ResourceLayoutError>
{
    crate::runtime::resource::arc_bytes::<Lists<T>>()
}
#[cfg(all(feature = "rt-multi-thread", feature = "time"))]
pub(crate) fn resource_entry_bytes<T>(
) -> Result<usize, crate::runtime::resource::ResourceLayoutError> {
    crate::runtime::resource::arc_bytes::<ListEntry<T>>()
}

#[cfg(all(test, not(loom), feature = "rt-multi-thread", feature = "time"))]
pub(crate) fn resource_entry_data_offset<T>() -> usize {
    std::alloc::Layout::new::<[std::sync::atomic::AtomicUsize; 2]>()
        .extend(std::alloc::Layout::new::<ListEntry<T>>())
        .unwrap()
        .1
}
