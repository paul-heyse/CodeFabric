...
impl Runtime {
        ...
        F::Output: Send + 'static,
    {
        ...
        if super::resource::is_owned(&self.handle) {
            return super::resource::spawn_async(&self.handle, future);
        }
    ...
    /// Runs a future after admitting Tokio's optional original future box.
    /// Governed callers use this fallible entry instead of `block_on`.
    #[cfg(all(feature = "rt-multi-thread", feature = "time"))]
    pub fn try_block_on<F: Future>(
        &self,
        future: F,
    ) -> Result<F::Output, super::resource::ResourceLayoutError> {
        let _receipt = super::resource::admit_block_on::<F>(&self.handle)?;
    ...
    pub fn shutdown_timeout(mut self, duration: Duration) {
        ...
        // Required local ownership never permits timeout to detach a live native
        // worker. Its profile explicitly requires the complete joined barrier.
        #[cfg(all(feature = "rt-multi-thread", feature = "time"))]
        if crate::runtime::resource::is_owned(&self.handle) {
            self.blocking_pool.shutdown(None);
            return;
        }
