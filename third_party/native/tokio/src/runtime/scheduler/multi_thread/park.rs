...
#[cfg(feature = "time")]
pub(super) fn resource_heap_bytes(workers: usize) -> Result<usize, crate::runtime::resource::ResourceLayoutError> {
    use crate::runtime::resource::{add, arc_bytes, mul};
    // The initial Parker plus one fresh Inner allocated by each Parker::clone.
    add(arc_bytes::<Shared>()?, mul(add(workers, 1)?, arc_bytes::<Inner>()?)?)
}
