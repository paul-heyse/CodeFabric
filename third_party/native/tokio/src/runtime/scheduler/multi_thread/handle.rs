...
pub(crate) struct Handle {
    ...
    #[cfg(feature = "time")]
    pub(crate) local_resource: Option<std::sync::Arc<crate::runtime::resource::ResourceState>>,
}

/// Cloneable native Arc owner. The external anchor outlives deallocation of the
/// original Handle allocation; no naked Arc or Weak escapes this wrapper.
#[derive(Clone, Debug)]
...
impl HandleRef {
    ...
    #[allow(dead_code)]
    pub(crate) fn as_ptr(&self) -> *const Handle {
...
impl Handle {
    ...
    pub(crate) fn shutdown(&self) {
        #[cfg(feature = "time")]
        if let Some(resource) = &self.local_resource {
            resource.close();
        }
        self.close();
        #[cfg(all(tokio_unstable, feature = "time"))]
...
impl task::Schedule for HandleRef {
