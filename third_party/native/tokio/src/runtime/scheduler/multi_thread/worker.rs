...
struct Core {
    ...
    run_queue: queue::Local<super::HandleRef>,
...
pub(crate) struct Shared {
    ...
    pub(super) inject: inject::Shared<super::HandleRef>,

    /// Coordinates idle workers
    idle: Idle,

    /// Collection of all active tasks spawned onto this executor.
    pub(crate) owned: OwnedTasks<super::HandleRef>,
...
struct Remote {
    ...
    pub(super) steal: queue::Steal<super::HandleRef>,
...
type Notified = task::Notified<super::HandleRef>;
...
pub(super) fn create(
    ...
    name: Option<String>,
    #[cfg(feature = "time")] local_resource: Option<
        std::sync::Arc<crate::runtime::resource::ResourceState>,
    >,
) -> (super::HandleRef, Launch) {
    ...
    let handle = super::HandleRef::new(Handle {
        name,
        ...
        shared: Shared {
            ...
            shutdown_cores: Mutex::new(Vec::with_capacity(size)),
    ...
    let mut launch = Launch(Vec::with_capacity(size));
...
impl Launch {
    pub(crate) fn launch(mut self) -> std::io::Result<()> {
        for worker in &self.0 {
            let worker = worker.clone();
            let task = runtime::spawn_blocking(move || run(worker));
            #[cfg(feature = "time")]
            if task.resource_denial().is_some() {
                return Err(std::io::ErrorKind::OutOfMemory.into());
            }
            drop(task);
        }
        // All workers were handed to the original blocking pool. On success
        // shutdown remains Runtime's responsibility.
        self.0.clear();
        Ok(())
    }
}
...
impl Drop for Launch {
    fn drop(&mut self) {
        // Admission or OS launch can fail after some workers started. Closing
        // alone is insufficient: unlaunched cores must also reach the original
        // all-cores shutdown barrier, otherwise queues and parks stay undrained.
        if let Some(worker) = self.0.first() {
            worker.handle.shutdown();
        }
        for worker in &self.0 {
            if let Some(mut core) = worker.core.take() {
                core.pre_shutdown(worker);
                worker.handle.shutdown_core(core);
            }
        }
    }
}
...
impl Context {
    ...
    fn maintain_local_timers_before_parking(
        ...
        let (should_yield, next_timer) = with_current(|maybe_cx| {
            ...
            assert_eq!(
                cx.worker.handle.as_ptr(),
                self.worker.handle.as_ptr(),
...
impl Worker {
    ...
    fn inject(&self) -> &inject::Shared<super::HandleRef> {
...
impl Overflow<super::HandleRef> for Handle {
    fn push(&self, task: task::Notified<super::HandleRef>) {
        self.push_remote_task(task);
    }

    fn push_batch<I>(&self, iter: I)
    where
        I: Iterator<Item = task::Notified<super::HandleRef>>,
    {
        unsafe {
...
pub(crate) fn resource_heap_bytes(
    ...
    use crate::runtime::resource::{add, arc_bytes, array_bytes, mul};
    let per_worker = add(std::mem::size_of::<Core>(), arc_bytes::<Worker>()?)?;
    let mut bytes = add(arc_bytes::<Handle>()?, mul(workers, per_worker)?)?;
    // Initial cores Vec, remotes and metrics exact-sized arrays, idle sleepers,
    // launch Vec and shutdown_cores Vec. The owned profile preallocates both
    // latter Vecs to workers before any worker is launched.
    bytes = add(bytes, array_bytes::<Box<Core>>(workers)?)?;
    bytes = add(bytes, array_bytes::<Remote>(workers)?)?;
    bytes = add(bytes, array_bytes::<WorkerMetrics>(workers)?)?;
    bytes = add(bytes, array_bytes::<usize>(workers)?)?;
    bytes = add(bytes, array_bytes::<Arc<Worker>>(workers)?)?;
    bytes = add(bytes, array_bytes::<Box<Core>>(workers)?)?;
    bytes = add(bytes, mul(workers, queue::resource_heap_bytes()?)?)?;
    bytes = add(bytes, super::park::resource_heap_bytes(workers)?)?;
    add(
        bytes,
        crate::runtime::task::resource_heap_bytes::<super::HandleRef>(workers)?,
    )
}
