    ...
    S: Schedule,
{
    ...
    pub(super) fn dealloc(self) {
        ...
        unsafe {
            // A field receipt would drop during `Cell::drop`, before Box frees
            // the Cell allocation. Move it out under the exclusive last-reference
            // guarantee and retain it across Box deallocation, including unwind.
            #[cfg(all(feature = "rt-multi-thread", feature = "time"))]
            let _allocation_receipt = (*self.cell.as_ptr()).resource_receipt.take();
