...
cfg_rt! {
    ...
    pub struct JoinHandle<T> {
        ...
        resource_error: Option<(super::Id, crate::runtime::resource::ResourceLayoutError)>,
        #[cfg(all(feature = "rt-multi-thread", feature = "time"))]
        rejected_id: Option<super::Id>,
        _p: PhantomData<T>,
    }
}
...
impl<T> JoinHandle<T> {
    ...
    pub(crate) fn resource_denied(
        ...
        Self {
            raw: None,
            resource_error: Some((id, error)),
            rejected_id: Some(id),
            _p: PhantomData,
        }
    }
    #[cfg(all(feature = "rt-multi-thread", feature = "time"))]
    pub(crate) fn resource_denial(&self) -> Option<crate::runtime::resource::ResourceLayoutError> {
        self.resource_error.map(|(_, error)| error)
    }
    fn raw(&self) -> RawTask {
        self.raw
            .expect("terminal resource-denied task has no native allocation")
    }
    ...
    pub(crate) fn set_join_waker(&mut self, waker: &Waker) {
        if self.raw.is_none_or(|raw| raw.try_set_join_waker(waker)) {
    ...
    pub fn abort_handle(&self) -> AbortHandle {
        #[cfg(all(feature = "rt-multi-thread", feature = "time"))]
        if let Some(id) = self.rejected_id {
            return AbortHandle::resource_denied(id);
        }
        self.raw().ref_inc();
...
impl<T> Future for JoinHandle<T> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if self.raw.is_none() {
            ...
            return Poll::Ready(Err(super::JoinError::resource_denied(id, error)));
        }
