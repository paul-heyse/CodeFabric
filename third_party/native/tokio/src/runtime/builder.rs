...
pub struct Builder {
    ...
    pub(super) local_resource: Option<std::sync::Arc<super::resource::ResourceState>>,
...
cfg_rt_multi_thread! {
    impl Builder {
        fn build_threaded_runtime(&mut self) -> io::Result<Runtime> {
            ...
            // Construct the owner first so a denied partial launch still executes
            // the original scheduler shutdown and complete blocking-thread join.
            let runtime=Runtime::from_parts(Scheduler::MultiThread(scheduler), handle, blocking_pool);
            let entered=runtime.enter();
            let launched=launch.launch();
            drop(entered);
            if let Err(error)=launched { drop(runtime); return Err(error); }
            Ok(runtime)
        }
    }
}
