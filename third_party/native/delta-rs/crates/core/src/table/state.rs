...
impl Snapshot {
    ...
    pub(crate) fn active_add_partition_batches_from(
        &self,
        batches: &[crate::kernel::SharedNativeRecordBatch],
    ) -> Result<Vec<RecordBatch>, DeltaTableError> {
        let mut expressions = vec![column_expr_ref!("path")];
        let mut fields = vec![StructField::not_null("path", DataType::STRING)];
        ...
        let evaluated_batches = batches.iter().map(|batch| {
            let batch = batch.try_to_ungoverned_record_batch()?;
    ...
    fn add_actions_batches_with_schema(
        ...
        let files = self
            .materialized_files()
            .map(|materialized_files| materialized_files.batches.as_ref().as_ref())
        ...
        let evaluated_batches = files.iter().map(|file| {
            // This public compatibility API returns bare batches. A native owner
            // cannot cross that boundary without the selected owned API.
            let legacy_file = file.try_to_ungoverned_record_batch()?;
            let file = add_missing_stats_parsed(&legacy_file, input_arrow_schema.as_ref())?;
