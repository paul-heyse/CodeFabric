...
use delta_kernel_default_engine::resource::OwnedJoinSet;
use tracing::log::*;
use uuid::Uuid;
...
fn channel_size() -> usize {
    if delta_kernel::resource::current_resource_scope().is_some() { return DEFAULT_WRITER_BATCH_CHANNEL_SIZE; }
...
fn max_concurrent_writers() -> usize {
    if delta_kernel::resource::current_resource_scope().is_some() { return 128; }
...
pub(crate) async fn write_streams(
    ...
    let (tx, mut rx) = delta_kernel_default_engine::resource::try_channel::<RecordBatch>(channel_size())?;

    let mut writer_handle = delta_kernel_default_engine::resource::try_spawn(async move {
    ...
    let mut worker_set = OwnedJoinSet::try_new()?;
    for mut stream in streams {
        let tx_clone = tx.clone();
        worker_set.spawn(async move {
            while let Some(maybe_batch) = stream.next().await {
                let batch = maybe_batch?;
                tx_clone.send(batch, WRITER_TASK_CLOSED_UNEXPECTEDLY_MSG).await?;
            }
            Ok::<(), DeltaTableError>(())
        })?;
    }
...
async fn write_data_plan(
    ...
    let mut join_set = OwnedJoinSet::try_new()?;
...
async fn write_cdc_plan(
    ...
    if partition_columns.is_empty() {
        let (tx_normal, mut rx_normal) = delta_kernel_default_engine::resource::try_channel::<RecordBatch>(channel_size())?;
        let (tx_cdf, mut rx_cdf) = delta_kernel_default_engine::resource::try_channel::<RecordBatch>(channel_size())?;

        let normal_writer_handle = delta_kernel_default_engine::resource::try_spawn(async move {
        ...
        let cdf_writer_handle = delta_kernel_default_engine::resource::try_spawn(async move {
        ...
        for mut partition_stream in partition_streams {
            ...
            let h = delta_kernel_default_engine::resource::try_spawn(async move {
                while let Some(maybe_batch) = partition_stream.next().await {
                    ...
                    while let Some(mut normal_batch) = normal_stream.try_next().await? {
                        ...
                        txn.send(normal_batch, "normal writer closed unexpectedly").await?;
                    }

                    let mut cdf_stream = cdf_df.execute_stream().await?;
                    while let Some(cdf_batch) = cdf_stream.try_next().await? {
                        txc.send(cdf_batch, "cdf writer closed unexpectedly").await?;
                    }
                }
                Ok::<(), DeltaTableError>(())
            })?;
