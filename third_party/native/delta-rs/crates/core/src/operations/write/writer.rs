...
use super::native_writer::AdmittedBufWriter;
...
fn upload_part_size() -> usize {
    if delta_kernel::resource::current_resource_scope().is_some() { return DEFAULT_UPLOAD_PART_SIZE; }
...
fn get_max_concurrency_tasks() -> usize {
    if delta_kernel::resource::current_resource_scope().is_some() { return DEFAULT_MAX_CONCURRENCY_TASKS; }
...
async fn upload_parquet_file(
    mut arrow_writer: AsyncArrowWriter<AdmittedBufWriter>,
    path: Path,
...
impl PartitionWriterConfig {
    ...
    pub fn try_new(
        ...
        if delta_kernel::resource::current_resource_scope().is_some() && max_concurrency_tasks.is_some_and(|maximum| maximum == 0 || maximum > DEFAULT_MAX_CONCURRENCY_TASKS) {
            return Err(delta_kernel::Error::from(delta_kernel::resource::ResourceExhausted { kind: "native_upload_concurrency", requested: max_concurrency_tasks.unwrap_or(0), limit: DEFAULT_MAX_CONCURRENCY_TASKS }).into());
        }
...
enum LazyArrowWriter {
    ...
    Writing(Path, AsyncArrowWriter<AdmittedBufWriter>),
}
    ...
    async fn write_batch(&mut self, batch: &RecordBatch) -> DeltaResult<()> {
        match self {
            LazyArrowWriter::Initialized(path, object_store, config) => {
                let writer = AdmittedBufWriter::try_new(
...
pub struct PartitionWriter {
    ...
    /// Stats columns, specific columns to collect stats from, takes precedence over num_indexed_cols
    stats_columns: Option<Vec<String>>,
    in_flight_writers: OwnedJoinSet<DeltaResult<(Path, usize, ParquetMetaData)>>,
}
...
impl PartitionWriter {
    ...
    pub fn try_with_config(
        ...
        Ok(Self {
            ...
            num_indexed_cols,
            stats_columns,
            in_flight_writers: OwnedJoinSet::try_new()?,
        })
    }
