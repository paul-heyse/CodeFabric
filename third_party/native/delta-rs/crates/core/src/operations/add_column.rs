...
use crate::kernel::{
    Action, EagerSnapshot, ProtocolExt as _, SnapshotMetadataRef, StructField,
...
fn plan_add_column_actions(
    ...
    let mut metadata = snapshot.metadata.try_clone_admitted(delta_kernel::resource::current_resource_scope())?;
    ...
    metadata = metadata.with_schema(crate::kernel::models::actions::admitted_schema_arc(new_table_schema)?)?;
