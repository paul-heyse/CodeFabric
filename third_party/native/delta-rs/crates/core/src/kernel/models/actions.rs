...
/// Move an already admitted native schema into its original Arc only after
/// reserving the allocation header; no deep clone is covered by sharing a scope.
pub(crate) fn admitted_schema_arc(schema: StructType) -> crate::DeltaResult<std::sync::Arc<StructType>> {
    if let Some(scope) = delta_kernel::resource::current_resource_scope() {
        scope.reserve(delta_kernel::resource::AllocationRequest {
            kind: "native_delta_schema_arc",
            bytes: std::mem::size_of::<StructType>() + 2 * std::mem::size_of::<usize>(),
        })?;
    }
