...
impl MaterializedFilesWire {
    ...
    fn into_materialized(
        ...
        let materialized_files = MaterializedFiles {
            ...
            batches: Arc::new(
                super::MaterializedFileBatches::unadmitted(deserialize_batches(self.batches)?)
...
fn serialize_batches(
    batches: &[super::stream::SharedNativeRecordBatch],
    ...
    let mut buffer = vec![];
    let mut writer = FileWriter::try_new(&mut buffer, batches[0].schema_ref().clone().as_ref())
...
impl Serialize for Snapshot {
    ...
    where
        S: serde::Serializer,
    {
        reject_governed_snapshot_wire().map_err(serde::ser::Error::custom)?;
...
impl<'de> Visitor<'de> for SnapshotVisitor {
        ...
        V: SeqAccess<'de>,
    {
        ...
        let listed_log_files = LogSegmentFiles {
            ascending_commit_files: ascending_commit_files.into(),
            ascending_compaction_files: ascending_compaction_files.into(),
            checkpoint_parts: checkpoint_parts.into(),
            latest_crc_file,
            latest_commit_file,
            ..Default::default()
        };
...
impl<'de> Deserialize<'de> for Snapshot {
    ...
    where
        D: Deserializer<'de>,
    {
        reject_governed_snapshot_wire().map_err(de::Error::custom)?;
...
// Snapshot IPC/Serde is a compatibility transport, not the selected exact-log
// native reconstruction route. Reject before its DTO/batch allocations; legacy
// callers outside a governed operation retain the unchanged wire behavior.
fn reject_governed_snapshot_wire() -> delta_kernel::DeltaResult<()> {
    if delta_kernel::resource::current_resource_scope().is_some() {
        Err(delta_kernel::resource::ResourceExhausted {
            kind: "native_snapshot_serde_compatibility", requested: 1, limit: 0,
        }.into())
    } else { Ok(()) }
}
