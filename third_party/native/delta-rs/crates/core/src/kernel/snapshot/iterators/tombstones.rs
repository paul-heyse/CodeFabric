...
pub struct TombstoneView {
    data: super::super::stream::SharedNativeRecordBatch,
    index: usize,
}

impl TombstoneView {
    /// Creates a new view into the specified file entry.
    pub(crate) fn new(data: super::super::stream::SharedNativeRecordBatch, index: usize) -> Self {
        Self { data, index }
    }

    /// Borrow the original full batch retained by this tombstone row view.
    ///
    /// This does not slice, project or replay the backing. Other rows in the same batch
    /// remain retained for this view's lifetime and must be included by resource owners.
    pub fn retained_batch(&self) -> &RecordBatch {
        self.data.record_batch()
    }

    /// Original admitted data and metadata owner retained by the row view.
    pub fn retained_native_batch(&self) -> &super::super::stream::SharedNativeRecordBatch {
        &self.data
    }
