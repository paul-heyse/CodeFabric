...
where
    S: Stream<Item = DeltaResult<NativeOwnedRecordBatch>>,
{
    type Item = DeltaResult<super::super::stream::SharedNativeRecordBatch>;
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        match this.stream.poll_next(cx) {
            Poll::Ready(Some(Ok(batch))) => {
                ...
                Poll::Ready(Some(result.and_then(|batch| Ok(batch.try_into_shared()?))))
            }
...
pub(crate) fn parse_stats_column_with_schema(
    sn: &KernelSnapshot,
    owned_batch: &NativeOwnedRecordBatch,
    stats_schema: KernelSchemaRef,
) -> DeltaResult<NativeOwnedRecordBatch> {
...
fn parse_stats_column_impl(
    ...
    let batch = owned_batch.record_batch();
    let mut owners = owned_batch.owners().clone().try_with_current()?;
    ...
    if let Some((stats_array, stats_owners)) = materialize_stats_array(
        owned_batch,
        stats_schema,
        stats_arrow_fields,
        stats_materialization,
    )? {
        owners = owners.try_merge(stats_owners)?;
...
fn materialize_stats_array(
    ...
    stats_arrow_fields: &Fields,
    stats_materialization: &FileStatsMaterialization,
) -> DeltaResult<Option<(Arc<StructArray>, NativeDataOwners)>> {
    let batch = owned_batch.record_batch();
    match stats_materialization.stats_source_policy() {
        ...
        StatsSourcePolicy::ParsedWithJsonFallback => {
            ...
            if let Some((idx, _)) = batch.schema_ref().column_with_name(FIELD_STATS_PARSED) {
                ...
                match project_struct_array(parsed, stats_arrow_fields) {
                    Ok(projected) => {
                        return Ok(Some((
                            Arc::new(projected),
                            owned_batch.owners().clone().try_with_current()?,
                        )));
                    }
                    Err(err) => {
                        if matches!(&err, DeltaTableError::KernelError(error) if error.is_resource_exhausted())
                        {
                            return Err(err);
                        }
                        debug!(
            ...
            parse_raw_stats_array(owned_batch, stats_schema).map(Some)
        }
    }
}
...
fn parse_raw_stats_array(
    owned_batch: &NativeOwnedRecordBatch,
    stats_schema: KernelSchemaRef,
) -> DeltaResult<(Arc<StructArray>, NativeDataOwners)> {
    let batch = owned_batch.record_batch();
    ...
    let stats_batch = owned_batch
        .try_transform(|batch| Ok::<_, DeltaTableError>(batch.project(&[stats_idx])?))?;
    let stats_data = Box::new(stats_batch.into_engine_data());
    let parsed = parse_json(stats_data, stats_schema)?;
    let parsed = ArrowEngineData::try_from_engine_data(parsed)?.into_owned_record_batch();
    let (array, owners) = parsed.into_struct_array();
    Ok((Arc::new(array), owners))
}
