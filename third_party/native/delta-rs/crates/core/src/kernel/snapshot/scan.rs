...
impl Scan {
    ...
    pub fn scan_metadata_from<
        T: Iterator<Item = super::stream::SharedNativeRecordBatch> + Send + 'static,
    >(
        &self,
        engine: Arc<dyn Engine>,
        ...
        let scan_inner = move || {
            ...
            let scan_row_data = existing_data
                .map_while(move |batch| {
                    match crate::kernel::arrow::engine_ext::ExpressionEvaluatorExt::evaluate_native(
                        evaluator.as_ref(),
                        batch.as_ref(),
                    ) {
                        Ok(data) => Some(Box::new(data.into_engine_data()) as Box<dyn EngineData>),
                        Err(err) => {
