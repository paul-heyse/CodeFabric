...
use self::retained::MaterializedFilesCollection;
...
enum ConflictReadSetInner<'a> {
    ...
    Owned {
        batches: Arc<Vec<SharedNativeRecordBatch>>,
...
pub struct Snapshot {
    ...
    /// Admission for new caches; this is policy, not a receipt for kernel/Arrow state.
    materialized_files_admission: Option<Arc<dyn MaterializedFilesAdmission>>,
}
...
impl Snapshot {
    ...
    pub async fn try_new_with_engine(
        ...
        Ok(Self {
            ...
            materialized_files_admission: None,
        })
    }
    ...
    pub async fn update(
        ...
        let snapshot = Arc::new(Self {
            ...
            materialized_files: None,
            materialized_files_admission: self.materialized_files_admission.clone(),
        });
    ...
    pub(crate) async fn active_add_batches(
        ...
        log_store: &dyn LogStore,
        options: ActiveAddOptions,
    ) -> DeltaResult<Vec<SharedNativeRecordBatch>> {
        self.files_with_stats_policy(log_store, options.predicate, options.stats)
            .try_collect()
            .await
    }
    ...
    /// Legacy bare batches are rejected when they carry native admission owners.
    pub fn files(
        &self,
        log_store: &dyn LogStore,
        predicate: Option<PredicateRef>,
    ) -> SendableRBStream {
        self.native_files(log_store, predicate)
            .map(|batch| Ok(batch?.try_to_ungoverned_record_batch()?))
            .boxed()
    }

    /// Stream the original native batches with their admission owners.
    pub fn native_files(
        &self,
        log_store: &dyn LogStore,
        predicate: Option<PredicateRef>,
    ) -> SendableNativeRBStream {
        ...
        let engine = match log_store.engine(None) {
            Ok(engine) => engine,
            Err(error) => return Box::pin(once(ready(Err(error)))),
        };
        if predicate.is_some() && self.config.skip_stats {
            return self.files_with_engine(engine.clone(), predicate);
        }
            ...
            .and_then(|materialized_files| materialized_files.full_table_seed())
        {
            ...
            None => self.files_with_engine(engine.clone(), predicate),
        }
    }

    fn cached_parsed_batches(&self) -> Option<SendableNativeRBStream> {
        self.cached_parsed_batches_for(AddStatsPolicy::None)
    }

    fn cached_parsed_batches_for(&self, stats: AddStatsPolicy) -> Option<SendableNativeRBStream> {
    ...
    pub(crate) fn files_from<T: Iterator<Item = SharedNativeRecordBatch> + Send + 'static>(
        &self,
        engine: Arc<dyn Engine>,
        predicate: Option<PredicateRef>,
    ...
    fn files_from_preserving_raw<T: Iterator<Item = SharedNativeRecordBatch> + Send + 'static>(
        &self,
        engine: Arc<dyn Engine>,
        predicate: Option<PredicateRef>,
        existing_version: Version,
    ...
    fn files_from_materialized<T: Iterator<Item = SharedNativeRecordBatch> + Send + 'static>(
        &self,
        engine: Arc<dyn Engine>,
        predicate: Option<PredicateRef>,
        existing_version: Version,
    ...
    fn files_from_materialized_with_options<
        T: Iterator<Item = SharedNativeRecordBatch> + Send + 'static,
    >(
        &self,
        engine: Arc<dyn Engine>,
        predicate: Option<PredicateRef>,
    ...
    async fn materialize_files_with_engine(
        ...
        // Admission and descriptor allocation precede native stream construction.
        // Kernel/Arrow worker allocation ownership is a separate engine obligation.
        let mut collection = MaterializedFilesCollection::try_new(
            self.as_ref(),
            self.materialized_files_admission.as_deref(),
        )?;
        let mut stream = match materialized_seed.and_then(|seed| seed.full_table_seed()) {
            Some(materialized_seed) => {
                let (existing_version, existing_data, existing_predicate) =
                    materialized_seed.into_parts();
            ...
            None => self.files_with_engine_preserving_raw(engine, None),
        };
        while let Some(batch) = stream.try_next().await? {
            collection.push(batch)?;
        }
        let materialized_files = Arc::new(collection.finish(self.as_ref()));
        Ok(Arc::new(
    ...
    pub(crate) fn tombstones(
        ...
        let mut builder = ReceiverStreamBuilder::<
            delta_kernel::engine::arrow_data::NativeOwnedRecordBatch,
        >::new(100);
        let tx = builder.tx();
        ...
        builder.spawn_blocking(move || {
            for res in remove_data {
                let batch =
                    ArrowEngineData::try_from_engine_data(res?.actions)?.into_owned_record_batch();
        ...
        builder
            ...
            .map(|maybe_batch| {
                maybe_batch.and_then(|batch| {
                    let filtered = batch.try_map(|batch| {
                        Ok::<_, DeltaTableError>(filter_record_batch(
                            &batch,
                            &is_not_null(batch.column(0))?,
                        )?)
                    })?;
                    let tombstones = TOMBSTONE_EVALUATOR
                        .evaluate_native(&filtered)?
                        .try_into_shared()?;
...
pub(crate) struct MaterializedFiles {
    ...
    pub(crate) batches: Arc<MaterializedFileBatches>,
    // Last field: release only after the owned identity is dropped.
    _identity_receipt: Option<MaterializedFilesReceipt>,
}
...
struct MaterializedFilesSeed {
    version: Version,
    existing_predicate: Option<PredicateRef>,
    batches: Arc<MaterializedFileBatches>,
}
...
impl IntoIterator for MaterializedFilesSeed {
    type Item = SharedNativeRecordBatch;
...
impl SharedRecordBatchIter {
    fn new(batches: Arc<MaterializedFileBatches>) -> Self {
        Self {
            batches,
            next_idx: 0,
        }
...
impl MaterializedFiles {
    fn full(snapshot: &Snapshot, batches: Vec<RecordBatch>) -> DeltaResult<Self> {
        ...
        Ok(Self {
            ...
            batches: Arc::new(MaterializedFileBatches::unadmitted(batches)?),
            _identity_receipt: None,
        })
    }
...
mod tests {
    ...
    async fn test_materialized_files_full_table_seed_shares_batches() -> TestResult {
        ...
        assert_eq!(
            ...
            materialized_files.batches.as_ref().as_ref()
        );
    ...
    async fn legacy_skip_stats_cache_reads_current_files_and_rebuilds_for_update() -> TestResult {
        ...
        let current_cache = snapshot
            ...
            .materialized_files()
            .expect("eager snapshot must contain materialized files");
        let mut legacy_cache = MaterializedFiles::full(snapshot.snapshot(), Vec::new());
        legacy_cache.batches = Arc::new(MaterializedFileBatches::unadmitted(
            current_cache
                .batches
                .iter()
        ...
        let zero_row_cache = MaterializedFiles::full(
            snapshot.snapshot(),
            vec![legacy_cache.batches[0].slice(0, 0)],
        );
        assert!(
