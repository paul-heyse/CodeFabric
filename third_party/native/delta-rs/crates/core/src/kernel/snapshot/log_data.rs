...
pub struct LogDataHandler<'a> {
    data: &'a [SharedNativeRecordBatch],
    config: &'a TableConfiguration,
}

impl<'a> LogDataHandler<'a> {
    pub(crate) fn new(data: &'a [SharedNativeRecordBatch], config: &'a TableConfiguration) -> Self {
        Self { data, config }
    }

    /// Borrow the original table configuration associated with these retained batches.
    /// This does not copy parsed schemas, metadata, protocol or properties.
    ...
    /// Borrow the original record batches retained by this handler.
    ///
    /// No projection, concatenation or replay occurs. A handler alone does not distinguish
    /// a lazy snapshot from a materialized empty cache; use Snapshot::retained_state or
    /// EagerSnapshot::retained_state when that distinction matters.
    pub fn retained_batches(&self) -> &'a [SharedNativeRecordBatch] {
        self.data
    }
    ...
    impl LogDataHandler<'_> {
        fn pick_stats(&self, column: &Column, stats_field: &'static str) -> Option<ArrayRef> {
            ...
            let results: Vec<_> = self
                .data
                .iter()
                .map(|data| -> DeltaResult<_> { Ok(evaluator.evaluate_native(data)?) })
                .try_collect()
                .ok()?;
            concat(
                &results
                    .iter()
                    .map(|result| result.column(0).as_ref())
                    .collect::<Vec<_>>(),
            )
            .ok()
        }
        ...
        fn row_counts(&self) -> Option<ArrayRef> {
            ...
            let results: Vec<_> = self
                ...
                .map(|data| -> DeltaResult<_> {
                    let batch = ROW_COUNTS_EVAL.evaluate_native(data)?;
                    let array = arrow_cast::cast(batch.column(0), &ArrowDataType::UInt64)?;
                    Ok((array, batch))
                })
                .try_collect()
                .ok()?;
