...
pub struct LogicalFileView {
    files: super::stream::SharedNativeRecordBatch,
    index: usize,
}

impl LogicalFileView {
    /// Creates a new view into the specified file entry.
    pub(crate) fn new(files: super::stream::SharedNativeRecordBatch, index: usize) -> Self {
        Self { files, index }
    }

    /// Borrow the original full batch retained by this row view.
    ///
    /// The batch may contain other rows. This is the actual backing, not a one-row
    /// projection; it can outlive the snapshot or batch container that created the view.
    pub fn retained_batch(&self) -> &RecordBatch {
        self.files.record_batch()
    }

    /// Borrow the native lifetime owners retained by this row view.
    pub fn retained_native_batch(&self) -> &super::stream::SharedNativeRecordBatch {
        &self.files
    }
