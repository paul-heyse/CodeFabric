...
/// One immutable native batch descriptor and its original allocation owners.
pub type SharedNativeRecordBatch =
    std::sync::Arc<delta_kernel::engine::arrow_data::NativeOwnedRecordBatch>;

/// Native batches retain their admitted backing and metadata through stream transfer.
pub type SendableNativeRBStream =
    Pin<Box<dyn Stream<Item = DeltaResult<SharedNativeRecordBatch>> + Send>>;

/// Legacy bare record-batch stream; governed producers require the owned stream API.
