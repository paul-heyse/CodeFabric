...
pub(crate) fn rb_from_scan_meta(
    metadata: ScanMetadata,
) -> DeltaResult<delta_kernel::engine::arrow_data::NativeOwnedRecordBatch> {
    let (underlying_data, selection_vector) = metadata.scan_files.into_parts();
    let batch = ArrowEngineData::try_from_engine_data(underlying_data)?.into_owned_record_batch();
    batch.try_map(|batch| {
...
pub(crate) trait ExpressionEvaluatorExt {
    ...
    fn evaluate_native(
        &self,
        batch: &delta_kernel::engine::arrow_data::NativeOwnedRecordBatch,
    ) -> DeltaResult<delta_kernel::engine::arrow_data::NativeOwnedRecordBatch>;
}
...
impl<T: ExpressionEvaluator + ?Sized> ExpressionEvaluatorExt for T {
    fn evaluate_arrow(&self, batch: RecordBatch) -> DeltaResult<RecordBatch> {
        ...
        Ok(
            ArrowEngineData::try_from_engine_data(T::evaluate(self, &engine_data)?)?
    ...
    fn evaluate_native(
        ...
        let engine_data = batch.try_clone()?.into_engine_data();
        Ok(
            ArrowEngineData::try_from_engine_data(T::evaluate(self, &engine_data)?)?
                .into_owned_record_batch()
                .try_retain(batch.owners().clone())?,
        )
    }
}
