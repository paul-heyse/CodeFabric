    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        match this.stream.poll_next(cx) {
            Poll::Ready(Some(Ok(scan_data))) => {
                ...
                let batch = ArrowEngineData::try_from_engine_data(data)?.into_owned_record_batch();
                let scan_files = batch.try_map(|batch| {
                ...
                let parsed_stats =
                    parse_stats_column_with_schema(snapshot.as_ref(), &scan_files, stats_schema)?
                        .try_into_shared()?;
                let retained_stats_batch = parsed_stats.clone();
                ...
                Poll::Ready(Some(Ok(ctx
                    ...
                    .map(|ctx| {
                        ...
                        ScanFileContext::new(ctx, stats, partitions, retained_stats_batch.clone())
                    })
                    .collect_vec())))
            }
...
fn extract_file_statistics(
    scan: &KernelScan,
    scan_config: &DeltaScanConfig,
    parsed_stats: crate::kernel::SharedNativeRecordBatch,
...
pub(crate) struct ScanFileContext {
    ...
    /// Retained native input and metadata lifetime for derived statistics.
    pub native_batch: crate::kernel::SharedNativeRecordBatch,
}
...
impl ScanFileContext {
    ...
    fn new(
        ...
        native_batch: crate::kernel::SharedNativeRecordBatch,
    ) -> Self {
        Self {
...
fn apply_file_selection(
    ...
    scan_data.scan_files = FilteredEngineData::try_new(
        Box::new(batch.try_clone()?.into_engine_data()),
        selection_vector,
    )?;
    Ok(scan_data)
}
