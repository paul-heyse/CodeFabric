...
use delta_kernel::{DeltaResult, Error as DeltaError, FileMeta, FileSlice, StorageHandler, OwnedFileMeta};
    ...
    fn get_or_create(
        ...
        let handler: Arc<dyn StorageHandler> = match self.handle.runtime_flavor() {
            RuntimeFlavor::MultiThread => Arc::new(ObjectStoreStorageHandler::new(
                store,
                TokioMultiThreadExecutor::try_new_current_shared(self.handle.clone())?,
            )),
            RuntimeFlavor::CurrentThread => Arc::new(ObjectStoreStorageHandler::new(
                store,
                Arc::new(TokioBackgroundExecutor::try_new()?),
            )),
