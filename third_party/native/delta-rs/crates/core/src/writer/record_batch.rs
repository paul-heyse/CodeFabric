...
impl DeltaWriter<RecordBatch> for RecordBatchWriter {
    ...
    async fn flush_and_commit(
        ...
        if self.arrow_schema_ref != self.original_schema_ref && self.should_evolve {
            ...
            let current_meta = table.snapshot()?.metadata().try_clone_admitted(delta_kernel::resource::current_resource_scope())?;
            let metadata = current_meta.with_schema(crate::kernel::models::actions::admitted_schema_arc(schema)?)?;
