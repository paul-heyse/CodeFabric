...
impl ArrayData {
    /// Original owner of already-admitted metadata, including bufferless data.
    pub fn resource_owner(&self) -> &arrow_schema::resource::ResourceOwnerHandle {
        &self.resource_owner
    }

    /// Adopt original ownership before capturing the required worker owner.
    pub fn try_capture_current_resource_owner(
        &mut self,
    ) -> Result<(), arrow_schema::resource::ResourceOwnerError> {
    ...
    fn inner_new_builder(
        ...
        ArrayDataBuilder {
            ...
            resource_owner: arrow_schema::resource::ResourceOwnerHandle::empty(),
        }
    }
    ...
    /// This legacy parts API releases the metadata root; resource-governed native
    /// reconstruction must use [`Self::into_parts_with_resource_owner`].
    pub fn into_parts(
        self,
    ) -> (
        DataType,
    ...
    /// Return original native parts together with their retained metadata root.
    /// Use this instead of `into_parts` when reconstructing an owned native Array.
    pub fn into_parts_with_resource_owner(
        self,
    ) -> (
        DataType,
        usize,
        ...
        Vec<Buffer>,
        Vec<ArrayData>,
        arrow_schema::resource::ResourceOwnerHandle,
    ) {
        (
            self.data_type,
            self.len,
...
impl ArrayDataBuilder {
    ...
    /// Create a fresh builder carrying the owner of the original native parts.
    ///
    /// This constructor cannot overwrite another original root. Use
    /// [`Self::try_with_resource_owner`] to combine an existing builder with a
    /// distinct owner: that operation requires fallible, admitted adoption.
    /// Capturing the required worker owner happens at build time and preserves
    /// this original owner when adoption is rejected.
    pub fn new_with_resource_owner(
        data_type: DataType,
        owner: arrow_schema::resource::ResourceOwnerHandle,
    ) -> Self {
        Self {
    ...
    /// Fallible transfer of an original native metadata root.
    pub fn try_with_resource_owner(
        mut self,
        owner: arrow_schema::resource::ResourceOwnerHandle,
    ) -> Result<Self, arrow_schema::resource::ResourceOwnerError> {
        self.resource_owner.try_inherit(&owner)?;
        Ok(self)
    }
    ...
    /// apply.
    pub unsafe fn build_unchecked(mut self) -> ArrayData {
        self.resource_owner.capture_current_or_latch();
        unsafe { self.skip_validation(true) }
            .build_inner(false)
            .unwrap()
    }
    ...
    fn build_inner(self, adopt_current: bool) -> Result<ArrayData, ArrowError> {
        let Self {
            data_type,
            len,
            null_count,
        ...
        if adopt_current {
            resource_owner.try_capture_current()?;
        }
...
mod resource_owner_tests {
    use super::*;
    use arrow_schema::resource::{ResourceOwnerError, RetainedResourceOwner, enter_resource_owner};
    ...
    #[test]
    fn resource_bufferless_data_clone_slice_and_builder_keep_original_owner() {
        let owner = owner();
        let weak = Arc::downgrade(&owner);
        let data = {
            let _guard = enter_resource_owner(owner.clone());
            ArrayData::builder(DataType::Null).len(5).build().unwrap()
        };
        drop(owner);
        let sliced = data.slice(1, 3);
        ...
        let data = sliced.into_builder().build().unwrap();
        assert!(weak.upgrade().is_some());
        drop(data);
    ...
    #[test]
    fn resource_owned_parts_roundtrip_retains_zero_buffer_root() {
        let owner = owner();
        let weak = Arc::downgrade(&owner);
        let data = {
            let _guard = enter_resource_owner(owner.clone());
            ArrayData::builder(DataType::Null).len(3).build().unwrap()
        };
        drop(owner);
        let (kind, len, nulls, offset, buffers, children, owner) =
            data.into_parts_with_resource_owner();
        let rebuilt = ArrayDataBuilder::new_with_resource_owner(kind, owner)
            .len(len)
            .nulls(nulls)
            .offset(offset)
            .buffers(buffers)
    ...
    #[test]
    fn resource_fallible_builder_rejects_adoption_and_unchecked_builder_latches() {
        let original = owner();
        let data = {
            let _guard = enter_resource_owner(original.clone());
            ArrayData::builder(DataType::Null).len(2).build().unwrap()
        };
        let original_handle = data.resource_owner().clone();
        let next = owner();
        let _guard = enter_resource_owner(next.clone());
        let error = data.clone().into_builder().build().unwrap_err();
        assert!(matches!(
            error,
            ArrowError::ResourceOwnerError(ResourceOwnerError {
                kind: "test owner adoption",
                ..
            })
        ));
        // SAFETY: DataType::Null with two rows requires no buffers or children.
        let rebuilt = unsafe { data.into_builder().build_unchecked() };
        assert!(next.failed.load(Ordering::SeqCst));
        assert!(rebuilt.resource_owner().same_owner(&original_handle));
    }

    #[test]
    fn resource_original_root_survives_rejected_fresh_builder_adoption() {
        let original = owner();
        let weak = Arc::downgrade(&original);
        let handle = arrow_schema::resource::ResourceOwnerHandle::new(original.clone());
        drop(original);
        let current = owner();
        let data = {
            let _guard = enter_resource_owner(current.clone());
            // SAFETY: Null arrays have no required backing buffers.
            unsafe {
                ArrayDataBuilder::new_with_resource_owner(DataType::Null, handle)
                    .len(3)
                    .build_unchecked()
            }
        };
        assert!(current.failed.load(Ordering::SeqCst));
        // Neither the required worker scope nor the caller retains original.
        // The rejected native output must still keep that root alive.
        assert!(weak.upgrade().is_some());
        drop(current);
        assert!(weak.upgrade().is_some());
        drop(data);
    ...
    #[test]
    fn resource_mixed_builder_adoption_rejects_without_returning_native_parts() {
        let first = owner();
        let second = owner();
        let first_weak = Arc::downgrade(&first);
        let second_weak = Arc::downgrade(&second);
        let builder = ArrayDataBuilder::new_with_resource_owner(
            DataType::Null,
            arrow_schema::resource::ResourceOwnerHandle::new(first.clone()),
        );
        let incoming = arrow_schema::resource::ResourceOwnerHandle::new(second.clone());
        drop(first);
        drop(second);
        let error = builder.try_with_resource_owner(incoming).unwrap_err();
        assert_eq!(error.kind, "test owner adoption");
