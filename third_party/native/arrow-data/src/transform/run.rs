    ...
    macro_rules! extend_nulls_impl {
        ($run_end_type:ty) => {{
            ...
            super::resource::reserve(
                &mut mutable.child_data[0].data.buffer1,
                std::mem::size_of::<$run_end_type>(),
                "transform run end null growth",
            )?;
...
fn build_extend_arrays<T: ArrowNativeType + std::ops::Add<Output = T> + CheckedAdd>(
    ...
    let mut run_ends_bytes = super::resource::vec(
        super::resource::count_bytes(
            length.min(buffer.len() / std::mem::size_of::<T>()),
            std::mem::size_of::<T>(),
            "transform run end scratch geometry",
        )?,
        "transform run end scratch",
    )?;
...
pub fn build_extend(array: &ArrayData) -> Result<Extend<'_>, arrow_schema::ArrowError> {
