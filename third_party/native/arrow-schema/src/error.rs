...
#[derive(Debug)]
pub enum ArrowError {
    /// Typed refusal to retain an admitted native metadata owner.
    ResourceOwnerError(crate::resource::ResourceOwnerError),
...
impl Display for ArrowError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ArrowError::ResourceOwnerError(error) => Display::fmt(error, f),
...
impl Error for ArrowError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            ArrowError::ResourceOwnerError(error) => Some(error),
...
impl From<crate::resource::ResourceOwnerError> for ArrowError {
    fn from(error: crate::resource::ResourceOwnerError) -> Self {
        Self::ResourceOwnerError(error)
    }
}
