...
pub struct Fields(
    Arc<[FieldRef]>,
    #[cfg_attr(feature = "serde", serde(skip))] crate::resource::ResourceOwnerHandle,
);
...
impl Fields {
    /// Original already-admitted collection owner, including empty field sets.
    pub fn resource_owner(&self) -> &crate::resource::ResourceOwnerHandle {
        &self.1
    }

    /// Adopt the original root before capturing the required worker owner.
    pub fn try_capture_current_resource_owner(
        &mut self,
    ) -> Result<(), crate::resource::ResourceOwnerError> {
        self.1.try_capture_current()
    }

    /// Retain an original metadata root when reconstructing a field collection.
    pub fn try_inherit_resource_owner(
        &mut self,
        original: &crate::resource::ResourceOwnerHandle,
    ) -> Result<(), crate::resource::ResourceOwnerError> {
        self.1.try_inherit(original)
    }

    pub(crate) fn capture_current_resource_owner(&mut self) {
        self.1.capture_current_or_latch();
    }

    pub(crate) fn transfer_resource_owner(
        &mut self,
        mut owner: crate::resource::ResourceOwnerHandle,
    ) {
        owner.capture_current_or_latch();
        self.1 = owner;
    }
    ...
    pub fn empty() -> Self {
        Self(
            Arc::new([]),
            crate::resource::ResourceOwnerHandle::capture(),
        )
    }
...
impl UnionFields {
    /// Original already-admitted collection owner, including empty unions.
    pub fn resource_owner(&self) -> &crate::resource::ResourceOwnerHandle {
        &self.1
    }

    /// Retain an original metadata root during native union reconstruction.
    pub fn try_inherit_resource_owner(
        &mut self,
        original: &crate::resource::ResourceOwnerHandle,
    ) -> Result<(), crate::resource::ResourceOwnerError> {
