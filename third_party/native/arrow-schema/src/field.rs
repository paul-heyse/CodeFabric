...
impl Field {
    /// Original already-admitted metadata owner, independent of semantic equality.
    pub fn resource_owner(&self) -> &crate::resource::ResourceOwnerHandle {
        &self.resource_owner
    }

    /// Adopt original ownership before selected native mutations or reconstruction.
    ...
    pub fn try_merge(&mut self, from: &Field) -> Result<(), ArrowError> {
        self.resource_owner.try_capture_current()?;
        self.resource_owner.try_inherit(from.resource_owner())?;
