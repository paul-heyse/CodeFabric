...
fn decode_hex_to_writer<W: Write>(hex_string: &str, writer: &mut W) -> Result<(), ArrowError> {
    ...
    for (pair_index, pair) in (&mut iter).enumerate() {
        ...
        if buffered == buffer.len() {
            writer.write_all(&buffer).map_err(|e| {
                resource::json_error(format_args!("failed to write binary data: {e}"))
            })?;
            buffered = 0;
        }
    }
    ...
    if buffered > 0 {
        writer
            ...
            .map_err(|e| resource::json_error(format_args!("failed to write binary data: {e}")))?;
    }
...
impl ArrayDecoder for FixedSizeBinaryArrayDecoder {
    fn decode(&mut self, tape: &Tape<'_>, pos: &[u32]) -> Result<ArrayRef, ArrowError> {
        ...
        resource::buffer(pos.len(), capacity, "JSON fixed binary values")?;
        resource::nulls(pos.len())?;
        let mut builder = FixedSizeBinaryBuilder::with_capacity(pos.len(), self.len);
        let mut scratch = resource::new_vec(capacity, "JSON string backing")?;
            ...
            match tape.get(*p) {
                TapeElement::String(idx) => {
                    ...
                    resource::reserve_vec(
                        &mut scratch,
                        string.len().div_ceil(2),
                        "JSON string backing",
                    )?;
                    scratch.reserve(string.len().div_ceil(2));
                    decode_hex_to_writer(string, &mut scratch)?;
                    if resource::current().is_some() && scratch.len() != capacity {
                        return Err(resource::json_error(format_args!(
                            "Incorrect fixed binary width: expected {capacity}, got {}",
                            scratch.len()
                        )));
                    }
...
impl ArrayDecoder for BinaryViewDecoder {
    fn decode(&mut self, tape: &Tape<'_>, pos: &[u32]) -> Result<ArrayRef, ArrowError> {
        let data_capacity = estimate_data_capacity(tape, pos)?;
        let mut builder = if resource::current().is_some() {
            let block = resource::view_buffers(pos.len(), data_capacity)?;
            BinaryViewBuilder::with_capacity(pos.len()).with_fixed_block_size(block)
        } else {
            BinaryViewBuilder::with_capacity(data_capacity)
        };
    ...
    for p in pos {
        match tape.get(*p) {
            TapeElement::String(idx) => {
                ...
                data_capacity =
                    resource::checked_add(data_capacity, decoded_len, "JSON string backing")?;
            }
            TapeElement::Null => {}
            _ => {
