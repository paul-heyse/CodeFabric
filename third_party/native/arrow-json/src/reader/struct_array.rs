...
impl FieldTapePositions {
    ...
    fn resize(&mut self, field_count: usize, row_count: usize) -> Result<(), ArrowError> {
        ...
        self.data.clear();
        resource::reserve_vec(&mut self.data, total_len, "JSON struct positions")?;
...
impl StructArrayDecoder {
    pub fn new(
        ...
        let mut decoders = resource::new_vec(fields.len(), "JSON struct decoders")?;
        for f in fields {
            decoders.push(ctx.make_decoder(f.data_type(), f.is_nullable() || is_nullable)?);
        }

        let struct_mode = ctx.struct_mode();
        let field_name_to_index =
            if struct_mode == StructMode::ObjectOnly && resource::current().is_none() {
...
impl ArrayDecoder for StructArrayDecoder {
    fn decode(&mut self, tape: &Tape<'_>, pos: &[u32]) -> Result<ArrayRef, ArrowError> {
        ...
        let mut child_arrays = resource::new_vec(fields.len(), "JSON struct arrays")?;
        for (field_idx, d) in self.decoders.iter_mut().enumerate() {
            let pos = self.field_tape_positions.field_positions(field_idx);
            child_arrays.push(d.decode(tape, pos).map_err(|error| match error {
                ArrowError::JsonError(message) => resource::json_error(format_args!(
                    "whilst decoding field '{}': {message}",
