        ...
        for p in pos {
            let value = match tape.get(*p) {
                ...
                TapeElement::String(idx) => {
                    ...
                    if matches!(self.data_type, DataType::Date64) {
                        resource::parser_error_capacity(s.len())?;
                    }
                    P::parse(s).ok_or_else(|| {
                        resource::json_error(format_args!("failed to parse \"{s}\" as {d}",))
                    })
                }
                ...
                TapeElement::F32(v) => {
                    ...
                    NumCast::from(v).ok_or_else(|| {
                        resource::json_error(format_args!("failed to parse {v} as {d}",))
                    })
                }
                TapeElement::I32(v) => NumCast::from(v).ok_or_else(|| {
                    resource::json_error(format_args!("failed to parse {v} as {d}",))
                }),
            ...
            match value {
                ...
                Err(ref error)
                    if self.ignore_type_conflicts && !resource::is_resource_error(error) =>
                {
                    builder.append_null()
                }
                Err(e) => return Err(e),
