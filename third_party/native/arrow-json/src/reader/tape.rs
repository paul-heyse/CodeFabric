...
impl<'a> Tape<'a> {
    ...
    pub fn error(&self, idx: u32, expected: &str) -> ArrowError {
        if resource::current().is_some() {
            return resource::json_error(format_args!("expected {expected} at tape index {idx}"));
        }
        let mut out = String::with_capacity(64);
        self.serialize(&mut out, idx);
        resource::json_error(format_args!("expected {expected} got {out}"))
    }
}
...
impl TapeDecoder {
    ...
    pub fn new(batch_size: usize, num_fields: usize) -> Self {
        Self::try_new(batch_size, num_fields).unwrap()
    }
    pub fn try_new(batch_size: usize, num_fields: usize) -> Result<Self, ArrowError> {
        let fields = resource::checked_mul(num_fields, 2, "JSON tape fields")?;
        let tokens_per_row = resource::checked_add(2, fields, "JSON tape fields")?;
        let mut offsets = resource::new_vec(
            resource::checked_add(
                resource::checked_mul(batch_size, fields, "JSON tape offsets")?,
                1,
                "JSON tape offsets",
            )?,
            "JSON tape offsets",
        ...
        let mut elements = resource::new_vec(
            resource::checked_mul(batch_size, tokens_per_row, "JSON tape elements")?.max(1),
            "JSON tape elements",
        )?;
        ...
        Ok(Self {
            ...
            batch_size,
            cur_row: 0,
            bytes: resource::new_vec(
                resource::checked_mul(fields, 8, "JSON string tape")?,
                "JSON string tape",
            )?,
            stack: resource::new_vec(0, "JSON decoder stack")?,
        })
    }
    ...
    pub fn decode(&mut self, buf: &[u8]) -> Result<usize, ArrowError> {
        ...
        while !iter.is_empty() {
            // A state transition emits at most one token and one string offset.
            // The object-key transition pushes three decoder states.
            resource::reserve_vec(&mut self.elements, 1, "JSON tape elements")?;
            resource::reserve_vec(&mut self.offsets, 1, "JSON tape offsets")?;
            let stack_growth = match self.stack.last() {
                Some(DecoderState::Object(_)) => 3,
                None
                | Some(DecoderState::List(_) | DecoderState::String | DecoderState::Escape) => 1,
                _ => 0,
            };
            resource::reserve_vec(&mut self.stack, stack_growth, "JSON decoder stack")?;
            ...
            match state {
                ...
                DecoderState::String => {
                    ...
                    resource::reserve_vec(&mut self.bytes, s.len(), "JSON string tape")?;
                ...
                state @ DecoderState::Value => {
                    ...
                    *state = match next!(iter) {
                        ...
                        b @ b'-' | b @ b'0'..=b'9' => {
                            resource::reserve_vec(&mut self.bytes, 1, "JSON string tape")?;
                            self.bytes.push(b);
                            DecoderState::Number
                        }
    ...
    pub fn finish(&self) -> Result<Tape<'_>, ArrowError> {
        ...
        let strings = simdutf8::basic::from_utf8(&self.bytes)
            .map_err(|_| resource::json_error(format_args!("Encountered non-UTF-8 data")))?;

        for offset in self.offsets.iter().copied() {
            if !strings.is_char_boundary(offset) {
                return Err(resource::json_error(format_args!(
                    "Encountered truncated UTF-8 sequence"
                )));
            }
        }
    ...
    match (low, high) {
        (0xDC00..=0xDFFF, 0xD800..=0xDBFF) => {
            ...
            char::from_u32(n).ok_or_else(|| {
                resource::json_error(format_args!("Invalid UTF-16 surrogate pair {n}"))
            })
        }
        _ => Err(resource::json_error(format_args!(
            "Invalid UTF-16 surrogate pair. High: {high:#X}, Low: {low:#X}"
        ))),
    }
}
