...
impl ArrayDecoder for MapArrayDecoder {
    fn decode(&mut self, tape: &Tape<'_>, pos: &[u32]) -> Result<ArrayRef, ArrowError> {
        resource::buffer(
            resource::checked_add(pos.len(), 1, "JSON map offsets")?,
            4,
            "JSON map offsets",
        )?;
        ...
        let mut key_pos = resource::new_vec(pos.len(), "JSON child positions")?;
        let mut value_pos = resource::new_vec(pos.len(), "JSON child positions")?;
        ...
        for p in pos.iter().copied() {
            ...
            while cur_idx < end_idx {
                ...
                resource::reserve_vec(&mut key_pos, 1, "JSON child positions")?;
                resource::reserve_vec(&mut value_pos, 1, "JSON child positions")?;
                key_pos.push(key);
                value_pos.push(value);
            }

            let offset = i32::from_usize(key_pos.len()).ok_or_else(|| {
                resource::json_error(format_args!("offset overflow decoding MapArray"))
            })?;
        ...
        resource::reserve(2 * std::mem::size_of::<ArrayRef>(), "JSON map columns")?;
        resource::collection_error_capacity(self.key_value_fields[0].name())?;
        resource::collection_error_capacity(self.key_value_fields[1].name())?;
        ...
        resource::collection_error_capacity(self.entries_field.name())?;
