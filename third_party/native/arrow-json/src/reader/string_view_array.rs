...
impl ArrayDecoder for StringViewArrayDecoder {
    fn decode(&mut self, tape: &Tape<'_>, pos: &[u32]) -> Result<ArrayRef, ArrowError> {
        ...
        for &p in pos {
            ...
            match tape.get(p) {
                TapeElement::String(idx) => {
                    ...
                    if s.len() > 12 {
                        data_capacity =
                            resource::checked_add(data_capacity, s.len(), "JSON string backing")?;
                    }
                }
                TapeElement::Null => {
                ...
                TapeElement::F32(_) if coerce => {
                    data_capacity =
                        resource::checked_add(data_capacity, 10, "JSON string backing")?;
                }
        ...
        let mut builder = if resource::current().is_some() {
            ...
            GenericByteViewBuilder::<StringViewType>::with_capacity(pos.len())
                .with_fixed_block_size(block)
        } else {
