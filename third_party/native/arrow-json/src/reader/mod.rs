...
use crate::resource::{self, ReaderResourcePolicy};
...
pub struct ReaderBuilder {
    ...
    schema: SchemaRef,
    // Native backing and decoder metadata must drop before their receipts.
...
impl ReaderBuilder {
    ...
    /// Attach opt-in allocation admission to construction and every synchronous decode call.
    /// Keep a policy clone with application-owned metadata after the decoder is dropped.
    pub fn with_resource_policy(mut self, policy: ReaderResourcePolicy) -> Self {
    ...
    /// Create a [`Decoder`]
    pub fn build_decoder(mut self) -> Result<Decoder, ArrowError> {
        self.resource_policy = resource::effective_policy(self.resource_policy.as_ref());
        let policy = self.resource_policy.clone();
        resource::scoped(policy.as_ref(), || {
            self.build_decoder_inner().map_err(resource::own_error)
        })
    }

    fn build_decoder_inner(self) -> Result<Decoder, ArrowError> {
        ...
        resource::schema_fields(&self.schema.fields, 0)?;
        let decoder = ctx.make_decoder(data_type.as_ref(), nullable)?;

        // Count without allocating the temporary flattened_fields vector.
        let num_fields = resource::schema_fields(&self.schema.fields, 0)?;

        Ok(Decoder {
            decoder,
            is_field: self.is_field,
            tape_decoder: TapeDecoder::try_new(self.batch_size, num_fields)?,
...
impl Decoder {
    ...
    pub fn decode(&mut self, buf: &[u8]) -> Result<usize, ArrowError> {
        resource::validate_reader_owner(self.resource_policy.as_ref())?;
        resource::scoped(self.resource_policy.as_ref(), || {
            self.tape_decoder.decode(buf).map_err(resource::own_error)
        })
    }
    ...
    pub fn serialize<S: Serialize>(&mut self, rows: &[S]) -> Result<(), ArrowError> {
        ...
        if self.resource_policy.is_some() {
            return Err(resource::ResourceExhausted {
                kind: "JSON Serialize allocation path",
                requested: 1,
                limit: 0,
            }
            .into());
        }
        resource::scoped(None, || self.tape_decoder.serialize(rows))
    }
    ...
    pub fn flush(&mut self) -> Result<Option<RecordBatch>, ArrowError> {
        ...
        let policy = self.resource_policy.clone();
        resource::scoped(policy.as_ref(), || {
            self.flush_inner().map_err(resource::own_error)
        })
    }

    fn flush_inner(&mut self) -> Result<Option<RecordBatch>, ArrowError> {
        ...
        let mut pos = resource::new_vec(tape.num_rows(), "JSON row positions")?;
        for _ in 0..tape.num_rows() {
            let next = tape.next(next_object, "row")?;
            pos.push(std::mem::replace(&mut next_object, next));
        }
        resource::reserve(
            std::mem::size_of::<ArrayRef>() * self.schema.fields.len(),
            "JSON record batch columns",
        )?;

        let decoded = self.decoder.decode(&tape, &pos)?;
        resource::claim(decoded.as_ref())?;
        self.tape_decoder.clear();

        if self.is_field
            && !self.schema.fields[0].is_nullable()
            && decoded.null_count() > 0
            && resource::current().is_some()
        {
            return Err(resource::json_error(format_args!(
                "non-nullable root field contains nulls"
            )));
        }
        let batch = match self.is_field {
            true => RecordBatch::try_new(self.schema.clone(), vec![decoded])?,
            false => {
                let value = decoded.as_struct();
                RecordBatch::try_new_with_options(
                    self.schema.clone(),
                    value.columns().to_vec(),
                    &arrow_array::RecordBatchOptions::new().with_row_count(Some(value.len())),
                )?
            }
        };
...
pub(crate) trait ArrayDecoder: Send {
...
fn make_decoder(
    ...
    if resource::current().is_some() {
        match data_type {
            DataType::FixedSizeBinary(n) | DataType::FixedSizeList(_, n) if *n < 0 => {
                return Err(resource::ResourceExhausted {
                    kind: "JSON negative native geometry",
                    requested: 1,
                    limit: 0,
                }
                .into());
            ...
            DataType::Map(field, ordered)
                if *ordered
                    || !matches!(field.data_type(), DataType::Struct(fields) if fields.len() == 2) =>
            {
                return Err(resource::ResourceExhausted {
                    kind: "JSON invalid map decoder geometry",
                    requested: 1,
                    limit: 0,
                }
                .into());
    ...
    if resource::current().is_some() && matches!(data_type, DataType::RunEndEncoded(..)) {
        return Err(crate::resource::ResourceExhausted {
            kind: "JSON RunEndEncoded allocation path",
            requested: 1,
            limit: 0,
        }
        .into());
    ...
    macro_rules! primitive_decoder {
        ($t:ty, $data_type:expr) => {
            resource::boxed(PrimitiveArrayDecoder::<$t>::new(ctx, $data_type))
        };
    }
    macro_rules! timestamp_decoder {
        ($t:ty, $data_type:expr, $tz:expr) => {{ resource::boxed(TimestampArrayDecoder::<$t, _>::new(ctx, $data_type, $tz)) }};
    }
    macro_rules! decimal_decoder {
        ($t:ty, $p:expr, $s:expr) => {
            resource::boxed(DecimalArrayDecoder::<$t>::new(ctx, $p, $s))
        };
    }

    downcast_integer! {
        *data_type => (primitive_decoder, data_type),
        DataType::Null => resource::boxed(NullArrayDecoder::new(ctx)),
        ...
        DataType::Timestamp(TimeUnit::Second, Some(ref tz)) => {
            resource::parser_error_capacity(tz.len())?;
            let tz: Tz = tz.parse()?;
        ...
        DataType::Boolean => resource::boxed(BooleanArrayDecoder::new(ctx)),
        DataType::Utf8 => resource::boxed(StringArrayDecoder::<i32>::new(ctx)),
        DataType::Utf8View => resource::boxed(StringViewArrayDecoder::new(ctx)),
        DataType::LargeUtf8 => resource::boxed(StringArrayDecoder::<i64>::new(ctx)),
        DataType::List(_) => resource::boxed(ListArrayDecoder::<i32>::new(ctx, data_type, is_nullable)?),
        DataType::LargeList(_) => resource::boxed(ListArrayDecoder::<i64>::new(ctx, data_type, is_nullable)?),
        DataType::ListView(_) => resource::boxed(ListViewArrayDecoder::<i32>::new(ctx, data_type, is_nullable)?),
        DataType::LargeListView(_) => resource::boxed(ListViewArrayDecoder::<i64>::new(ctx, data_type, is_nullable)?),
        DataType::FixedSizeList(_, _) => resource::boxed(FixedSizeListArrayDecoder::new(ctx, data_type, is_nullable)?),
        DataType::Struct(_) => resource::boxed(StructArrayDecoder::new(ctx, data_type, is_nullable)?),
        DataType::Binary => resource::boxed(BinaryArrayDecoder::<i32>::default()),
        DataType::LargeBinary => resource::boxed(BinaryArrayDecoder::<i64>::default()),
        DataType::FixedSizeBinary(len) => resource::boxed(FixedSizeBinaryArrayDecoder::new(len)),
        DataType::BinaryView => resource::boxed(BinaryViewDecoder::default()),
        DataType::Map(_, _) => resource::boxed(MapArrayDecoder::new(ctx, data_type, is_nullable)?),
        DataType::RunEndEncoded(ref r, _) => match r.data_type() {
            DataType::Int16 => resource::boxed(RunEndEncodedArrayDecoder::<Int16Type>::new(ctx, data_type, is_nullable)?),
            DataType::Int32 => resource::boxed(RunEndEncodedArrayDecoder::<Int32Type>::new(ctx, data_type, is_nullable)?),
            DataType::Int64 => resource::boxed(RunEndEncodedArrayDecoder::<Int64Type>::new(ctx, data_type, is_nullable)?),
            d => unreachable!("unsupported run end index type: {d}"),
        },
        _ if resource::current().is_some() => Err(resource::ResourceExhausted { kind: "JSON unsupported native decoder allocation path", requested: 1, limit: 0 }.into()),
