...
pub enum ParquetError {
    ...
    General(String),
    /// An opt-in native reader resource admission was denied.
...
impl std::fmt::Display for ParquetError {
    fn fmt(&self, fmt: &mut std::fmt::Formatter) -> std::fmt::Result {
        match &self {
            ParquetError::ResourceExhausted(error) => {
                write!(fmt, "Reader resource exhausted: {error}")
            }
...
impl Error for ParquetError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            ...
            ParquetError::ResourceExhausted(e) => Some(e),
            _ => None,
        }
    }
}

impl From<crate::resource::ResourceExhausted> for ParquetError {
...
impl From<io::Error> for ParquetError {
    fn from(e: io::Error) -> ParquetError {
        if e.kind() == io::ErrorKind::OutOfMemory {
            if let Some(error) = crate::resource::io_resource_failure() {
                return ParquetError::ResourceExhausted(error);
            }
        }
...
impl From<ArrowError> for ParquetError {
    fn from(e: ArrowError) -> ParquetError {
        if let ArrowError::ResourceOwnerError(error) = e {
            return ParquetError::ResourceExhausted(crate::resource::ResourceExhausted {
                kind: error.kind,
...
impl From<ParquetError> for ArrowError {
    fn from(p: ParquetError) -> Self {
        match p {
            ...
            other => Self::ParquetError(format!("{other}")),
        }
    }
}
