...
impl RleEncoder {
    ...
    /// Preflight a native append including pending run/group values and its flush.
    pub fn try_reserve_values(&mut self, additional: usize) -> crate::errors::Result<()> {
        let Some(policy) = crate::resource::current() else { return Ok(()); };
        let count = additional.checked_add(self.num_buffered_values)
            .and_then(|n| n.checked_add(self.repeat_count))
            .ok_or(crate::resource::ResourceExhausted { kind: "writer RLE values", requested: usize::MAX, limit: policy.limits().output_values })?;
        crate::resource::ReaderResourcePolicy::check(count, policy.limits().output_values, "writer RLE values")?;
        let groups = count.checked_add(BIT_PACK_GROUP_SIZE - 1).map(|n| n / BIT_PACK_GROUP_SIZE)
            .ok_or(crate::resource::ResourceExhausted { kind: "writer RLE layout", requested: usize::MAX, limit: isize::MAX as usize })?;
        // Exact upper bound used by max_buffer_size, plus ten bytes for a
        // pending 64-bit VLQ run-length prefix at an append boundary.
        let bytes = groups.checked_mul(1 + usize::from(self.bit_width)).and_then(|n| n.checked_add(10))
            .ok_or(crate::resource::ResourceExhausted { kind: "writer RLE layout", requested: usize::MAX, limit: isize::MAX as usize })?;
        self.bit_writer.try_reserve_bytes(bytes)
    }
