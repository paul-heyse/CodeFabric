...
impl<T: DataType> Encoder<T> for ByteStreamSplitEncoder<T> {
    fn put(&mut self, values: &[T::T]) -> Result<()> {
        crate::resource::extend_writer_bytes(&mut self.buffer, <T as DataType>::T::slice_as_bytes(values))?;
    ...
    fn flush_buffer(&mut self) -> Result<Bytes> {
        ...
        crate::resource::reserve_vec(&mut encoded, self.buffer.len(), "writer split output")?;
        encoded.resize(self.buffer.len(), 0);
        ...
        self.buffer.clear();
        crate::resource::own_writer_vec(encoded)
    }
...
impl<T: DataType> Encoder<T> for VariableWidthByteStreamSplitEncoder<T> {
    fn put(&mut self, values: &[T::T]) -> Result<()> {
        ...
        let data_len = values.len().checked_mul(self.type_width).ok_or(crate::resource::ResourceExhausted { kind: "writer split layout", requested: usize::MAX, limit: isize::MAX as usize })?;
        // Ensure enough capacity for the new data
        crate::resource::reserve_vec(&mut self.buffer, data_len, "writer split values")?;
