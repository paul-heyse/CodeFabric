...
pub fn get_encoder<T: DataType>(
    ...
    let encoder: Box<dyn Encoder<T>> = match encoding {
        Encoding::PLAIN => {
            if let Some(policy) = crate::resource::current() { policy.reserve(std::mem::size_of::<PlainEncoder<T>>(), "writer encoder owner")?; }
            Box::new(PlainEncoder::try_new()?)
        },
        ...
        Encoding::RLE => {
            if let Some(policy) = crate::resource::current() { policy.reserve(std::mem::size_of::<RleValueEncoder<T>>(), "writer encoder owner")?; }
            Box::new(RleValueEncoder::new())
        },
        Encoding::DELTA_BINARY_PACKED => {
            if let Some(policy) = crate::resource::current() { policy.reserve(std::mem::size_of::<DeltaBitPackEncoder<T>>(), "writer encoder owner")?; }
            Box::new(DeltaBitPackEncoder::try_new()?)
        },
        Encoding::DELTA_LENGTH_BYTE_ARRAY => {
            if let Some(policy) = crate::resource::current() { policy.reserve(std::mem::size_of::<DeltaLengthByteArrayEncoder<T>>(), "writer encoder owner")?; }
            Box::new(DeltaLengthByteArrayEncoder::try_new()?)
        },
        Encoding::DELTA_BYTE_ARRAY => {
            if let Some(policy) = crate::resource::current() { policy.reserve(std::mem::size_of::<DeltaByteArrayEncoder<T>>(), "writer encoder owner")?; }
            Box::new(DeltaByteArrayEncoder::try_new()?)
        },
        Encoding::BYTE_STREAM_SPLIT => {
            if let Some(policy) = crate::resource::current() {
                let bytes = if T::get_physical_type() == Type::FIXED_LEN_BYTE_ARRAY { std::mem::size_of::<VariableWidthByteStreamSplitEncoder<T>>() } else { std::mem::size_of::<ByteStreamSplitEncoder<T>>() };
                policy.reserve(bytes, "writer encoder owner")?;
            }
...
impl<T: DataType> PlainEncoder<T> {
    ...
    pub fn try_new() -> Result<Self> {
        Ok(Self { buffer: vec![], bit_writer: BitWriter::try_new(256)?, _phantom: PhantomData })
    }
}
...
impl<T: DataType> Encoder<T> for PlainEncoder<T> {
    ...
    #[inline]
    fn flush_buffer(&mut self) -> Result<Bytes> {
        self.bit_writer.try_reserve_bytes(0)?;
        crate::resource::extend_writer_bytes(&mut self.buffer, self.bit_writer.flush_buffer())?;
        self.bit_writer.clear();
        crate::resource::own_writer_vec(std::mem::take(&mut self.buffer))
    }
    ...
    fn put(&mut self, values: &[T::T]) -> Result<()> {
        if crate::resource::current().is_some() {
            if T::get_physical_type() == Type::BOOLEAN {
                self.bit_writer.try_reserve_bytes(values.len().div_ceil(8))?;
            } else {
                let bytes = values.iter().try_fold(0usize, |total, value| {
                    let (base, payload) = value.dict_encoding_size();
                    let bytes = match T::get_physical_type() {
                        Type::BYTE_ARRAY => base.checked_add(payload)?,
                        Type::FIXED_LEN_BYTE_ARRAY => payload,
                        _ => base,
                    };
                    total.checked_add(bytes)
                }).ok_or(crate::resource::ResourceExhausted { kind: "writer plain bytes", requested: usize::MAX, limit: isize::MAX as usize })?;
                crate::resource::reserve_vec(&mut self.buffer, bytes, "writer plain bytes")?;
            }
        }
...
impl<T: DataType> Encoder<T> for RleValueEncoder<T> {
    ...
    fn put(&mut self, values: &[T::T]) -> Result<()> {
        ...
        if self.encoder.is_none() {
            ...
            crate::resource::reserve_vec(&mut buffer, DEFAULT_RLE_BUFFER_LEN, "writer RLE buffer")?;
            buffer.extend_from_slice(&[0; 4]);
            self.encoder = Some(RleEncoder::new_from_buf(1, buffer));
        }
        let rle_encoder = self.encoder.as_mut().unwrap();
        rle_encoder.try_reserve_values(values.len())?;
        for value in values {
    ...
    fn flush_buffer(&mut self) -> Result<Bytes> {
        ...
        crate::resource::own_writer_vec(buf)
    }
...
impl<T: DataType> DeltaBitPackEncoder<T> {
    ...
    /// Construct bounded native state; grow the large default byte arena lazily
    /// under admission instead of allocating its ordinary 1 MiB hint eagerly.
    pub fn try_new() -> Result<Self> {
        if crate::resource::current().is_none() { return Ok(Self::new()); }
        Self::assert_supported_type();
        let mini_block_size = match T::T::PHYSICAL_TYPE { Type::INT32 => 32, Type::INT64 => 64, _ => unreachable!() };
        let block_size = mini_block_size * DEFAULT_NUM_MINI_BLOCKS;
        let mut deltas = Vec::new();
        crate::resource::reserve_vec(&mut deltas, block_size, "writer delta block")?;
        deltas.resize(block_size, 0);
        Ok(Self {
            page_header_writer: BitWriter::try_new(MAX_PAGE_HEADER_WRITER_SIZE)?,
            bit_writer: BitWriter::try_new(0)?, total_values: 0, first_value: 0,
            current_value: 0, block_size, mini_block_size, num_mini_blocks: DEFAULT_NUM_MINI_BLOCKS,
            values_in_block: 0, deltas, _phantom: PhantomData,
        })
    }
    ...
    fn flush_block_values(&mut self) -> Result<()> {
        ...
        // One min-delta VLQ (at most 10 bytes), one bit-width byte per
        // mini-block, and each padded native value at most 64 bits.
        if self.values_in_block > 0 {
            self.bit_writer.try_reserve_bytes(10 + self.num_mini_blocks + self.block_size * 8)?;
        }
...
impl<T: DataType> Encoder<T> for DeltaBitPackEncoder<T> {
    ...
    fn flush_buffer(&mut self) -> Result<Bytes> {
        ...
        self.page_header_writer.try_reserve_bytes(MAX_PAGE_HEADER_WRITER_SIZE)?;
        self.bit_writer.try_reserve_bytes(0)?;
        let mut buffer = Vec::new();
        crate::resource::extend_writer_bytes(&mut buffer, self.page_header_writer.flush_buffer())?;
        crate::resource::extend_writer_bytes(&mut buffer, self.bit_writer.flush_buffer())?;
        ...
        crate::resource::own_writer_vec(buffer)
    }
...
impl<T: DataType> DeltaLengthByteArrayEncoder<T> {
    ...
    pub fn try_new() -> Result<Self> {
        Ok(Self { len_encoder: DeltaBitPackEncoder::try_new()?, data: vec![], encoded_size: 0, _phantom: PhantomData })
    }
}
...
impl<T: DataType> Encoder<T> for DeltaLengthByteArrayEncoder<T> {
    fn put(&mut self, values: &[T::T]) -> Result<()> {
        ...
        crate::resource::reserve_vec(&mut lengths, values.len(), "writer delta lengths")?;
        crate::resource::reserve_vec(&mut self.data, values.len(), "writer delta values")?;
        lengths.extend(val_it().map(|byte_array| byte_array.len() as i32));
    ...
    fn flush_buffer(&mut self) -> Result<Bytes> {
        ...
        crate::resource::extend_writer_bytes(&mut total_bytes, &lengths)?;
        for byte_array in &self.data {
            crate::resource::extend_writer_bytes(&mut total_bytes, byte_array.data())?;
        }
        self.data.clear();
        self.encoded_size = 0;

        crate::resource::own_writer_vec(total_bytes)
    }
...
impl<T: DataType> DeltaByteArrayEncoder<T> {
    ...
    pub fn try_new() -> Result<Self> {
        Ok(Self { prefix_len_encoder: DeltaBitPackEncoder::try_new()?, suffix_writer: DeltaLengthByteArrayEncoder::try_new()?, previous: vec![], _phantom: PhantomData })
    }
}
...
impl<T: DataType> Encoder<T> for DeltaByteArrayEncoder<T> {
    fn put(&mut self, values: &[T::T]) -> Result<()> {
        ...
        crate::resource::reserve_vec(&mut prefix_lengths, values.len(), "writer delta prefixes")?;
        crate::resource::reserve_vec(&mut suffixes, values.len(), "writer delta suffixes")?;
        ...
        for byte_array in values {
            ...
            self.previous.clear();
            crate::resource::extend_writer_bytes(&mut self.previous, current)?;
        }
    ...
    fn flush_buffer(&mut self) -> Result<Bytes> {
        match T::get_physical_type() {
            Type::BYTE_ARRAY | Type::FIXED_LEN_BYTE_ARRAY => {
                ...
                crate::resource::extend_writer_bytes(&mut total_bytes, &suffixes)?;
