...
impl<T: DataType> Storage for KeyStorage<T> {
    ...
    fn try_push(&mut self, value: &Self::Value) -> Result<Self::Key> {
        crate::resource::reserve_vec(&mut self.uniques, 1, "writer dictionary values")?;
        Ok(self.push(value))
    }
...
impl<T: DataType> DictEncoder<T> {
    ...
    pub fn try_new(desc: ColumnDescPtr) -> Result<Self> {
        Ok(Self {
            interner: Interner::try_new(KeyStorage { uniques: vec![], size_in_bytes: 0, type_length: desc.type_length() as usize })?,
            indices: vec![],
        })
    }
    ...
    pub fn write_dict(&self) -> Result<Bytes> {
        let mut plain_encoder = PlainEncoder::<T>::try_new()?;
    ...
    pub fn write_indices(&mut self) -> Result<Bytes> {
        ...
        crate::resource::reserve_vec(&mut buffer, buffer_len.saturating_add(1), "writer dictionary RLE buffer")?;
        ...
        self.indices.clear();
        crate::resource::own_writer_vec(encoder.consume())
    }

    fn put_one(&mut self, value: &T::T) -> Result<()> {
        self.indices.push(self.interner.try_intern(value)?);
        Ok(())
    }
...
impl<T: DataType> Encoder<T> for DictEncoder<T> {
    fn put(&mut self, values: &[T::T]) -> Result<()> {
        crate::resource::reserve_vec(&mut self.indices, values.len(), "writer dictionary indices")?;
        for i in values {
            self.put_one(i)?;
        }
        Ok(())
