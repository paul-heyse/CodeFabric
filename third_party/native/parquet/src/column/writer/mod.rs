...
pub fn get_column_writer<'a>(
    ...
    try_get_column_writer(descr, props, page_writer).expect("column writer construction failed")
}

/// Fallible constructor preserving native codec/resource admission failures.
/// Governed callers must use this path instead of the compatibility constructor.
pub fn try_get_column_writer<'a>(
    descr: ColumnDescPtr,
    props: WriterPropertiesPtr,
    page_writer: Box<dyn PageWriter + 'a>,
    ...
    Ok(match descr.physical_type() {
        Type::BOOLEAN => {
            ColumnWriter::BoolColumnWriter(ColumnWriterImpl::try_new(descr, props, page_writer)?)
        }
        Type::INT32 => {
            ColumnWriter::Int32ColumnWriter(ColumnWriterImpl::try_new(descr, props, page_writer)?)
        }
        Type::INT64 => {
            ColumnWriter::Int64ColumnWriter(ColumnWriterImpl::try_new(descr, props, page_writer)?)
        }
        Type::INT96 => {
            ColumnWriter::Int96ColumnWriter(ColumnWriterImpl::try_new(descr, props, page_writer)?)
        }
        Type::FLOAT => {
            ColumnWriter::FloatColumnWriter(ColumnWriterImpl::try_new(descr, props, page_writer)?)
        }
        Type::DOUBLE => {
            ColumnWriter::DoubleColumnWriter(ColumnWriterImpl::try_new(descr, props, page_writer)?)
        }
        Type::BYTE_ARRAY => ColumnWriter::ByteArrayColumnWriter(ColumnWriterImpl::try_new(
            descr,
            props,
            page_writer,
        )?),
        Type::FIXED_LEN_BYTE_ARRAY => ColumnWriter::FixedLenByteArrayColumnWriter(
            ColumnWriterImpl::try_new(descr, props, page_writer)?,
        ),
    })
}
    ...
    pub fn update_dictionary_location(mut self, dictionary_len: usize) -> Result<Self> {
        if dictionary_len > 0 {
            ...
            if let Some(offset_index) = self.offset_index.as_mut() {
                ...
                for location in offset_index.page_locations.try_values_mut()?.iter_mut() {
...
pub struct GenericColumnWriter<'a, E: ColumnValueEncoder> {
    ...
    /// the native EncodingMask preserves deterministic encoding order without allocation
    ...
    // Drop all native writer fields before the policy receipts. Escaping page Bytes also
    // retain this exact original owner independently of the column writer.
...
impl<'a, E: ColumnValueEncoder> GenericColumnWriter<'a, E> {
    ...
    pub fn new(
        ...
        Self::try_new(descr, props, page_writer).expect("column writer construction failed")
    }

    /// Construct a column writer without unwrapping fallible native codec/encoder admission.
    pub fn try_new(
        descr: ColumnDescPtr,
        props: WriterPropertiesPtr,
        page_writer: Box<dyn PageWriter + 'a>,
        ...
        if resource_policy.is_some() {
            if props.bloom_filter_properties(descr.path()).is_some() {
                return Err(crate::resource::ResourceExhausted { kind: "writer bloom filter outside admitted profile", requested: 1, limit: 0 }.into());
            }
            if matches!(descr.logical_type_ref(), Some(LogicalType::Geometry { .. } | LogicalType::Geography { .. })) {
                return Err(crate::resource::ResourceExhausted { kind: "writer geospatial statistics outside admitted profile", requested: 1, limit: 0 }.into());
            }
        }
        let codec = props.compression(descr.path());
        let codec_options = CodecOptionsBuilder::default().build();
        let compressor = create_codec(codec, &codec_options)?;
        let encoder = E::try_new(&descr, props.as_ref())?;
        if let Some(policy) = &resource_policy {
            // Both V1 level encoders begin with the native four-byte prefix.
            if props.writer_version() == WriterVersion::PARQUET_1_0 {
                policy.reserve(4, "writer level prefix")?;
                policy.reserve(4, "writer level prefix")?;
            }
            if props.statistics_enabled(descr.path()) != EnabledStatistics::None {
                for max in [descr.max_rep_level(), descr.max_def_level()] {
                    if max > 0 {
                        let bytes = (max as usize + 1) * std::mem::size_of::<i64>();
                        policy.reserve(bytes, "writer level histogram")?;
                        policy.reserve(bytes, "writer level histogram")?;
                    }
                }
            }
        ...
        let mut encodings = EncodingMask::default();
    ...
    pub(crate) fn write_batch_internal(
        ...
        let _resource_scope = crate::resource::enter(self.resource_policy.clone());
    ...
    pub fn close(mut self) -> Result<ColumnCloseResult> {
        ...
        let offset_index = self.offset_index_builder.map(|b| b.try_build()).transpose()?;
    ...
    fn write_mini_batch(
        ...
        if let Some(policy) = crate::resource::current() {
            crate::resource::ReaderResourcePolicy::check(num_levels, policy.limits().page_values, "writer batch values")?;
        }
        self.def_levels_encoder.try_reserve_values(num_levels)?;
        self.rep_levels_encoder.try_reserve_values(num_levels)?;
    ...
    fn update_column_offset_index(
        ...
        if self.column_index_builder.valid() || self.page_metrics.num_buffered_values == self.page_metrics.num_page_nulls as u32 {
            self.column_index_builder.try_reserve_page()?;
            if let Some(stat) = page_statistics {
                if let Some(policy) = crate::resource::current() {
                    for value in [stat.min_bytes_opt(), stat.max_bytes_opt()].into_iter().flatten() {
                        // Truncation copies at most the original length; failed
                        // byte increment may allocate once then copy original.
                        policy.reserve(value.len(), "writer index statistic bytes")?;
                        if self.can_truncate_value() { policy.reserve(value.len(), "writer index statistic retry")?; }
                    }
                }
            }
        }
        ...
        self.column_index_builder.try_append_histograms(
        ...
        if let Some(builder) = self.offset_index_builder.as_mut() {
            builder.try_reserve_page(page_variable_length_bytes.is_some())?;
    ...
    fn truncate_statistics(&self, statistics: Statistics) -> Result<Statistics> {
        if let Some(policy) = crate::resource::current() {
            if matches!(&statistics, Statistics::ByteArray(_) | Statistics::FixedLenByteArray(_)) {
                for bytes in [statistics.min_bytes_opt(), statistics.max_bytes_opt()].into_iter().flatten() {
                    crate::resource::ReaderResourcePolicy::check(bytes.len(), policy.limits().string_bytes, "writer statistic bytes")?;
                    // truncate_min makes one copy. truncate_max can first copy
                    // the truncation prefix, fail increment, then copy all bytes.
                    policy.reserve(bytes.len(), "writer statistic copy")?;
                    policy.reserve(bytes.len(), "writer statistic truncation fallback")?;
                }
            }
        }
        ...
        Ok(match statistics {
            Statistics::ByteArray(stats) if stats._internal_has_min_max_set() => {
                ...
                Statistics::ByteArray(
                    ValueStatistics::new(
                        Some(crate::data_type::ByteArray::from(crate::resource::own_writer_vec(min)?).into()),
                        Some(crate::data_type::ByteArray::from(crate::resource::own_writer_vec(max)?).into()),
    ...
    pub(crate) fn add_data_page(&mut self) -> Result<()> {
        ...
        let page_statistics = page_statistics
            ...
            .map(|stats| self.truncate_statistics(Statistics::from(stats))).transpose()?;
            ...
            WriterVersion::PARQUET_1_0 => {
                ...
                if max_rep_level > 0 {
                    self.rep_levels_encoder
                        .flush_to(|data| crate::resource::extend_writer_bytes(&mut buffer, data))?;
                }
                ...
                crate::resource::extend_writer_bytes(&mut buffer, &values_data.buf)?;
                let uncompressed_size = buffer.len();

                if let Some(ref mut cmpr) = self.compressor {
                    let mut compressed_buf = Vec::new();
                    if self.resource_policy.is_none() {
                        compressed_buf.reserve(uncompressed_size);
                    }
                ...
                let data_page = Page::DataPage {
                    buf: crate::resource::own_writer_vec(buffer)?,
        ...
        if self.encoder.has_dictionary() && !self.page_writer.defers_dictionary_ordering() {
            if crate::resource::current().is_some() && self.data_pages.len() == self.data_pages.capacity() {
                let count = self.data_pages.len().checked_add(1).ok_or(crate::resource::ResourceExhausted { kind: "writer pending pages", requested: usize::MAX, limit: isize::MAX as usize })?;
                crate::resource::admit_writer_slots::<CompressedPage>(count, "writer pending pages")?;
                self.data_pages.try_reserve_exact(1).map_err(|_| crate::resource::ResourceExhausted { kind: "writer pending pages allocation", requested: count, limit: count })?;
            }
    ...
    fn build_column_metadata(&mut self) -> Result<ColumnChunkMetaData> {
        ...
        let mut builder = ColumnChunkMetaData::builder(self.descr.clone())
            ...
            .set_encodings_mask(self.encodings)
            .set_page_encoding_stats(std::mem::take(&mut self.encoding_stats))
    ...
    fn write_data_page(&mut self, page: CompressedPage) -> Result<()> {
        ...
        match self.encoding_stats.last_mut() {
            ...
            _ => {
                ...
                crate::resource::reserve_vec(&mut self.encoding_stats, 1, "writer encoding statistics")?;
    ...
    fn write_dictionary_page(&mut self) -> Result<()> {
        ...
        let compressed_page = {
            ...
            if let Some(ref mut cmpr) = self.compressor {
                ...
                if self.resource_policy.is_none() {
                    output_buf.reserve(uncompressed_size);
                }
                cmpr.compress(&page.buf, &mut output_buf)?;
                page.buf = crate::resource::own_writer_vec(output_buf)?;
            } else if let Some(policy) = &self.resource_policy {
                policy.admit_bytes_owner()?;
                page.buf = policy.own_bytes(page.buf);
            }
