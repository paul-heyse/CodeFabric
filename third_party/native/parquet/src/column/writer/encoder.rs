...
impl<T: DataType> ColumnValueEncoder for ColumnValueEncoderImpl<T> {
    ...
    fn try_new(descr: &ColumnDescPtr, props: &WriterProperties) -> Result<Self> {
        ...
        let dict_encoder = if dict_supported { Some(DictEncoder::try_new(descr.clone())?) } else { None };
    ...
    fn write_gather(&mut self, values: &Self::Values, indices: &[usize]) -> Result<()> {
        ...
        crate::resource::reserve_vec(&mut slice, indices.len(), "writer gathered values")?;
        slice.extend(indices.iter().map(|idx| values[*idx].clone()));
        self.write_slice(&slice)
    }
