...
impl<T: DataType> ColumnValueDecoder for ColumnValueDecoderImpl<T> {
    ...
    fn read(&mut self, out: &mut Self::Buffer, num_values: usize) -> Result<usize> {
        ...
        let start = out.len();
        crate::resource::reserve_vec(out, num_values, "primitive decoded values")?;
...
impl DefinitionLevelDecoder for DefinitionLevelDecoderImpl {
    fn read_def_levels(
        ...
        // TODO: Push vec into decoder (#5177)
        let start = out.len();
        crate::resource::reserve_vec(out, num_levels, "decoded levels")?;
    ...
    fn skip_def_levels(&mut self, num_levels: usize) -> Result<(usize, usize)> {
        ...
        while level_skip < num_levels {
            ...
            let additional = to_read.saturating_sub(buf.len());
            crate::resource::reserve_vec(&mut buf, additional, "level decoder scratch")?;
