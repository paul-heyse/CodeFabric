...
impl Default for InMemoryPageStore {
    fn default() -> Self {
        Self { blobs: Vec::new(), resident: 0, resource_policy: crate::resource::current() }
    }
}
...
impl PageStore for InMemoryPageStore {
    fn put(&mut self, value: Bytes) -> Result<PageKey> {
        ...
        crate::resource::reserve_vec(&mut self.blobs, 1, "writer page store slots")?;
...
impl PageStoreFactory for InMemoryPageStoreFactory {
    fn create(&self, _args: &PageStoreArgs<'_>) -> Result<Box<dyn PageStore>> {
        if let Some(policy) = crate::resource::current() {
            policy.reserve(std::mem::size_of::<InMemoryPageStore>(), "writer page store owner")?;
        }
