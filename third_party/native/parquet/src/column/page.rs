...
impl CompressedPage {
    ...
    pub(crate) fn to_thrift_header(&self) -> Result<PageHeader> {
        ...
        // page_stats_to_thrift allocates one min/max Vec each and, for the
        // backwards-compatible representation, a second clone of each. Admit
        // those exact borrowed payload lengths before invoking native conversion.
        if let Some(policy) = crate::resource::current() {
            let statistics = match &self.compressed_page {
                Page::DataPage { statistics, .. } | Page::DataPageV2 { statistics, .. } => statistics.as_ref(),
                Page::DictionaryPage { .. } => None,
            };
            if let Some(stats) = statistics {
                for value in [stats.min_bytes_opt(), stats.max_bytes_opt()].into_iter().flatten() {
                    crate::resource::ReaderResourcePolicy::check(value.len(), policy.limits().string_bytes, "writer page statistics bytes")?;
                    policy.reserve(value.len(), "writer page statistics clone")?;
