...
impl ParquetMetaDataReader {
    ...
    async fn load_page_index_with_remainder<F: MetadataFetch>(
        ...
        let bytes = match &remainder {
            ...
            _ => {
                self.admit_encoded_metadata(usize::try_from(range.end - range.start)?)?;
                fetch.fetch(range.start..range.end).await?
            }
        };
    ...
    fn admit_encoded_metadata(&self, bytes: usize) -> Result<()> {
        if let Some(policy) = crate::resource::effective(
            self.metadata_options
                .as_ref()
                .and_then(|o| o.resource_policy())
                .cloned(),
        ) {
            ...
            policy.reserve(bytes, "encoded metadata backing")?;
        }
        Ok(())
    }
    ...
    fn parse_metadata<R: ChunkReader>(&mut self, chunk_reader: &R) -> Result<ParquetMetaData> {
        ...
        self.admit_encoded_metadata(metadata_len)?;
    ...
    async fn load_metadata<F: MetadataFetch>(
        ...
        self.admit_encoded_metadata(usize::try_from(file_size - footer_start)?)?;
        ...
        if length > suffix_len - FOOTER_SIZE {
            ...
            self.admit_encoded_metadata(length)?;
            let meta = fetch
    ...
    async fn load_metadata_via_suffix<F: MetadataSuffixFetch>(
        ...
        self.admit_encoded_metadata(prefetch)?;
        ...
        if length > suffix_len - FOOTER_SIZE {
            self.admit_encoded_metadata(metadata_offset)?;
