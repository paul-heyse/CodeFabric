...
impl ParquetMetaDataPushDecoder {
    ...
    pub fn try_new_with_metadata(file_len: u64, metadata: ParquetMetaData) -> Result<Self> {
        crate::resource::validate_thread_owner(metadata.resource_policy())?;
        let policy = metadata.resource_policy().cloned();
        let _scope = crate::resource::enter(policy.clone());
        let mut new_self = Self::try_new(file_len)?;
        if let Some(policy) = policy {
            let options_layout = std::alloc::Layout::new::<[std::sync::atomic::AtomicUsize; 2]>()
                .extend(std::alloc::Layout::new::<ParquetMetaDataOptions>())
                .map_err(|_| crate::resource::ResourceExhausted {
                    kind: "metadata parser options layout",
                    requested: usize::MAX,
                    limit: isize::MAX as usize,
                })?
                .0
                .pad_to_align();
            policy.reserve(options_layout.size(), "metadata parser options owner")?;
            new_self.metadata_parser =
                new_self
                    .metadata_parser
                    .with_metadata_options(Some(Arc::new(
                        ParquetMetaDataOptions::new().with_resource_policy(policy.clone()),
                    )));
            policy.reserve(
                std::mem::size_of::<ParquetMetaData>(),
                "pushed metadata state",
            )?;
        }
    ...
    pub fn push_ranges(&mut self, ranges: Vec<Range<u64>>, buffers: Vec<Bytes>) -> Result<()> {
        ...
        let policy = self.metadata_parser.resource_policy();
        self.buffers.try_set_resource_policy(policy.clone())?;
        let _scope = crate::resource::enter(policy);
        self.buffers.try_push_ranges(ranges, buffers)?;
        Ok(())
    }
    ...
    pub fn push_range(&mut self, range: Range<u64>, buffer: Bytes) -> Result<()> {
        ...
        self.buffers.try_set_resource_policy(policy.clone())?;
        let _scope = crate::resource::enter(policy);
        self.buffers.try_push_range(range, buffer)?;
        Ok(())
    }
    ...
    pub fn try_decode(&mut self) -> Result<DecodeResult<ParquetMetaData>> {
        ...
        self.buffers.try_set_resource_policy(policy.clone())?;
        let _scope = crate::resource::enter(policy.clone());
        if let DecodeState::ReadingPageIndex(metadata) = &self.state {
            if let Some(policy) = &policy {
                if !metadata
                    .resource_policy()
                    .is_some_and(|original| original.same_owner(policy))
                {
                    return Err(crate::resource::ResourceExhausted {
                        kind: "foreign pushed metadata owner",
                        requested: 1,
                        limit: 0,
                    }
                    .into());
            ...
            match std::mem::replace(&mut self.state, DecodeState::Intermediate) {
                DecodeState::ReadingFooter => {
                    ...
                    if !self.buffers.has_range(&footer_range) {
                        ...
                        return needs_range(footer_range);
                    }
                ...
                DecodeState::ReadingPageIndex(mut metadata) => {
                    ...
                    if !self.buffers.has_range(&page_index_range) {
                        ...
                        return needs_range(page_index_range);
                    }
...
fn needs_range(range: Range<u64>) -> Result<DecodeResult<ParquetMetaData>> {
    ...
    let mut ranges = crate::resource::vec_with_capacity(1, "metadata requested ranges")?;
    ranges.push(range);
    Ok(DecodeResult::NeedsData(ranges))
}
