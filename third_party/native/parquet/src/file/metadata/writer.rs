...
impl<'a, W: Write> ThriftMetadataWriter<'a, W> {
    ...
    fn finalize_column_indexes(&mut self) -> Result<Option<ParquetColumnIndex>> {
        ...
        if !all_none {
            if let Some(groups) = &column_indexes {
                crate::resource::admit_writer_slots::<Vec<ColumnIndexMetaData>>(groups.len(), "writer final column_indexes groups")?;
                for columns in groups { crate::resource::admit_writer_slots::<ColumnIndexMetaData>(columns.len(), "writer final column_indexes columns")?; }
            }
        }
    ...
    fn finalize_offset_indexes(&mut self) -> Result<Option<ParquetOffsetIndex>> {
        ...
        if !all_none {
            if let Some(groups) = &offset_indexes {
                crate::resource::admit_writer_slots::<Vec<OffsetIndexMetaData>>(groups.len(), "writer final offset_indexes groups")?;
                for columns in groups { crate::resource::admit_writer_slots::<OffsetIndexMetaData>(columns.len(), "writer final offset_indexes columns")?; }
            }
        }
    ...
    pub fn finish(mut self) -> Result<ParquetMetaData> {
        ...
        crate::resource::admit_writer_slots::<ColumnOrder>(self.schema_descr.num_columns(), "writer column orders")?;
