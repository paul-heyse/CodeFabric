...
pub(crate) fn parquet_metadata_with_encryption(
    ...
    if options.and_then(|o| o.resource_policy()).is_some()
        && (encrypted_footer || file_decryption_properties.is_some())
    {
        return Err(crate::resource::ResourceExhausted {
            kind: "unsupported bounded encryption profile",
            requested: 1,
            limit: 0,
        }
        .into());
    ...
    Ok(metadata.with_resource_policy(resource_policy))
}
