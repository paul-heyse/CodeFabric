...
fn read_row_group(
    ...
    prot.admit_collection::<ColumnChunkMetaData>(schema_descr.num_columns())?;
...
pub(crate) fn parquet_metadata_from_bytes(
    ...
    let resource_policy = crate::resource::effective(
        options
            .and_then(ParquetMetaDataOptions::resource_policy)
            .cloned(),
    );
    if let Some(policy) = &resource_policy {
        crate::resource::ReaderResourcePolicy::check(
            buf.len(),
            policy.limits().footer_bytes,
            "footer bytes",
        )?;
    }
    let mut prot = ThriftSliceInputProtocol::new(buf).with_resource_policy(resource_policy.clone());

    let _scope = crate::resource::enter(resource_policy.clone());
        ...
        match field_ident.id {
            ...
            2 => {
                ...
                if schema_descr.is_some() {
                    ...
                    let val = parquet_schema_from_array(val)?;
                    let descriptor = SchemaDescriptor::try_new(val)?;
                    if let Some(policy) = &resource_policy {
                        policy.reserve(
                            std::mem::size_of::<SchemaDescriptor>()
                                + 2 * std::mem::size_of::<usize>(),
                            "schema descriptor owner",
                        )?;
                    }
                    schema_descr = Some(Arc::new(descriptor));
                }
            }
            3 => {
            ...
            4 => {
                ...
                // check for list of struct
                validate_list_type(ElementType::Struct, &list_ident)?;
                prot.admit_collection::<RowGroupMetaData>(list_ident.size as usize)?;
    ...
    Ok(ParquetMetaData::new(fmd, row_groups).with_resource_policy(resource_policy))
}
...
pub(super) fn serialize_column_meta_data<W: Write>(
    ...
    w.write_list_begin(Encoding::ELEMENT_TYPE, column_chunk.encodings().count())?;
    for encoding in column_chunk.encodings() { encoding.write_thrift(w)?; }
    if w.write_path_in_schema() {
        let path = column_chunk.column_descr.path().parts();
        w.write_field_begin(FieldType::List, 3, 2)?;
        w.write_list_begin(String::ELEMENT_TYPE, path.len())?;
        for part in path { part.write_thrift(w)?; }
    ...
    if should_write_column_stats(column_chunk) {
        ...
        let stats = column_chunk.statistics().map(|stats| {
            let min = stats.min_bytes_opt();
            let max = stats.max_bytes_opt();
            Statistics {
                min: if stats.is_min_max_backwards_compatible() { min } else { None },
                max: if stats.is_min_max_backwards_compatible() { max } else { None },
                min_value: if stats.is_min_max_deprecated() { None } else { min },
                max_value: if stats.is_min_max_deprecated() { None } else { max },
                null_count: stats.null_count_opt().and_then(|v| i64::try_from(v).ok()),
                distinct_count: stats.distinct_count_opt().and_then(|v| i64::try_from(v).ok()),
                is_min_value_exact: Some(stats.min_is_exact()),
                is_max_value_exact: Some(stats.max_is_exact()),
            }
        });
        ...
        // Write the original borrowed histograms directly. The wire fields
        // match SizeStatistics without allocating cloned Vec<i64> values.
        if column_chunk.unencoded_byte_array_data_bytes.is_some()
            || column_chunk.repetition_level_histogram.is_some()
            || column_chunk.definition_level_histogram.is_some()
        {
            w.write_field_begin(FieldType::Struct, 16, last_field_id)?;
            let mut histogram_field = if let Some(bytes) = column_chunk.unencoded_byte_array_data_bytes { bytes.write_thrift_field(w, 1, 0)? } else { 0 };
            for (field, histogram) in [(2, column_chunk.repetition_level_histogram()), (3, column_chunk.definition_level_histogram())] {
                if let Some(histogram) = histogram {
                    w.write_field_begin(FieldType::List, field, histogram_field)?;
                    w.write_list_begin(i64::ELEMENT_TYPE, histogram.len())?;
                    for value in histogram.values() { value.write_thrift(w)?; }
