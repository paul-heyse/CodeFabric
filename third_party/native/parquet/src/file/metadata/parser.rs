...
mod inner {
    ...
    impl MetadataParser {
        ...
        pub(crate) fn resource_policy(&self) -> Option<crate::resource::ReaderResourcePolicy> {
            crate::resource::effective(
                self.metadata_options
                    .as_ref()
                    .and_then(|options| options.resource_policy())
                    .cloned(),
            )
        }
...
pub(crate) fn parse_column_index(
    ...
    let mut index =
        crate::resource::vec_with_capacity(metadata.row_groups().len(), "column index row groups")?;
    for (rg_idx, x) in metadata.row_groups().iter().enumerate() {
        let mut columns =
            crate::resource::vec_with_capacity(x.columns().len(), "column index columns")?;
        for (col_idx, c) in x.columns().iter().enumerate() {
            let column = match c.column_index_range() {
                Some(r) => {
...
pub(crate) fn parse_offset_index(
    ...
    let mut all_indexes =
        crate::resource::vec_with_capacity(row_groups.len(), "offset index row groups")?;
    for (rg_idx, x) in row_groups.iter().enumerate() {
        let mut row_group_indexes =
            crate::resource::vec_with_capacity(x.columns().len(), "offset index columns")?;
        for (col_idx, c) in x.columns().iter().enumerate() {
            ...
            match result {
                ...
                Err(e) => {
                    if offset_index_policy == PageIndexPolicy::Required
                        || matches!(e, ParquetError::ResourceExhausted(_))
                    {
                        return Err(e);
                    } else {
...
mod resource_tests {
    ...
    use std::sync::Arc;
    #[test]
    fn resource_native_optional_offset_index_does_not_swallow_capacity_failure() {
        use crate::file::metadata::{
        ...
        let schema = Arc::new(SchemaDescriptor::new(Arc::new(
            Type::group_type_builder("schema")
                .with_fields(vec![Arc::new(
                    Type::primitive_type_builder("x", crate::basic::Type::INT32)
                        .build()
                        .unwrap(),
                )])
                .build()
        ...
        let column = ColumnChunkMetaData::builder(schema.column(0))
            ...
            .set_offset_index_length(Some(2))
            .build()
            .unwrap();
        let row_group = RowGroupMetaData::builder(schema.clone())
            .set_column_metadata(vec![column])
            .build()
            .unwrap();
        let file = FileMetaData::new(1, 0, None, None, schema, None);
        let mut metadata = ParquetMetaData::new(file, vec![row_group]);
        let (policy, _) = policy("offset index page locations");
        let _guard = policy.enter_thread();
        let error = parse_offset_index(
            &mut metadata,
            PageIndexPolicy::Optional,
            &bytes::Bytes::from_static(&[0x19, 0x1c]),
            0,
        )
        .unwrap_err();
        assert!(matches!(
