...
impl PartialEq for ParquetMetaData {
    fn eq(&self, other: &Self) -> bool {
        let equal = self.file_metadata == other.file_metadata && self.row_groups == other.row_groups && self.column_index == other.column_index && self.offset_index == other.offset_index;
        #[cfg(feature = "encryption")]
        let equal = equal && self.file_decryptor == other.file_decryptor;
        equal
    }
}

impl ParquetMetaData {
    /// Admission owner for allocations retained by this decoded metadata.
...
impl PartialEq for FileMetaData {
    fn eq(&self, other: &Self) -> bool {
        let equal = self.version == other.version && self.num_rows == other.num_rows && self.created_by == other.created_by && self.key_value_metadata == other.key_value_metadata && self.schema_descr == other.schema_descr && self.column_orders == other.column_orders;
        #[cfg(feature = "encryption")]
        let equal = equal && self.encryption_algorithm == other.encryption_algorithm && self.footer_signing_key_metadata == other.footer_signing_key_metadata;
        equal
    }
}
...
impl PartialEq for RowGroupMetaData {
    fn eq(&self, other: &Self) -> bool {
        let equal = self.columns == other.columns && self.num_rows == other.num_rows && self.sorting_columns == other.sorting_columns && self.total_byte_size == other.total_byte_size && self.schema_descr == other.schema_descr && self.file_offset == other.file_offset && self.ordinal == other.ordinal;
        equal
    }
}
...
impl RowGroupMetaData {
    ...
    /// Fallible native builder; admits its initial column collection before construction.
    pub fn try_builder(schema_descr: SchemaDescPtr) -> Result<RowGroupMetaDataBuilder> {
        crate::resource::admit_writer_slots::<ColumnChunkMetaData>(schema_descr.num_columns(), "writer row group columns")?;
        Ok(RowGroupMetaDataBuilder::new(schema_descr))
    }

    /// Clone this metadata after admission of the new native collections.
    pub fn try_clone(&self) -> Result<Self> {
        ...
        crate::resource::admit_writer_slots::<ColumnChunkMetaData>(self.columns.len(), "writer cloned column metadata")?;
        if let Some(columns) = &self.sorting_columns { crate::resource::admit_writer_slots::<SortingColumn>(columns.len(), "writer sorting columns")?; }
        for column in &self.columns { column.admit_clone()?; }
        Ok(self.clone())
    }
...
impl PartialEq for ColumnChunkMetaData {
    fn eq(&self, other: &Self) -> bool {
        let equal = self.column_descr == other.column_descr && self.encodings == other.encodings && self.file_path == other.file_path && self.file_offset == other.file_offset && self.num_values == other.num_values && self.compression == other.compression && self.total_compressed_size == other.total_compressed_size && self.total_uncompressed_size == other.total_uncompressed_size && self.data_page_offset == other.data_page_offset && self.index_page_offset == other.index_page_offset && self.dictionary_page_offset == other.dictionary_page_offset && self.statistics == other.statistics && self.geo_statistics == other.geo_statistics && self.encoding_stats == other.encoding_stats && self.bloom_filter_offset == other.bloom_filter_offset && self.bloom_filter_length == other.bloom_filter_length && self.offset_index_offset == other.offset_index_offset && self.offset_index_length == other.offset_index_length && self.column_index_offset == other.column_index_offset && self.column_index_length == other.column_index_length && self.unencoded_byte_array_data_bytes == other.unencoded_byte_array_data_bytes && self.repetition_level_histogram == other.repetition_level_histogram && self.definition_level_histogram == other.definition_level_histogram;
        #[cfg(feature = "encryption")]
        let equal = equal && self.column_crypto_metadata == other.column_crypto_metadata && self.encrypted_column_metadata == other.encrypted_column_metadata && self.plaintext_footer_mode == other.plaintext_footer_mode;
        equal
    }
}

impl ColumnChunkMetaData {
    fn admit_clone(&self) -> Result<()> {
        let Some(policy) = crate::resource::current() else { return Ok(()); };
        if self.geo_statistics.is_some() { return Err(crate::resource::ResourceExhausted { kind: "writer geospatial metadata clone outside admitted profile", requested: 1, limit: 0 }.into()); }
        #[cfg(feature = "encryption")]
        if self.column_crypto_metadata.is_some() || self.encrypted_column_metadata.is_some() { return Err(crate::resource::ResourceExhausted { kind: "writer encrypted metadata clone outside admitted profile", requested: 1, limit: 0 }.into()); }
        if let Some(path) = &self.file_path { policy.reserve(path.len(), "writer metadata file path")?; }
        if let Some(ParquetPageEncodingStats::Full(stats)) = &self.encoding_stats { crate::resource::admit_writer_slots::<PageEncodingStats>(stats.len(), "writer encoding statistics clone")?; }
        for values in [self.repetition_level_histogram.as_ref(), self.definition_level_histogram.as_ref()].into_iter().flatten() { crate::resource::admit_writer_slots::<i64>(values.len(), "writer level histogram clone")?; }
        // Native Statistics contains inline scalar values or Bytes-backed byte
        // values. Their clone retains the original allocation; it does not copy
        // payload. Optional geospatial/encryption allocations rejected above.
        Ok(())
    }
...
impl ColumnIndexBuilder {
    ...
    pub(crate) fn try_reserve_page(&mut self) -> Result<()> {
        crate::resource::reserve_vec(&mut self.null_pages, 1, "writer index null pages")?;
        crate::resource::reserve_vec(&mut self.min_values, 1, "writer index minima")?;
        crate::resource::reserve_vec(&mut self.max_values, 1, "writer index maxima")?;
        crate::resource::reserve_vec(&mut self.null_counts, 1, "writer index null counts")
    }

    pub(crate) fn try_append_histograms(&mut self, rep: &Option<LevelHistogram>, def: &Option<LevelHistogram>) -> Result<()> {
        if !self.valid { return Ok(()); }
        if let Some(values) = rep { crate::resource::reserve_vec(self.repetition_level_histograms.get_or_insert_with(Vec::new), values.len(), "writer index repetition histogram")?; }
        if let Some(values) = def { crate::resource::reserve_vec(self.definition_level_histograms.get_or_insert_with(Vec::new), values.len(), "writer index definition histogram")?; }
        self.append_histograms(rep, def);
        Ok(())
    }
    ...
    where
        T: ParquetValueType,
    {
        let mut min_values = crate::resource::vec_with_capacity(self.min_values.len(), "writer index minimum references")?;
        let mut max_values = crate::resource::vec_with_capacity(self.max_values.len(), "writer index maximum references")?;
        min_values.extend(self.min_values.iter().map(|v| v.as_slice()));
        max_values.extend(self.max_values.iter().map(|v| v.as_slice()));
...
impl OffsetIndexBuilder {
    ...
    pub(crate) fn try_reserve_page(&mut self, variable_bytes: bool) -> Result<()> {
        crate::resource::reserve_vec(&mut self.first_row_index_array, 1, "writer offset rows")?;
        if variable_bytes { crate::resource::reserve_vec(self.unencoded_byte_array_data_bytes_array.get_or_insert_with(Vec::new), 1, "writer offset variable bytes")?; }
        Ok(())
    }

    pub(crate) fn try_reserve_offset(&mut self) -> Result<()> {
        crate::resource::reserve_vec(&mut self.offset_array, 1, "writer offset locations")?;
        crate::resource::reserve_vec(&mut self.compressed_page_size_array, 1, "writer offset sizes")
    }

    pub(crate) fn try_build(self) -> Result<OffsetIndexMetaData> {
        let mut locations = crate::resource::vec_with_capacity(self.offset_array.len(), "writer final offset locations")?;
        locations.extend(self.offset_array.iter().zip(&self.compressed_page_size_array).zip(&self.first_row_index_array).map(|((offset, size), row)| PageLocation { offset: *offset, compressed_page_size: *size, first_row_index: *row }));
        OffsetIndexMetaData::try_new(locations, self.unencoded_byte_array_data_bytes_array)
    }
    ...
    pub fn build(self) -> OffsetIndexMetaData {
        let locations = self
            ...
            .collect::<Vec<_>>();
        OffsetIndexMetaData::try_new(locations, self.unencoded_byte_array_data_bytes_array).expect("offset index ownership admission")
    }
}
