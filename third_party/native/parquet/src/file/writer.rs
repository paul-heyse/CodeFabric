...
#[cfg(test)]
use crate::column::writer::get_column_writer;
...
use crate::column::{
    ...
    writer::{ColumnWriter, try_get_column_writer},
};
...
pub struct TrackedWrite<W: Write> {
    inner: BufWriter<W>,
    bytes_written: usize,
    _resource_policy: Option<crate::resource::ReaderResourcePolicy>,
}

impl<W: Write> TrackedWrite<W> {
    /// Create a new [`TrackedWrite`] from a [`Write`]
    pub fn new(inner: W) -> Self {
        Self::try_new(inner).expect("tracked writer admission")
    }

    /// Admit the standard BufWriter's fixed 8 KiB buffer before construction.
    pub fn try_new(inner: W) -> Result<Self> {
        let resource_policy = crate::resource::current();
        if let Some(policy) = &resource_policy { policy.reserve(8192, "writer I/O buffer")?; }
        Ok(Self { inner: BufWriter::with_capacity(8192, inner), bytes_written: 0, _resource_policy: resource_policy })
    }
...
impl<W: Write + Send> SerializedFileWriter<W> {
    ...
    pub fn new(buf: W, schema: TypePtr, properties: WriterPropertiesPtr) -> Result<Self> {
        if crate::resource::current().is_some() {
            if properties.content_defined_chunking().is_some() {
                return Err(crate::resource::ResourceExhausted { kind: "writer content-defined chunking outside admitted profile", requested: 1, limit: 0 }.into());
            }
            #[cfg(feature = "encryption")]
            if properties.file_encryption_properties().is_some() {
                return Err(crate::resource::ResourceExhausted { kind: "writer encryption outside admitted profile", requested: 1, limit: 0 }.into());
            }
        }
        let mut buf = TrackedWrite::try_new(buf)?;

        let schema_descriptor = SchemaDescriptor::try_new(schema.clone())?;

        #[cfg(feature = "encryption")]
        let file_encryptor = Self::get_file_encryptor(&properties, &schema_descriptor)?;

        if let Some(policy) = crate::resource::current() {
            let (layout, _) = std::alloc::Layout::new::<[usize; 2]>().extend(std::alloc::Layout::new::<SchemaDescriptor>()).map_err(|_| crate::resource::ResourceExhausted { kind: "writer schema owner layout", requested: usize::MAX, limit: isize::MAX as usize })?;
            policy.reserve(layout.pad_to_align().size(), "writer schema owner")?;
        }
    ...
    pub fn next_row_group(&mut self) -> Result<SerializedRowGroupWriter<'_, W>> {
        ...
        crate::resource::reserve_vec(&mut self.row_groups, 1, "writer row groups")?;
        crate::resource::reserve_vec(&mut self.bloom_filters, 1, "writer row bloom descriptors")?;
        crate::resource::reserve_vec(&mut self.column_indexes, 1, "writer row column indexes")?;
        crate::resource::reserve_vec(&mut self.offset_indexes, 1, "writer row offset indexes")?;
        ...
        if let Some(policy) = crate::resource::current() {
            policy.reserve(std::mem::size_of_val(&on_close), "writer row close callback")?;
        }
        let row_group_writer = SerializedRowGroupWriter::try_new(
            self.descr.clone(),
            self.props.clone(),
            &mut self.buf,
            ordinal,
    ...
    fn write_metadata(&mut self) -> Result<ParquetMetaData> {
        ...
        if let Some(policy) = crate::resource::current() {
            let count = self.props.key_value_metadata().map_or(0, Vec::len).checked_add(self.kv_metadatas.len()).ok_or(crate::resource::ResourceExhausted { kind: "writer footer key values", requested: usize::MAX, limit: isize::MAX as usize })?;
            crate::resource::admit_writer_slots::<KeyValue>(count, "writer footer key values")?;
            for pair in self.props.key_value_metadata().into_iter().flatten().chain(&self.kv_metadatas) {
                policy.string(pair.key.len())?;
                if let Some(value) = &pair.value { policy.string(value.len())?; }
            }
            policy.string(self.props.created_by().len())?;
        }
    ...
    /// Append already-owned metadata after admitting this writer's Vec growth.
    pub fn try_append_key_value_metadata(&mut self, kv_metadata: KeyValue) -> Result<()> {
        crate::resource::validate_thread_owner(self.resource_policy.as_ref())?;
        let _scope = crate::resource::enter(self.resource_policy.clone());
        crate::resource::reserve_vec(&mut self.kv_metadatas, 1, "writer appended key values")?;
        self.kv_metadatas.push(kv_metadata);
        Ok(())
    }
...
impl<'a, W: Write + Send> SerializedRowGroupWriter<'a, W> {
    ...
    pub fn new(
        ...
        Self::try_new(schema_descr, properties, buf, row_group_index, on_close).expect("row group admission")
    }

    /// Construct native row-group collections after fallible capacity admission.
    pub fn try_new(
        schema_descr: SchemaDescPtr,
        properties: WriterPropertiesPtr,
        buf: &'a mut TrackedWrite<W>,
        ...
        Ok(Self {
            ...
            column_chunks: crate::resource::vec_with_capacity(num_columns, "writer row column metadata")?,
            bloom_filters: crate::resource::vec_with_capacity(num_columns, "writer row bloom filters")?,
            column_indexes: crate::resource::vec_with_capacity(num_columns, "writer row index descriptors")?,
            offset_indexes: crate::resource::vec_with_capacity(num_columns, "writer row offset descriptors")?,
    ...
    fn get_on_close(&mut self) -> Result<(&mut TrackedWrite<W>, OnCloseColumnChunk<'_>)> {
        ...
        if let Some(policy) = crate::resource::current() { policy.reserve(std::mem::size_of_val(&on_close), "writer column close callback")?; }
        Ok((self.buf, Box::new(on_close)))
    }
        ...
        ) -> Result<C>,
    {
        ...
        Ok(match self.next_column_desc() {
            Some(column) => {
                ...
                if let Some(policy) = crate::resource::current() { policy.reserve(std::mem::size_of_val(&page_writer), "writer serialized page owner")?; }
                Some(factory(
                    column,
                    props,
                    Box::new(page_writer),
    ...
    pub fn next_column(&mut self) -> Result<Option<SerializedColumnWriter<'_>>> {
        self.next_column_with_factory(|descr, props, page_writer, on_close| {
            let column_writer = try_get_column_writer(descr, props, page_writer)?;
    ...
    fn finish_appended_column(
        ...
        // Move the original metadata collections, changing only file-relative
        // offsets. No cloned histogram/statistics/encoding allocation is needed.
        ...
        #[cfg(feature = "encryption")]
        let builder = if let Some(file_encryptor) = self.file_encryptor.as_ref() {
            let column = self.descr.column(self.column_index - 1);
            builder.set_column_crypto_metadata(get_column_crypto_metadata(file_encryptor.properties(), &column))
        } else { builder };
        close.metadata = builder.build()?;

        if let Some(offsets) = close.offset_index.as_mut() {
            for location in offsets.page_locations.try_values_mut()? {
    ...
    pub fn close(mut self) -> Result<RowGroupMetaDataPtr> {
        if self.row_group_metadata.is_none() {
            ...
            if let Some(columns) = self.props.sorting_columns() { crate::resource::admit_writer_slots::<crate::file::metadata::SortingColumn>(columns.len(), "writer sorting columns")?; }
            let row_group_metadata = RowGroupMetaData::try_builder(self.descr.clone())?
            ...
            let cloned = row_group_metadata.try_clone()?;
            if let Some(policy) = crate::resource::current() {
                let layout = std::alloc::Layout::new::<[usize; 2]>().extend(std::alloc::Layout::new::<RowGroupMetaData>()).map_err(|_| crate::resource::ResourceExhausted { kind: "writer row group Arc layout", requested: usize::MAX, limit: isize::MAX as usize })?.0.pad_to_align();
                policy.reserve(layout.size(), "writer row group Arc")?;
            }
            self.row_group_metadata = Some(Arc::new(cloned));
