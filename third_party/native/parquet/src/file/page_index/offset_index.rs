...
impl Copy for PageLocation {}

/// Offset-index metadata whose original vector backing retains native receipts.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct OffsetIndexMetaData {
    /// Page locations in the original admitted backing.
    pub page_locations: crate::resource::OwnedVec<PageLocation>,
    /// Optional original admitted unencoded page sizes.
    pub unencoded_byte_array_data_bytes: Option<crate::resource::OwnedVec<i64>>,
}
...
impl<'a, R: ThriftCompactInputProtocol<'a>> ReadThrift<'a, R> for OffsetIndexMetaData {
    fn read_thrift(prot: &mut R) -> Result<Self> {
        let raw = RawOffsetIndexMetaData::read_thrift(prot)?;
        Self::try_new(raw.page_locations, raw.unencoded_byte_array_data_bytes)
    }
}
impl WriteThrift for OffsetIndexMetaData {
    ...
    fn write_thrift<W: Write>(&self, writer: &mut ThriftCompactOutputProtocol<W>) -> Result<()> {
        ...
        if let Some(values) = &self.unencoded_byte_array_data_bytes { values.write_thrift_field(writer, 2, 1)?; }
        writer.write_struct_end()
    }
}
crate::write_thrift_field!(OffsetIndexMetaData, FieldType::Struct);

impl OffsetIndexMetaData {
    /// Retain already-admitted native vectors without exposing an unowned move.
    pub fn try_new(page_locations: Vec<PageLocation>, unencoded_byte_array_data_bytes: Option<Vec<i64>>) -> Result<Self> {
        Ok(Self {
            page_locations: crate::resource::OwnedVec::try_new(page_locations)?,
            unencoded_byte_array_data_bytes: unencoded_byte_array_data_bytes.map(crate::resource::OwnedVec::try_new).transpose()?,
        })
    }
    ...
    pub fn unencoded_byte_array_data_bytes(&self) -> Option<&Vec<i64>> {
        self.unencoded_byte_array_data_bytes.as_deref()
    }
    ...
    pub(super) fn try_from_fast<'a, R: ThriftCompactInputProtocol<'a>>(
        ...
        let mut page_locations = crate::resource::vec_with_capacity(
            list_ident.size as usize,
        ...
        Self::try_new(page_locations, unencoded_byte_array_data_bytes)
    }
}
...
mod tests {
    ...
    fn test_offset_idx_roundtrip() {
        ...
        test_roundtrip(OffsetIndexMetaData {
            page_locations: crate::resource::OwnedVec::try_new(page_locations.clone()).unwrap(),
            unencoded_byte_array_data_bytes: Some(crate::resource::OwnedVec::try_new(unenc).unwrap()),
        });
        test_roundtrip(OffsetIndexMetaData {
            page_locations: crate::resource::OwnedVec::try_new(page_locations).unwrap(),
