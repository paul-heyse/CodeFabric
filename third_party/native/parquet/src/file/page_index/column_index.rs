...
impl<T: ParquetValueType> PrimitiveColumnIndex<T> {
    pub(crate) fn try_new(
        ...
        let mut min_values =
            crate::resource::vec_with_capacity(len, "primitive column index minima")?;
        let mut max_values =
            crate::resource::vec_with_capacity(len, "primitive column index maxima")?;
...
impl ByteArrayColumnIndex {
    pub(crate) fn try_new(
        ...
        let min_len = byte_length(&min_values)?;
        let max_len = byte_length(&max_values)?;
        let mut min_bytes =
            crate::resource::vec_with_capacity(min_len, "byte column index minima")?;
        min_bytes.resize(min_len, 0u8);
        let mut max_bytes =
            crate::resource::vec_with_capacity(max_len, "byte column index maxima")?;
        max_bytes.resize(max_len, 0u8);
        let offsets_len = len
            .checked_add(1)
            .ok_or(crate::resource::ResourceExhausted {
                kind: "column index offsets",
                requested: usize::MAX,
                limit: isize::MAX as usize,
            })?;
        let mut min_offsets =
            crate::resource::vec_with_capacity(offsets_len, "byte column index minimum offsets")?;
        min_offsets.resize(offsets_len, 0usize);
        let mut max_offsets =
            crate::resource::vec_with_capacity(offsets_len, "byte column index maximum offsets")?;
        max_offsets.resize(offsets_len, 0usize);
