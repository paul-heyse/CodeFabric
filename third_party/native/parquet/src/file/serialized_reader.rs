...
impl<R: 'static + ChunkReader> SerializedFileReader<R> {
    ...
    /// Returns an error if the Parquet file does not exist or is corrupt.
    pub fn new_with_options(chunk_reader: R, mut options: ReadOptions) -> Result<Self> {
        if let Some(policy) = options.props.resource_policy().cloned() {
            options.metadata_options = options.metadata_options.with_resource_policy(policy);
        }
...
impl<R: 'static + ChunkReader> RowGroupReader for SerializedRowGroupReader<'_, R> {
    ...
    fn get_column_page_reader(&self, i: usize) -> Result<Box<dyn PageReader>> {
        ...
        let props = Arc::clone(&self.props);
        Ok(Box::new(SerializedPageReader::new_with_owned_page_locations(
...
pub(crate) fn decode_page(
    ...
    decode_page_with_policy(page_header, buffer, physical_type, decompressor, None)
}

fn decode_page_with_policy(
    page_header: PageHeader,
    buffer: Bytes,
    physical_type: Type,
    decompressor: Option<&mut Box<dyn Codec>>,
    ...
    let _scope = crate::resource::enter(policy.cloned());
    if let Some(policy) = policy {
        let limits = policy.limits();
        crate::resource::ReaderResourcePolicy::check(
            usize::try_from(page_header.compressed_page_size)?,
            limits.page_bytes,
        ...
        let values = page_header
            .dictionary_page_header
            .as_ref()
            .map(|h| h.num_values)
            .or_else(|| page_header.data_page_header.as_ref().map(|h| h.num_values))
            .or_else(|| {
                page_header
                    .data_page_header_v2
                    .as_ref()
        ...
        // Bytes::from_owner uses a fixed owner/header allocation as well as page backing.
    ...
    let buffer = match decompressor {
        Some(decompressor) if can_decompress => {
            ...
            if let Some(policy) = policy {
                policy.reserve(uncompressed_page_size, "page decompression")?;
            }
    ...
    let buffer = match policy {
        Some(policy) => policy.own_bytes(buffer),
        None => buffer,
    };
...
enum SerializedPageReaderState {
    ...
    Pages {
        ...
        page_locations: crate::resource::OwnedVecCursor<PageLocation>,
...
impl<R: ChunkReader> SerializedPageReader<R> {
    ...
    pub fn new_with_properties(
        ...
        let policy = crate::resource::effective(props.resource_policy().cloned());
        let _scope = crate::resource::enter(policy);
        let locations = page_locations.map(crate::resource::OwnedVec::try_new).transpose()?;
        Self::new_with_owned_page_locations(reader, meta, total_rows, locations, props)
    }

    pub(crate) fn new_with_owned_page_locations(
        reader: Arc<R>,
        meta: &ColumnChunkMetaData,
        total_rows: usize,
        page_locations: Option<crate::resource::OwnedVec<PageLocation>>,
        props: ReaderPropertiesPtr,
    ) -> Result<Self> {
        let resource_policy = crate::resource::effective(props.resource_policy().cloned());
        let decompressor = crate::compression::create_admitted_decoder(
            meta.compression(),
            props.codec_options(),
        ...
        let state = match page_locations {
            Some(locations) => {
                ...
                SerializedPageReaderState::Pages {
                    page_locations: locations.into_cursor(),
                    dictionary_page,
                    total_rows,
                    page_index: 0,
                }
        ...
        let mut context = SerializedPageReaderContext {
            resource_policy,
            ..Default::default()
        };
...
impl SerializedPageReaderContext {
    fn read_page_header<T: Read>(
        ...
        let mut prot =
            ThriftReadInputProtocol::new(input).with_resource_policy(self.resource_policy.clone());
        if self.read_stats {
        ...
        match self.page_crypto_context(page_index, dictionary_page) {
            None => {
                let mut prot = ThriftReadInputProtocol::new(input)
                    .with_resource_policy(self.resource_policy.clone());
                if self.read_stats {
                    Ok(PageHeader::read_thrift(&mut prot)?)
                } else {
                    use crate::file::metadata::thrift::PageHeader;
...
impl<R: ChunkReader> PageReader for SerializedPageReader<R> {
    fn get_next_page(&mut self) -> Result<Option<Page>> {
        crate::resource::validate_thread_owner(self.context.resource_policy.as_ref())?;
        loop {
            let page = match &mut self.state {
                SerializedPageReaderState::Values {
                    ...
                    *remaining -= data_len as u64;
                    ...
                    if let Some(policy) = &self.context.resource_policy {
                        crate::resource::ReaderResourcePolicy::check(
                            data_len,
                            policy.limits().page_bytes,
                            "compressed page bytes",
                        )?;
                        policy.reserve(data_len, "compressed page backing")?;
                    }
                    ...
                    let page = decode_page_with_policy(
                        header,
                        buffer,
                        self.physical_type,
                        self.decompressor.as_mut(),
                        self.context.resource_policy.as_ref(),
                    )?;
                ...
                SerializedPageReaderState::Pages {
                    ...
                    if let Some(policy) = &self.context.resource_policy {
                        ...
                        policy.reserve(page_len, "compressed page backing")?;
                    }
...
mod tests {
    ...
    fn get_serialized_page_reader<R: ChunkReader>(
        ...
        SerializedPageReader::new_with_owned_page_locations(
...
mod resource_admission_tests {
    ...
    use crate::file::metadata::thrift::DictionaryPageHeader;
    use crate::resource::tests::policy;
    use std::sync::atomic::{AtomicBool, Ordering};

    struct Tripwire(Arc<AtomicBool>);
    impl Codec for Tripwire {
        fn compress(&mut self, _: &[u8], _: &mut Vec<u8>) -> Result<()> {
            unreachable!()
        }
        fn decompress(&mut self, _: &[u8], _: &mut Vec<u8>, _: Option<usize>) -> Result<usize> {
            self.0.store(true, Ordering::Release);
            panic!("decoder ran before admission")
        }
    }
    fn header(uncompressed: i32) -> PageHeader {
        PageHeader {
    ...
    #[test]
    fn resource_page_admission_precedes_native_decompression() {
        for (deny, bytes) in [("page decompression", 4096), ("", 8193)] {
            let (policy, _) = policy(deny);
            let invoked = Arc::new(AtomicBool::new(false));
            let mut codec: Box<dyn Codec> = Box::new(Tripwire(invoked.clone()));
            let result = decode_page_with_policy(
                header(bytes),
            ...
            assert!(matches!(result, Err(ParquetError::ResourceExhausted(_))));
            assert!(!invoked.load(Ordering::Acquire));
        }
    }
    #[test]
    fn resource_native_page_backing_retains_admission_after_reader_drop() {
