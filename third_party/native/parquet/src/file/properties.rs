...
pub struct WriterProperties {
    ...
    resource_owner: WriterPropertiesResourceOwner,
}

/// Legacy infallible deep clones do not transfer an original allocation charge.
/// Governed callers use try_clone, which reserves the copy before allocation.
#[derive(Debug)]
struct WriterPropertiesResourceOwner(Option<crate::resource::ReaderResourcePolicy>);
impl Clone for WriterPropertiesResourceOwner {fn clone(&self)->Self{Self(None)}}
...
impl WriterProperties {
    /// Build default writer properties after admitting its created-by String.
    pub fn try_new() -> crate::errors::Result<Self> { Ok(Self::try_builder()?.build()) }

    /// Fallible native default builder. Later caller-supplied collection/string
    /// properties retain their own separate admission obligation.
    pub fn try_builder() -> crate::errors::Result<WriterPropertiesBuilder> {
        let policy = crate::resource::current();
        if let Some(policy) = &policy { policy.string(DEFAULT_CREATED_BY.len())?; }
        let mut builder = WriterPropertiesBuilder::default();
        builder.resource_owner = WriterPropertiesResourceOwner(policy);
        Ok(builder)
    }

    /// Deep-copy original native property backing only after full copy admission.
    /// Shared encryption configuration is outside the selected local profile.
    pub fn try_clone(&self) -> crate::errors::Result<Self> {
        let policy=crate::resource::current();
        if let Some(policy)=&policy {
            #[cfg(feature="encryption")]
            if self.file_encryption_properties.is_some() {
                return Err(crate::resource::ResourceExhausted{kind:"writer encryption owner",requested:1,limit:0}.into());
            }
            policy.string(self.created_by.len())?;
            if let Some(metadata)=&self.key_value_metadata {
                policy.collection(metadata.len(),std::mem::size_of::<KeyValue>(),"writer metadata copy")?;
                for entry in metadata {policy.string(entry.key.len())?;if let Some(value)=&entry.value {policy.string(value.len())?;}}
            }
            if let Some(columns)=&self.sorting_columns {policy.collection(columns.len(),std::mem::size_of::<SortingColumn>(),"writer sorting columns copy")?;}
            // std1.98 HashMap clones its full native bucket allocation. Its
            // reported capacity is bucket_count-1 below8, otherwise7/8 buckets.
            let capacity=self.column_properties.capacity();
            if capacity!=0 {
                let overflow=||crate::resource::ResourceExhausted{kind:"writer properties hash layout",requested:usize::MAX,limit:isize::MAX as usize};
                let buckets=if capacity<7 {capacity.checked_add(1)}else{capacity.checked_div(7).and_then(|n|n.checked_mul(8))}.ok_or_else(overflow)?;
                policy.collection(buckets,std::mem::size_of::<(ColumnPath,ColumnProperties)>(),"writer column properties copy")?;
                // Native SIMD control group is16bytes on the selected target;
                //32 covers every compiled native group width, plus data alignment.
                let alignment=std::mem::align_of::<(ColumnPath,ColumnProperties)>().max(32);
                policy.reserve(buckets.checked_add(32).and_then(|n|n.checked_add(alignment-1)).ok_or_else(overflow)?,"writer hash control copy")?;
            }
            for path in self.column_properties.keys() {
                policy.collection(path.parts().len(),std::mem::size_of::<String>(),"writer column path copy")?;
                for part in path.parts(){policy.string(part.len())?;}
            }
        }
        let mut value=self.clone();
        value.resource_owner=WriterPropertiesResourceOwner(policy);
        Ok(value)
    }
...
impl Default for WriterPropertiesBuilder {
    ...
    fn default() -> Self {
        Self {
            ...
            resource_owner: WriterPropertiesResourceOwner(None),
        }
    }
}
...
impl WriterPropertiesBuilder {
    ...
    /// Build created-by text from borrowed pieces after exact backing admission.
    pub fn try_set_created_by_parts(mut self, prefix:&str, value:&str)->crate::errors::Result<Self>{
        let len=prefix.len().checked_add(value.len()).ok_or(crate::resource::ResourceExhausted{kind:"writer created-by layout",requested:usize::MAX,limit:isize::MAX as usize})?;
        if let Some(policy)=crate::resource::current(){
            policy.string(len)?;
            // Builder's existing allocations keep their original owner. If a
            // foreign phase attempts mutation it must first use admitted copy.
            if self.resource_owner.0.as_ref().is_some_and(|original|!original.same_owner(&policy)) {
                return Err(crate::resource::ResourceExhausted{kind:"writer properties owner mismatch",requested:1,limit:0}.into());
            }
            self.resource_owner=WriterPropertiesResourceOwner(Some(policy));
        }
        let mut created_by=String::with_capacity(len);created_by.push_str(prefix);created_by.push_str(value);self.created_by=created_by;
        Ok(self)
    }
...
impl From<WriterProperties> for WriterPropertiesBuilder {
    fn from(props: WriterProperties) -> Self {
        WriterPropertiesBuilder {
            ...
            resource_owner: props.resource_owner,
        }
    }
}
...
impl ReaderProperties {
    /// Installed opt-in native reader admission policy.
    pub fn resource_policy(&self) -> Option<&crate::resource::ReaderResourcePolicy> {
...
impl ReaderPropertiesBuilder {
    /// Install opt-in native allocation admission.
    pub fn set_resource_policy(
        mut self,
        policy: Option<crate::resource::ReaderResourcePolicy>,
    ) -> Self {
        self.resource_policy = crate::resource::effective(policy);
        self
    }
