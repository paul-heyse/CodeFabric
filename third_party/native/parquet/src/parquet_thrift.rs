...
impl From<ThriftProtocolError> for ParquetError {
    ...
    fn from(e: ThriftProtocolError) -> Self {
        match e {
            ThriftProtocolError::ResourceExhausted(e) => e.into(),
...
impl From<crate::resource::ResourceExhausted> for ThriftProtocolError {
...
pub(crate) trait ThriftCompactInputProtocol<'a> {
    fn resource_policy(&self) -> Option<&crate::resource::ReaderResourcePolicy> {
        None
    }
    fn admit_collection<T>(&self, count: usize) -> ThriftProtocolResult<()> {
        if let Some(policy) = self.resource_policy() {
            policy.collection(count, std::mem::size_of::<T>(), "thrift collection backing")?;
        }
        Ok(())
    }
    ...
    T: ReadThrift<'a, R> + WriteThrift,
{
    ...
    prot.admit_collection::<T>(list_ident.size as usize)?;
