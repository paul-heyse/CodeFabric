...
impl BitWriter {
    ...
    /// Fallible admission of native byte backing before constructing this writer.
    pub fn try_new(initial_capacity: usize) -> crate::errors::Result<Self> {
        let mut buffer = Vec::new();
        crate::resource::reserve_vec(&mut buffer, initial_capacity, "writer bit buffer")?;
        Ok(Self::new_from_buf(buffer))
    }

    /// Reserve room for appended encoded bytes plus the existing partial u64.
    /// Subsequent infallible native bit operations cannot grow within this bound.
    pub fn try_reserve_bytes(&mut self, additional: usize) -> crate::errors::Result<()> {
        if crate::resource::current().is_none() { return Ok(()); }
        let count = additional.checked_add(8).ok_or(crate::resource::ResourceExhausted {
            kind: "writer bit buffer", requested: usize::MAX, limit: isize::MAX as usize,
        })?;
        crate::resource::reserve_vec(&mut self.buffer, count, "writer bit buffer")
    }
