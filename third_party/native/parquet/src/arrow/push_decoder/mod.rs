...
impl ParquetPushDecoderBuilder {
    ...
    pub fn build(self) -> Result<ParquetPushDecoder, ParquetError> {
        let resource_policy = crate::resource::effective(self.metadata.resource_policy().cloned());
        ...
        let row_groups = match row_groups {
            Some(groups) => groups,
            None => {
                let mut groups = crate::resource::vec_with_capacity(
                    parquet_metadata.num_row_groups(),
                    "selected row groups",
                )?;
                groups.extend(0..parquet_metadata.num_row_groups());
                groups
            }
        };
...
impl ParquetPushDecoder {
    ...
    pub fn push_range(&mut self, range: Range<u64>, data: Bytes) -> Result<(), ParquetError> {
        let _scope = crate::resource::enter(self.resource_policy.clone());
        let mut ranges = crate::resource::vec_with_capacity(1, "single pushed range")?;
        let mut buffers = crate::resource::vec_with_capacity(1, "single pushed buffer")?;
        ranges.push(range);
        buffers.push(data);
        self.push_ranges(ranges, buffers)
    }
