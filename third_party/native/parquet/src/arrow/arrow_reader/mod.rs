...
impl ArrowReaderOptions {
    /// Install opt-in fallible admission before native metadata and page decoding.
    pub fn with_resource_policy(mut self, policy: crate::resource::ReaderResourcePolicy) -> Self {
        self.metadata_options = self.metadata_options.with_resource_policy(policy);
        self
    }
...
impl ArrowReaderMetadata {
    ...
    pub fn try_new(metadata: Arc<ParquetMetaData>, options: ArrowReaderOptions) -> Result<Self> {
        crate::resource::require_owned_metadata(metadata.resource_policy())?;
        let _scope = crate::resource::enter(metadata.resource_policy().cloned());
...
impl<T: ChunkReader + 'static> ParquetRecordBatchReaderBuilder<T> {
    ...
    pub fn build(self) -> Result<ParquetRecordBatchReader> {
        let _scope = crate::resource::enter(self.metadata.resource_policy().cloned());
        let Self {
            input,
            metadata,
            schema: _,
...
impl<T: ChunkReader + 'static> ReaderPageIterator<T> {
    ...
    fn next_page_reader(&mut self, rg_idx: usize) -> Result<SerializedPageReader<T>> {
        ...
        let properties = crate::file::properties::ReaderProperties::builder()
            .set_resource_policy(self.metadata.resource_policy().cloned())
            .build();
...
impl Iterator for ParquetRecordBatchReader {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let result = self.next_inner().and_then(|batch| {
            if let (Some(policy), Some(batch)) = (&self.resource_policy, &batch) {
                policy.retain_record_batch(batch)?;
            }
            Ok(batch)
        });
        result.map_err(|arrow_err| arrow_err.into()).transpose()
    }
}
...
impl ParquetRecordBatchReader {
    ...
    pub fn try_new_with_row_groups(
        ...
        let _scope = crate::resource::enter(row_groups.metadata().resource_policy().cloned());
