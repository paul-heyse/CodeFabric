...
impl ViewBuffer {
    ...
    pub fn try_append_block(&mut self, block: Buffer) -> Result<u32> {
        crate::resource::reserve_vec(&mut self.buffers, 1, "byte view block descriptors")?;
...
impl ValuesBuffer for ViewBuffer {
    ...
    fn try_with_capacity(capacity: usize) -> Result<Self> {
        Ok(Self {
            views: crate::resource::vec_with_capacity(capacity, "byte views")?,
            buffers: Vec::new(),
        })
    }

    fn try_reserve_exact(&mut self, additional: usize) -> Result<()> {
        crate::resource::reserve_vec(&mut self.views, additional, "byte views")
    }
