...
impl<I: OffsetSizeTrait> OffsetBuffer<I> {
    ...
    pub fn try_push(&mut self, data: &[u8], validate_utf8: bool) -> Result<()> {
        ...
        crate::resource::reserve_vec(&mut self.values, data.len(), "byte array values")?;
        crate::resource::reserve_vec(&mut self.offsets, 1, "byte array offsets")?;
    ...
    pub fn extend_from_dictionary<K: ArrowNativeType, V: ArrowNativeType>(
        ...
        crate::resource::reserve_vec(&mut self.offsets, keys.len(), "dictionary expanded offsets")?;
    ...
    pub fn try_into_array(
        ...
        crate::resource::admit_arrow_owners(2)?;
        Ok(self.into_array(null_buffer, data_type))
    }
...
impl<I: OffsetSizeTrait> ValuesBuffer for OffsetBuffer<I> {
    ...
    fn try_with_capacity(capacity: usize) -> Result<Self> {
        let count = capacity
            .checked_add(1)
            .ok_or_else(|| general_err!("offset capacity overflow"))?;
        let mut offsets = crate::resource::vec_with_capacity(count, "byte array offsets")?;
        offsets.push(I::default());
        Ok(Self {
            offsets,
            values: Vec::new(),
    ...
    fn try_reserve_exact(&mut self, additional: usize) -> Result<()> {
        crate::resource::reserve_vec(&mut self.offsets, additional, "byte array offsets")
    }
    ...
    fn pad_nulls(
        ...
        let additional = (read_offset + levels_read + 1).saturating_sub(self.offsets.len());
        crate::resource::reserve_vec(&mut self.offsets, additional, "byte array null offsets")?;
        self.offsets
...
mod tests {
    ...
    #[test]
    fn resource_dictionary_expansion_denied_before_selected_value_copy() {
        use crate::resource::{ResourceExhausted, enter, tests::policy};
        let (policy, _) = policy("dictionary expanded values");
        let _scope = enter(Some(policy));
        let mut output = OffsetBuffer::<i32>::try_with_capacity(0).unwrap();
        let error = output
            .extend_from_dictionary(&[0_i32, 0, 0], &[0_i32, 4], b"abcd")
            .unwrap_err();
        assert!(matches!(
            error,
            crate::errors::ParquetError::ResourceExhausted(ResourceExhausted {
                kind: "dictionary expanded values",
                requested: 4,
                ..
            })
        ));
        assert!(output.values.is_empty());
        assert_eq!(output.offsets, [0]);
    }
}
