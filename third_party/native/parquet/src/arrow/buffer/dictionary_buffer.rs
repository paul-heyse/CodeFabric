...
impl<K: ArrowNativeType + Ord, V: OffsetSizeTrait> DictionaryBuffer<K, V> {
    ...
    pub fn spill_values(&mut self) -> Result<&mut OffsetBuffer<V>> {
        match self {
            ...
            Self::Dict { keys, values } => {
                let mut spilled = OffsetBuffer::try_with_capacity(0)?;
    ...
    pub fn into_array(
        ...
        match self {
            ...
            Self::Values { values } => {
                ...
                let array = values.try_into_array(null_buffer, value_type)?;
...
impl<K: ArrowNativeType, V: OffsetSizeTrait> ValuesBuffer for DictionaryBuffer<K, V> {
    ...
    fn try_with_capacity(capacity: usize) -> Result<Self> {
        Ok(Self::Values {
            values: OffsetBuffer::try_with_capacity(capacity)?,
        })
    }
    ...
    fn try_reserve_exact(&mut self, additional: usize) -> Result<()> {
        match self {
            Self::Dict { keys, .. } => {
                crate::resource::reserve_vec(keys, additional, "dictionary keys")
            }
            Self::Values { values, .. } => values.try_reserve_exact(additional),
        }
    }
    ...
    fn pad_nulls(
        ...
        match self {
            Self::Dict { keys, .. } => {
                crate::resource::reserve_vec(
                    keys,
                    (read_offset + levels_read).saturating_sub(keys.len()),
                    "dictionary null keys",
                )?;
...
fn pack_values_impl<K: ArrowDictionaryKeyType, T: ByteArrayType>(
    ...
    let (values, bytes) = if crate::resource::current().is_some() {
        let offsets = array
            .len()
            .checked_add(1)
            .ok_or(crate::resource::ResourceExhausted {
                kind: "dictionary offset capacity",
                requested: usize::MAX,
                limit: isize::MAX as usize,
            })?;
        crate::resource::admit_dictionary_buffers::<K::Native>(
            array.len(),
            array.value_data().len(),
            Some((offsets, std::mem::size_of::<T::Offset>())),
        )?;
        crate::resource::admit_dictionary_table::<usize>(array.len())?;
        crate::resource::admit_shared_owner::<arrow_array::DictionaryArray<K>>()?;
        (array.len(), array.value_data().len())
    } else {
        (1024, 1024)
    };
    let mut builder =
        GenericByteDictionaryBuilder::<K, T>::with_capacity(array.len(), values, bytes);
    for x in array {
        match x {
...
fn pack_fixed_values_impl<K: ArrowDictionaryKeyType>(
    ...
    let values = if crate::resource::current().is_some() {
        let bytes = array
            .len()
            .checked_mul(array.value_length() as usize)
            .ok_or(crate::resource::ResourceExhausted {
                kind: "dictionary value capacity",
                requested: usize::MAX,
                limit: isize::MAX as usize,
            })?;
        crate::resource::admit_dictionary_buffers::<K::Native>(array.len(), bytes, None)?;
        crate::resource::admit_dictionary_table::<usize>(array.len())?;
        crate::resource::admit_shared_owner::<arrow_array::DictionaryArray<K>>()?;
        array.len()
    } else {
...
mod tests {
    ...
    #[test]
    fn resource_native_dictionary_packing_denies_before_builder_table() {
        let input = StringArray::from(vec![Some("alpha"), None, Some("beta"), Some("alpha")]);
        let (policy, _) = crate::resource::tests::policy("dictionary hash table");
        let _scope = crate::resource::enter(Some(policy));
        let error =
            pack_values_impl::<arrow_array::types::Int32Type, arrow_array::types::Utf8Type>(&input)
                .unwrap_err();
        assert!(matches!(
            error,
            ParquetError::ResourceExhausted(crate::resource::ResourceExhausted {
    ...
    #[test]
    fn resource_native_dictionary_packing_preserves_long_values_and_nulls() {
        let values: Vec<_> = (0..1500)
            .map(|index| (index % 9 != 0).then(|| index.to_string()))
            .collect();
        ...
        let packed =
            pack_values_impl::<arrow_array::types::Int32Type, arrow_array::types::Utf8Type>(&input)
                .unwrap();
        assert_eq!(
            cast(packed.as_ref(), &ArrowType::Utf8).unwrap().as_ref(),
            &input
        );
        ...
        assert!(
            requests
                ...
                .any(|request| request.kind == "dictionary builder keys"
                    && request.bytes == input.len() * 4)
        );
        assert!(
            requests
                .iter()
                .any(|request| request.kind == "dictionary builder values"
                    && request.bytes == input.value_data().len())
        );
    }
