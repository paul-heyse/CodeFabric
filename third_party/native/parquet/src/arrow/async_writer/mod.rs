...
pub trait AsyncFileWriter: Send {
    ...
    /// Fallible entrypoint used by governed writers. Implementations must admit
    /// the future's Box before returning it and preserve original authority.
    fn try_write(&mut self, bs: Bytes) -> Result<BoxFuture<'_, Result<()>>> {
        if crate::resource::current().is_some() {
            return Err(crate::resource::ResourceExhausted { kind: "async writer requires admitted future", requested: 1, limit: 0 }.into());
        }
        Ok(self.write(bs))
    }

    /// Fallible finalization entrypoint; the default preserves ungoverned use.
    fn try_complete(&mut self) -> Result<BoxFuture<'_, Result<()>>> {
        if crate::resource::current().is_some() {
            return Err(crate::resource::ResourceExhausted { kind: "async writer requires admitted completion", requested: 1, limit: 0 }.into());
        }
        Ok(self.complete())
    }
...
impl AsyncFileWriter for Box<dyn AsyncFileWriter + '_> {
    ...
    fn try_write(&mut self, bs: Bytes) -> Result<BoxFuture<'_, Result<()>>> { self.as_mut().try_write(bs) }
    fn try_complete(&mut self) -> Result<BoxFuture<'_, Result<()>>> { self.as_mut().try_complete() }
...
pub struct AsyncArrowWriter<W> {
    ...
    sync_writer: ArrowWriter<crate::resource::OwnedWriteBuffer>,
...
impl<W: AsyncFileWriter> AsyncArrowWriter<W> {
    ...
    pub fn try_new_with_options(
        ...
        let sync_writer = ArrowWriter::try_new_with_options(crate::resource::OwnedWriteBuffer::new(), arrow_schema, options)?;
    ...
    /// Append metadata through the admitted synchronous writer boundary.
    pub fn try_append_key_value_metadata(&mut self, kv_metadata: KeyValue) -> Result<()> { self.sync_writer.try_append_key_value_metadata(kv_metadata) }
    ...
    pub async fn finish(&mut self) -> Result<ParquetMetaData> {
        ...
        self.do_write().await?;
        self.async_writer.try_complete()?.await?;
    ...
    async fn do_write(&mut self) -> Result<()> {
        let buffer = self.sync_writer.inner_mut().try_take_bytes()?;
