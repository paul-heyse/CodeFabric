...
fn apply_hint(parquet: DataType, hint: DataType) -> Result<DataType> {
    Ok(match (&parquet, &hint) {
        ...
        (_, DataType::Dictionary(_, value)) => {
            ...
            let hinted = apply_hint(parquet, super::resource::clone_data_type(value.as_ref())?)?;
    ...
    match (info.logical_type_ref(), info.converted_type()) {
        ...
        (Some(LogicalType::Timestamp(timestamp)), _) => Ok(DataType::Timestamp(
            ...
            if timestamp.is_adjusted_to_u_t_c {
                Some(super::resource::timezone("UTC")?)
            } else {
                None
            },
        )),
