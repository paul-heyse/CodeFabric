...
pub(crate) fn parquet_to_arrow_schema_and_fields(
    ...
    if let Some(policy) = crate::resource::ReaderResourcePolicy::current() {
        if let Some(values) = key_value_metadata {
            resource::admit_metadata(
                &policy,
                values.iter().filter_map(|kv| {
                    kv.value
                        .as_ref()
                        .map(|value| (kv.key.as_str(), value.as_str()))
                }),
            )?;
        }
    }
...
pub fn parquet_to_arrow_field_levels_with_virtual(
    ...
    let mut parquet_field = match complex::convert_schema(schema, mask, hint)? {
        ...
        None if virtual_columns.is_empty() => {
            resource::native_struct_fields(0)?;
        ...
        match &mut parquet_field.field_type {
            ParquetFieldType::Group { children } => {
                ...
                for virtual_column in virtual_columns {
                    ...
                    crate::resource::reserve_vec(&mut fields_vec, 1, "virtual field references")?;
                    crate::resource::reserve_vec(children, 1, "virtual child descriptors")?;
                ...
                // Update the fields
                resource::native_struct_fields(fields_vec.len())?;
...
fn get_arrow_schema_from_metadata(encoded_meta: &str) -> Result<Schema> {
    if let Some(policy) = crate::resource::ReaderResourcePolicy::current() {
        return resource::decode_embedded_schema(&policy, encoded_meta);
    }
...
pub(crate) fn try_add_encoded_arrow_schema_to_metadata(schema: &Schema, props: &mut WriterProperties) -> Result<()> {
    native_resource::admit(schema, props)?;
    add_encoded_arrow_schema_to_metadata(schema, props);
    Ok(())
}
...
impl<'a> ArrowSchemaConverter<'a> {
    ...
    pub fn convert(&self, schema: &Schema) -> Result<SchemaDescriptor> {
        crate::resource::admit_writer_slots::<Arc<Type>>(schema.fields().len(), "writer schema root fields")?;
        let fields = schema
            .fields()
            .iter()
            .map(|field| arrow_to_parquet_type(field, self.coerce_types).and_then(resource::parquet_type_owner))
            .collect::<Result<_>>()?;
        let group = Type::group_type_builder(self.schema_root)
            .with_fields(fields)
            .build()?;
        SchemaDescriptor::try_new(resource::parquet_type_owner(group)?)
    }
}
...
fn arrow_to_parquet_type(field: &Field, coerce_types: bool) -> Result<Type> {
    arrow_to_parquet_type_at(field, coerce_types, 1)
}
fn arrow_to_parquet_type_at(field: &Field, coerce_types: bool, depth: usize) -> Result<Type> {
    if let Some(policy) = crate::resource::current() {
        crate::resource::ReaderResourcePolicy::check(depth, policy.limits().schema_depth, "writer schema conversion depth")?;
        // Geospatial extension deserialization can allocate before the Parquet
        // logical type is built. Reject its unselected profile at this boundary.
        #[cfg(feature = "geospatial")]
        if field.extension_type_name() == Some("geoarrow.wkb") { return Err(crate::resource::ResourceExhausted { kind: "writer geospatial extension outside admitted profile", requested: 1, limit: 0 }.into()); }
    }
    ...
    match field.data_type() {
        ...
        | DataType::LargeListView(f) => {
            let field_ref = if coerce_types && f.name() != PARQUET_LIST_ELEMENT_NAME {
                ...
                if let Some(policy) = crate::resource::current() { policy.string(PARQUET_LIST_ELEMENT_NAME.len())?; }
                let ff = resource::clone_field(f)?.with_name(PARQUET_LIST_ELEMENT_NAME);
                resource::parquet_type_owner(arrow_to_parquet_type_at(&ff, coerce_types, depth + 1)?)?
            } else {
                resource::parquet_type_owner(arrow_to_parquet_type_at(f, coerce_types, depth + 1)?)?
            };

            crate::resource::admit_writer_slots::<Arc<Type>>(1, "writer schema list children")?;
            crate::resource::admit_writer_slots::<Arc<Type>>(1, "writer schema list wrapper")?;
            Type::group_type_builder(name)
                .with_fields(vec![resource::parquet_type_owner(
        ...
        DataType::Struct(fields) => {
            ...
            crate::resource::admit_writer_slots::<Arc<Type>>(fields.len(), "writer schema struct children")?;
            let fields = fields
                .iter()
                .map(|f| arrow_to_parquet_type_at(f, coerce_types, depth + 1).and_then(resource::parquet_type_owner))
                .collect::<Result<_>>()?;
            Type::group_type_builder(name)
                .with_fields(fields)
                .with_repetition(repetition)
            ...
            if let DataType::Struct(struct_fields) = field.data_type() {
                ...
                let fix_map_field = |name: &str, fld: &Arc<Field>| -> Result<Arc<Type>> {
                    if coerce_types && fld.name() != name {
                        if let Some(policy) = crate::resource::current() { policy.string(name.len())?; }
                        let f = resource::clone_field(fld)?.with_name(name);
                        resource::parquet_type_owner(arrow_to_parquet_type_at(&f, coerce_types, depth + 1)?)
                    } else {
                        resource::parquet_type_owner(arrow_to_parquet_type_at(fld, coerce_types, depth + 1)?)
                    }
                };
                ...
                crate::resource::admit_writer_slots::<Arc<Type>>(2, "writer schema map children")?;
                crate::resource::admit_writer_slots::<Arc<Type>>(1, "writer schema map wrapper")?;
                Type::group_type_builder(name)
                    .with_fields(vec![resource::parquet_type_owner(
        ...
        DataType::Dictionary(_, value) => {
            ...
            let dict_field = resource::clone_field(field)?.with_data_type(resource::clone_data_type(value)?);
            arrow_to_parquet_type_at(&dict_field, coerce_types, depth + 1)
        }
        DataType::RunEndEncoded(_, value_field) => {
            let ree_value_field = resource::clone_field(field)?
                .with_data_type(resource::clone_data_type(value_field.data_type())?);
            arrow_to_parquet_type_at(&ree_value_field, coerce_types, depth + 1)
        }
    }
}
