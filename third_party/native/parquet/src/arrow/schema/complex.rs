...
impl ParquetField {
    ...
    fn into_list(self, name: &str) -> Result<Self> {
        resource::native_field(name)?;
        resource::native_children(1)?;
        Ok(ParquetField {
            ...
            arrow_type: DataType::List(Arc::new(Field::new(
                name,
                resource::clone_data_type(&self.arrow_type)?,
                false,
            ))),
...
impl Visitor {
    fn visit_primitive(
        ...
        let primitive_arrow_data_type = match repetition {
            Repetition::REPEATED if context.treat_repeated_as_list_arrow_hint => {
                ...
                arrow_field
                    .map(|f| resource::clone_data_type(f.data_type()))
                    .transpose()?
            }
            _ => context
                .data_type
                .as_ref()
                .map(resource::clone_data_type)
                .transpose()?,
        };
    ...
    fn visit_struct(
        ...
        resource::native_struct_fields(parquet_fields.len())?;
        resource::native_children(parquet_fields.len())?;
        ...
        for (idx, parquet_field) in parquet_fields.iter().enumerate() {
            let data_type = match arrow_fields {
                Some(fields) => {
                    ...
                    Some(resource::clone_data_type(field.data_type())?)
                }
                None => None,
            };
    ...
    fn visit_map(
        ...
        let maybe_key = {
            let context = VisitorContext {
                ...
                def_level,
                data_type: arrow_key
                    .map(|x| resource::clone_data_type(x.data_type()))
                    .transpose()?,
                // Key is not repeated
        ...
        match (maybe_key, maybe_value) {
            (Some(key), Some(value)) => {
                ...
                let field_metadata = match arrow_map {
                    Some(field) => resource::clone_metadata(field.metadata())?,
                    _ => HashMap::default(),
                };

                resource::native_field(map_key_value.name())?;
                resource::native_struct_fields(2)?;
                resource::native_children(2)?;
    ...
    fn visit_list(
        ...
        // Compare the same legacy <list-name>_tuple spelling without an
        // allocated formatting temporary.
        if items.len() != 1
            || (!repeated_field.is_list()
                && !repeated_field.has_single_repeated_child()
                && (repeated_field.name() == "array"
                    || repeated_field.name().strip_suffix("_tuple") == Some(list_type.name())))
        {
...
pub(super) fn convert_virtual_field(
    ...
    Ok(ParquetField {
        ...
        def_level,
        nullable,
        arrow_type: resource::clone_data_type(arrow_field.data_type())?,
...
fn convert_field(
    ...
    resource::native_field(name)?;
    let data_type = resource::clone_data_type(&field.arrow_type)?;
