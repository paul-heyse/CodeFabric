...
pub trait ValuesBuffer {
    ...
    /// Fallible preallocation under the current synchronous reader scope.
    fn try_with_capacity(capacity: usize) -> Result<Self>
    where
        Self: Sized;

    /// Admit the complete new backing before growing.
...
impl<T: Copy + Default> ValuesBuffer for Vec<T> {
    ...
    fn try_with_capacity(capacity: usize) -> Result<Self> {
        crate::resource::vec_with_capacity(capacity, "primitive values")
    }
    fn try_reserve_exact(&mut self, additional: usize) -> Result<()> {
        crate::resource::reserve_vec(self, additional, "primitive values")
    }
    ...
    fn pad_nulls(
        ...
        crate::resource::reserve_vec(
            self,
            (read_offset + levels_read).saturating_sub(self.len()),
            "primitive null padding",
        )?;
