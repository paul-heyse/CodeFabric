    ...
    CV: ColumnValueDecoder<Buffer = V>,
{
    ...
    fn read_one_batch(&mut self, batch_size: usize) -> Result<usize> {
        ...
        if self.values.is_none() {
            self.values = Some(V::try_with_capacity(0)?);
        }
        let values = self.values.as_mut().unwrap();
        let compact_bitmap = &mut self.compact_bitmap;

        if padding_threshold.is_none() {
            values.try_reserve_exact(self.capacity_hint.saturating_sub(self.num_values))?;
        }
            ...
            .read_records_with_reservation(
                ...
                |values, values_to_read, levels_to_read, def_levels| {
                    let output_slots = if let Some(threshold) = padding_threshold {
                        ...
                        crate::resource::reserve_bitmap(
                            bitmap,
                            batch_levels.len(),
                            "compact validity bitmap",
                        )?;
        ...
        if self.padding_threshold.is_some() {
            ...
            if values_read < item_count {
                values.try_reserve_exact(item_count - values_read)?;
                values.pad_nulls(
                    self.values_written,
                    values_read,
                    item_count,
            ...
            values.try_reserve_exact(levels_read - values_read)?;
            values.pad_nulls(
                self.num_values,
                values_read,
                levels_read,
