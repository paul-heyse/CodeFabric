...
impl ArrayReader for StructArrayReader {
    ...
    fn consume_batch(&mut self) -> Result<ArrayRef> {
        if self.children.is_empty() {
            crate::resource::admit_shared_owner::<StructArray>()?;
            return Ok(Arc::new(StructArray::from(Vec::new())));
        }

        let mut children_arrays =
            crate::resource::vec_with_capacity(self.children.len(), "struct child arrays")?;
        for reader in &mut self.children {
            children_arrays.push(reader.consume_batch()?);
        }
        ...
        let nulls = if self.nullable {
            ...
            crate::resource::reserve_bitmap(&mut bitmap, def_levels.len(), "struct validity")?;
