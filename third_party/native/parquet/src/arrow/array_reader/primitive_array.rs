...
pub trait IntoBuffer {
    fn into_buffer(self, target_type: &ArrowType) -> Result<Buffer>;
}

macro_rules! native_buffer {
    ($($t:ty),*) => {
        $(impl IntoBuffer for Vec<$t> {
            fn into_buffer(self, _target_type: &ArrowType) -> Result<Buffer> {
                crate::resource::admit_arrow_owners(1)?;
                Ok(Buffer::from_vec(self))
            }
        })*
    };
}
...
impl IntoBuffer for Vec<bool> {
    fn into_buffer(self, _target_type: &ArrowType) -> Result<Buffer> {
        ...
        crate::resource::reserve_bitmap(&mut builder, self.len(), "boolean values bitmap")?;
        builder.append_slice(&self);
        Ok(builder.finish().into_inner())
    }
}

impl IntoBuffer for Vec<Int96> {
    fn into_buffer(self, target_type: &ArrowType) -> Result<Buffer> {
        let mut builder = crate::resource::vec_with_capacity(self.len(), "int96 timestamp output")?;
        match target_type {
    ...
    if width != 0 {
        if let Some(policy) = crate::resource::current() {
            ...
            crate::resource::ReaderResourcePolicy::check(
                ...
                "primitive conversion bytes",
            )?;
            // Arrow59.2 PrimitiveArray::unary collects an exact-size iterator
            // into a Vec of the destination primitive type before wrapping it.
            policy.reserve(bytes, "primitive converted values")?;
            crate::resource::admit_arrow_owners(1)?;
        }
    }
    Ok(())
...
fn coerce_i32(array: &Int32Array, target_type: &ArrowType) -> Result<ArrayRef> {
    let width = match target_type {
        ArrowType::UInt8 | ArrowType::Int8 => 1,
        ArrowType::UInt16 | ArrowType::Int16 => 2,
        ArrowType::Date64 | ArrowType::Timestamp(_, _) | ArrowType::Decimal64(_, _) => 8,
        ArrowType::Decimal128(_, _) => 16,
        ArrowType::Decimal256(_, _) => 32,
        _ => 0,
    };
    admit_primitive_conversion(array.len(), width)?;
...
fn pack_dictionary_impl<K: ArrowDictionaryKeyType, V: ArrowPrimitiveType>(
    ...
    let keys = if crate::resource::current().is_some() {
        let bytes = values
            .len()
            .checked_mul(std::mem::size_of::<V::Native>())
            .ok_or(crate::resource::ResourceExhausted {
                kind: "dictionary value capacity",
                requested: usize::MAX,
                limit: isize::MAX as usize,
            })?;
        crate::resource::admit_dictionary_buffers::<K::Native>(values.len(), bytes, None)?;
        // Native PrimitiveDictionaryBuilder uses HashMap<Value<V::Native>,
        // usize>; Value<T> is a single-field wrapper of T in Arrow59.2.
        crate::resource::admit_dictionary_table::<(V::Native, usize)>(values.len())?;
        crate::resource::admit_shared_owner::<arrow_array::DictionaryArray<K>>()?;
        values.len()
    } else {
        1024
    };
    let mut builder = PrimitiveDictionaryBuilder::<K, V>::with_capacity(keys, values.len());
...
mod tests {
    use super::*;
    #[test]
    fn resource_native_primitive_dictionary_packing_admits_full_input_length() {
        let values = Int32Array::from(
            (0..1500)
                .map(|value| (value % 9 != 0).then_some(value))
                .collect::<Vec<_>>(),
        );
        let (policy, admission) = crate::resource::tests::policy("");
        let _scope = crate::resource::enter(Some(policy));
        let packed = pack_dictionary_impl::<
            arrow_array::types::Int32Type,
            arrow_array::types::Int32Type,
        >(&values)
        .unwrap();
        assert_eq!(
            arrow::compute::cast(packed.as_ref(), &ArrowType::Int32)
                .unwrap()
                .as_ref(),
            &values
        );
        assert!(
            admission
                ...
                .unwrap()
                .iter()
                .any(|request| request.kind == "dictionary builder keys"
                    && request.bytes == values.len() * 4)
        );
    }
