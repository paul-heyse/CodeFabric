...
impl<OffsetSize: OffsetSizeTrait> ArrayReader for ListViewArrayReader<OffsetSize> {
    ...
    fn consume_batch(&mut self) -> Result<ArrayRef> {
        ...
        // Native From<ListArray> creates two Vec<OffsetSize> of exactly len:
        // one for offsets and one for sizes, then two shared Buffer owners.
        let bytes = list_array
            .len()
            .checked_mul(std::mem::size_of::<OffsetSize>())
            .ok_or(crate::resource::ResourceExhausted {
                kind: "list view geometry",
                requested: usize::MAX,
                limit: isize::MAX as usize,
            })?;
        crate::resource::admit_output_allocation(bytes, "list view offsets")?;
        crate::resource::admit_output_allocation(bytes, "list view sizes")?;
        crate::resource::admit_arrow_owners(2)?;
        crate::resource::admit_shared_owner::<GenericListViewArray<OffsetSize>>()?;
