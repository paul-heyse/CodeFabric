...
impl ArrayReader for FixedLenByteArrayReader {
    ...
    fn consume_batch(&mut self) -> Result<ArrayRef> {
        ...
        let converted_width = match &self.data_type {
            ArrowType::Decimal32(_, _) | ArrowType::Interval(IntervalUnit::YearMonth) => 4,
            ArrowType::Decimal64(_, _) | ArrowType::Interval(IntervalUnit::DayTime) => 8,
            ArrowType::Decimal128(_, _) => 16,
            ArrowType::Decimal256(_, _) => 32,
            ArrowType::Float16 => 2,
            _ => 0,
        };
        // Arrow PrimitiveArray::from_unary collects one exact-length native
        // Vec before constructing its Buffer, with the null bitmap shared.
        super::primitive_array::admit_primitive_conversion(len, converted_width)?;
...
impl ValuesBuffer for FixedLenByteArrayBuffer {
        ...
        match self.byte_length {
            Some(width) => {
                let bytes = additional
                    .checked_mul(width)
                    .ok_or_else(|| general_err!("fixed width capacity overflow"))?;
                crate::resource::reserve_vec(&mut self.buffer, bytes, "fixed length values")?;
            }
            None => self.reserve_exact(additional),
        }
        Ok(())
    }
    ...
    fn pad_nulls(
        ...
        let additional =
            ((read_offset + levels_read) * byte_length).saturating_sub(self.buffer.len());
        crate::resource::reserve_vec(&mut self.buffer, additional, "fixed length null padding")?;
        self.buffer
            ...
            None => {
                ...
                if out.buffer.is_empty() {
                    if let Some(values_capacity) = out.values_capacity.take() {
                        ...
                        out.buffer = crate::resource::vec_with_capacity(
                            byte_capacity,
                            "fixed length values",
                        )?;
                    }
        ...
        match self.decoder.as_mut().unwrap() {
            ...
            Decoder::Dict { decoder } => {
                ...
                decoder.read(num_values, |keys| {
                    crate::resource::reserve_vec(
                        &mut out.buffer,
                        keys.len().saturating_mul(self.byte_length),
                        "fixed length dictionary values",
                    )?;
                    for key in keys {
            ...
            Decoder::Delta { decoder } => {
                ...
                crate::resource::reserve_vec(
                    &mut out.buffer,
                    to_read.saturating_mul(self.byte_length),
                    "fixed length delta values",
                )?;
