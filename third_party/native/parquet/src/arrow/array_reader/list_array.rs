...
impl<OffsetSize: OffsetSizeTrait> ArrayReader for ListArrayReader<OffsetSize> {
    ...
    fn consume_batch(&mut self) -> Result<ArrayRef> {
        ...
        let mut list_offsets: Vec<OffsetSize> =
            crate::resource::vec_with_capacity(levels_len.saturating_add(1), "list offsets")?;
        let mut validity = self.nullable.then(|| BooleanBufferBuilder::new(0));
        if let Some(validity) = validity.as_mut() {
            crate::resource::reserve_bitmap(validity, levels_len, "list validity")?;
        }
        ...
        let nulls = if let Some(builder) = validity {
            ...
            Some(NullBuffer::from(builder))
        } else {
            None
        };
        let field = match &self.data_type {
            ArrowType::List(field) | ArrowType::LargeList(field) => field.clone(),
            _ => return Err(general_err!("list reader has a non-list type")),
        };
        crate::resource::admit_arrow_owners(1)?;
        crate::resource::admit_shared_owner::<GenericListArray<OffsetSize>>()?;
        // Move the already-admitted offsets into Arrow and preserve the original
        // child ArrayRef. The native constructor avoids a second offset payload
        // copy and the recursive child ArrayData descriptor round-trip.
