...
impl ArrayReader for ByteViewArrayReader {
    ...
    fn consume_batch(&mut self) -> Result<ArrayRef> {
        ...
        let array = buffer.try_into_array(null_buffer, &self.data_type)?;
...
impl ColumnValueDecoder for ByteViewArrayColumnValueDecoder {
    ...
    fn set_dict(
        ...
        let mut buffer = ViewBuffer::try_with_capacity(num_values)?;
        let mut decoder =
    ...
    fn read_impl<const VALIDATE_UTF8: bool>(
        ...
        let block_id = {
            if output.buffers.last().is_some_and(|x| x.ptr_eq(&self.buf)) {
                ...
                output.try_append_block(self.buf.clone())?
            }
        };
        ...
        crate::resource::reserve_vec(&mut output.views, to_read, "byte views")?;
...
impl ByteViewArrayDecoderDictionary {
    ...
    fn read(&mut self, output: &mut ViewBuffer, dict: &ViewBuffer, len: usize) -> Result<usize> {
        ...
        crate::resource::reserve_vec(&mut output.views, len, "byte views")?;
...
impl ByteViewArrayDecoderDeltaLength {
    fn new(data: Bytes, validate_utf8: bool) -> Result<Self> {
        ...
        let mut lengths = crate::resource::vec_with_capacity(values, "byte view delta lengths")?;
        lengths.resize(values, 0);
    ...
    fn read(&mut self, output: &mut ViewBuffer, len: usize) -> Result<usize> {
        ...
        let block_id = output.try_append_block(bytes)?;
...
impl ByteViewArrayDecoderDelta {
    ...
    fn read(&mut self, output: &mut ViewBuffer, len: usize) -> Result<usize> {
        ...
        crate::resource::reserve_vec(&mut output.views, to_reserve, "byte views")?;

        // array buffer only have long strings
        let mut array_buffer: Vec<u8> =
            crate::resource::vec_with_capacity(4096, "byte view delta values")?;
        ...
        let read = if !self.validate_utf8 {
            ...
            let mut utf8_validation_buffer =
                crate::resource::vec_with_capacity(4096, "byte view utf8 scratch")?;
            ...
            let v = self.decoder.read(len, |bytes| {
                ...
                if bytes.len() > 12 {
                    ...
                    crate::resource::reserve_vec(
                        &mut utf8_validation_buffer,
                        bytes.len(),
                        "byte view utf8 scratch",
                    )?;
        ...
        let actual_block_id = output.try_append_block(Buffer::from_vec(array_buffer))?;
