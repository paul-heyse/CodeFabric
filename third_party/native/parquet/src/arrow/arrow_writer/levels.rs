...
use arrow_array::{Array, ArrayRef, Int32Array, OffsetSizeTrait, RunArray, downcast_run_array, downcast_dictionary_array};
...
fn level_capacity_error(kind: &'static str) -> ParquetError {
    crate::resource::ResourceExhausted { kind, requested: usize::MAX, limit: isize::MAX as usize }.into()
}
fn admit_logical_nulls(array: &dyn Array) -> Result<()> {
    let Some(policy) = crate::resource::current() else { return Ok(()); };
    fn bitmap(policy: &crate::resource::ReaderResourcePolicy, rows: usize) -> Result<()> {
        let bytes = rows.checked_add(7).map(|v| v / 8).and_then(|v| v.checked_add(63)).map(|v| v & !63).ok_or_else(|| level_capacity_error("writer logical null bitmap"))?;
        crate::resource::ReaderResourcePolicy::check(bytes, policy.limits().output_bytes, "writer logical null bitmap")?;
        policy.reserve(bytes, "writer logical null bitmap")?;
        policy.reserve(arrow_buffer::allocation_owner_layout().size(), "writer logical null buffer owner")?;
        Ok(())
    }
    match array.data_type() {
        DataType::Null if !array.is_empty() => bitmap(&policy, array.len())?,
        DataType::Dictionary(_, _) => {
            arrow_array::downcast_dictionary_array!(array => {
                let values = array.values();
                // This writer admits dictionaries only over is_leaf types.
                // Ordinary leaf logical_nulls clones an existing bitmap; Null
                // creates one. The dictionary creates one bitmap if values
                // supply logical nulls, including a Some all-valid bitmap.
                if values.data_type() == &DataType::Null && !values.is_empty() { bitmap(&policy, values.len())?; }
                if values.nulls().is_some() || (values.data_type() == &DataType::Null && !values.is_empty()) { bitmap(&policy, array.len())?; }
            }, _ => return Err(level_capacity_error("writer dictionary implementation")));
        }
        _ => {}
    }
    Ok(())
...
pub(crate) fn calculate_array_levels(array: &ArrayRef, field: &Field) -> Result<Vec<ArrayLevels>> {
    ...
    if crate::resource::current().is_some() {
        builder.plan_capacity(0..array.len())?;
        builder.try_visit_leaves(&mut |leaf| leaf.reserve_planned_capacity())?;
    }
...
impl LevelInfoBuilder {
    ...
    fn try_new(field: &Field, parent_ctx: LevelContext, array: &ArrayRef) -> Result<Self> {
        Self::try_new_at(field, parent_ctx, array, 1)
    }
    fn try_new_at(field: &Field, parent_ctx: LevelContext, array: &ArrayRef, depth: usize) -> Result<Self> {
        if let Some(policy) = crate::resource::current() {
            crate::resource::ReaderResourcePolicy::check(depth, policy.limits().schema_depth, "writer levels depth")?;
            crate::resource::ReaderResourcePolicy::check(array.len(), policy.limits().output_values, "writer input array rows")?;
        }
        if parent_ctx.def_level > i16::MAX - 2 || parent_ctx.rep_level == i16::MAX { return Err(level_capacity_error("writer level overflow")); }
        ...
        match array.data_type() {
            d if is_leaf(d) => {
                let levels = ArrayLevels::try_new(parent_ctx, is_nullable, array.clone())?;
            ...
            DataType::RunEndEncoded(_, value_field) => {
                // Selected checkpoint/CPG schemas do not currently contain
                // REE. Its take expansion remains a separate admitted-kernel
                // obligation; reject before its index/array allocations.
                if crate::resource::current().is_some() { return Err(level_capacity_error("writer REE take admission required")); }
                ...
                Self::try_new_at(&flat_field, parent_ctx, &flat, depth + 1)
            }
            DataType::Struct(children) => {
                ...
                let mut child_builders = crate::resource::vec_with_capacity(children.len(), "writer level child builders")?;
                for (f, a) in children.iter().zip(array.columns()) { child_builders.push(Self::try_new_at(f, ctx, a, depth + 1)?); }
            ...
            | DataType::LargeListView(child) => {
                ...
                if let Some(policy) = crate::resource::current() { policy.reserve(std::mem::size_of::<Self>(), "writer level child Box")?; }
                Ok(match field.data_type() {
                    DataType::List(_) => {
                        let list = array.as_list();
                        let child = Self::try_new_at(child.as_ref(), ctx, list.values(), depth + 1)?;
                    ...
                    DataType::Map(_, _) => {
                        ...
                        let entries = arrow_array::try_make_array(map.entries().try_to_data()?)?;
                        let child = Self::try_new_at(child.as_ref(), ctx, &entries, depth + 1)?;
                        let is_last = child.child_has_no_nested_rep();
    ...
    fn try_visit_leaves(&mut self, visit: &mut impl FnMut(&mut ArrayLevels) -> Result<()>) -> Result<()> {
        match self {
            Self::Primitive(leaf) => visit(leaf)?,
            Self::List(child, ..) | Self::LargeList(child, ..) | Self::FixedSizeList(child, ..)
            | Self::ListView(child, ..) | Self::LargeListView(child, ..) => child.try_visit_leaves(visit)?,
            Self::Struct(children, ..) => { for child in children { child.try_visit_leaves(visit)?; } }
        }
        Ok(())
    }

    fn plan_placeholders(&mut self, count: usize) -> Result<()> {
        self.try_visit_leaves(&mut |leaf| leaf.plan_levels(count))
    }

    /// Capacity-only walk over original offsets. Null parents may cause this
    /// walk to include child ranges later skipped by the native writer: that is
    /// a conservative structural admission, never an inferred empty result.
    fn plan_capacity(&mut self, range: Range<usize>) -> Result<()> {
        match self {
            Self::Primitive(leaf) => leaf.plan_levels(range.end - range.start)?,
            Self::Struct(children, _, nulls) => {
                let null_count = nulls.as_ref().map_or(0, |n| n.slice(range.start, range.end - range.start).null_count());
                for child in children { child.plan_capacity(range.clone())?; child.plan_placeholders(null_count)?; }
            }
            Self::List(child, _, offsets, nulls, _) => Self::plan_list(child, offsets, nulls.as_ref(), range)?,
            Self::LargeList(child, _, offsets, nulls, _) => Self::plan_list(child, offsets, nulls.as_ref(), range)?,
            Self::FixedSizeList(child, _, size, nulls) => {
                let start = range.start.checked_mul(*size).ok_or_else(|| level_capacity_error("writer fixed list range"))?;
                let end = range.end.checked_mul(*size).ok_or_else(|| level_capacity_error("writer fixed list range"))?;
                child.plan_capacity(start..end)?;
                let placeholders = if *size == 0 { range.end - range.start } else { nulls.as_ref().map_or(0, |n| n.slice(range.start, range.end - range.start).null_count()) };
                child.plan_placeholders(placeholders)?;
            }
            Self::ListView(child, _, offsets, sizes, nulls) => Self::plan_list_view(child, offsets, sizes, nulls.as_ref(), range)?,
            Self::LargeListView(child, _, offsets, sizes, nulls) => Self::plan_list_view(child, offsets, sizes, nulls.as_ref(), range)?,
        }
        Ok(())
    }

    fn plan_list<O: OffsetSizeTrait>(child: &mut Self, offsets: &[O], nulls: Option<&NullBuffer>, range: Range<usize>) -> Result<()> {
        child.plan_capacity(offsets[range.start].as_usize()..offsets[range.end].as_usize())?;
        let placeholders = range.filter(|&row| offsets[row] == offsets[row + 1] || nulls.is_some_and(|n| n.is_null(row))).count();
        child.plan_placeholders(placeholders)
    }

    fn plan_list_view<O: OffsetSizeTrait>(child: &mut Self, offsets: &[O], sizes: &[O], nulls: Option<&NullBuffer>, range: Range<usize>) -> Result<()> {
        for row in range {
            let start = offsets[row].as_usize();
            let size = sizes[row].as_usize();
            let end = start.checked_add(size).ok_or_else(|| level_capacity_error("writer list view range"))?;
            child.plan_capacity(start..end)?;
            if size == 0 || nulls.is_some_and(|n| n.is_null(row)) { child.plan_placeholders(1)?; }
        }
        Ok(())
    }

    fn try_finish(self) -> Result<Vec<ArrayLevels>> {
        if crate::resource::current().is_none() { return Ok(self.finish()); }
        fn count(node: &LevelInfoBuilder) -> Result<usize> {
            Ok(match node {
                LevelInfoBuilder::Primitive(_) => 1,
                LevelInfoBuilder::List(c, ..) | LevelInfoBuilder::LargeList(c, ..) | LevelInfoBuilder::FixedSizeList(c, ..)
                | LevelInfoBuilder::ListView(c, ..) | LevelInfoBuilder::LargeListView(c, ..) => count(c)?,
                LevelInfoBuilder::Struct(children, ..) => children.iter().try_fold(0usize, |n, child| n.checked_add(count(child)?).ok_or_else(|| level_capacity_error("writer leaf count")))?,
            })
        }
        fn append(node: LevelInfoBuilder, output: &mut Vec<ArrayLevels>) {
            match node {
                LevelInfoBuilder::Primitive(leaf) => output.push(leaf),
                LevelInfoBuilder::List(c, ..) | LevelInfoBuilder::LargeList(c, ..) | LevelInfoBuilder::FixedSizeList(c, ..)
                | LevelInfoBuilder::ListView(c, ..) | LevelInfoBuilder::LargeListView(c, ..) => append(*c, output),
                LevelInfoBuilder::Struct(children, ..) => { for child in children { append(child, output); } }
            }
        }
        let mut output = crate::resource::vec_with_capacity(count(&self)?, "writer leaf descriptors")?;
...
impl LevelData {
    ...
    fn append_run(&mut self, value: i16, count: usize) {
        ...
        match self {
            ...
            Self::Materialized(values) if values.is_empty() && crate::resource::current().is_none() => {
...
impl ArrayLevels {
    fn try_new(ctx: LevelContext, is_nullable: bool, array: ArrayRef) -> Result<Self> {
        ...
        admit_logical_nulls(array.as_ref())?;
    ...
    fn plan_levels(&mut self, additional: usize) -> Result<()> {
        self.planned_levels = self.planned_levels.checked_add(additional).ok_or_else(|| level_capacity_error("writer expanded levels"))?;
        if let Some(policy) = &self.resource_policy { crate::resource::ReaderResourcePolicy::check(self.planned_levels, policy.limits().output_values, "writer expanded levels")?; }
        Ok(())
    }
    fn reserve_planned_capacity(&mut self) -> Result<()> {
        for stream in [&mut self.def_levels, &mut self.rep_levels] {
            if let LevelData::Materialized(values) = stream { crate::resource::reserve_vec(values, self.planned_levels, "writer materialized levels")?; }
        }
        crate::resource::reserve_vec(&mut self.non_null_indices, self.planned_levels, "writer non-null indices")
    }
