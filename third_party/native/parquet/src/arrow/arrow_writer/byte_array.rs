...
impl FallbackEncoder {
    ...
    fn new(descr: &ColumnDescPtr, props: &WriterProperties) -> Result<Self> {
        ...
        if let Some(policy) = crate::resource::current() {
            let count = match encoding { Encoding::DELTA_LENGTH_BYTE_ARRAY => 1, Encoding::DELTA_BYTE_ARRAY => 2, _ => 0 };
            for _ in 0..count { policy.reserve(std::mem::size_of::<DeltaBitPackEncoder<Int32Type>>(), "writer delta encoder owner")?; }
        }
        let encoder = match encoding {
            Encoding::PLAIN => FallbackEncoderImpl::Plain { buffer: vec![] },
            Encoding::DELTA_LENGTH_BYTE_ARRAY => FallbackEncoderImpl::DeltaLength {
                buffer: vec![],
                lengths: Box::new(DeltaBitPackEncoder::try_new()?),
            },
            Encoding::DELTA_BYTE_ARRAY => FallbackEncoderImpl::Delta {
                buffer: vec![],
                last_value: vec![],
                prefix_lengths: Box::new(DeltaBitPackEncoder::try_new()?),
                suffix_lengths: Box::new(DeltaBitPackEncoder::try_new()?),
            },
            _ => {
    ...
    fn encode<T>(&mut self, values: T, indices: impl ExactSizeIterator<Item = usize>) -> Result<()>
    where
        ...
        match &mut self.encoder {
            FallbackEncoderImpl::Plain { buffer } => {
                for idx in indices {
                    ...
                    crate::resource::extend_writer_bytes(buffer, (value.len() as u32).as_bytes())?;
                    crate::resource::extend_writer_bytes(buffer, value)?;
            ...
            FallbackEncoderImpl::Delta {
                ...
                for idx in indices {
                    ...
                    last_value.clear();
                    crate::resource::extend_writer_bytes(last_value, value)?;

                    crate::resource::extend_writer_bytes(buffer, &value[prefix_length..])?;
    ...
    fn flush_data_page(
        ...
        let (buf, encoding) = match &mut self.encoder {
            ...
            FallbackEncoderImpl::DeltaLength { buffer, lengths } => {
                ...
                let mut out = Vec::new();
                crate::resource::extend_writer_bytes(&mut out, &lengths)?;
                crate::resource::extend_writer_bytes(&mut out, buffer)?;
                buffer.clear();
            ...
            FallbackEncoderImpl::Delta {
                ...
                let mut out = Vec::new();
                crate::resource::extend_writer_bytes(&mut out, &prefix_lengths)?;
                crate::resource::extend_writer_bytes(&mut out, &suffix_lengths)?;
                crate::resource::extend_writer_bytes(&mut out, buffer)?;
                buffer.clear();
                last_value.clear();
        ...
        Ok(DataPageValues {
            buf: crate::resource::own_writer_vec(buf)?,
...
impl Storage for ByteArrayStorage {
    ...
    fn try_push(&mut self, value: &Self::Value) -> Result<Self::Key> {
        let bytes = value.len().checked_add(4).ok_or(crate::resource::ResourceExhausted { kind: "writer dictionary payload", requested: usize::MAX, limit: isize::MAX as usize })?;
        crate::resource::reserve_vec(&mut self.page, bytes, "writer dictionary payload")?;
        crate::resource::reserve_vec(&mut self.values, 1, "writer dictionary ranges")?;
        Ok(self.push(value))
    }
...
impl DictEncoder {
    ...
    where
        ...
        T::Item: AsRef<[u8]>,
    {
        crate::resource::reserve_vec(&mut self.indices, indices.len(), "writer dictionary indices")?;

        for idx in indices {
            let value = values.value(idx);
            let interned = self.interner.try_intern(value.as_ref())?;
    ...
    fn flush_dict_page(self) -> Result<DictionaryPage> {
        let storage = self.interner.into_inner();

        Ok(DictionaryPage {
            buf: crate::resource::own_writer_vec(storage.page)?,
    ...
    fn flush_data_page(
        ...
        crate::resource::reserve_vec(&mut buffer, buffer_len, "writer dictionary RLE buffer")?;
        ...
        Ok(DataPageValues {
            buf: crate::resource::own_writer_vec(encoder.consume())?,
            num_values,
...
impl ColumnValueEncoder for ByteArrayEncoder {
    ...
    where
        Self: Sized,
    {
        let dictionary = if props.dictionary_enabled(descr.path()) {
            Some(DictEncoder { interner: Interner::try_new(ByteArrayStorage::default())?, indices: Vec::new(), variable_length_bytes: 0 })
        } else { None };
...
fn encode<T, I>(values: T, indices: I, encoder: &mut ByteArrayEncoder) -> Result<()>
where
    T: ArrayAccessor + Copy,
...
) -> Result<Option<(ByteArray, ByteArray)>>
where
    T: ArrayAccessor,
    T::Item: Copy + Ord + AsRef<[u8]>,
{
    let Some(first_idx) = valid.next() else { return Ok(None); };
    ...
    crate::resource::reserve_vec(&mut min_bytes, min.as_ref().len(), "writer byte statistics")?;
    crate::resource::reserve_vec(&mut max_bytes, max.as_ref().len(), "writer byte statistics")?;
    min_bytes.extend_from_slice(min.as_ref());
    max_bytes.extend_from_slice(max.as_ref());
    Ok(Some((crate::resource::own_writer_vec(min_bytes)?.into(), crate::resource::own_writer_vec(max_bytes)?.into())))
}
