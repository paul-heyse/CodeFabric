...
use super::schema::{try_add_encoded_arrow_schema_to_metadata, decimal_length_from_precision};
...
use crate::column::writer::{
    ColumnCloseResult, ColumnWriter, GenericColumnWriter, try_get_column_writer,
};
...
impl<W: Write + Send> ArrowWriter<W> {
    ...
    pub fn try_new(
        ...
        let properties = match props { Some(props) => props, None => WriterProperties::try_new()? };
        let options = ArrowWriterOptions::from_properties(properties);
    ...
    pub fn try_new_with_options(
        ...
        if !options.skip_arrow_metadata {
            ...
            try_add_encoded_arrow_schema_to_metadata(&arrow_schema, &mut props)?;
        }
        ...
        if let Some(policy) = crate::resource::current() {
            let (layout, _) = std::alloc::Layout::new::<[usize; 2]>().extend(std::alloc::Layout::new::<WriterProperties>()).map_err(|_| crate::resource::ResourceExhausted { kind: "writer properties layout", requested: usize::MAX, limit: isize::MAX as usize })?;
            policy.reserve(layout.pad_to_align().size(), "writer properties owner")?;
        }
        ...
        let mut row_group_writer_factory =
            ArrowRowGroupWriterFactory::try_new(&file_writer, arrow_schema.clone())?;
    ...
    pub fn write(&mut self, batch: &RecordBatch) -> Result<()> {
        ...
        let _scope = crate::resource::enter(self.resource_policy.clone());
        if batch.num_rows() == 0 {
            return Ok(());
        }
    ...
    /// Append metadata through the admitted native collection boundary.
    pub fn try_append_key_value_metadata(&mut self, kv_metadata: KeyValue) -> Result<()> { self.writer.try_append_key_value_metadata(kv_metadata) }
...
impl ArrowWriterOptions {
    /// Construct options without allocating and discarding default properties.
    pub fn from_properties(properties: WriterProperties) -> Self {
        Self { properties, skip_arrow_metadata: false, schema_root: None, schema_descr: None, page_store_factory: None }
    }

    /// Fallible default options for an active native resource policy.
    pub fn try_new() -> Result<Self> { Ok(Self::from_properties(WriterProperties::try_new()?)) }
...
impl ArrowColumnChunkData {
    ...
    fn push(&mut self, value: Bytes) -> Result<()> {
        crate::resource::reserve_vec(&mut self.keys, 1, "writer page keys")?;
    ...
    fn push_dictionary(&mut self, value: Bytes) -> Result<()> {
        crate::resource::reserve_vec(&mut self.dictionary_keys, 1, "writer dictionary page keys")?;
        let len = value.len();
        let key = self.store.put(value)?;
...
struct StreamingColumnChunkPages {
    store: Box<dyn PageStore>,
    dictionary_keys: IntoIter<PageKey>,
...
impl StreamingColumnChunkPages {
    fn new(data: ArrowColumnChunkData) -> Self {
        Self {
            store: data.store,
            dictionary_keys: data.dictionary_keys.into_iter(),
...
impl Iterator for StreamingColumnChunkPages {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let key = self.dictionary_keys.next().or_else(|| self.keys.next())?;
        if let Err(error) = crate::resource::validate_thread_owner(self.resource_policy.as_ref()) {
...
impl ArrowPageWriter {
    ...
    fn try_new(store: Box<dyn PageStore>) -> Result<Self> {
        if let Some(policy) = crate::resource::current() {
            let (layout, _) = std::alloc::Layout::new::<[usize; 2]>()
                .extend(std::alloc::Layout::new::<Mutex<ArrowColumnChunkData>>())
                .map_err(|_| crate::resource::ResourceExhausted { kind: "writer shared chunk layout", requested: usize::MAX, limit: isize::MAX as usize })?;
            policy.reserve(layout.pad_to_align().size(), "writer shared chunk owner")?;
            policy.reserve(std::mem::size_of::<Self>(), "writer page writer owner")?;
        }
        Ok(Self {
...
impl PageWriter for ArrowPageWriter {
    fn write_page(&mut self, page: CompressedPage) -> Result<PageWriteSpec> {
        ...
        let header = if crate::resource::current().is_some() && self.page_encryptor_mut().is_none() {
            crate::resource::writer_thrift_bytes(&page_header)?
        } else {
...
pub fn compute_leaves(field: &Field, array: &ArrayRef) -> Result<Vec<ArrowLeafColumn>> {
    ...
    crate::resource::admit_writer_slots::<ArrowLeafColumn>(levels.len(), "writer public leaf descriptors")?;
...
impl ArrowRowGroupWriterFactory {
    ...
    pub fn new<W: Write + Send>(
        ...
        Self::try_new(file_writer, arrow_schema).expect("row group factory admission")
    }

    /// Admit the default native page-store factory before creating its Arc.
    pub fn try_new<W: Write + Send>(
        file_writer: &SerializedFileWriter<W>,
        arrow_schema: SchemaRef,
    ) -> Result<Self> {
        let schema = Arc::clone(file_writer.schema_descr_ptr());
        let props = Arc::clone(file_writer.properties());
        if let Some(policy) = crate::resource::current() { policy.reserve(2 * std::mem::size_of::<usize>(), "writer page store factory")?; }
        Ok(Self {
            schema,
            arrow_schema,
            props,
    ...
    pub fn create_column_writers(&self, row_group_index: usize) -> Result<Vec<ArrowColumnWriter>> {
        ...
        crate::resource::reserve_vec(&mut writers, self.schema.num_columns(), "writer column descriptors")?;
    ...
    fn column_writer_factory(&self, row_group_idx: usize) -> ArrowColumnWriterFactory {
        ArrowColumnWriterFactory::from_page_store_factory(self.page_store_factory.clone())
...
pub fn get_column_writers(
    ...
    let mut writers = Vec::new();
    crate::resource::reserve_vec(&mut writers, parquet.num_columns(), "writer column descriptors")?;
...
impl ArrowColumnWriterFactory {
    ...
    fn from_page_store_factory(page_store_factory: Arc<dyn PageStoreFactory>) -> Self {
        Self { page_store_factory, #[cfg(feature = "encryption")] row_group_index: 0, #[cfg(feature = "encryption")] file_encryptor: None }
    }
    ...
    fn create_page_writer(
        ...
        Ok(Box::new(
            ArrowPageWriter::try_new(store)?.with_encryptor(page_encryptor),
        ))
    }
    ...
    fn create_page_writer(
        ...
        let args = PageStoreArgs::new(column_index, column_descriptor);
        let store = self.page_store_factory.create(&args)?;
        Ok(Box::new(ArrowPageWriter::try_new(store)?))
    }
    ...
    fn get_arrow_column_writer(
        ...
        let col = |desc: &ColumnDescPtr| -> Result<ArrowColumnWriter> {
            ...
            let writer = try_get_column_writer(desc.clone(), props.clone(), page_writer)?;
        ...
        let bytes = |desc: &ColumnDescPtr| -> Result<ArrowColumnWriter> {
            let page_writer = self.create_page_writer(desc, out.len())?;
            let chunk = page_writer.buffer.clone();
            let writer = GenericColumnWriter::try_new(desc.clone(), props.clone(), page_writer)?;
            Ok(ArrowColumnWriter {
                chunk,
...
#[cfg(test)]
mod resource_writer_tests {
    use super::*;
    use crate::basic::{Compression, ZstdLevel};
    ...
    fn factory(
        ...
        dictionary: bool,
    ) -> (ArrowRowGroupWriterFactory, SchemaRef) {
        ...
        (
            ArrowRowGroupWriterFactory::new(&writer, schema.clone()),
            schema,
        )
    }

    #[test]
    fn resource_writer_codec_denial_is_typed_through_both_native_factories() {
        for data_type in [ArrowDataType::Int32, ArrowDataType::Utf8] {
            ...
            let (policy, _) = policy("zstd codec owner");
            let _guard = policy.enter_thread();
            assert!(matches!(
                factory.create_column_writers(0),
                Err(ParquetError::ResourceExhausted(ResourceExhausted {
                    kind: "zstd codec owner",
                    ..
                }))
            ));
        }
        let schema = Arc::new(
            crate::schema::parser::parse_message_type("message test { REQUIRED INT32 value; }")
                .unwrap(),
        );
        let props = Arc::new(
            WriterProperties::builder()
    ...
    #[test]
    fn resource_writer_original_compressed_page_bytes_outlive_closed_column() {
        for codec in [Compression::ZSTD(ZstdLevel::default()), Compression::SNAPPY] {
            for version in [WriterVersion::PARQUET_1_0, WriterVersion::PARQUET_2_0] {
                for dictionary in [false, true] {
                    let (factory, schema) =
                        factory(ArrowDataType::Int32, codec, version, dictionary);
                    let array: ArrayRef = Arc::new(arrow_array::Int32Array::from(vec![7; 32]));
                    let leaves = compute_leaves(schema.field(0), &array).unwrap();
                    let (policy, admission) = policy("");
                    let guard = policy.enter_thread();
                    let mut column = factory.create_column_writers(0).unwrap().pop().unwrap();
                    column.write(&leaves[0]).unwrap();
                    let chunk = column.close().unwrap();
                    let mut pages = StreamingColumnChunkPages::new(chunk.data)
                        .collect::<Result<Vec<_>>>()
                        .unwrap();
                    let original = pages.pop().unwrap();
                    assert!(!original.is_empty());
                    let last_slice = original.slice(..1);
                    drop((pages, original, guard, policy, chunk.close));
                    assert!(
                        admission.live.load(Ordering::Acquire) > 0,
                        "last original page slice must retain codec/output receipts"
                    );
                    assert!(
                        admission
                            .requests
                            ...
                            .unwrap()
                            .iter()
                            .any(|request| request.kind == "writer page backing owner")
                    );
                    drop(last_slice);
    ...
    #[test]
    fn resource_writer_backing_or_assembly_denial_prevents_page_publication() {
        for denied in ["writer page assembly", "writer page backing owner", "writer thrift buffer", "writer page statistics clone", "writer page store slots", "writer page keys"] {
            ...
            let array: ArrayRef = Arc::new(arrow_array::Int32Array::from(vec![7; 32]));
            let leaves = compute_leaves(schema.field(0), &array).unwrap();
            let (policy, admission) = policy(denied);
            let guard = policy.enter_thread();
            let mut column = factory.create_column_writers(0).unwrap().pop().unwrap();
            column.write(&leaves[0]).unwrap();
            let result = column.close();
            assert!(
                matches!(&result, Err(ParquetError::ResourceExhausted(ResourceExhausted { kind, .. })) if *kind == denied),
    ...
    #[test]
    fn resource_writer_dictionary_and_plain_admission_fail_before_publication() {
        for (kind, dictionary) in [
            ("writer dictionary hash table", true),
            ("writer dictionary indices", true),
            ("writer dictionary payload", true),
            ("writer dictionary ranges", true),
            ("writer byte statistics", true),
            ("writer page assembly", false),
        ] {
            let (factory, schema) = factory(ArrowDataType::Utf8, Compression::SNAPPY, WriterVersion::PARQUET_1_0, dictionary);
            let array: ArrayRef = Arc::new(arrow_array::StringArray::from(vec!["different", "values", "different"]));
            let leaves = compute_leaves(schema.field(0), &array).unwrap();
            let (policy, admission) = policy(kind);
            let guard = policy.enter_thread();
            let mut column = factory.create_column_writers(0).unwrap().pop().unwrap();
            assert!(matches!(column.write(&leaves[0]), Err(ParquetError::ResourceExhausted(error)) if error.kind == kind));
            assert!(column.chunk.lock().unwrap().keys.is_empty());
            drop((column, guard, policy));
            assert_eq!(admission.live.load(Ordering::Acquire), 0);
        }
    }

    #[test]
    fn resource_writer_header_last_slice_retains_original_bank() {
        let (factory, schema) = factory(ArrowDataType::Int32, Compression::SNAPPY, WriterVersion::PARQUET_1_0, false);
        ...
        let chunk = column.close().unwrap();
        let mut pages = StreamingColumnChunkPages::new(chunk.data);
        let header = pages.next().unwrap().unwrap();
        let slice = header.slice(..1);
        drop((pages, header, chunk.close, guard, policy));
    ...
    #[test]
    fn resource_writer_guarded_utf8_round_trip_both_versions() {
        for version in [WriterVersion::PARQUET_1_0, WriterVersion::PARQUET_2_0] {
            for dictionary in [false, true] {
                let (factory, schema) = factory(ArrowDataType::Utf8, Compression::SNAPPY, version, dictionary);
                let array: ArrayRef = Arc::new(arrow_array::StringArray::from(vec!["first", "second", "prefix_long", "prefix_short"]));
                let leaves = compute_leaves(schema.field(0), &array).unwrap();
                let (policy, admission) = policy("");
                let guard = policy.enter_thread();
                let mut column = factory.create_column_writers(0).unwrap().pop().unwrap();
                ...
                let chunk = column.close().unwrap();
                drop(guard);
                // The sink is deliberately outside this buffer-family probe.
                let mut output = SerializedFileWriter::new(Vec::new(), factory.schema.root_schema_ptr(), factory.props.clone()).unwrap();
                let mut group = output.next_row_group().unwrap();
                chunk.append_to_row_group(&mut group).unwrap();
                group.close().unwrap();
                let output = output.into_inner().unwrap();
                drop(policy);
                assert_eq!(admission.live.load(Ordering::Acquire), 0);
                let mut reader = crate::arrow::arrow_reader::ParquetRecordBatchReaderBuilder::try_new(Bytes::from(output)).unwrap().build().unwrap();
                let actual = reader.next().unwrap().unwrap();
                assert_eq!(actual.column(0).as_ref(), array.as_ref());
            }
        }
    }

    #[test]
    fn resource_writer_prebuilt_column_rejects_new_required_owner() {
        let (factory, schema) = factory(
            ArrowDataType::Int32,
            Compression::ZSTD(ZstdLevel::default()),
            WriterVersion::PARQUET_1_0,
        ...
        let array: ArrayRef = Arc::new(arrow_array::Int32Array::from(vec![7; 32]));
        let leaves = compute_leaves(schema.field(0), &array).unwrap();
        let mut unowned = factory.create_column_writers(0).unwrap().pop().unwrap();
        let (required, admission) = policy("");
        let before = admission.live.load(Ordering::Acquire);
        let _guard = required.enter_thread();
        assert!(matches!(
            unowned.write(&leaves[0]),
            Err(ParquetError::ResourceExhausted(_))
        ));
        assert_eq!(admission.live.load(Ordering::Acquire), before);
    }
}
