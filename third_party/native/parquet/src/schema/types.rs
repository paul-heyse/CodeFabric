...
pub enum Type {
    ...
    GroupType {
        /// Fields of this group type; original backing drops before its owner.
        fields: Vec<TypePtr>,
        /// Basic information about the type.
        basic_info: BasicTypeInfo,
    },
...
impl<'a> PrimitiveTypeBuilder<'a> {
    ...
    pub fn build(self) -> Result<Type> {
        if let Some(policy) = crate::resource::current() {
            if matches!(self.logical_type, Some(LogicalType::Geometry(_) | LogicalType::Geography(_))) { return Err(crate::resource::ResourceExhausted { kind: "writer geospatial type outside admitted profile", requested: 1, limit: 0 }.into()); }
            policy.string(self.name.len())?;
        }
        let mut basic_info = BasicTypeInfo {
            ...
            id: self.id,
            _resource_policy: crate::resource::RetainedPolicy::capture(),
        };
...
pub struct BasicTypeInfo {
    ...
    logical_type: Option<LogicalType>,
    id: Option<i32>,
    _resource_policy: crate::resource::RetainedPolicy,
}
...
impl SchemaDescriptor {
    /// Construct a descriptor with fallible admission for the native schema
    /// caches. The scope is installed by guarded native metadata readers.
    pub fn try_new(tp: TypePtr) -> Result<Self> {
        if let Some(policy) = crate::resource::current() {
            fn visit(
                tp: &Type,
                ...
                *nodes = nodes.checked_add(1).ok_or(ResourceExhausted {
                    kind: "schema nodes",
                    requested: usize::MAX,
                    limit: policy.limits().collection_entries,
                })?;
                ...
                ReaderResourcePolicy::check(depth, policy.limits().schema_depth, "schema depth")?;
                let path_bytes =
                    path_bytes
                        .checked_add(tp.name().len())
                        .ok_or(ResourceExhausted {
                            kind: "schema path bytes",
                            requested: usize::MAX,
                            limit: policy.limits().output_bytes,
                        })?;
                match tp {
                    Type::PrimitiveType { .. } => {
                        // Native build_tree uses Vec<String>::extend from an exact
                        // size iterator. Rust's Vec allocates at least four String
                        // slots on the first growth; all strings copy path bytes.
                        policy.reserve(
                            depth
                                .max(4)
                                .checked_mul(std::mem::size_of::<String>())
                                .ok_or(ResourceExhausted {
                                    kind: "schema column path",
                                    requested: usize::MAX,
                                    limit: policy.limits().output_bytes,
                                })?,
                            "schema column path",
                        )?;
                        policy.reserve(path_bytes, "schema column path strings")?;
                        policy.reserve(
                            std::mem::size_of::<ColumnDescriptor>()
                    ...
                    Type::GroupType { fields, .. } => {
                        ...
                        for child in fields {
                            let (child_leaves, child_depth) =
                                visit(child, policy, depth + 1, path_bytes, nodes)?;
                            leaves += child_leaves;
                            maximum_depth = maximum_depth.max(child_depth);
                        }
                        Ok((leaves, maximum_depth))
                    }
                }
            }
            let mut leaves = 0;
            ...
            for field in tp.get_fields() {
                let (field_leaves, field_depth) = visit(field, &policy, 1, 0, &mut nodes)?;
                leaves += field_leaves;
                depth = depth.max(field_depth);
            }
            policy.collection(
                leaves,
                std::mem::size_of::<ColumnDescPtr>(),
                "schema leaf descriptors",
            )?;
            policy.collection(leaves, std::mem::size_of::<usize>(), "schema leaf mapping")?;
            // Match new's initial depth and every complete Vec growth allocation.
            let mut capacity = 16_usize;
            loop {
                policy.reserve(
                    capacity * std::mem::size_of::<&str>(),
                    "schema traversal path",
                )?;
                if capacity >= depth {
                    break;
                }
                capacity = capacity
                    ...
                    .ok_or(crate::resource::ResourceExhausted {
                        kind: "schema traversal depth",
                        requested: usize::MAX,
                        limit: policy.limits().schema_depth,
                    })?;
            }
        }
        Ok(Self::new(tp))
...
pub(crate) fn parquet_schema_from_array<'a>(elements: Vec<SchemaElement<'a>>) -> Result<TypePtr> {
    ...
    let mut schema_nodes = crate::resource::vec_with_capacity(1, "schema root descriptors")?; // there should only be one element when done

    // turn into iterator so we can take ownership of elements of the vector
    let mut elements = elements.into_iter();

    while index < num_elements {
        let t = schema_from_array_helper(&mut elements, num_elements, index, 0)?;
        index = t.0;
        crate::resource::reserve_vec(&mut schema_nodes, 1, "schema root descriptors")?;
...
fn schema_from_array_helper<'a>(
    ...
    if let Some(policy) = crate::resource::current() {
        policy.string(element.name.len())?;
        policy.reserve(
            std::mem::size_of::<Type>() + 2 * std::mem::size_of::<usize>(),
            "schema type node",
        )?;
    }
    ...
    match element.num_children {
        ...
        Some(n) => {
            ...
            let mut fields =
                crate::resource::vec_with_capacity(usize::try_from(n)?, "schema children")?;
            let mut next_index = index + 1;
            for _ in 0..n {
                let child_result =
                    schema_from_array_helper(elements, num_elements, next_index, depth + 1)?;
