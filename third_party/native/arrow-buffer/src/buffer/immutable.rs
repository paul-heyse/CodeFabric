...
impl Buffer {
    ...
    /// Register this original allocation only if it has no existing reservation.
    /// Returns whether registration occurred. Existing claims are preserved,
    /// including when this buffer is a slice or clone of the original allocation.
    /// This is lifetime registration, not fallible allocation admission.
    #[cfg(feature = "pool")]
    pub fn claim_if_unclaimed(&self, pool: &dyn MemoryPool) -> bool {
        self.data.claim_if_unclaimed(pool)
    }

    /// Fallibly append an independent reservation without releasing originals.
    ///
    /// `max_claims` is a finite caller-admitted bound on retained reservations.
    /// Before constructing a combined wrapper, `admit_wrapper` receives its
    /// exact native allocation layout. `reserve_new` must admit its own boxed
    /// reservation before constructing it. Both callbacks run under the original
    /// allocation's claim mutex and must not reenter this buffer's claim APIs.
    /// Any refusal leaves the original reservation and count unchanged.
    ///
    /// This registers lifetimes for payloads admitted before allocation. It does
    /// not authorize native reallocation; resize callbacks remain advisory.
    #[cfg(feature = "pool")]
    pub fn try_add_claim<E>(
        &self,
        max_claims: usize,
        admit_wrapper: impl FnOnce(std::alloc::Layout) -> Result<(), E>,
        reserve_new: impl FnOnce(usize) -> Result<Box<dyn crate::MemoryReservation>, E>,
    ) -> Result<(), crate::BufferClaimError<E>> {
        self.data
            .try_add_claim(max_claims, admit_wrapper, reserve_new)
    }
}
