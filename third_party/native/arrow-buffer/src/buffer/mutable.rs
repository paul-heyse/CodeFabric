...
/// Allocation-free failure of a fallible native buffer layout or allocation.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct BufferAllocationError {
    /// Requested complete native allocation size.
    pub requested: usize,
...
impl std::fmt::Display for BufferAllocationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "native buffer allocation failed: {} bytes aligned to {}",
            self.requested, self.alignment
        )
    }
}
...
mod fallible_allocation_tests {
    ...
    #[test]
    fn fallible_native_geometry_matches_infallible_growth() {
        let mut fallible = MutableBuffer::try_with_capacity(1).unwrap();
        let mut native = MutableBuffer::with_capacity(1);
        assert_eq!(fallible.capacity(), native.capacity());
        for len in [1, 63, 64, 65, 127, 128, 257] {
            let layout = fallible.try_reserve_layout(len - fallible.len()).unwrap();
            fallible.try_resize(len, 0xab).unwrap();
            native.resize(len, 0xab);
            assert_eq!(fallible, native);
            assert_eq!(fallible.capacity(), native.capacity());
            if let Some(layout) = layout {
                assert_eq!(fallible.capacity(), layout.size());
            }
        }
    }

    #[test]
    fn fallible_native_layout_denial_keeps_original_allocation() {
        let mut buffer = MutableBuffer::try_from_len_zeroed(3).unwrap();
        assert_eq!(buffer.capacity(), 3);
        buffer.as_slice_mut().copy_from_slice(b"old");
        let pointer = buffer.as_ptr();
        assert!(buffer.try_reserve(usize::MAX).is_err());
        assert_eq!(buffer.as_ptr(), pointer);
        assert_eq!(buffer.len(), 3);
        assert_eq!(buffer.capacity(), 3);
        assert_eq!(buffer.as_slice(), b"old");
        assert!(MutableBuffer::try_with_capacity(usize::MAX).is_err());
        assert!(MutableBuffer::try_from_len_zeroed(usize::MAX).is_err());
        buffer.try_resize(4, b'!').unwrap();
        assert_eq!(buffer.as_slice(), b"old!");
    }
}
...
pub struct MutableBuffer {
    ...
    #[cfg(feature = "pool")]
    reservation: Mutex<crate::bytes::ReservationState>,
}

impl MutableBuffer {
    /// Compute the exact native layout used by [`Self::try_with_capacity`]
    /// without allocating. Callers can admit this complete layout before work.
    pub fn try_capacity_layout(capacity: usize) -> Result<Layout, BufferAllocationError> {
        let error = BufferAllocationError {
            requested: capacity,
            alignment: ALIGNMENT,
        };
        let rounded = capacity.checked_add(63).ok_or(error)? & !63;
        Layout::from_size_align(rounded, ALIGNMENT).map_err(|_| error)
    }

    /// Fallible equivalent of [`Self::with_capacity`]. This is an allocator
    /// primitive, not resource admission; callers must admit its layout first.
    pub fn try_with_capacity(capacity: usize) -> Result<Self, BufferAllocationError> {
        let layout = Self::try_capacity_layout(capacity)?;
        let data = if layout.size() == 0 {
            dangling_ptr()
        } else {
            // SAFETY: layout is valid and nonzero. The returned allocation is
            // owned exclusively by the new buffer and freed using this layout.
            NonNull::new(unsafe { std::alloc::alloc(layout) }).ok_or(BufferAllocationError {
        ...
        Ok(Self {
            ...
            layout,
            #[cfg(feature = "pool")]
            reservation: Mutex::new(crate::bytes::ReservationState::default()),
        })
    }

    /// Fallible equivalent of [`Self::from_len_zeroed`]. Its exact allocation
    /// has `len` bytes and [`ALIGNMENT`] alignment, without capacity rounding.
    pub fn try_from_len_zeroed(len: usize) -> Result<Self, BufferAllocationError> {
        let error = BufferAllocationError {
            requested: len,
            alignment: ALIGNMENT,
        };
        let layout = Layout::from_size_align(len, ALIGNMENT).map_err(|_| error)?;
        let data = if len == 0 {
            dangling_ptr()
        } else {
            // SAFETY: valid nonzero layout; ownership transfers to this buffer.
            NonNull::new(unsafe { std::alloc::alloc_zeroed(layout) }).ok_or(error)?
        };
        Ok(Self {
            data,
            len,
    ...
    /// Return the exact replacement allocation required by [`Self::try_reserve`]
    /// without allocating. `None` means the existing backing already suffices.
    /// The original allocation remains live while a replacement is admitted.
    pub fn try_reserve_layout(
        &self,
        additional: usize,
    ) -> Result<Option<Layout>, BufferAllocationError> {
        let error = BufferAllocationError {
            requested: usize::MAX,
            alignment: self.layout.align(),
        };
        let required = self.len.checked_add(additional).ok_or(error)?;
        if required <= self.layout.size() {
            return Ok(None);
        }
        let rounded = required.checked_add(63).ok_or(error)? & !63;
        let capacity = rounded.max(self.layout.size().checked_mul(2).ok_or(error)?);
        let layout = Layout::from_size_align(capacity, self.layout.align()).map_err(|_| {
    ...
    /// Fallibly grow with the same native geometry as [`Self::reserve`]. On
    /// layout or allocator failure the original pointer, length and capacity
    /// are unchanged. This method does not perform resource admission.
    pub fn try_reserve(&mut self, additional: usize) -> Result<(), BufferAllocationError> {
        let Some(layout) = self.try_reserve_layout(additional)? else {
            return Ok(());
        };
        // SAFETY: layout is valid and nonzero. Realloc receives the exclusively
        // owned original pointer and its exact allocation layout. On null it
        // leaves that original allocation untouched; fields change only after
        // success. No other alias or asynchronous work can access &mut self.
        let raw = unsafe {
            if self.layout.size() == 0 {
                std::alloc::alloc(layout)
            } else {
                std::alloc::realloc(self.data.as_ptr(), self.layout, layout.size())
            }
        };
        let data = NonNull::new(raw).ok_or(BufferAllocationError {
            requested: layout.size(),
            alignment: layout.align(),
        })?;
        self.data = data;
    ...
    /// Fallible equivalent of [`Self::resize`], using [`Self::try_reserve`].
    pub fn try_resize(&mut self, new_len: usize, value: u8) -> Result<(), BufferAllocationError> {
        if new_len > self.len {
            let diff = new_len - self.len;
            self.try_reserve(diff)?;
            // SAFETY: the reserved region [len, new_len) is exclusively owned
            // and inside the allocation. Initialization precedes length update.
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        ...
        Self {
            ...
            layout,
            #[cfg(feature = "pool")]
            reservation: std::sync::Mutex::new(crate::bytes::ReservationState::default()),
        }
    }
    ...
    pub fn claim(&self, pool: &dyn MemoryPool) {
        *self.reservation.lock().unwrap() =
            crate::bytes::ReservationState::new(pool.reserve(self.capacity()));
    }
}
