...
#[cfg(feature = "pool")]
use crate::pool::{BufferClaimError, MemoryPool, MemoryReservation};
#[cfg(feature = "pool")]
use std::sync::Mutex;
...
pub(super) struct ReservationState {
    value: Option<Box<dyn MemoryReservation>>,
    claims: usize,
}
#[cfg(feature = "pool")]
impl ReservationState {
    ...
    pub(crate) fn as_mut(&mut self) -> Option<&mut Box<dyn MemoryReservation>> {
        self.value.as_mut()
    }
    pub(crate) fn new(value: Box<dyn MemoryReservation>) -> Self {
        Self {
            value: Some(value),
            claims: 1,
        }
...
#[cfg(feature = "pool")]
#[derive(Debug)]
struct CombinedReservation {
    original: Box<dyn MemoryReservation>,
    additional: Box<dyn MemoryReservation>,
}
#[cfg(feature = "pool")]
impl MemoryReservation for CombinedReservation {
    fn size(&self) -> usize {
        self.additional.size()
    }
    fn resize(&mut self, size: usize) {
        self.original.resize(size);
        self.additional.resize(size);
    }
}
...
pub(crate) struct Bytes {
    ...
    #[cfg(feature = "pool")]
    pub(super) reservation: Mutex<ReservationState>,
}
...
impl Bytes {
    ...
    pub(crate) fn claim(&self, pool: &dyn MemoryPool) {
        *self.reservation.lock().unwrap() = ReservationState {
            value: Some(pool.reserve(self.capacity())),
            claims: 1,
        };
    }

    /// Register only when no original claim exists. The lock makes the test and
    /// registration atomic with respect to other claims.
    #[cfg(feature = "pool")]
    pub(crate) fn claim_if_unclaimed(&self, pool: &dyn MemoryPool) -> bool {
        let mut reservation = self.reservation.lock().unwrap();
        if reservation.value.is_some() {
            return false;
        }
        reservation.value = Some(pool.reserve(self.capacity()));
        reservation.claims = 1;
        true
    }

    #[cfg(feature = "pool")]
    pub(crate) fn try_add_claim<E>(
        &self,
        max_claims: usize,
        admit_wrapper: impl FnOnce(std::alloc::Layout) -> Result<(), E>,
        reserve_new: impl FnOnce(usize) -> Result<Box<dyn MemoryReservation>, E>,
    ) -> Result<(), BufferClaimError<E>> {
        let mut state = self.reservation.lock().unwrap();
        let claims = state
            .claims
            .checked_add(1)
            .ok_or(BufferClaimError::Capacity {
                requested: usize::MAX,
                limit: max_claims,
            })?;
        if claims > max_claims {
            return Err(BufferClaimError::Capacity {
                requested: claims,
                limit: max_claims,
            });
        }
        if state.value.is_some() {
            admit_wrapper(std::alloc::Layout::new::<CombinedReservation>())
                .map_err(BufferClaimError::Admission)?;
        }
        let additional = reserve_new(self.capacity()).map_err(BufferClaimError::Admission)?;
        // Both fallible callbacks completed while the original stayed intact.
        // Moving it into the already-admitted wrapper cannot release it.
        state.value = Some(match state.value.take() {
            Some(original) => Box::new(CombinedReservation {
                original,
                additional,
            }),
            None => additional,
    ...
    fn resize_reservation(&self, new_size: usize) {
        ...
        if let Some(mut reservation) = guard.value.take() {
            // Resize the reservation
            reservation.resize(new_size);

            // Put it back
            guard.value = Some(reservation);
        }
    }
