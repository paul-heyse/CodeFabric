...
#[path = "last_checkpoint_hint_resource.rs"]
mod resource;
...
pub(crate) struct LastCheckpointHint {
    ...
    #[serde(skip)]
    resource_owner: resource::HintResourceOwner,
}
...
impl LastCheckpointHint {
    ...
    pub(crate) fn path(log_root: &Url) -> DeltaResult<Url> {
        crate::path::resource::try_join_url(log_root, LAST_CHECKPOINT_FILE_NAME)
    }
    ...
    pub(crate) fn try_read(
        ...
        crate::path::resource::admit_file_slice_descriptors(1)?;
        match storage.read_files(vec![(file_path, None)])?.next() {
            Some(Ok(data)) => {
                if let Some(limits) = storage.json_resource_limits() {
                    limits.inspect(&data)?;
                }
                let decoded = match crate::resource::current_resource_scope() {
                    Some(scope) => {
                        let limits = storage.json_resource_limits().ok_or(
                            crate::resource::ResourceExhausted {
                                kind: "native_checkpoint_hint_json_policy",
                                requested: 1,
                                limit: 0,
                            },
                        )?;
                        Self::from_bytes_with_resources(&data, scope, limits)
                    }
                    None => Self::from_bytes_with_oversized_fields_dropped(&data)
                        .map_err(Error::MalformedJson),
                };
                let result = match decoded {
                    Ok(hint) => Some(hint),
                    Err(error) if error.is_resource_exhausted() => return Err(error),
                    Err(error) => {
                        warn!("invalid _last_checkpoint JSON: {error}");
                        None
                    }
                };
                if let Some(hint) = &result {
                    info!(version = hint.version, size = hint.size, parts = hint.parts);
                }
                Ok(result)
            }
