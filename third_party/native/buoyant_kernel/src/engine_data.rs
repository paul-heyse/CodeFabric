...
pub use crate::schema::OwnedVisitorSelection;
...
impl<'a> ListItem<'a> {
    ...
    /// Borrow an original string without allocating a materialized copy.
    pub fn get_borrowed(&self, list_index: usize) -> &str {
        self.values.value(self.offsets.start + list_index)
    }
...
impl<'a> MapItem<'a> {
    /// Original entry count, including null-valued entries which materialize skips.
    pub fn len(&self) -> usize {
        self.offsets.len()
    }

    /// Whether the original map has no entries.
    pub fn is_empty(&self) -> bool {
        self.offsets.is_empty()
    }

    /// Borrow original entries before any owned map/string allocation.
    pub fn borrowed_entries(&self) -> impl Iterator<Item = (&str, Option<&str>)> {
        self.offsets.clone().map(|index| {
            (
                self.keys.value(index),
                self.values
                    .is_valid(index)
                    .then(|| self.values.value(index)),
            )
        })
    }
...
pub trait FilteredRowVisitor {
    ...
    /// Return original per-operation selector ownership. The default rejects a
    /// required policy before touching a legacy process-static initializer.
    fn try_selection(&self) -> DeltaResult<OwnedVisitorSelection> {
        crate::schema::visitor_selection::legacy_selection(||self.selected_column_names_and_types())
    }
        ...
        Self: Sized,
    {
        ...
        let selection=self.try_selection()?;
        let column_names=selection.as_ref().0;
...
impl<V: FilteredRowVisitor> RowVisitor for FilteredVisitorBridge<'_, V> {
    ...
    fn try_selection(&self)->DeltaResult<OwnedVisitorSelection>{self.visitor.try_selection()}
...
pub trait RowVisitor {
    ...
    {
        let selection=self.try_selection()?;
        data.visit_rows(selection.as_ref().0, self)
    }
}
