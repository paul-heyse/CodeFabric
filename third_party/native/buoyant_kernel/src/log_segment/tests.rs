...
async fn test_create_checkpoint_stream_returns_checkpoint_batches_as_is_if_schema_has_no_file_actions(
    ...
    let log_segment = LogSegment::try_new(
        LogSegmentFiles {
            checkpoint_parts: vec![create_log_path(&checkpoint_one_file)].into(),
...
async fn test_commit_cover(
    ...
    .await;
    let cover = log_segment.find_commit_cover().unwrap();
...
fn checkpoint_sidecars_distinguishes_empty_from_absent() -> DeltaResult<()> {
    ...
    let log_segment = LogSegment::try_new(
        LogSegmentFiles {
            checkpoint_parts: vec![create_log_path_with_size(&checkpoint_file, 1)].into(),
            ascending_commit_files: vec![commit.clone()].into(),
...
async fn test_get_file_actions_schema_v1_parquet_with_hint(
    ...
    let log_segment = LogSegment::try_new(
        LogSegmentFiles {
            checkpoint_parts: vec![create_log_path_with_size(&checkpoint_file, cp_size)].into(),
            ascending_commit_files: vec![commit_v2.clone()].into(),
...
async fn test_checkpoint_stream_sets_has_partition_values_parsed() -> DeltaResult<()> {
    ...
    let log_segment = LogSegment::try_new(
        LogSegmentFiles {
            checkpoint_parts: vec![create_log_path_with_size(&checkpoint_file, checkpoint_size)].into(),
            latest_commit_file: Some(create_log_path("file:///00000000000000000001.json")),
            ..Default::default()
        },
        log_root,
    ...
    fn normalize(log_segment: LogSegment) -> LogSegment {
        ...
        LogSegment {
            ...
            listed: LogSegmentFiles {
                ...
                ascending_commit_files: vec![].into(),
...
async fn test_segment_crc_filtering(#[case] case: CrcPruningCase) {
    ...
    let after = seg.segment_after_version(case.crc_version).unwrap();
