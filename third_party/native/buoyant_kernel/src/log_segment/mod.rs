...
pub(crate) struct LogSegment {
    ...
    pub log_root: crate::OwnedLogUrl,
    ...
    pub(crate) last_checkpoint_metadata: Option<Arc<LastCheckpointHint>>,
}
...
impl LogSegment {
    ...
    pub(crate) fn new_for_version_zero(
        ...
        Ok(Self {
            ...
            log_root: crate::OwnedLogUrl::try_copy(&log_root)?,
            ...
            listed: LogSegmentFiles {
                ...
                ascending_commit_files: OwnedLogPaths::try_from_iter([Ok(commit_file)])?,
                ..Default::default()
            },
        })
    }
    ...
    pub(crate) fn try_new(
        ...
        last_checkpoint_metadata: Option<Arc<LastCheckpointHint>>,
    ) -> DeltaResult<Self> {
        listed_files.ascending_commit_files.validate()?;
        listed_files.ascending_compaction_files.validate()?;
        listed_files.checkpoint_parts.validate()?;
        ...
        let checkpoint_version =
            if let Some(checkpoint_file) = listed_files.checkpoint_parts.first() {
                ...
                listed_files
                    ...
                    .try_retain(|log_path| version < log_path.version)?;
                Some(version)
            } else {
                None
            };
    ...
    pub(crate) fn for_snapshot_impl(
        ...
        let checkpoint_hint = checkpoint_hint.map(Arc::new);
        // LastCheckpointHint native parse admission includes this original Arc header.
    ...
    pub(crate) fn for_timestamp_conversion(
        ...
        if !commits.is_empty() {
            ...
            commits.try_drain_prefix(start_idx)?;
        }
    ...
    pub(crate) fn new_with_commit_appended(
        ...
        new_log_segment
            .listed
            .ascending_commit_files
            .try_push(tail_commit_file.clone())?;
    ...
    pub(crate) fn try_new_with_checkpoint(&self, checkpoint: ParsedLogPath) -> DeltaResult<Self> {
        ...
        new_log_segment.listed.checkpoint_parts = OwnedLogPaths::try_from_iter([Ok(checkpoint)])?;
    ...
    pub(crate) fn try_new_with_crc_file(&self, crc_file: ParsedLogPath<Url>) -> DeltaResult<Self> {
        ...
        let crc_file = crc_file.try_with_file_metadata(0, 0)?;
        let mut new_log_segment = self.clone();
    ...
    pub(crate) fn find_commit_cover(&self) -> DeltaResult<crate::OwnedFileMetas> {
        ...
        let mut selected_files = crate::OwnedFileMetas::default();
        for next in all_files {
            match last_pushed {
                ...
                Some(prev) if prev.version == next.version => {
                    selected_files.pop();
                    debug!("Selecting {next:?}, it covers a wider range");
                }
                // Skip later files whose start overlaps with the previous end
                Some(previous) if matches!(previous.file_type, LogPathFileType::CompactedCommit { hi } if next.version <= hi) => {
            ...
            selected_files.try_push_copy(&next.location)?;
        }
    ...
    fn get_file_actions_schema_and_sidecars(
        &self,
        engine: &dyn Engine,
    ) -> DeltaResult<(Option<SchemaRef>, crate::OwnedFileMetas)> {
        ...
        let Some(checkpoint) = self.listed.checkpoint_parts.first() else {
            return Ok((None, crate::OwnedFileMetas::default()));
        };
        ...
        match &checkpoint.file_type {
            MultiPartCheckpoint { .. } => {
                ...
                Ok((Some(schema), crate::OwnedFileMetas::default()))
            }
            ...
            SinglePartCheckpoint | UuidCheckpoint if checkpoint.extension.as_str() == "parquet" => {
                ...
                if checkpoint_schema.field(SIDECAR_NAME).is_some() {
                    ...
                    Ok((Some(checkpoint_schema), crate::OwnedFileMetas::default()))
                }
            }
            _ => Ok((None, crate::OwnedFileMetas::default())),
        }
    }
    ...
    > {
        ...
        let (file_actions_schema, sidecar_files) = if need_file_actions {
            ...
            (None, crate::OwnedFileMetas::default())
        };
        ...
        let mut checkpoint_file_meta = crate::OwnedFileMetas::default();
        for file in &self.listed.checkpoint_parts { checkpoint_file_meta.try_push_copy(&file.location)?; }
    ...
    fn extract_sidecar_refs(
    ...
    ) -> DeltaResult<crate::OwnedFileMetas> {
        ...
        let batches = match checkpoint.extension.as_str() {
            ...
            _ => return Ok(crate::OwnedFileMetas::default()),
        };
        ...
        // Convert to FileMeta
        let mut files = crate::OwnedFileMetas::default();
        for sidecar in &visitor.sidecars {
            files.try_push_owned(sidecar.to_filemeta(&self.log_root)?)?;
        }
        Ok(files)
    }
    ...
    pub(crate) fn segment_after_version(&self, start_v_exclusive: Version) -> DeltaResult<Self> {
        ...
        Ok(LogSegment {
            ...
            listed: LogSegmentFiles {
                ...
                checkpoint_parts: OwnedLogPaths::default(),
    ...
    ) -> DeltaResult<(OwnedLogPaths, OwnedLogPaths)> {
        let above_lo = |v: Version| lo_exclusive.is_none_or(|lo| lo < v);
        let commits = OwnedLogPaths::try_from_iter(self
            .listed
            .ascending_commit_files
            .iter()
            ...
            .cloned()
            .map(Ok))?;
        let compactions = OwnedLogPaths::try_from_iter(self
            .listed
    ...
    pub(crate) fn commits_since_log_compaction_or_checkpoint(&self) -> u64 {
        ...
        let max_compaction_end = self
            ...
            .iter()
            .fold(0, |cur, f| {
                if let LogPathFileType::CompactedCommit { hi } = f.file_type
                {
                    Version::max(cur, hi)
                } else {
