...
use crate::crc::{
    ...
    read_crc_file_or_none, size_to_u64,
};
...
impl LogSegment {
    ...
    pub(crate) fn pick_latest_base_crc(
        ...
        let disk = match preferred_disk_crc {
            Some(file) => read_crc_file_or_none(engine, file)?,
            None => None,
        };
        Ok(disk.or_else(|| in_memory_base.cloned()))
    }
    ...
    pub(crate) fn read_latest_crc(&self, engine: &dyn Engine) -> DeltaResult<Option<Arc<Crc>>> {
    ...
    pub(crate) fn build_incremental_crc_from_base(
        ...
        let scope = engine.resource_scope();
        if let Some(histogram) = base_crc.file_stats().and_then(|s| s.file_size_histogram()) {
            crate::crc::resource::reserve(
                &scope,
                "crc_replay_histogram_seed",
                crate::crc::resource::histogram_bytes(histogram)?,
            )?;
        }
        ...
        base_crc
            .try_clone_admitted(scope.clone())?
            .try_apply_admitted(delta, self.end_version, scope)
    }
    ...
    pub(crate) fn build_incremental_crc_delta(
        ...
        let scope = crate::crc::resource::select_scope(engine.resource_scope(), None)?;
        crate::crc::resource::reserve(
            &scope,
            "crc_replay_diagnostics",
            crate::crc::resource::diagnostic_bytes(0)?,
        )?;
        self.build_incremental_crc_delta_inner(engine, base_version, seed_histogram, scope.clone())
            .map_err(|error| crate::crc::resource::retain_error(error, &scope))
    }

    fn build_incremental_crc_delta_inner(
        &self,
        engine: &dyn Engine,
        base_version: Version,
        seed_histogram: Option<FileSizeHistogram>,
        scope: Option<std::sync::Arc<crate::resource::NativeResourceScope>>,
    ) -> DeltaResult<CrcDelta> {
        require!(
        ...
        // Both descriptor vectors and cloned URLs are derived from this original log segment.
        // The filtered reference iterator has no exact lower size hint: include
        // every RawVec growth layout, separately from the exact FileMeta vector.
        let count = self.listed.ascending_commit_files.len();
        let mut descriptor_bytes = crate::crc::resource::add(
            crate::crc::resource::mul(
                crate::crc::resource::add(count, 4)?,
                4 * std::mem::size_of::<&crate::path::ParsedLogPath>(),
            )?,
            crate::crc::resource::mul(count, std::mem::size_of::<crate::FileMeta>())?,
        )?;
        for file in &self.listed.ascending_commit_files {
            descriptor_bytes =
                crate::crc::resource::add(descriptor_bytes, file.location.location.as_str().len())?;
        }
        crate::crc::resource::reserve(&scope, "crc_replay_file_descriptors", descriptor_bytes)?;
        ...
        acc.delta.resource_owner = crate::crc::resource::CrcResourceOwner(scope);
...
impl RowVisitor for CrcReplayVisitor<'_> {
    ...
    fn visit<'a>(&mut self, row_count: usize, getters: &[&'a dyn GetData<'a>]) -> DeltaResult<()> {
        ...
        self.admit_original_getters(row_count, getters)?;
...
impl CrcReplayVisitor<'_> {
    fn admit_original_getters<'a>(
        &self,
        row_count: usize,
        getters: &[&'a dyn GetData<'a>],
    ) -> DeltaResult<()> {
        use crate::crc::resource::{add, diagnostic_bytes, map_layout, mul, reserve};
        let scope = &self.acc.delta.resource_owner.0;
        if scope.is_none() {
            return Ok(());
        }
        // Getter schema/type failures can allocate a bounded diagnostic before
        ...
        let mut bytes = add(
            ...
            map_layout::<String, crate::actions::SetTransaction>(add(
                self.acc.delta.set_transactions.capacity(),
                row_count,
            )?)?,
        )?;
        for row in 0..row_count {
            ...
            for column in [
                ...
                if let Some(value) = getters[column].get_str(row, "CRC borrowed string")? {
                    // Native materialized value + independently owned map key / file marker.
                    strings = add(strings, mul(value.len(), 2)?)?;
                }
            }
            for column in [protocol_start + 2, protocol_start + 3, metadata_start + 6] {
                if let Some(list) = getters[column].get_list(row, "CRC borrowed list")? {
                    bytes = add(
                        bytes,
                        mul(
                            list.len(),
                    ...
                    for index in 0..list.len() {
                        strings = add(strings, mul(list.get_borrowed(index).len(), 2)?)?;
                    }
                }
            }
            if let Some(map) =
                getters[metadata_start + 8].get_map(row, "CRC borrowed configuration")?
            {
                bytes = add(bytes, map_layout::<String, String>(map.len())?)?;
                for (key, value) in map.borrowed_entries() {
                    strings = add(strings, add(key.len(), value.map_or(0, str::len))?)?;
                }
            }
            bytes = add(bytes, add(strings, diagnostic_bytes(strings)?)?)?;
        }
        reserve(scope, "crc_replay_owned_batch", bytes)
    }
}
