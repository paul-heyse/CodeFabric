...
use crate::arrow::array::{
    Array, ArrayRef, AsArray, ListArray, MapArray, RecordBatch, RecordBatchOptions, StructArray,
};
use crate::arrow::datatypes::{DataType as ArrowDataType, FieldRef, Schema as ArrowSchema};
...
pub(crate) fn apply_schema(array: &dyn Array, schema: &DataType) -> DeltaResult<RecordBatch> {
    ...
    resource::arc::<ArrowSchema>("native_apply_schema_RecordBatch_schema")?;
...
fn transform_struct(
    ...
    // Borrow original descriptors: cloning StructArray would allocate a second columns Vec.
    let input_col_count = struct_array.num_columns();
    let mut transformed_fields =
        resource::new_vec::<FieldRef>(input_col_count, "native_apply_schema_fields")?;
    let mut transformed_cols =
        resource::new_vec::<ArrayRef>(input_col_count, "native_apply_schema_columns")?;
    for ((sa_col, input_field), target_field) in struct_array
        .columns()
        .iter()
        ...
        .zip(target_fields)
    {
        ...
        scalar_resource::prepare_field_metadata(target_field)?;
        ...
        transformed_fields.push(resource::field(
            &target_field.name,
            transformed_col.data_type(),
            target_field.nullable,
            Some(arrow_metadata),
        )?);
        transformed_cols.push(transformed_col);
    }
    if transformed_cols.len() != input_col_count {
        return Err(Error::internal_error(resource::format(format_args!(
    ...
    Ok(StructArray::try_new_with_length(
        resource::fields(transformed_fields)?,
        transformed_cols,
...
fn apply_schema_to_list(
    ...
    let element_path = resource::path(relative_path, LIST_ARRAY_ROOT)?;
    ...
    scalar_resource::prepare_field_id_metadata(element_id)?;
    let transformed_arrow_element_field = resource::field(
        arrow_element_field.name(),
        transformed_arrow_values.data_type(),
        target_inner_type.contains_null,
        Some(parquet_field_id_metadata(element_id)),
    )?;
    Ok(ListArray::try_new(
        transformed_arrow_element_field,
        offset_buffer,
...
fn apply_schema_to_map(
    ...
    let ArrowDataType::Map(arrow_map_field, ordered) = ma.data_type() else {
        unreachable!("native MapArray data type")
    };
    let input_fields = ma.entries().fields();
    let key_path = resource::path(relative_path, MAP_KEY_DEFAULT)?;
    let value_path = resource::path(relative_path, MAP_VALUE_DEFAULT)?;
    let key_id = ancestor
    ...
    let value_id = ancestor
        ...
        .flatten();
    let transformed_key =
        apply_schema_to_inner(ma.keys(), &kernel_map_type.key_type, ancestor, &key_path)?;
    let transformed_value = apply_schema_to_inner(
        ma.values(),
        &kernel_map_type.value_type,
        ancestor,
        &value_path,
    )?;
    scalar_resource::prepare_field_id_metadata(key_id)?;
    let key_field = resource::field(
        input_fields[0].name(),
        transformed_key.data_type(),
        false,
        Some(parquet_field_id_metadata(key_id)),
    )?;
    scalar_resource::prepare_field_id_metadata(value_id)?;
    let value_field = resource::field(
        input_fields[1].name(),
        transformed_value.data_type(),
        kernel_map_type.value_contains_null,
        Some(parquet_field_id_metadata(value_id)),
    )?;
    let mut fields = resource::new_vec::<FieldRef>(2, "native_apply_schema_map_fields")?;
    fields.push(key_field);
    fields.push(value_field);
    let fields = resource::fields(fields)?;
    let mut columns = resource::new_vec::<ArrayRef>(2, "native_apply_schema_map_columns")?;
    columns.push(transformed_key);
    columns.push(transformed_value);
    let entries = StructArray::try_new(fields.clone(), columns, ma.entries().nulls().cloned())?;
    let entries_field = resource::field(
...
fn apply_schema_to_inner(
    ...
    let array: ArrayRef = match schema {
        Struct(stype) => {
            let value = apply_schema_to_struct(array, stype)?;
            resource::arc::<StructArray>("native_apply_schema_struct_owner")?;
            Arc::new(value)
        }
        Array(atype) => {
            let value = apply_schema_to_list(array, atype, ancestor, relative_path)?;
            resource::arc::<ListArray>("native_apply_schema_list_owner")?;
            Arc::new(value)
        }
        Map(mtype) => {
            let value = apply_schema_to_map(array, mtype, ancestor, relative_path)?;
            resource::arc::<MapArray>("native_apply_schema_map_owner")?;
            Arc::new(value)
        }
        _ => {
