...
use crate::arrow::array::{
    Array as ArrowArray, ArrayRef as ArrowArrayRef, GenericListArray, MapArray, OffsetSizeTrait,
    PrimitiveArray, RecordBatch, RecordBatchOptions, StringArray, StructArray, make_array,
    new_null_array,
};
use crate::arrow::buffer::NullBuffer;
use crate::arrow::compute::{CastOptions, cast_with_options};
use crate::arrow::datatypes::{
    DataType as ArrowDataType, Field as ArrowField, FieldRef as ArrowFieldRef,
    Fields as ArrowFields, Int64Type, Schema as ArrowSchema, SchemaRef as ArrowSchemaRef,
};
use crate::arrow::json::writer::make_encoder;
use crate::arrow::json::{EncoderOptions, ReaderBuilder};
...
use crate::parquet::arrow::{PARQUET_FIELD_ID_META_KEY, ProjectionMask};
...
use crate::transforms::{SchemaTransform, transform_output_type};
...
macro_rules! prim_array_cmp {
    ( $left_arr: ident, $right_arr: ident, $(($data_ty: pat, $prim_ty: ty)),+ ) => {
        ...
        $(
            $data_ty => {
                let Some(prim_array) = $left_arr.as_primitive_opt::<$prim_ty>() else {
                    return Err(Error::InvalidExpressionEvaluation(crate::engine::arrow_expression::expression_resource::format(
                        format_args!("Cannot cast to primitive array: {}", $left_arr.data_type()))?));
                };
                let Some(list_array) = $right_arr.as_list_opt::<i32>() else {
                    return Err(Error::InvalidExpressionEvaluation(crate::engine::arrow_expression::expression_resource::format(
                        format_args!("Cannot cast to list array: {}", $right_arr.data_type()))?));
                };
                crate::engine::arrow_expression::expression_resource::in_list(prim_array, list_array)?;
                crate::arrow::compute::kernels::comparison::in_list(prim_array, list_array)
            }
        )+
            _ => Err(ArrowError::CastError(
                        crate::engine::arrow_expression::expression_resource::format(format_args!("Bad Comparison between: {:?} and {:?}",
                            $left_arr.data_type(),
                            $right_arr.data_type()))?
                        )
                )
...
pub(crate) fn parse_json(
    ...
    let json_strings =
        ArrowEngineData::try_from_engine_data(json_strings)?.into_owned_record_batch();
    let result = json_strings.try_map(|batch| parse_json_impl(batch.column(0).as_ref(), schema))?;
    crate::engine::arrow_expression::expression_resource::boxed::<ArrowEngineData>("native_parse_json_engine_data")?;
    Ok(Box::new(ArrowEngineData::from_native_owned(result)))
}
...
pub(crate) fn parse_json_impl(
    ...
    match json_strings.data_type() {
        ...
        dt => Err(Error::Generic(crate::engine::arrow_expression::expression_resource::format(format_args!(
...
fn parse_json_inner<'a>(
    ...
    use crate::engine::arrow_expression::scalar_resource;
    // The generic Cow transform builds temporary field vectors even if it returns
    // Borrowed. Probe the selected leaf family without constructing descriptors.
    fn needs_relaxation(schema: &StructType, depth: usize) -> DeltaResult<bool> {
        if depth > 64 {
            return Err(crate::resource::ResourceExhausted { kind: "native_stats_schema_depth", requested: depth, limit: 64 }.into());
        }
        for field in schema.fields() {
            match field.data_type() {
                DataType::Primitive(PrimitiveType::Timestamp | PrimitiveType::TimestampNtz | PrimitiveType::Date | PrimitiveType::Decimal(_)) => return Ok(true),
                DataType::Struct(child) if needs_relaxation(child, depth + 1)? => return Ok(true),
                _ => {}
            }
        }
        Ok(false)
    }
    let arrow_target = Arc::new(scalar_resource::prepare_schema(schema.as_ref())?);
    if !needs_relaxation(&schema, 1)? {
        return decode_with_arrow_json(json_strings, num_rows, arrow_target);
    }
    let admission = crate::schema::resource::SchemaRelaxationAdmission::begin(&schema)?;
    if admission.is_none() && arrow_schema_59::resource::current_resource_owner().is_some() {
        return Err(crate::resource::ResourceExhausted { kind: "native_stats_relaxation_scope", requested: 1, limit: 0 }.into());
    }
    match StringifyFailureProneLeaves.transform_struct(schema.as_ref()) {
        Cow::Borrowed(_) => decode_with_arrow_json(json_strings, num_rows, arrow_target),
        Cow::Owned(relaxed) => {
            let relaxed = match admission { Some(admission) => admission.finish(relaxed)?, None => relaxed };
            let arrow_relaxed = Arc::new(scalar_resource::prepare_schema(&relaxed)?);
...
fn decode_with_arrow_json<'a>(
    ...
    use crate::engine::arrow_expression::expression_resource;
    if num_rows == 0 {
        let mut columns=expression_resource::new_vec(schema.fields().len(),"native_empty_json_columns")?;
        for field in schema.fields() {
            columns.push(expression_resource::empty_column(field.data_type())?);
        }
        return Ok(RecordBatch::try_new_with_options(schema,columns,&RecordBatchOptions::new().with_row_count(Some(0)))?);
    }
    ...
    if let Some(batch) = decoder.flush()? {
        if batch.num_rows() != num_rows {
            return Err(Error::Generic(expression_resource::format(format_args!(
                "Unexpected number of rows decoded. Got {}, expected{}", batch.num_rows(), num_rows
            ))?));
        }
        return Ok(batch);
    }
    Err(Error::generic(expression_resource::format(format_args!(
...
#[internal_api]
pub(crate) fn safe_cast_back(decoded: RecordBatch, target: &ArrowSchemaRef) -> DeltaResult<RecordBatch> {
    let opts = CastOptions {
        safe: true,
        ..Default::default()
    };
    let (_, columns, row_count) = decoded.into_parts();
    let mut result = crate::engine::arrow_expression::expression_resource::new_vec(
        columns.len(), "native_stats_cast_columns",
    )?;
    for (array, field) in columns.into_iter().zip(target.fields().iter()) {
        result.push(safe_cast_array(array, field.data_type(), &opts)?);
    }
    let columns = result;
...
fn safe_cast_array(
    ...
    match target {
        ArrowDataType::Struct(target_fields) => {
            ...
            let mut new_children = crate::engine::arrow_expression::expression_resource::new_vec(
                s.num_columns(), "native_stats_cast_struct_columns",
            )?;
            for (child, field) in s.columns().iter().zip(target_fields.iter()) {
                new_children.push(safe_cast_array(child.clone(), field.data_type(), opts)?);
            }
            crate::engine::arrow_expression::expression_resource::arc::<StructArray>("native_stats_cast_struct_owner")?;
        ...
        _ => {
            crate::engine::arrow_expression::expression_resource::stats_cast(array.as_ref(), target)?;
            let output = cast_with_options(&array, target, opts)?;
            output.try_visit_buffers(&mut |buffer| arrow_data_59::try_retain_fresh_buffer(buffer))?;
            Ok(output)
        },
    }
}
...
) -> DeltaResult<crate::engine::arrow_data::NativeOwnedRecordBatch> {
    let filtered = filtered_data.apply_selection_vector()?;
    let arrow_data = ArrowEngineData::try_from_engine_data(filtered)?;
    Ok(arrow_data.into_owned_record_batch())
}

// This immutable native byte owner keeps the admitted JSON bank and neutral
// Arrow owner with the actual encoded Vec across Bytes clone/slice and async IO.
// Field declaration order keeps backing destruction before receipt release.
struct OwnedJsonBacking {
    bytes: Vec<u8>,
    _native: crate::engine::arrow_data::NativeDataOwners,
    _arrow: arrow_schema_59::resource::ResourceOwnerHandle,
}
impl AsRef<[u8]> for OwnedJsonBacking {
    fn as_ref(&self)->&[u8]{&self.bytes}
}

/// Serialize actual native filtered batches to owned line-delimited JSON bytes.
/// Ordinary null struct fields remain omitted; map keys with null values remain
/// present, preserving the Delta partition-values contract without constructing
/// a new custom encoder for every map row.
#[internal_api]
pub(crate) fn to_json_bytes(
    data: impl Iterator<Item = DeltaResult<FilteredEngineData>> + Send,
) -> DeltaResult<bytes::Bytes> {
    ...
    for chunk in data {
        ...
        resource::admit(std::alloc::Layout::array::<ArrowArrayRef>(batch.num_columns()).map_err(|_|crate::resource::ResourceExhausted{kind:"native_json_writer_columns",requested:batch.num_columns(),limit:isize::MAX as usize})?,"native_json_writer_columns")?;
        let array=StructArray::from(batch.clone());
        let field=resource::field("",array.data_type(),false,None)?;
        let mut encoder=make_encoder(&field,&array,&options)?;
        for row in 0..batch.num_rows() {encoder.try_encode_line(row,&mut bytes)?;}
    }
    let native=crate::engine::arrow_data::NativeDataOwners::current();
    let arrow=arrow_schema_59::resource::ResourceOwnerHandle::capture();
    // bytes 1.12.1 uses repr(C) Owned<T> { AtomicUsize ref_cnt, T owner }
    // allocated by Box before exposing the original backing; no payload copy.
    let layout=std::alloc::Layout::new::<std::sync::atomic::AtomicUsize>().extend(std::alloc::Layout::new::<OwnedJsonBacking>()).map_err(|_|crate::resource::ResourceExhausted{kind:"native_json_bytes_owner",requested:usize::MAX,limit:isize::MAX as usize})?.0.pad_to_align();
    resource::admit(layout,"native_json_bytes_owner")?;
    Ok(bytes::Bytes::from_owner(OwnedJsonBacking{bytes,_native:native,_arrow:arrow}))
}
...
mod tests {
    ...
    use crate::schema::{
        ArrayType, ColumnMetadataKey, DataType, MapType, MetadataColumnSpec, MetadataValue,
        StructField, StructType, schema_ref,
    };
