...
use super::arrow_expression::{expression_resource as resource, scalar_resource};
...
impl EnsureDataTypes {
    ...
    fn ensure_data_types(
        ...
        match (kernel_type, arrow_type) {
            (DataType::Primitive(_), _) if arrow_type.is_primitive() => {
                check_cast_compat(scalar_resource::prepare_type(kernel_type)?, arrow_type)
            }
            ...
            (DataType::Variant(variant_fields), ArrowDataType::Struct(arrow_fields)) => {
                require!(arrow_fields.len() == variant_fields.num_fields(), {
                    let unexpected = resource::joined_names(|| {
                        arrow_fields
                            .iter()
                            .map(|f| f.name().as_str())
                            .filter(|name| variant_fields.field(name).is_none())
                    })?;
                    make_arrow_error(resource::format(format_args!(
                ...
                for kernel_field in variant_fields.fields() {
                    let Some(arrow_field) = arrow_fields
                        ...
                        return Err(make_arrow_error(resource::format(format_args!(
            ...
            (DataType::Map(kernel_map_type), ArrowDataType::Map(arrow_map_type, _)) => {
                let ArrowDataType::Struct(fields) = arrow_map_type.data_type() else {
                    return Err(make_arrow_error(resource::format(format_args!(
                        "Arrow map type wasn't a struct."
                    ))?));
                };
                ...
                match self.mode {
                    ...
                    ValidationMode::TypesAndNames | ValidationMode::Full => {
                        ...
                        require!(kernel_fields.num_fields() == found_fields, {
                            let missing_field_names = resource::joined_names(|| {
                                kernel_fields
                                    .field_names()
                                    .filter(|kernel_field| {
                                        !arrow_fields
                                            .iter()
                                            .any(|field| field.name() == *kernel_field)
                                    })
                                    .take(5)
            ...
            _ => Err(make_arrow_error(resource::format(format_args!(
    ...
    fn ensure_nullability(
        ...
        {
            Err(Error::Generic(resource::format(format_args!(
...
impl PartialEq<String> for MetadataValue {
    fn eq(&self, other: &String) -> bool {
        // Match the existing Display text directly. The sink keeps consuming
        // after a mismatch so serde's formatter does not allocate an IO error.
        struct Compare<'a> {
            expected: &'a str,
            offset: usize,
            equal: bool,
        }
        impl std::fmt::Write for Compare<'_> {
        ...
        let mut sink = Compare {
            expected: other,
            offset: 0,
            equal: true,
        };
        std::fmt::write(&mut sink, format_args!("{self}")).is_ok()
            && sink.equal
            && sink.offset == other.len()
    }
}
