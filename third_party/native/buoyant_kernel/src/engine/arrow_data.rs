...
use crate::arrow::array::types::{
    Date32Type, Decimal128Type, Float32Type, Float64Type, GenericStringType, Int8Type, Int16Type,
...
#[path = "native_owned_batch.rs"]
mod native_owned_batch;
pub use native_owned_batch::{NativeDataOwners, NativeOwnedRecordBatch};
...
pub trait EngineDataArrowExt {
    ...
    /// Transfer the native batch and its exact allocation lifetime owners together.
    fn try_into_owned_record_batch(self) -> DeltaResult<NativeOwnedRecordBatch>;
}

impl EngineDataArrowExt for Box<dyn EngineData> {
    fn try_into_owned_record_batch(self) -> DeltaResult<NativeOwnedRecordBatch> {
        Ok(ArrowEngineData::try_from_engine_data(self)?.into_owned_record_batch())
    }
    fn try_into_record_batch(self) -> DeltaResult<RecordBatch> {
        self.try_into_owned_record_batch()?
...
impl ArrowEngineData {
    ...
    /// Transfer data and native allocation lifetimes as one owned value.
    pub fn into_owned_record_batch(self) -> NativeOwnedRecordBatch {
        NativeOwnedRecordBatch::from_parts(self.data, self.native_owners)
    }

    /// Reconstruct engine data from an explicitly owned native batch.
    pub fn from_native_owned(batch: NativeOwnedRecordBatch) -> Self {
        let (data, native_owners) = batch.into_parts();
        Self {
            data,
            native_owners,
        }
    ...
    pub fn native_owners(&self) -> &NativeDataOwners {
        &self.native_owners
    }

    /// Preserve the owners of an input whose arrays or metadata are shared by this output.
    pub fn try_inherit_owners(mut self, owners: NativeDataOwners) -> DeltaResult<Self> {
        self.native_owners = self.native_owners.try_merge(owners)?;
        Ok(self)
    }
...
impl EngineData for ArrowEngineData {
    ...
    fn visit_rows(
        ...
        let entries = leaf_columns
            ...
            .try_fold(0usize, |n, column| {
                n.checked_add(column.path().len().max(1))
            })
            .ok_or_else(|| crate::resource::ResourceExhausted {
                kind: "native_visitor_prefix_count",
                requested: usize::MAX,
                limit: isize::MAX as usize,
            })?;
        let depth = leaf_columns
            .iter()
            .map(|column| column.path().len())
            .max()
            .unwrap_or(0)
            .max(1);
        if depth > 64 {
            return Err(crate::resource::ResourceExhausted {
                kind: "native_visitor_path_depth",
                requested: depth,
                limit: 64,
            }
            .into());
        ...
        for (column, data_type) in leaf_columns.iter().zip(leaf_types.iter()) {
            ...
            column_map.insert(leaf, ColumnState::AwaitingGetter(data_type));
            for length in (1..column.path().len()).rev() {
                let mut parent = resource::new_vec(length, "native_visitor_parent_key")?;
                parent.extend(column.path()[..length].iter().map(String::as_str));
                column_map.entry(parent).or_insert(ColumnState::Parent);
            }
        }
        debug!(
        ...
        let mut path = resource::new_vec(depth, "native_visitor_path")?;
        Self::extract_columns(&mut path, &mut column_map, &self.data)?;
        let mut getters = resource::new_vec(leaf_columns.len(), "native_visitor_getters")?;
        for column in leaf_columns {
            path.clear();
            path.extend(column.path().iter().map(String::as_str));
            match column_map.get(path.as_slice()) {
                Some(ColumnState::HasGetter(getter)) => getters.push(*getter),
                _ => {
                    return Err(resource::kernel_diagnostic(
                        Error::MissingColumn,
                        format_args!("Column {column} not found in the data"),
                    ));
                }
            }
        }
    ...
    fn append_columns(
        ...
        use crate::engine::arrow_expression::{expression_resource as resource, scalar_resource};
        resource::check_evaluator_owner(&self.native_owners)?;
        if columns.len() != schema.num_fields() {
            return Err(Error::Generic(resource::format(format_args!(
                "number of columns({}) must match number of fields({}) in a schema",
                self.data.num_columns().saturating_add(columns.len()),
                self.data.num_columns().saturating_add(schema.num_fields())
            ))?));
        }
        let count = self
            .data
            ...
            .ok_or_else(|| crate::resource::ResourceExhausted {
                kind: "native_append_column_count",
                requested: usize::MAX,
                limit: isize::MAX as usize,
            })?;
        let rows = if self.data.num_columns() == 0 {
            columns
                .first()
                .map_or(0, |column| column.array_elements().len())
        } else {
            self.data.num_rows()
        };
        let schema = scalar_resource::prepare_schema(schema.as_ref())?;
        let mut combined_fields = resource::new_vec(count, "native_append_fields")?;
        combined_fields.extend(self.data.schema_ref().fields().iter().cloned());
        combined_fields.extend(schema.fields().iter().cloned());
        // prepare_schema admits the immediate Arc<ArrowSchema>; use that native
        // descriptor slot for this actual combined schema, without another Arc.
        let combined_schema = Arc::new(ArrowSchema::new(resource::fields(combined_fields)?));
        let mut combined_columns = resource::new_vec(count, "native_append_columns")?;
        combined_columns.extend(self.data.columns().iter().cloned());
        for column in columns {
            // Empty existing batches still have an explicit row count; preserve
            // native RecordBatch's equality requirement before decoding columns.
            if column.array_elements().len() != rows {
                return Err(Error::Generic(resource::format(format_args!(
                    "all columns in a record batch must have the same length: {} != {}",
                    column.array_elements().len(),
                    rows
                ))?));
            }
            combined_columns.push(column.to_arrow()?);
        }
        let data = RecordBatch::try_new(combined_schema, combined_columns)?;
        let native_owners = self.native_owners.clone().try_with_current()?;
        resource::boxed::<ArrowEngineData>("native_append_engine_data")?;
    ...
    fn apply_selection_vector(
        ...
        use crate::engine::arrow_expression::expression_resource as resource;
        if selection_vector.len() > self.len() {
            return Err(Error::InvalidSelectionVector(resource::format(
                format_args!(
        ...
        // Empty and all-true short vectors select every original row.
        if selection_vector.iter().all(|value| *value) {
            return Ok(self);
        }
        if self.len() > selection_vector.capacity() {
            resource::admit(
                std::alloc::Layout::array::<bool>(self.len()).map_err(|_| {
                    crate::resource::ResourceExhausted {
                        kind: "native_selection_vector",
                        requested: self.len(),
                        limit: isize::MAX as usize,
                    }
                })?,
                "native_selection_vector",
            )?;
            selection_vector
                .try_reserve_exact(self.len() - selection_vector.len())
                .map_err(|_| crate::resource::ResourceExhausted {
                    kind: "native_selection_vector",
                    requested: self.len(),
                    limit: 0,
                })?;
        ...
        let selection = resource::boolean_values(&selection_vector)?;
        let owned = self
            .into_owned_record_batch()
            .try_map(|batch| Ok::<_, Error>(filter_record_batch(&batch, &selection)?))?;
        resource::boxed::<Self>("native_filtered_engine_data")?;
        Ok(Box::new(Self::from_native_owned(owned)))
    }
...
impl ArrowEngineData {
    fn extract_columns<'a>(
        path: &mut Vec<&'a str>,
        column_map: &mut HashMap<Vec<&'a str>, ColumnState<'a>>,
            ...
            if let Some(state) = column_map.get_mut(path.as_slice()) {
                match state {
                    ...
                    ColumnState::HasGetter(_) => {
                        return Err(
                            crate::engine::arrow_expression::expression_resource::kernel_diagnostic(
                                Error::InternalError,
                                format_args!(
                ...
                debug!("Skipping unmasked path {}", ColumnName::display_path(path));
            }
            path.pop();
        }
        Ok(())
    ...
    fn extract_leaf_column<'a>(
        ...
        let result: Result<&'a dyn GetData<'a>, _> = match data_type {
            ...
            &DataType::BYTE => {
                debug!("Pushing int8 array for {}", ColumnName::display_path(path));
                col.as_primitive_opt::<Int8Type>()
                    .map(|a| a as _)
                    .ok_or("byte")
            }
            &DataType::SHORT => {
                debug!("Pushing int16 array for {}", ColumnName::display_path(path));
                col.as_primitive_opt::<Int16Type>()
                    .map(|a| a as _)
                    .ok_or("short")
            }
            &DataType::INTEGER => {
                debug!("Pushing int32 array for {}", ColumnName::display_path(path));
            ...
            &DataType::LONG => {
                debug!("Pushing int64 array for {}", ColumnName::display_path(path));
            ...
            &DataType::FLOAT => {
                debug!("Pushing float array for {}", ColumnName::display_path(path));
            ...
            &DataType::DATE => {
                debug!("Pushing date array for {}", ColumnName::display_path(path));
            ...
            DataType::Array(_) => {
                debug!("Pushing list for {}", ColumnName::display_path(path));
                col_as_list().ok_or("array<string>")
            }
            DataType::Map(_) => {
                debug!("Pushing map for {}", ColumnName::display_path(path));
...
mod tests {
    ...
    use super::{ArrowEngineData, extract_record_batch};
    use crate::actions::{LOG_PROTOCOL_SCHEMA, Metadata, Protocol, get_commit_schema};
    ...
    use crate::expressions::ArrayData;
    use crate::schema::{ArrayType, ColumnName, DataType, StructField, StructType, schema_ref};
