...
use super::expression_resource;
use crate::arrow::array::types::*;
use crate::arrow::array::{
    self as arrow_array, try_make_array, Array, ArrayBuilder, ArrayRef, AsArray, BooleanArray,
    Datum, ListArray, MapArray, MutableArrayData, RecordBatch, StringArray,
    StructArray,
};
...
pub(super) fn extract_column(
    ...
    loop {
        let child = parent
            ...
            .ok_or_else(|| expression_resource::arrow_diagnostic(ArrowError::SchemaError, format_args!("No such field: {field_name}")))?;
        ...
        parent = child
            ...
            .ok_or_else(|| expression_resource::arrow_diagnostic(ArrowError::SchemaError, format_args!("Not a struct: {field_name}")))?;
    }
}
...
fn evaluate_struct_expression(
    ...
    if fields.len() != output_schema.num_fields() {
        return Err(expression_resource::kernel_diagnostic(Error::Generic, format_args!(
    ...
    let mut output_cols =
        expression_resource::new_vec::<ArrayRef>(fields.len(), "native_struct_expression_columns")?;
    let mut output_fields =
        expression_resource::new_vec(fields.len(), "native_struct_expression_fields")?;
    for (expr, field) in fields.iter().zip(output_schema.fields()) {
        let column = evaluate_expression(expr, batch, Some(field.data_type()))?;
        output_fields.push(expression_resource::field(
            field.name(),
            column.data_type(),
            field.nullable,
            None,
    ...
    let null_buffer = if let Some(predicate_expr) = nullability_predicate {
        ...
        let bool_array = predicate_array
            ...
            .ok_or_else(|| expression_resource::kernel_diagnostic(Error::Generic, format_args!("Nullability predicate must evaluate to boolean")))?;
        let values = bool_array.values();
        let combined = match bool_array.nulls() {
            Some(nulls) => {
                expression_resource::bitmap_binary(values.len())?;
                let combined = values & nulls.inner();
                arrow_data_59::try_retain_fresh_buffer(combined.inner())?;
                combined
            }
            None => values.clone(),
        };
    ...
    let data = StructArray::try_new_with_length(
        expression_resource::fields(output_fields)?,
        output_cols,
        null_buffer,
        batch.num_rows(),
    )?;
    expression_resource::arc::<StructArray>("native_struct_expression_owner")?;
    Ok(Arc::new(data))
}
...
fn evaluate_struct_patch_expression(
    ...
    let mut output_cols = expression_resource::new_vec::<ArrayRef>(
        output_schema.num_fields(),
        "native_struct_patch_columns",
    )?;
    ...
    let mut next_output_type = || {
        output_schema_iter
            ...
            .ok_or_else(|| expression_resource::kernel_diagnostic(Error::Generic, format_args!("Too few fields in output schema")))
    };
    ...
    let source_data: &dyn ProvidesColumnByName = match source_array {
        Some(ref array) => array
            .as_any()
            .downcast_ref::<StructArray>()
            .ok_or_else(|| expression_resource::kernel_diagnostic(Error::Generic, format_args!("Input path must point to a struct")))?,
        None => batch,
    };
    ...
    for input_field in source_data.schema_fields() {
        ...
        if field_patch.is_none_or(|patch| patch.keep_input) {
            let _ = next_output_type()?; // Validate capacity before pushing the borrowed column.
    ...
    if used_field_patches < required_count {
        return Err(expression_resource::kernel_diagnostic(Error::Generic, format_args!("Some non-optional field patches reference invalid input field names")));
    }
    ...
    if output_schema_iter.next().is_some() {
        return Err(expression_resource::kernel_diagnostic(Error::Generic, format_args!("Too many fields in output schema")));
    }

    // Build the final struct, preserving null bitmap for nested patches
    let mut output_fields =
        expression_resource::new_vec(output_cols.len(), "native_struct_patch_fields")?;
    for (column, field) in output_cols.iter().zip(output_schema.fields()) {
        output_fields.push(expression_resource::field(
            field.name(),
            column.data_type(),
            column.is_nullable(),
    ...
    expression_resource::arc::<StructArray>("native_struct_patch_owner")?;
    Ok(Arc::new(data))
}
...
pub fn evaluate_expression(
    ...
    match (expression, result_type) {
        ...
        (Struct(..), dt) => Err(expression_resource::kernel_diagnostic(Error::Generic, format_args!(
        ...
        (StructPatch(_), _) => Err(expression_resource::kernel_diagnostic(Error::Generic, format_args!("Data type is required to evaluate struct patch expressions"))),
        (Predicate(pred), None | Some(&DataType::BOOLEAN)) => {
            let result = evaluate_predicate(pred, batch, false)?;
            expression_resource::arc::<BooleanArray>("native_predicate_expression_owner")?;
            Ok(Arc::new(result))
        }
        (Predicate(_), Some(data_type)) => Err(expression_resource::kernel_diagnostic(Error::Generic, format_args!(
        ...
        (Unary(UnaryExpression { op: ToJson, expr }), result_type) => match result_type {
            ...
            Some(data_type) => Err(expression_resource::kernel_diagnostic(Error::Generic, format_args!(
        ...
        (Binary(BinaryExpression { op, left, right }), _) => {
            ...
            expression_resource::arithmetic(left_arr.as_ref(), right_arr.as_ref())?;
            let output = eval(&left_arr, &right_arr)?;
            super::scalar_resource::retain_fresh_buffers(output.as_ref())?;
        ...
        (
            ...
            let mut arrays =
                expression_resource::new_vec(exprs.len(), "native_expression_coalesce_columns")?;
        ...
        (Opaque(OpaqueExpression { op, exprs }), _) => {
                ...
                .downcast_ref::<ArrowOpaqueExpressionOpAdaptor>()
            {
                ...
                None => Err(expression_resource::kernel_diagnostic(Error::Unsupported, format_args!(
        ...
        (ParseJson(p), _) => {
            ...
            match parse_json_impl(json_arr.as_ref(), p.output_schema.clone()) {
                Ok(batch) => {
                    expression_resource::arc::<StructArray>("native_parse_json_struct_array")?;
                    Ok(Arc::new(StructArray::from(batch)) as ArrayRef)
                }
                ...
                Err(e) => {
                    ...
                    let schema = super::scalar_resource::prepare_schema(p.output_schema.as_ref())?;
                    let mut columns = expression_resource::new_vec(
                        p.output_schema.num_fields(),
                        "native_parse_json_null_columns",
                    )?;
                    for field in p.output_schema.fields() {
                        let kind = super::scalar_resource::prepare_null(
                            field.data_type(),
                            json_arr.len(),
                        )?;
                        let mut builder = arrow_array::make_builder(&kind, json_arr.len());
                        Scalar::append_null(builder.as_mut(), field.data_type(), json_arr.len())?;
                        let column = builder.try_finish()?;
                        super::scalar_resource::retain_fresh_buffers(column.as_ref())?;
                        columns.push(column);
                    }
                    let nulls = expression_resource::null_bitmap(json_arr.len())?;
                    ...
                    expression_resource::arc::<StructArray>("native_parse_json_null_owner")?;
                    Ok(Arc::new(output))
                }
            }
        }
        (MapToStruct(m), Some(DataType::Struct(output_schema))) => {
            ...
            expression_resource::arc::<StructArray>("native_map_to_struct_owner")?;
            Ok(Arc::new(result) as ArrayRef)
        }
        (MapToStruct(_), dt) => Err(expression_resource::kernel_diagnostic(Error::Generic, format_args!(
            "MapToStruct expression requires a DataType::Struct result type, but got {dt:?}"
        ))),
        (Unknown(name), _) => Err(expression_resource::kernel_diagnostic(Error::Unsupported, format_args!("Unknown expression: {name:?}"))),
    }
}
...
fn evaluate_array_expression(
    ...
    let mut element_arrays =
        expression_resource::new_vec(exprs.len(), "native_expression_array_columns")?;
    for expr in exprs {
        element_arrays.push(evaluate_expression(expr, batch, element_kernel_type)?);
    }

    let element_type = expression_resource::clone_type(
        element_arrays
            .first()
            .ok_or_else(|| expression_resource::kernel_diagnostic(Error::Generic, format_args!("Array expression requires at least one element")))?
            .data_type(),
    )?;
    ...
    let array_data = expression_resource::array_data(&element_arrays)?;
    let total_len = num_rows.checked_mul(n).ok_or_else(|| {
        expression_resource::kernel_diagnostic(Error::Generic, format_args!(
            "Array expression length overflows usize: num_rows={num_rows} * inputs={n}"
        ))
    })?;
    let refs = expression_resource::data_refs(&array_data)?;
    let mut mutable = MutableArrayData::try_new(&refs, false, total_len)?;
    for row in 0..num_rows {
        for col in 0..n {
            mutable.try_extend(col, row, row + 1)?;
        }
    }
    let values = try_make_array(mutable.try_freeze()?)?;
    ...
    let count = num_rows
        ...
        .ok_or(crate::resource::ResourceExhausted {
            kind: "native_expression_list_offsets",
            requested: usize::MAX,
            limit: isize::MAX as usize,
        })?;
    let mut raw_offsets =
        expression_resource::new_vec::<i32>(count, "native_expression_list_offsets")?;
    for row in 0..=num_rows {
        raw_offsets.push((row * n) as i32);
    }
    expression_resource::admit(
        arrow_buffer_59::allocation_owner_layout(),
        "native_expression_list_offset_owner",
    )?;
    let offsets = OffsetBuffer::<i32>::new(raw_offsets.into());
    expression_resource::admit(
        std::alloc::Layout::array::<u8>(LIST_ARRAY_ROOT.len()).expect("static name"),
        "native_expression_list_field_name",
    )?;
    expression_resource::arc::<ArrowField>("native_expression_list_field")?;
    ...
    expression_resource::arc::<ListArray>("native_expression_list_array")?;
    super::scalar_resource::retain_fresh_buffers(&list)?;
    ...
    let to_type = match dir {
        ...
        ViewCast::ToNonView => match field.data_type() {
            ...
            other => {
                ...
                expression_resource::clone_type(other)?
            }
        },
    };
    let new_field = expression_resource::field(field.name(), &to_type, field.is_nullable(), Some(expression_resource::clone_metadata(field.metadata())?))?;
    let output = match vals.data_type() {
        ArrowDataType::List(_) => {
            let list = vals.as_list::<i32>();
            let values = cast_view_leaf(list.values(), &to_type)?;
            expression_resource::arc::<arrow_array::ListArray>("native_view_list_owner")?;
            Arc::new(arrow_array::ListArray::try_new(new_field.clone(), list.offsets().clone(), values, list.nulls().cloned())?) as ArrayRef
        }
        ArrowDataType::LargeList(_) => {
            let list = vals.as_list::<i64>();
            let values = cast_view_leaf(list.values(), &to_type)?;
            expression_resource::arc::<arrow_array::LargeListArray>("native_view_list_owner")?;
            Arc::new(arrow_array::LargeListArray::try_new(new_field.clone(), list.offsets().clone(), values, list.nulls().cloned())?) as ArrayRef
        }
        ArrowDataType::ListView(_) => {
            let list = vals.as_list_view::<i32>();
            match dir {
                ViewCast::ToNonView => normalize_list_view(list, new_field.clone())?,
                ViewCast::ToView => {
                    let values = cast_view_leaf(list.values(), &to_type)?;
                    expression_resource::arc::<arrow_array::ListViewArray>("native_view_list_owner")?;
                    Arc::new(arrow_array::ListViewArray::try_new(new_field.clone(),list.offsets().clone(),list.sizes().clone(),values,list.nulls().cloned())?) as ArrayRef
                }
            }
        }
        ArrowDataType::LargeListView(_) => {
            let list = vals.as_list_view::<i64>();
            match dir {
                ViewCast::ToNonView => normalize_list_view(list, new_field.clone())?,
                ViewCast::ToView => {
                    let values = cast_view_leaf(list.values(), &to_type)?;
                    expression_resource::arc::<arrow_array::LargeListViewArray>("native_view_list_owner")?;
                    Arc::new(arrow_array::LargeListViewArray::try_new(new_field.clone(),list.offsets().clone(),list.sizes().clone(),values,list.nulls().cloned())?) as ArrayRef
                }
            }
        }
        _ => unreachable!("validated list container"),
    };
    super::scalar_resource::retain_fresh_buffers(output.as_ref())?;
    Ok(output)
}

/// Original ListView casting expands valid row ranges in row order. Use Arrow's
/// admitted MutableArrayData range extension instead of an unbounded take-index
/// Vec, preserving overlapping ranges and the original parent null bitmap.
fn normalize_list_view<O: arrow_array::OffsetSizeTrait>(
    ...
    for row in 0..list.len() {
        if list.is_valid(row) {
            expanded = expanded.checked_add(list.value_sizes()[row].as_usize())
                .ok_or_else(|| expression_resource::offset_limit(usize::MAX,O::MAX_OFFSET))?;
            if expanded > O::MAX_OFFSET {return Err(expression_resource::offset_limit(expanded,O::MAX_OFFSET).into());}
        }
    }
    let count=list.len().checked_add(1).ok_or_else(||expression_resource::offset_limit(usize::MAX,isize::MAX as usize))?;
    let mut offsets=expression_resource::new_vec::<O>(count,"native_list_view_offsets")?;
    offsets.push(O::usize_as(0));
    let source=list.values().try_to_data()?;
    let refs=[&source];
    let mut mutable=MutableArrayData::try_new(&refs,false,expanded)?;
    let mut current=0usize;
    for row in 0..list.len() {
        if list.is_valid(row) {
            let start=list.value_offsets()[row].as_usize();
            let len=list.value_sizes()[row].as_usize();
            mutable.try_extend(0,start,start+len)?;
            current+=len;
        }
        offsets.push(O::usize_as(current));
    }
    let values=try_make_array(mutable.try_freeze()?)?;
    let values=if values.data_type()==field.data_type(){values}else{cast_view_leaf(&values,field.data_type())?};
    expression_resource::admit(arrow_buffer_59::allocation_owner_layout(),"native_list_view_offset_owner")?;
    let list=arrow_array::GenericListArray::<O>::try_new(field,OffsetBuffer::new(offsets.into()),values,list.nulls().cloned())?;
    expression_resource::arc::<arrow_array::GenericListArray<O>>("native_list_view_array_owner")?;
    super::scalar_resource::retain_fresh_buffers(&list)?;
    Ok(Arc::new(list))
}

fn cast_view_leaf(vals: &ArrayRef, target: &ArrowDataType) -> DeltaResult<ArrayRef> {
    expression_resource::view_cast(vals.as_ref(), target)?;
    let output = cast(vals, target)?;
    super::scalar_resource::retain_fresh_buffers(output.as_ref())?;
    Ok(output)
}
...
fn arrow_convert_to_non_view_type(vals: Arc<dyn Array>) -> DeltaResult<Arc<dyn Array>> {
    match vals.data_type() {
        ...
        ArrowDataType::Utf8View => cast_view_leaf(&vals, &ArrowDataType::Utf8),
        ArrowDataType::BinaryView => cast_view_leaf(&vals, &ArrowDataType::Binary),
        _ => Ok(vals),
    }
}
...
fn arrow_convert_to_view_type(vals: Arc<dyn Array>) -> DeltaResult<Arc<dyn Array>> {
    match vals.data_type() {
        ...
        ArrowDataType::Utf8 | ArrowDataType::LargeUtf8 => {
            cast_view_leaf(&vals, &ArrowDataType::Utf8View)
        }
        ArrowDataType::Binary | ArrowDataType::LargeBinary => {
            cast_view_leaf(&vals, &ArrowDataType::BinaryView)
        }
        _ => Ok(vals),
    }
}
...
pub fn evaluate_predicate(
    ...
    let result = evaluate_predicate_impl(predicate, batch, inverted)?;
    super::scalar_resource::retain_fresh_buffers(&result)?;
    Ok(result)
}

fn evaluate_predicate_impl(
    predicate: &Predicate,
    batch: &RecordBatch,
    inverted: bool,
) -> DeltaResult<BooleanArray> {
    ...
    let maybe_inverted = |result: Cow<'_, BooleanArray>| match inverted {
        true => { expression_resource::predicate_bitmaps(result.len(), 1)?; not(&result) },
    ...
    match predicate {
        BooleanExpression(expr) => {
            ...
            match arr.as_any().downcast_ref::<BooleanArray>() {
                ...
                None => Err(expression_resource::kernel_diagnostic(Error::Generic, format_args!("expected boolean array"))),
            }
        }
        ...
        Unary(UnaryPredicate { op, expr }) => {
            ...
            expression_resource::logical_nulls(arr.as_ref())?;
            expression_resource::predicate_bitmaps(arr.len(), 1)?;
            Ok(eval_op_fn(&arr)?)
        }
        Binary(BinaryPredicate { op, left, right }) => {
            ...
            let eval_in = || match (left, right) {
                    ...
                    if let Some(string_arr) = left.as_string_opt::<i32>() {
                        if let Some(list_arr) = right.as_list_opt::<i32>() {
                            if list_arr.value_type() == ArrowDataType::Utf8 {
                                expression_resource::in_list(string_arr, list_arr)?;
                ...
                (Expression::Literal(lit), Expression::Literal(Scalar::Array(ad))) => {
                    ...
                    Ok(expression_resource::repeated_boolean(exists, 1)?)
                }
                (l, r) => Err(expression_resource::kernel_diagnostic(Error::InvalidExpressionEvaluation, format_args!(
            ...
            expression_resource::comparison(left.as_ref(), right.as_ref())?;
        ...
        Junction(JunctionPredicate { op, preds }) => {
            ...
            preds
                ...
                .reduce(|left, right| {
                    ...
                    expression_resource::predicate_bitmaps(left.len(), 2)?;
                    Ok(reducer(&left, &right)?)
                })
                .unwrap_or_else(|| Ok(expression_resource::repeated_boolean(default, batch.num_rows())?))
        }
        ...
        Unknown(name) => Err(expression_resource::kernel_diagnostic(Error::Unsupported, format_args!("Unknown predicate: {name:?}"))),
    }
}
    ...
    match array_ref.data_type() {
        ArrowDataType::Struct(_) => {
            let struct_array = array_ref.as_struct_opt().ok_or_else(|| {
                expression_resource::arrow_diagnostic(ArrowError::InvalidArgumentError, format_args!(
            ...
            let field = expression_resource::field("root", struct_array.data_type(), true, None)?;
            ...
            let count =
                num_rows
                    .checked_add(1)
                    .ok_or(arrow_schema_59::resource::ResourceOwnerError {
                        kind: "native_json_offsets",
                        requested: usize::MAX,
                        limit: isize::MAX as usize,
                    })?;
            let mut offsets = expression_resource::new_vec::<i32>(count, "native_json_offsets")?;
            offsets.push(0);
            let mut nulls = expression_resource::null_builder(num_rows)?;
            for row in 0..num_rows {
                if struct_array.is_null(row) {
                    nulls.append_null();
                } else {
                    encoder.try_encode_with_limit(row, &mut data, i32::MAX as usize)?;
            ...
            expression_resource::admit(
                arrow_buffer_59::allocation_owner_layout(),
                "native_json_offset_owner",
            )?;
            expression_resource::admit(
                arrow_buffer_59::allocation_owner_layout(),
                "native_json_value_owner",
            )?;
            ...
            expression_resource::arc::<StringArray>("native_json_string_array")?;
            array
                .try_visit_buffers(&mut |buffer| arrow_data_59::try_retain_fresh_buffer(buffer))?;
            Ok(Arc::new(array))
        }
        _ => Err(expression_resource::arrow_diagnostic(ArrowError::InvalidArgumentError, format_args!(
...
pub fn coalesce_arrays(
    ...
    let Some((first, rest)) = arrays.split_first() else {
        return Err(expression_resource::arrow_diagnostic(ArrowError::InvalidArgumentError, format_args!(
            "The default engine currently does not support empty COALESCE statements"
        )));
    };

    // Validate against the expected output type, if provided
    if let Some(result_type) = result_type {
        let result_type = super::scalar_resource::prepare_type(result_type).map_err(|error| {
            if let crate::Error::ResourceExhausted(denied) = error {
                ArrowError::ResourceOwnerError(arrow_schema_59::resource::ResourceOwnerError {
                    kind: denied.kind,
    ...
    let array_data = expression_resource::array_data(arrays)?;

    // Build result
    let refs = expression_resource::data_refs(&array_data)?;
    let mut mutable = MutableArrayData::try_new(&refs, false, first.len())?;
    ...
    try_make_array(mutable.try_freeze()?)
}
...
fn evaluate_map_to_struct(
    ...
    let map_array = map_arr.as_any().downcast_ref::<MapArray>().ok_or_else(|| {
        expression_resource::format(format_args!("MapToStruct requires a MapArray as input"))
            .map(Error::generic)
    ...
    let map_keys = map_array
        ...
        .ok_or_else(|| {
            expression_resource::format(format_args!("MapToStruct requires maps with string keys"))
                .map(Error::generic)
                .unwrap_or_else(Error::from)
        })?;
    let map_values = map_array
        ...
        .downcast_ref::<StringArray>()
        .ok_or_else(|| {
            expression_resource::format(format_args!(
                "MapToStruct requires maps with string values"
            ))
            .map(Error::generic)
            .unwrap_or_else(Error::from)
        })?;
    ...
    let mut fields =
        expression_resource::new_vec(output_schema.num_fields(), "native_map_to_struct_fields")?;
    fields.extend(output_schema.fields());
    let arrow_schema = super::scalar_resource::prepare_schema(output_schema)?;
    let mut builders = expression_resource::new_vec::<Box<dyn ArrayBuilder>>(
        fields.len(),
        "native_map_to_struct_builders",
    )?;
    let mut target_types =
        expression_resource::new_vec(fields.len(), "native_map_to_struct_types")?;
    for (field, arrow_field) in fields.iter().zip(arrow_schema.fields()) {
        let prim = match field.data_type() {
            DataType::Primitive(p) => p,
            other => {
                return Err(Error::Generic(expression_resource::format(format_args!(
                    "MapToStruct only supports primitive target types, got {other:?}"
                ))?))
            }
        };
        target_types.push(prim);
        super::scalar_resource::prepare_constructor(arrow_field.data_type(), num_rows)?;
        builders.push(arrow_array::make_builder(arrow_field.data_type(), num_rows));
    }
    let mut field_indices =
        expression_resource::hash_map(fields.len(), "native_map_to_struct_index")?;
    for (i, field) in fields.iter().enumerate() {
        field_indices.insert(field.name().as_str(), i);
    }
    let mut string_bytes =
        expression_resource::new_vec::<usize>(fields.len(), "native_map_to_struct_offset_census")?;
    string_bytes.resize(fields.len(), 0);
    ...
    let mut matched_entry_idx =
        expression_resource::new_vec::<i32>(fields.len(), "native_map_to_struct_matches")?;
    matched_entry_idx.resize(fields.len(), -1);
        ...
        for (i, field) in fields.iter().enumerate() {
            ...
            if entry_idx >= entry_start && map_values.is_valid(entry_idx as usize) {
                ...
                if matches!(
                    ...
                    string_bytes[i] = string_bytes[i]
                        .checked_add(raw.len())
                        .filter(|size| *size <= i32::MAX as usize)
                        .ok_or(crate::resource::ResourceExhausted {
                            kind: "native_map_to_struct_string_offsets",
                            requested: usize::MAX,
                            limit: i32::MAX as usize,
                        })?;
                }
                expression_resource::partition_parse(raw)?;
                match parse_partition_scalar(target_types[i], raw)? {
                    Some(scalar) => {
                        super::scalar_resource::prepare_append(&scalar, 1)?;
                        scalar.append_to(builder, 1)?;
                    }
                    None => {
                        super::scalar_resource::prepare_append(
    ...
    let mut output_columns =
        expression_resource::new_vec(fields.len(), "native_map_to_struct_columns")?;
...
mod tests {
    ...
    use super::*;
    use crate::arrow::array::{
        new_null_array, ArrayRef, BinaryArray, BooleanArray, Int32Array, Int64Array,
