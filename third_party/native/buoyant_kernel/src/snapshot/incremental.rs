...
impl Snapshot {
    ...
    pub(super) fn try_new_from(
        ...
        let log_root = existing_log_segment.log_root.try_clone_url_admitted()?;
        ...
        new_log_segment
            ...
            .try_retain(|log_path| existing_snapshot_version < log_path.version)?;
        ...
        ascending_commit_files.try_extend(new_log_segment.listed.ascending_commit_files)?;
        let mut ascending_compaction_files = Self::files_after_checkpoint(
            &existing_log_segment.listed.ascending_compaction_files,
            new_checkpoint_version,
        )?;
        ascending_compaction_files.try_extend(new_log_segment.listed.ascending_compaction_files)?;
        ...
        // `incremental_replay`.
        let base_crc =
            combined_log_segment.pick_latest_base_crc(engine, existing_snapshot.crc())?;
    ...
    ) -> DeltaResult<crate::OwnedLogPaths> {
        crate::OwnedLogPaths::try_from_iter(files.iter()
            .filter(|file| checkpoint_version.is_none_or(|version| file.version > version))
            .cloned().map(Ok))
    }
}
