...
use crate::actions::set_transaction::{SetTransactionScanner, is_set_txn_expired};
use crate::actions::{DomainMetadata, INTERNAL_DOMAIN_PREFIX};
use crate::checkpoint::{
    CheckpointSpec, CheckpointWriter, DEFAULT_FILE_ACTIONS_PER_SIDECAR_HINT, V2CheckpointConfig,
};
use crate::clustering::{CLUSTERING_DOMAIN_NAME, parse_clustering_columns};
use crate::committer::{Committer, PublishMetadata};
use crate::crc::{
    Crc, CrcDelta, DomainMetadataState, FileStats, SetTransactionState, try_write_crc_file,
};
...
use crate::table_features::{ColumnMappingMode, TableFeature, physical_to_logical_column_name};
...
impl Snapshot {
    ...
    fn try_new_from_log_segment(
        ...
        let (metadata, protocol) = match crc_at_version.as_deref() {
            Some(c) => (
                c.metadata.try_clone_admitted(engine.resource_scope())?,
                c.protocol.try_clone_admitted(engine.resource_scope())?,
            ),
            None => {
    ...
    pub(crate) fn new_post_commit(
        ...
        let new_crc = self
            ...
            .as_deref()
            .map(|base| {
                base.try_clone_admitted(crate::resource::current_resource_scope())?
                    .try_apply_admitted(
                        crc_delta,
                        new_version,
        ...
        match try_write_crc_file(engine, &crc_path.location, crc) {
            Ok(()) => {
                ...
                let new_snapshot = Arc::new(Snapshot::new_with_crc(
                    new_log_segment,
                    self.table_configuration().try_clone_admitted()?,
                    self.crc.clone(),
                )?);
...
mod tests {
    ...
    use test_utils::table_builder::{
        FeatureSet, LogState, TestTableBuilder, VersionTarget, checkpoint_json_stats, unpartitioned,
    };
    ...
    use crate::history_manager::{HistoryCommitType, latest_version_as_of};
    ...
    use crate::path::ParsedLogPath;
    use crate::schema::{DataType, StructField, StructType, schema_ref};
    ...
    fn create_snapshot_with_commit_file_absent_from_log_segment(
        ...
        let checkpoint_parts = vec![
            ParsedLogPath::try_from(crate::FileMeta {
