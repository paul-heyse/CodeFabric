...
pub use path::resource::{OwnedFileMeta, OwnedFileMetas, OwnedLogPaths, OwnedLogUrl};
...
pub trait StorageHandler: AsAny {
    /// Optional finite geometry for native CRC/checkpoint JSON ingress.
    fn json_resource_limits(&self) -> Option<resource::JsonResourceLimits> {
        resource::current_json_resource_limits()
    }
...
pub trait Engine: AsAny {
    /// Optional explicit native allocation owner. Cloning it shares already admitted
    /// reservations; every new native allocation still needs its own admission.
    fn resource_scope(&self) -> Option<Arc<resource::NativeResourceScope>> {
        resource::current_resource_scope()
    }
