...
use crate::transaction::schema_evolution::{
    SchemaEvolutionResult, SchemaOperation, apply_schema_operations,
};
...
impl AlterTableTransactionBuilder<Modifying> {
    ...
    pub fn build(
        ...
        let mut evolved_metadata = table_config
            .metadata()
            .try_clone_admitted(_engine.resource_scope())?
            .with_schema(evolved_schema.clone())?;
        if let Some(id) = new_max_column_id {
            // u64 decimal formatting emits at most twenty bytes; String's
            // formatter grows geometrically with a minimum eight-byte capacity.
            crate::path::resource::reserve(&crate::resource::current_resource_scope(), "native_column_mapping_id_string", 64)?;
            evolved_metadata = evolved_metadata.with_configuration_entry(COLUMN_MAPPING_MAX_COLUMN_ID, &id.to_string())?;
        }
