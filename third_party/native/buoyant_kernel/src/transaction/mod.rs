...
use crate::actions::{
    CommitInfo, DomainMetadata, LOG_METADATA_SCHEMA, LOG_PROTOCOL_SCHEMA, LOG_REMOVE_SCHEMA,
    LOG_TXN_SCHEMA, MAX_VALUES, MIN_VALUES, Metadata, NULL_COUNT, NUM_RECORDS, Protocol,
    SetTransaction, TIGHT_BOUNDS, as_log_add_schema,
};
use crate::committer::{
    CommitMetadata, CommitProtocolMetadata, CommitResponse, CommitType, Committer,
};
use crate::crc::{CrcDelta, FileStatsDelta, is_incremental_safe_operation};
use crate::engine_data::FilteredEngineData;
use crate::error::Error;
use crate::expressions::UnaryExpressionOp::ToJson;
use crate::expressions::{
    ArrayData, ColumnName, ExpressionStructPatch, ExpressionStructPatchBuilder, Scalar, col, lit,
};
...
use crate::schema::{
    ArrayType, SchemaRef, SchemaStructPatchBuilder, StructField, StructType, lazy_schema_ref,
};
...
use crate::{
    DataType, DeltaResult, Engine, EngineData, Expression, IntoEngineData, RowVisitor,
    Version,
};
...
impl<S> Transaction<S> {
    ...
    pub fn commit(self, engine: &dyn Engine) -> DeltaResult<CommitResult<S>> {
        let _resource_scope = crate::resource::enter_resource_scope(
            engine.resource_scope(),
            engine.storage_handler().json_resource_limits(),
        )?;
...
impl<S> Transaction<S> {
    ...
    fn into_committed(
        self,
        file_meta: crate::OwnedFileMeta,
        crc_delta: CrcDelta,
    ) -> DeltaResult<CommittedTransaction> {
        let parsed_commit = file_meta.into_parsed()?.filter(ParsedLogPath::is_commit)
            .ok_or_else(|| Error::invalid_log_path("Committed path was not a native commit path"))?;
        ...
        let (post_commit_stats, post_commit_snapshot) = match &self.read_snapshot_opt {
            ...
            None => {
                ...
                let crc = crc_delta
                    .try_into_crc_for_version_zero_admitted()?
                    .ok_or_else(|| {
    ...
    fn build_crc_delta(
        ...
        let mut bytes = crate::crc::resource::add(
            crate::crc::resource::map_layout::<String, DomainMetadata>(dm_changes.len())?,
            crate::crc::resource::map_layout::<String, SetTransaction>(
                self.set_transactions.len(),
            )?,
        )?;
        for dm in &dm_changes {
            bytes = crate::crc::resource::add(
                bytes,
                crate::crc::resource::add(dm.domain().len(), dm.configuration().len())?,
            )?;
            bytes = crate::crc::resource::add(bytes, dm.domain().len())?;
        }
        for txn in &self.set_transactions {
            bytes =
                crate::crc::resource::add(bytes, crate::crc::resource::mul(txn.app_id.len(), 2)?)?;
        }
        bytes = crate::crc::resource::add(
            bytes,
            crate::crc::resource::metadata_bytes(self.effective_table_config.metadata())?,
        )?;
        bytes = crate::crc::resource::add(
            bytes,
            crate::crc::resource::protocol_bytes(self.effective_table_config.protocol())?,
        )?;
        if let Some(histogram) = &file_stats.net_histogram {
            bytes = crate::crc::resource::add(bytes, crate::crc::resource::histogram_bytes(histogram)?)?;
        }
        bytes = crate::crc::resource::add(bytes, std::mem::size_of::<CrcDelta>())?;
        crate::crc::resource::reserve(&scope, "crc_transaction_delta", bytes)?;
        ...
        Ok(CrcDelta {
            ...
            protocol: self
                ...
                .then(|| self.effective_table_config.protocol().try_clone_admitted(scope.clone()))
                .transpose()?,
            metadata: self
                .should_emit_metadata
                .then(|| self.effective_table_config.metadata().try_clone_admitted(scope.clone()))
                .transpose()?,
            domain_metadata,
            set_transactions,
            in_commit_timestamp,
            is_incremental_safe,
            resource_owner: crate::crc::resource::CrcResourceOwner(scope),
        })
    }
...
mod tests {
    ...
    use crate::schema::{MapType, schema_ref};
    use crate::table_features::ColumnMappingMode;
    use crate::transaction::create_table::create_table;
    use crate::transaction::data_layout::DataLayout;
    use crate::utils::test_utils::{
        CapturingReporter, install_thread_local_metrics_reporter, load_test_table,
