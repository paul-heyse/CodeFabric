...
pub(crate) use crate::expressions::{ColumnName, column_name};
use crate::reserved_field_ids::FILE_NAME;
use crate::table_features::{
    ColumnMappingMode, validate_and_extract_column_mapping_annotations, validate_column_mapping_id,
};
...
pub(crate) mod visitor_selection;
pub use visitor_selection::OwnedVisitorSelection;
...
pub(crate) trait ToSchema {
    ...
    fn resource_schema_bytes() -> DeltaResult<usize> {
        Err(crate::resource::ResourceExhausted{kind:"native_derived_schema_layout",requested:1,limit:0}.into())
    }
    fn try_to_schema() -> DeltaResult<StructType> {
        let bytes=Self::resource_schema_bytes()?;
        visitor_selection::admit(bytes,"native_derived_schema")?;
        let schema=Self::to_schema();
        match crate::resource::current_resource_scope(){
            Some(scope)=>resource::attach_derived_schema_owner(schema,scope,bytes),
            None=>Ok(schema),
        }
    }
}
...
impl StructField {
    ...
    fn apply_physical_metadata(&mut self, column_mapping_mode: ColumnMappingMode) {
        let base_metadata = &mut self.metadata;
        ...
        match column_mapping_mode {
            ColumnMappingMode::Id => {
                let Some(MetadataValue::Number(fid)) = field_id else {
                    ...
                    // this has a field Id
                    warn!(
                        "StructField with name {} is missing field id in the Id column mapping mode",
                        self.name()
                    );
                    debug_assert!(false);
                    return;
            ...
            ColumnMappingMode::Name => {
                ...
                let Some(MetadataValue::Number(fid)) = field_id else {
                    warn!(
                        "StructField with name {} is missing field id in the Name column mapping mode",
                        self.name()
                    );
                    debug_assert!(false);
                    return;
...
pub struct StructType {
    ...
    resource_owner: resource::SchemaResourceOwner,
}
...
impl StructType {
    ...
    pub fn try_new(fields: impl IntoIterator<Item = StructField>) -> DeltaResult<Self> {
        ...
        Ok(Self {
            ...
            fields: field_map,
            metadata_columns,
            resource_owner: resource::SchemaResourceOwner::default(),
        })
    }
    ...
    pub(crate) fn make_physical(
        ...
        let result = transformer.transform_struct(self).map(|s| s.into_owned());
        match admission {
...
impl ColumnNamesAndTypes {
    pub(crate) fn into_parts(self)->(Vec<ColumnName>,Vec<DataType>){(self.0,self.1)}
    #[internal_api]
...
impl Serialize for StructType {
        ...
        S: serde::Serializer,
    {
        ...
        struct BorrowedFields<'a>(&'a IndexMap<String, StructField>);
        ...
        // Serialize the original native fields. The previous temporary helper
        // deep-cloned the entire recursive schema before writing any bytes.
        let mut state = serializer.serialize_struct("StructTypeSerDeHelper", 2)?;
        state.serialize_field("type", &self.type_name)?;
        state.serialize_field("fields", &BorrowedFields(&self.fields))?;
        state.end()
    }
}
...
impl<'a> SchemaTransform<'a> for MakePhysical<'a> {
    ...
    fn transform_struct_field(
        ...
        self.transform_inner(field.name(), |this| {
            ...
            // Move an already transformed child graph into its parent. Calling
            // with_name here deep-cloned that graph and its metadata again, then
            // immediately discarded the old metadata through with_metadata.
            let mut field = field.into_owned();
            field.name = physical_name.to_owned();
            field.apply_physical_metadata(this.column_mapping_mode);
            Ok(Cow::Owned(field))
        })
    }
