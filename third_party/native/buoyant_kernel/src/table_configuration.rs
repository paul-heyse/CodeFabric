...
use crate::scan::data_skipping::stats_schema::{
    StatsConfig, StripFieldMetadataTransform, expected_stats_schema, stats_column_names,
};
...
use crate::schema::{SchemaRef, StructField, StructType, schema_has_invariants};
#[cfg(feature = "nanosecond-timestamps")]
use crate::table_features::validate_timestamp_nanos_feature_support;
use crate::table_features::{
    ColumnMappingMode, EnablementCheck, FeatureRequirement, FeatureType, KernelSupport,
    LEGACY_WRITER_FEATURES, MAX_VALID_WRITER_VERSION, MIN_VALID_RW_VERSION, Operation,
    TABLE_FEATURES_MIN_READER_VERSION, TABLE_FEATURES_MIN_WRITER_VERSION, TableFeature,
    V3_VALIDATOR, check_reader_version_range, column_mapping_mode, extract_enabled_reader_features,
...
#[path = "table_configuration_resource.rs"]
mod resource;
...
pub(crate) struct TableConfiguration {
    ...
    table_root: Url,
    version: Version,
    resource_owner: resource::ConfigurationResourceOwner,
}
...
impl TableConfiguration {
    ...
    pub(crate) fn try_new_with_schema(
        ...
        Self::try_new_inner(
            metadata,
            base.protocol
                .try_clone_admitted(crate::resource::current_resource_scope())?,
            base.clone_table_root_admitted()?,
            base.version,
            logical_schema,
        )
    }
    ...
    fn try_new_inner(
        ...
        let admission = resource::ConfigurationAdmission::begin(&metadata, &logical_schema)?;
        let result =
            Self::build_configuration(metadata, protocol, table_root, version, logical_schema);
        match admission {
    ...
    fn build_configuration(
        ...
        let physical_data_schema_without_partition_columns = {
            // Filter by corresponding logical names: physical names may differ
            // under column mapping. Admission and original owner attach through
            // the native fallible schema filter before its deep copies occur.
            let mut logical_fields = logical_schema.fields();
            Arc::new(physical_schema.with_fields_filtered(|_| {
                logical_fields.next().is_some_and(|logical_field| {
                    !partition_columns.contains(logical_field.name().as_str())
                })
            })?)
        };
        let logical_schema_without_partition_columns =
            Arc::new(logical_schema.with_fields_filtered(|field| {
        ...
        let table_config = Self {
            ...
            table_root,
            version,
            resource_owner: resource::ConfigurationResourceOwner::default(),
        };
    ...
    pub(crate) fn try_new_from(
        ...
        if new_metadata.is_none() && new_protocol.is_none() {
            return Ok(Self {
                version: new_version,
                ..table_configuration.try_clone_admitted()?
            });
        }
        ...
        Self::try_new(
            ...
            table_configuration.clone_table_root_admitted()?,
            new_version,
        )
    }
...
mod test {
    ...
    use crate::Error;
    use crate::actions::{MIN_VALUES, Metadata, Protocol};
    use crate::schema::{ColumnName, DataType, SchemaRef, StructField, StructType, schema_ref};
    use crate::table_features::{
        ColumnMappingMode, FeatureType, Operation, TABLE_FEATURES_MIN_READER_VERSION,
        TABLE_FEATURES_MIN_WRITER_VERSION, TableFeature,
    };
    use crate::table_properties::{
        COLUMN_MAPPING_MODE, ENABLE_ICEBERG_COMPAT_V1, ENABLE_ICEBERG_COMPAT_V2,
        ENABLE_ICEBERG_COMPAT_V3, ENABLE_IN_COMMIT_TIMESTAMPS, ENABLE_ROW_TRACKING,
        ROW_TRACKING_SUSPENDED, TableProperties,
    };
    ...
    fn reject_protocol_version_below_minimum(
        ...
        let expected = if rv < 1 {
            format!(
                "Invalid protocol action in the delta log: min_reader_version must be >= 1, got {rv}"
            )
        } else {
            format!(
                "Invalid protocol action in the delta log: min_writer_version must be >= 1, got {wv}"
            )
        };
    ...
    fn test_timestamp_nanos_validation_integration(#[case] ts_type: DataType) {
        ...
        assert_result_error_with_message(
            result,
            "Unsupported: Table contains TIMESTAMP_NANOS or TIMESTAMP_NANOS_NTZ columns but does not have the required 'timestampNanos' and 'timestampNtz' features in reader and writer features",
        );
    ...
    fn test_timestamp_ntz_validation_integration() {
        ...
        assert_result_error_with_message(
            result,
            "Unsupported: Table contains TIMESTAMP_NTZ columns but does not have the required 'timestampNtz' feature in reader and writer features",
        );
    ...
    fn test_variant_validation_integration() {
        ...
        assert_result_error_with_message(
            result,
            "Unsupported: Table contains VARIANT columns but does not have the required 'variantType' feature in reader and writer features",
        );
