...
impl StorageHandler for MeteredStorageHandler {
    fn json_resource_limits(&self) -> Option<crate::resource::JsonResourceLimits> {
        self.inner.json_resource_limits()
    }

    fn list_from(
        &self,
        path: &Url,
    ) -> DeltaResult<Box<dyn Iterator<Item = DeltaResult<crate::OwnedFileMeta>>>> {
        let start = Instant::now();
        let inner = self.inner.list_from(path)?;
        Ok(Box::new(MetricsIterator::<_, crate::OwnedFileMeta>::new(
            inner,
...
mod tests {
    ...
    impl StorageHandler for StubStorageHandler {
        fn list_from(
            ...
            let results: Vec<_> = self.list_results.iter().cloned().map(crate::OwnedFileMeta::unadmitted).map(Ok).collect();
