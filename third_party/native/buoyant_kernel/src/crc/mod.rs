...
#[allow(unused)]
pub(crate) use file_stats::{FileStatsDelta, is_incremental_safe_operation, size_to_u64};
...
pub struct Crc {
    ...
    /// Last field: owned state drops before its original scope receipt. Legacy Clone clears it.
    pub(crate) resource_owner: resource::CrcResourceOwner,
}

impl Crc {
    pub(crate) fn attach_admitted_owner(
        &mut self,
        scope: Option<std::sync::Arc<crate::resource::NativeResourceScope>>,
    ) {
        self.metadata.attach_admitted_owner(scope.clone());
        self.protocol.attach_admitted_owner(scope.clone());
        self.resource_owner = resource::CrcResourceOwner(scope);
    }
...
impl Crc {
    ...
    pub(crate) fn try_from_json_bytes(bytes: &[u8], version: Version) -> DeltaResult<Self> {
        ...
        let raw: CrcRaw = serde_json::from_slice(bytes).map_err(|error| {
            if scope.is_some() {
...
impl Crc {
    /// Borrow this original CRC's admitted native owner, if any. Sharing the Arc
    /// does not authorize a deep clone or an independently moved-out field.
    pub fn resource_scope(&self) -> Option<&std::sync::Arc<crate::resource::NativeResourceScope>> {
        self.resource_owner.0.as_ref()
    }

    /// Parse the original native CRC after finite JSON and allocation admission.
    /// The receipt stays on this CRC; this does not charge separately moved fields.
    #[delta_kernel_derive::internal_api]
    pub(crate) fn try_from_json_bytes_admitted(
        bytes: &[u8],
        version: Version,
        scope: Option<std::sync::Arc<crate::resource::NativeResourceScope>>,
        limits: Option<crate::resource::JsonResourceLimits>,
    ) -> DeltaResult<Self> {
        // Resolve required worker policy before inspecting or reserving. None
        // inherits a live native phase; an explicit foreign scope is rejected by
        // enter_resource_scope before any charge can go to the wrong owner.
        let _guard = crate::resource::enter_resource_scope(
            scope.or_else(crate::resource::current_resource_scope),
            limits.or_else(crate::resource::current_json_resource_limits),
        )?;
        let scope = crate::resource::current_resource_scope();
        let limits = crate::resource::current_json_resource_limits();
        if let Some(limits) = limits {
            let shape = limits.inspect(bytes)?;
            resource::reserve(
                &scope,
                "crc_decode_and_conversion",
                resource::decode_bytes(shape)?,
            )?;
        } else if scope.is_some() {
            return Err(crate::resource::ResourceExhausted {
                kind: "crc_json_policy",
                requested: 1,
                limit: 0,
            }
            .into());
        ...
        (|| {
            let mut crc = Self::try_from_json_bytes(bytes, version)?;
            crc.attach_admitted_owner(scope.clone());
            Ok(crc)
        })()
        .map_err(|error| resource::retain_error(error, &scope))
    }

    /// Pre-admit a complete deep copy before cloning the original native fields.
    pub fn try_clone_admitted(
        &self,
        scope: Option<std::sync::Arc<crate::resource::NativeResourceScope>>,
    ) -> DeltaResult<Self> {
        let scope = resource::select_scope(scope, self.resource_owner.0.clone())?;
        resource::reserve(&scope, "crc_deep_clone", resource::clone_bytes(self)?)?;
