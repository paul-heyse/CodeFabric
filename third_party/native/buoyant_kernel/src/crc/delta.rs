...
pub(crate) struct CrcDelta {
    ...
    pub(crate) resource_owner: super::resource::CrcResourceOwner,
}
...
mod tests {
    ...
    use crate::crc::{FileSizeHistogram, SetTransactionState, is_incremental_safe_operation};
...
impl Crc {
    pub(crate) fn try_apply_admitted(
        self,
        delta: CrcDelta,
        new_version: Version,
        scope: Option<std::sync::Arc<crate::resource::NativeResourceScope>>,
    ) -> crate::DeltaResult<Self> {
        let scope = super::resource::select_scope(
            scope,
            self.resource_owner
                .0
                .clone()
                .or_else(|| delta.resource_owner.0.clone()),
        )?;
        super::resource::reserve(
            &scope,
            "crc_apply",
            super::resource::apply_bytes(&self, &delta)?,
        )?;
        let mut result = self.apply(delta, new_version);
        result.attach_admitted_owner(scope);
        Ok(result)
    }
}
impl CrcDelta {
    pub(crate) fn try_into_crc_for_version_zero_admitted(self) -> crate::DeltaResult<Option<Crc>> {
        let scope =
            crate::resource::current_resource_scope().or_else(|| self.resource_owner.0.clone());
        let bytes = super::resource::add(
            super::resource::add(
                super::resource::delta_bytes(&self)?,
                super::resource::map_layout::<String, DomainMetadata>(
                    self.domain_metadata.capacity(),
                )?,
            )?,
            std::mem::size_of::<Crc>()
                + 2 * std::mem::size_of::<usize>()
                + super::resource::diagnostic_bytes(0)?,
        )?;
        super::resource::reserve(&scope, "crc_version_zero", bytes)?;
        Ok(self.into_crc_for_version_zero().map(|mut crc| {
            crc.attach_admitted_owner(scope);
            crc
        }))
    }
}
