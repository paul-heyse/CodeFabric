...
pub(crate) fn try_write_crc_file(engine: &dyn Engine, path: &Url, crc: &Crc) -> DeltaResult<()> {
    ...
    try_write_crc_file_inner(engine, path, crc, &scope)
        .map_err(|error| super::resource::retain_error(error, &scope))
}

fn try_write_crc_file_inner(
    engine: &dyn Engine,
    path: &Url,
    crc: &Crc,
    scope: &Option<Arc<crate::resource::NativeResourceScope>>,
) -> DeltaResult<()> {
    // Crc::serialize constructs a temporary CrcRaw. Reserve its complete native
    // clone layout before validation/serialization, including bounded Debug
    // diagnostics for a degraded histogram-bearing FileStatsState.
    let clone_bytes = super::resource::clone_bytes(crc)?;
    super::resource::reserve(
        scope,
        "crc_serialize_preflight",
        super::resource::add(clone_bytes, super::resource::diagnostic_bytes(clone_bytes)?)?,
    )?;
    require!(
    ...
    let data = if let Some(scope) = scope {
        // Count the exact output with the unchanged native serializer. The sink
        // allocates nothing; map iteration order changes neither encoded length
        // nor semantics. A second admitted CrcRaw conversion writes into an
        // exact-capacity Vec, with a hard limit preventing accidental growth.
        let mut counter = JsonByteCounter(0);
        serde_json::to_writer(&mut counter, crc)?;
        scope.reserve(crate::resource::AllocationRequest {
            kind: "crc_serialize_output",
            bytes: super::resource::add(
                super::resource::add(clone_bytes, counter.0)?,
                std::mem::size_of::<OwnedCrcBytes>() + 4 * std::mem::size_of::<usize>(),
            )?,
        })?;
        let mut data = ExactJsonOutput {
            bytes: Vec::with_capacity(counter.0),
            limit: counter.0,
        };
        serde_json::to_writer(&mut data, crc)?;
        Bytes::from_owner(OwnedCrcBytes {
            bytes: data.bytes,
            _scope: scope.clone(),
        })
    } else {
        serde_json::to_vec(crc)?.into()
    };
    engine
        .storage_handler()
        .put(path, data, false /* overwrite */)
}
...
struct ExactJsonOutput {
    bytes: Vec<u8>,
    limit: usize,
}
impl Write for ExactJsonOutput {
    fn write(&mut self, bytes: &[u8]) -> io::Result<usize> {
...
struct OwnedCrcBytes {
    bytes: Vec<u8>,
    _scope: Arc<crate::resource::NativeResourceScope>,
}
impl AsRef<[u8]> for OwnedCrcBytes {
    fn as_ref(&self) -> &[u8] {
        &self.bytes
    }
}
