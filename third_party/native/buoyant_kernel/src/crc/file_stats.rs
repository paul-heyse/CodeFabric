...
impl FileStatsDelta {
    ...
    pub(crate) fn try_compute_for_txn(
        ...
        let bins =
            bin_boundaries.map_or_else(FileSizeHistogram::default_boundary_count, <[i64]>::len);
        super::resource::reserve(
            &scope,
            "crc_transaction_histogram",
            super::resource::add(
                super::resource::mul(bins, 3 * std::mem::size_of::<i64>())?,
...
mod tests {
    use rstest::rstest;
    use test_utils::{IntoArray, generate_batch};
