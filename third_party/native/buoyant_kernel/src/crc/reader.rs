...
pub(crate) fn try_read_crc_file(engine: &dyn Engine, crc_path: &ParsedLogPath) -> DeltaResult<Crc> {
    ...
    Crc::try_from_json_bytes_admitted(
        &data,
        crc_path.version,
        engine.resource_scope(),
        storage.json_resource_limits(),
    )
}
...
pub(crate) fn read_crc_file_or_none(
    ...
    crate::resource::optional_native(try_read_crc_file(engine, crc_file))
...
mod tests {
    ...
    fn test_read_crc_file() {
        ...
        assert!(
            dms["delta.clustering"]
                .configuration()
                .contains("clusteringColumns")
        );
        assert!(
            dms["delta.rowTracking"]
                .configuration()
