...
use super::*;
use crate::engine_data::{GetData, RowVisitor, TypedGetData as _, OwnedVisitorSelection};
...
impl RowVisitor for MetadataVisitor {
    fn try_selection(&self)->DeltaResult<OwnedVisitorSelection>{
        OwnedVisitorSelection::try_from_schema(&Metadata::try_to_schema()?,Some(METADATA_NAME))
    }
...
impl RowVisitor for SelectionVectorVisitor {
    fn try_selection(&self)->DeltaResult<OwnedVisitorSelection>{OwnedVisitorSelection::try_from_paths(&[&["output"]],&[DataType::BOOLEAN])}
...
impl RowVisitor for ProtocolVisitor {
    fn try_selection(&self)->DeltaResult<OwnedVisitorSelection>{
        OwnedVisitorSelection::try_from_schema(&Protocol::try_to_schema()?,Some(PROTOCOL_NAME))
    }
...
impl RowVisitor for AddVisitor {
    fn try_selection(&self)->DeltaResult<OwnedVisitorSelection>{
        OwnedVisitorSelection::try_from_schema(&Add::try_to_schema()?,Some(ADD_NAME))
    }
...
impl RowVisitor for RemoveVisitor {
    fn try_selection(&self)->DeltaResult<OwnedVisitorSelection>{
        OwnedVisitorSelection::try_from_schema(&Remove::try_to_schema()?,Some(REMOVE_NAME))
    }
...
impl RowVisitor for CdcVisitor {
    fn try_selection(&self)->DeltaResult<OwnedVisitorSelection>{
        OwnedVisitorSelection::try_from_schema(&Cdc::try_to_schema()?,Some(CDC_NAME))
    }
...
impl RowVisitor for SetTransactionVisitor {
    fn try_selection(&self)->DeltaResult<OwnedVisitorSelection>{
        OwnedVisitorSelection::try_from_schema(&SetTransaction::try_to_schema()?,Some(SET_TRANSACTION_NAME))
    }
...
pub(crate) struct SidecarVisitor {
    ...
    resource_scope: Option<Arc<crate::resource::NativeResourceScope>>,
}
...
impl SidecarVisitor {
    ...
    fn visit_sidecar<'a>(
        ...
        Ok(Sidecar {
            ...
            resource_owner: Default::default(),
        })
    }
}
...
impl RowVisitor for SidecarVisitor {
    fn try_selection(&self)->DeltaResult<OwnedVisitorSelection>{
        OwnedVisitorSelection::try_from_schema(&Sidecar::try_to_schema()?,Some(SIDECAR_NAME))
    }
    ...
    fn visit<'a>(&mut self, row_count: usize, getters: &[&'a dyn GetData<'a>]) -> DeltaResult<()> {
        ...
        let admission = super::resource::ActionAdmission::try_new()?;
        self.resource_scope = admission.scope();
        let result = (|| {
            for i in 0..row_count {
                // Borrow the original path and map before native getter copies.
                if getters[0].get_str(i, "sidecar.path")?.is_some() {
                    admission.sidecar(i, getters)?;
                    if self.sidecars.len() == self.sidecars.capacity() {
                        let capacity = crate::crc::resource::mul(self.sidecars.capacity(), 2)?.max(4);
                        if let Some(scope) = &self.resource_scope {
                            scope.reserve(crate::resource::AllocationRequest {
                                kind: "native_sidecar_descriptors",
                                bytes: crate::crc::resource::mul(capacity, std::mem::size_of::<Sidecar>())?,
                            })?;
                        }
                        self.sidecars.reserve_exact(capacity - self.sidecars.len());
                    }
                    let path = getters[0].get(i, "sidecar.path")?;
                    let mut sidecar = Self::visit_sidecar(i, path, getters)?;
                    sidecar.attach_admitted_owner(admission.scope());
...
impl RowVisitor for DomainMetadataVisitor {
    fn try_selection(&self)->DeltaResult<OwnedVisitorSelection>{
        OwnedVisitorSelection::try_from_schema(&DomainMetadata::try_to_schema()?,Some(DOMAIN_METADATA_NAME))
    }
...
pub(crate) fn visit_metadata_at<'a>(
    ...
    let admission = super::resource::ActionAdmission::try_new()?;
    let result = visit_metadata_at_inner(row_index, getters, &admission);
    admission.finish(result)
}

fn visit_metadata_at_inner<'a>(
    row_index: usize,
    getters: &[&'a dyn GetData<'a>],
    admission: &super::resource::ActionAdmission,
) -> DeltaResult<Option<Metadata>> {
    require!(
        getters.len() == 9,
        Error::InternalError(format!(
    ...
    admission.metadata(row_index, getters)?;
    ...
    metadata.attach_admitted_owner(admission.scope());
    Ok(Some(metadata))
}
...
pub(crate) fn visit_protocol_at<'a>(
    ...
    let admission = super::resource::ActionAdmission::try_new()?;
    let result = visit_protocol_at_inner(row_index, getters, &admission);
    admission.finish(result)
}

fn visit_protocol_at_inner<'a>(
    row_index: usize,
    getters: &[&'a dyn GetData<'a>],
    admission: &super::resource::ActionAdmission,
) -> DeltaResult<Option<Protocol>> {
    ...
    admission.protocol(row_index, getters)?;
    ...
    let mut protocol = Protocol::try_new(
        min_reader_version,
        min_writer_version,
        reader_features,
        writer_features,
    )?;
    protocol.attach_admitted_owner(admission.scope());
    Ok(Some(protocol))
}
...
impl RowVisitor for InCommitTimestampVisitor {
    fn try_selection(&self)->DeltaResult<OwnedVisitorSelection>{OwnedVisitorSelection::try_from_paths(&[&["commitInfo","inCommitTimestamp"]],&[DataType::LONG])}
