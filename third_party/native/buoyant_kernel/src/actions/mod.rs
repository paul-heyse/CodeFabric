...
pub(crate) struct Metadata {
    ...
    #[serde(skip)]
    #[kernel_resource_owner]
    resource_owner: resource::ActionResourceOwner,
}
...
impl Metadata {
    ...
    /// Original format options, borrowed for native allocation admission.
    ...
    pub(crate) fn parse_schema(&self) -> DeltaResult<StructType> {
        match (
            ...
            crate::resource::current_json_resource_limits(),
        ) {
            (Some(scope), Some(limits)) => {
                StructType::try_from_json_with_resources(&self.schema_string, scope, limits)
            }
            (Some(_), None) => Err(crate::resource::ResourceExhausted {
                kind: "schema_json_policy",
                requested: 1,
                limit: 0,
            }.into()),
            (None, limits) => {
                if let Some(limits) = limits {
                    limits.inspect(self.schema_string.as_bytes())?;
                }
    ...
    pub(crate) fn with_schema(self, schema: SchemaRef) -> DeltaResult<Self> {
        if let Some(scope) = crate::resource::current_resource_scope() {
            // The other metadata fields become newly admitted copies too; an
            // updated schema string must not replace their original owner.
            let mut metadata = self.try_clone_admitted(Some(scope.clone()))?;
            metadata.schema_string = schema.to_json_admitted(&scope)?;
            Ok(metadata)
        } else {
            self.resource_owner.validate_current_scope()?;
            Ok(Self {
                schema_string: serde_json::to_string(&schema)?,
                ..self
            })
    ...
    pub(crate) fn with_configuration_entry(
        ...
        let scope = crate::crc::resource::select_scope(None, self.resource_scope().cloned())?;
        if let Some(scope) = &scope {
            // Copy original backing into the current allocation policy before
            // modifying an action retained by an earlier operation.
            if self.resource_scope().is_none_or(|original| !std::sync::Arc::ptr_eq(original, scope)) {
                self = self.try_clone_admitted(Some(scope.clone()))?;
            }
            let mut bytes = crate::crc::resource::add(key.len(), value.len())?;
            if !self.configuration.contains_key(key) && self.configuration.len() == self.configuration.capacity() {
                let capacity = crate::crc::resource::add(crate::crc::resource::mul(self.configuration.capacity(), 2)?, 1)?;
                bytes = crate::crc::resource::add(bytes, crate::crc::resource::map_layout::<String, String>(capacity)?)?;
            }
            scope.reserve(crate::resource::AllocationRequest { kind: "native_metadata_configuration_entry", bytes })?;
        }
        self.configuration.insert(key.to_owned(), value.to_owned());
        self.attach_admitted_owner(scope);
        Ok(self)
    }
...
impl IntoEngineData for Metadata {
    fn into_engine_data(
        ...
        self.resource_owner.validate_current_scope()?;
        let _native_conversion = self.admit_engine_data_conversion()?;
...
pub(crate) struct Sidecar {
    ...
    #[serde(skip)]
    #[kernel_resource_owner]
    pub(crate) resource_owner: resource::ActionResourceOwner,
}
...
impl Sidecar {
    ...
    pub(crate) fn to_filemeta(&self, log_root: &Url) -> DeltaResult<crate::OwnedFileMeta> {
        if let Some(scope) = crate::resource::current_resource_scope() {
            scope.reserve(crate::resource::AllocationRequest { kind: "native_sidecar_size_diagnostic", bytes: 128 })?;
        }
        let size = self.size_in_bytes.try_into().map_err(|_| Error::generic(format!(
            "Failed to convert sidecar size {} to usize", self.size_in_bytes,
        )))?;
        let directory = crate::path::resource::try_join_url(log_root, "_sidecars/")?;
        crate::OwnedFileMeta::try_from_reference(&directory, &self.path, self.modification_time, size)
    }
}
