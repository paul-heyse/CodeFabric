...
pub(crate) struct CheckpointManifestReader {
    ...
    log_root: crate::OwnedLogUrl,
    is_complete: bool,
    manifest_file: crate::OwnedFileMeta,
}
...
impl CheckpointManifestReader {
    ...
    pub(crate) fn try_new(
        ...
        Ok(Self {
            ...
            is_complete: false,
            manifest_file: crate::OwnedFileMeta::try_copy_url(&manifest.location.location, manifest.location.last_modified, manifest.location.size)?,
        })
    }
    ...
    #[allow(unused)]
    pub(crate) fn extract_sidecars(self) -> DeltaResult<crate::OwnedFileMetas> {
        require!(
            self.is_complete,
        ...
        let mut sidecars = crate::OwnedFileMetas::default();
        for sidecar in &self.sidecar_visitor.sidecars {
            sidecars.try_push_owned(sidecar.to_filemeta(&self.log_root)?)?;
        }
        Ok(sidecars)
    }
}
