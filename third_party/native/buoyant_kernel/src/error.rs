...
pub enum Error {
    /// Native admission failed; optional metadata readers must not swallow it.
    #[error(transparent)]
    ResourceExhausted(crate::resource::ResourceExhausted),
...
impl Error {
    /// Whether this is a native resource rejection, including backtrace wrappers.
    pub fn is_resource_exhausted(&self) -> bool {
        match self {
            Self::ResourceExhausted(_) => true,
            Self::Backtraced { source, .. } => source.is_resource_exhausted(),
            _ => false,
        }
    }
    ...
    pub fn with_backtrace(self) -> Self {
        // Stack unwinding/symbol capture has no native decode-layout bound.
        // A governed operation keeps its bounded error instead; diagnostics may
        // add independently admitted backtraces outside this decoder contract.
        if crate::resource::current_resource_scope().is_some() {
            return self;
        }
...
impl From<crate::resource::ResourceExhausted> for Error {
    fn from(error: crate::resource::ResourceExhausted) -> Self {
...
impl From<ArrowError> for Error {
    fn from(value: ArrowError) -> Self {
        ...
        {
            // Native readers may wrap Arrow ExternalError inside a Parquet error
            // and back into Arrow. Preserve the static typed pressure through
            // that source chain; never classify a failure by its display text.
            let mut source: &(dyn std::error::Error + 'static) = &value;
            loop {
                if let Some(ArrowError::ResourceOwnerError(pressure)) =
                    source.downcast_ref::<ArrowError>()
                {
                    return crate::resource::ResourceExhausted {
                        kind: pressure.kind,
                ...
                if let Some(pressure) = source
                    .downcast_ref::<crate::arrow::json::resource::ResourceExhausted>()
                {
                    return crate::resource::ResourceExhausted {
                        kind: pressure.kind,
                        requested: pressure.requested,
                ...
                if let Some(pressure) =
                    source.downcast_ref::<crate::parquet::resource::ResourceExhausted>()
                {
                    return crate::resource::ResourceExhausted {
                        kind: pressure.kind,
                        requested: pressure.requested,
