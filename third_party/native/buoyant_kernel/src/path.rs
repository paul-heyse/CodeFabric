...
#[internal_api]
pub(crate) struct ParsedLogPath<Location: AsUrl = FileMeta>(Arc<ParsedLogPathFields<Location>>);

#[derive(Debug, PartialEq, Eq)]
#[internal_api]
pub(crate) struct ParsedLogPathFields<Location: AsUrl = FileMeta> {
    ...
    owner: ParsedPathOwner,
}

#[derive(Debug)]
struct ParsedPathOwner(Option<Arc<crate::resource::NativeResourceScope>>);
impl PartialEq for ParsedPathOwner { fn eq(&self, _: &Self) -> bool { true } }
impl Eq for ParsedPathOwner {}
impl<Location: AsUrl + std::fmt::Debug> std::fmt::Debug for ParsedLogPath<Location> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ParsedLogPath").field("location", &self.location)
            .field("filename", &self.filename).field("extension", &self.extension)
            .field("version", &self.version).field("file_type", &self.file_type).finish()
    }
}
impl<Location: AsUrl> Deref for ParsedLogPath<Location> {
    type Target = ParsedLogPathFields<Location>;
...
impl<Location: AsUrl> ParsedLogPath<Location> {
    #[cfg(test)]
    pub(crate) fn from_test_parts(location: Location, filename: String, extension: String, version: Version, file_type: LogPathFileType) -> Self {
        Self(Arc::new(ParsedLogPathFields { location, filename, extension, version, file_type, owner: ParsedPathOwner(None) }))
    }
    ...
    pub(crate) fn try_from(location: Location) -> DeltaResult<Option<ParsedLogPath<Location>>> {
        resource::validate(None)?;
        Self::try_from_owned(location, None)
    }

    pub(crate) fn validate_owner(&self) -> DeltaResult<()> { resource::validate(self.0.owner.0.as_ref()) }
    pub fn resource_scope(&self) -> Option<&Arc<crate::resource::NativeResourceScope>> { self.0.owner.0.as_ref() }

    pub(crate) fn try_from_owned(location: Location, owner: Option<Arc<crate::resource::NativeResourceScope>>) -> DeltaResult<Option<Self>> {
        resource::validate(owner.as_ref())?;
        let url = location.as_url();
        let length = url.path().len();
        let bytes = crate::crc::resource::add(
            crate::crc::resource::mul(crate::crc::resource::add(length, 8)?, 4 * std::mem::size_of::<&str>() + 4)?,
            std::mem::size_of::<ParsedLogPathFields<Location>>() + 2 * std::mem::size_of::<usize>(),
        )?;
        resource::reserve(&owner, "native_parsed_log_path", bytes)?;
        ...
        Ok(Some(Self(Arc::new(ParsedLogPathFields {
            location, filename, extension, version, file_type,
            owner: ParsedPathOwner(owner),
        }))))
    }
...
impl ParsedLogPath<Url> {
    /// A new native URL copy requires separate allocation admission. The
    /// synchronous caller retains its operation scope until consuming this URL.
    pub(crate) fn try_clone_url_admitted(&self) -> DeltaResult<Url> {
        self.validate_owner()?;
        resource::reserve(&self.0.owner.0, "native_log_url_copy", self.location.as_str().len())?;
        Ok(self.location.clone())
    }
    pub(crate) fn try_with_file_metadata(&self, last_modified: i64, size: u64) -> DeltaResult<ParsedLogPath<FileMeta>> {
        let owner = self.0.owner.0.clone();
        self.validate_owner()?;
        let location = self.try_clone_url_admitted()?;
        ParsedLogPath::try_from_owned(FileMeta { location, last_modified, size }, owner)?.ok_or_else(|| Error::internal_error("Original parsed path failed native parse"))
    }
    ...
    fn create_path(table_root: &Url, filename: String) -> DeltaResult<Self> {
        let owner = resource::scope();
        let bytes = crate::crc::resource::mul(crate::crc::resource::add(table_root.as_str().len(), filename.len() + DELTA_LOG_DIR_WITH_SLASH.len() + 8)?, 8)?;
        resource::reserve(&owner, "native_log_path_join", bytes)?;
        let location = resource::try_join_url(&resource::try_join_url(table_root, DELTA_LOG_DIR_WITH_SLASH)?, &filename)?;
        Self::try_from_owned(location, owner)?.ok_or_else(|| {
    ...
    pub(crate) fn new_commit(table_root: &Url, version: Version) -> DeltaResult<Self> {
        resource::reserve(&resource::scope(), "native_log_filename", 256)?;
...
pub(crate) struct LogRoot {
    table_root: resource::OwnedLogUrl,
    log_root: resource::OwnedLogUrl,
}
...
impl LogRoot {
    ...
    pub(crate) fn new(mut table_root: Url) -> DeltaResult<Self> {
        resource::reserve(&resource::scope(), "native_log_root_normalization", crate::crc::resource::mul(crate::crc::resource::add(table_root.as_str().len(), 8)?, 4)?)?;
        if !table_root.path().ends_with('/') {
            let new_path = format!("{}/", table_root.path());
            table_root.set_path(&new_path);
        }
        let log_root = resource::try_join_url(&table_root, DELTA_LOG_DIR_WITH_SLASH)?;
        Ok(Self {
            table_root: resource::OwnedLogUrl::try_copy(&table_root)?,
            log_root: resource::OwnedLogUrl::try_copy(&log_root)?,
        })
    }
    ...
    pub(crate) fn new_commit_path(&self, version: Version) -> DeltaResult<ParsedLogPath<Url>> {
        ...
        let filename = format!("{version:020}.json");
        let owner = resource::scope();
        resource::reserve(&owner, "native_log_path_join", crate::crc::resource::mul(crate::crc::resource::add(self.log_root().as_str().len(), filename.len() + 8)?, 4)?)?;
        let path = resource::try_join_url(self.log_root(), &filename)?;
        ParsedLogPath::try_from_owned(path, owner)?.ok_or_else(|| {
            Error::internal_error(format!("Attempted to create an invalid path: {filename}"))
        })
    }
    ...
    pub(crate) fn new_staged_commit_path(
        ...
        let owner = resource::scope();
        resource::reserve(&owner, "native_log_path_join", crate::crc::resource::mul(crate::crc::resource::add(self.log_root().as_str().len(), filename.len() + STAGED_COMMITS_DIR.len() + 8)?, 8)?)?;
        let path = resource::try_join_url(&resource::try_join_url(self.log_root(), STAGED_COMMITS_DIR)?, &filename)?;
        ParsedLogPath::try_from_owned(path, owner)?.ok_or_else(|| {
            Error::internal_error(format!("Attempted to create an invalid path: {filename}"))
        })
    }
...
pub(crate) mod tests {
    ...
    fn test_unknown_invalid_patterns() {
        ...
        assert!(
            matches!(
                result,
                Ok(Some(ref path)) if path.file_type == LogPathFileType::Unknown
            ),
    ...
    #[test]
    fn test_should_list() {
        let mut path = ParsedLogPath::from_test_parts(table_log_dir_url(), String::new(), String::new(), 0, LogPathFileType::Commit);
        ...
        for (file_type, should_list) in [
            ...
            Arc::get_mut(&mut path.0).unwrap().file_type = file_type;
            assert_eq!(
                path.should_list(),
                should_list,
                "file_type: {:?}",
