...
impl Display for ColumnName {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        display_path_parts(self.path.iter().map(String::as_str), f)
    }
}

impl ColumnName {
    /// Format an existing borrowed path with the same escaping as ColumnName,
    /// without creating a temporary owned name during native row extraction.
    pub(crate) fn display_path<'a>(path: &'a [&'a str]) -> impl Display + 'a {
        struct Borrowed<'a>(&'a [&'a str]);
        impl Display for Borrowed<'_> {
            fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
                display_path_parts(self.0.iter().copied(), f)
            }
        }
        Borrowed(path)
    }
