...
use crate::path::{LogPathFileType, ParsedLogPath, may_begin_listable_log_path};
...
pub(crate) struct LogSegmentFiles {
    pub ascending_commit_files: OwnedLogPaths,
    pub ascending_compaction_files: OwnedLogPaths,
    pub checkpoint_parts: OwnedLogPaths,
...
pub(crate) fn list_from_storage(
    ...
    crate::path::resource::reserve(
        &crate::path::resource::scope(),
        "native_log_listing_prefix",
        crate::crc::resource::mul(crate::crc::resource::add(log_root.as_str().len(), 64)?, 8)?,
    )?;
...
fn group_checkpoint_parts(parts: OwnedLogPaths) -> DeltaResult<HashMap<u32, OwnedLogPaths>> {
    if parts.is_empty() {
        return Ok(HashMap::new());
    }
    let scope = crate::resource::current_resource_scope();
    if let Some(scope) = &scope {
        scope.reserve(crate::resource::AllocationRequest {
            kind: "native_checkpoint_group_map",
            bytes: crate::crc::resource::map_layout::<u32, OwnedLogPaths>(parts.len())?,
        })?;
    }
    let mut checkpoints: HashMap<u32, OwnedLogPaths> = HashMap::with_capacity(parts.len());
    for part_file in parts {
        match &part_file.file_type {
            ...
            | MultiPartCheckpoint {
                ...
                checkpoints.insert(1, OwnedLogPaths::try_from_iter([Ok(part_file)])?);
            }
            MultiPartCheckpoint {
                part_num: 1,
                num_parts,
                ...
                checkpoints.insert(*num_parts, OwnedLogPaths::try_from_iter([Ok(part_file)])?);
            }
            MultiPartCheckpoint {
                part_num,
                num_parts,
...
fn find_complete_checkpoint_version(
    ascending_files: &[ParsedLogPath],
) -> DeltaResult<Option<Version>> {
    // Preserve native per-version grouping and arbitrary equivalent-checkpoint
    // choice, but admit original group descriptors before collecting any path.
    let mut latest = None;
    let mut group = OwnedLogPaths::default();
    let mut version = None;
    for file in ascending_files
        .iter()
        .filter(|file| file.is_checkpoint() && should_process_log_file(file))
    {
        if version.is_some_and(|version| version != file.version) {
            if group_checkpoint_parts(std::mem::take(&mut group))?
                .iter()
                .any(|(n, parts)| parts.len() == *n as usize)
            {
                latest = version;
            }
        }
        version = Some(file.version);
        group.try_push(file.clone())?;
    }
    if group_checkpoint_parts(group)?
        .iter()
        .any(|(n, parts)| parts.len() == *n as usize)
    {
        latest = version;
...
struct ListingAccumulator {
    ...
    /// ends
    pending_checkpoint_parts: OwnedLogPaths,
...
impl ListingAccumulator {
    fn process_file(&mut self, file: ParsedLogPath) -> DeltaResult<()> {
        ...
        match file.file_type {
            Commit | StagedCommit => self.output.ascending_commit_files.try_push(file)?,
            ...
            SinglePartCheckpoint | UuidCheckpoint | MultiPartCheckpoint { .. } => {
                self.pending_checkpoint_parts.try_push(file)?
            }
            Crc => {
    ...
    fn maybe_flush_and_advance(&mut self, file_version: Version) -> DeltaResult<()> {
    ...
    fn flush_checkpoint_group(&mut self, version: Version) -> DeltaResult<()> {
...
impl LogSegmentFiles {
    ...
    pub(crate) fn build_log_segment_files(
        ...
        let log_tail = OwnedLogPaths::try_from_iter(log_tail.into_iter().map(Ok))?;
    ...
    pub(crate) fn ascending_commit_files(&self) -> &OwnedLogPaths {
    ...
    pub(crate) fn ascending_commit_files_mut(&mut self) -> &mut OwnedLogPaths {
        &mut self.ascending_commit_files
    }

    pub(crate) fn checkpoint_parts(&self) -> &OwnedLogPaths {
    ...
    pub(crate) fn list_commits(
        ...
        let log_tail = OwnedLogPaths::try_from_iter(log_tail.into_iter().map(Ok))?;
        let log_tail_start_version = log_tail.first().map(|f| f.version);
        let mut listed_commits = OwnedLogPaths::default();
    ...
    pub(crate) fn list_with_checkpoint_hint(
        ...
        if latest_checkpoint.version != checkpoint_metadata.version {
            info!(
                ...
                checkpoint_metadata.version, latest_checkpoint.version
            );
    ...
    pub(crate) fn list_with_backward_checkpoint_scan(
        ...
        let mut windows: Vec<OwnedLogPaths> = Vec::new();
        ...
        while upper > 0 {
            ...
            let window_files = OwnedLogPaths::try_from_iter(list_from_storage(
                storage,
                log_root,
                lower,
                upper - 1,
            ...
            if windows.len() == windows.capacity() {
                let capacity = crate::crc::resource::mul(windows.capacity(), 2)?.max(4);
                crate::path::resource::reserve(
                    &crate::path::resource::scope(),
                    "native_log_scan_windows",
                    crate::crc::resource::mul(capacity, std::mem::size_of::<OwnedLogPaths>())?,
                )?;
                windows.reserve_exact(capacity - windows.len());
            }
