...
pub(crate) struct SequentialPhase<P: LogReplayProcessor> {
    ...
    checkpoint_parts: crate::OwnedFileMetas,
}
...
pub(crate) enum AfterSequential<P: LogReplayProcessor> {
    ...
    Done(P),
    /// Parallel phase needed - distribute files for parallel processing.
    Parallel { processor: P, files: crate::OwnedFileMetas },
}
    ...
    pub(crate) fn try_new(
        ...
        let checkpoint_manifest_phase = match log_segment.listed.checkpoint_parts.as_slice() {
            [single_part] => Some(CheckpointManifestReader::try_new(
                engine,
                single_part,
                log_segment.log_root.try_clone_url_admitted()?,
            )?),
            _ => None,
        };

        let mut checkpoint_parts = crate::OwnedFileMetas::default();
        for path in &log_segment.listed.checkpoint_parts {
            checkpoint_parts.try_push_copy(&path.location)?;
        }
