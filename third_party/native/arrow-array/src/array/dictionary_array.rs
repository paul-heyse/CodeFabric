...
pub struct DictionaryArray<K: ArrowDictionaryKeyType> {
    ...
    /// Values are ordered.
    is_ordered: bool,
    // Drop backing and metadata before releasing original resource ownership.
...
impl<K: ArrowDictionaryKeyType> DictionaryArray<K> {
    ...
    pub fn try_new(mut keys: PrimitiveArray<K>, values: ArrayRef) -> Result<Self, ArrowError> {
        ...
        let mut resource_owner = keys.resource_owner().clone();
        resource_owner.capture_current_or_latch();
        keys.resource_owner = resource_owner.clone();
        Ok(Self {
            resource_owner,
            data_type,
            keys,
    ...
    pub unsafe fn new_unchecked(mut keys: PrimitiveArray<K>, values: ArrayRef) -> Self {
...
impl<T: ArrowDictionaryKeyType> From<ArrayData> for DictionaryArray<T> {
    fn from(data: ArrayData) -> Self {
        let (data_type, len, nulls, offset, mut buffers, mut child_data, mut resource_owner) =
        ...
        if let DataType::Dictionary(key_data_type, _) = &data_type {
            ...
            let keys_data =
                ArrayDataBuilder::new_with_resource_owner(T::DATA_TYPE, resource_owner.clone())
                    .len(len)
                    .offset(offset)
                    .buffers(vec![buffer])
                    .nulls(nulls);
            let keys = PrimitiveArray::<T>::from(unsafe { keys_data.build_unchecked() });
...
impl<T: ArrowDictionaryKeyType> From<DictionaryArray<T>> for ArrayData {
    fn from(array: DictionaryArray<T>) -> Self {
        // Safe constructors give the keys the parent owner only after original
        // key ownership has been preserved/adopted. Transfer that root once;
        // rebinding keys separately could create a reverse adoption edge.
        let len = array.keys.len();
        let (_, keys, nulls) = array.keys.into_parts();
        let offset = 0;
        let builder =
            ArrayDataBuilder::new_with_resource_owner(array.data_type, array.resource_owner)
                .len(len)
                .offset(offset)
                .nulls(nulls)
                .buffers(vec![keys.into_inner()])
...
unsafe impl<T: ArrowDictionaryKeyType> Array for DictionaryArray<T> {
    ...
    fn try_admit_to_data(&self) -> Result<(), arrow_schema::ArrowError> {
        crate::native_resource::prepare_data(self)
    }
    ...
    fn try_visit_buffers(
        ...
        self.keys.try_visit_buffers(visitor)?;
        self.values.try_visit_buffers(visitor)?;
        Ok(())
    }

    fn try_to_data(&self) -> Result<ArrayData, arrow_schema::ArrowError> {
        crate::native_resource::to_data(self)
    }
...
unsafe impl<K: ArrowDictionaryKeyType, V: Sync> Array for TypedDictionaryArray<'_, K, V> {
    ...
    fn resource_owner(&self) -> &arrow_schema::resource::ResourceOwnerHandle {
        self.dictionary.resource_owner()
    }
    ...
    fn try_visit_buffers(
        ...
        self.dictionary.try_visit_buffers(visitor)
    }

    fn try_to_data(&self) -> Result<ArrayData, arrow_schema::ArrowError> {
        self.dictionary.try_to_data()
    }
    fn try_admit_to_data(&self) -> Result<(), arrow_schema::ArrowError> {
        self.dictionary.try_admit_to_data()
    }
    fn to_data(&self) -> ArrayData {
