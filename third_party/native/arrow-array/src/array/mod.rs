...
pub unsafe trait Array: std::fmt::Debug + Send + Sync {
    ...
    /// Original native allocation lifetime. Custom arrays default to unowned.
    /// Sharing this handle does not admit new backing or metadata allocations.
    fn resource_owner(&self) -> &arrow_schema::resource::ResourceOwnerHandle {
        static EMPTY: arrow_schema::resource::ResourceOwnerHandle =
            arrow_schema::resource::ResourceOwnerHandle::empty();
        &EMPTY
    }

    /// Returns the underlying data with its retained resource owner.
    fn to_data(&self) -> ArrayData;

    /// Visits existing native backing without allocating array descriptors.
    ///
    /// The callback may fail and stops traversal immediately. Custom implementors
    /// must explicitly implement this seam; the default never invokes `to_data`.
    fn try_visit_buffers(
        &self,
        visitor: &mut dyn FnMut(&arrow_buffer::Buffer) -> Result<(), arrow_schema::ArrowError>,
    ) -> Result<(), arrow_schema::ArrowError> {
        let _ = visitor;
        let error = arrow_schema::resource::ResourceOwnerError {
            kind: "custom array buffer visitation unavailable",
            requested: 1,
            limit: 0,
        };
        if let Some(owner) = arrow_schema::resource::current_resource_owner() {
            owner.record_failure(error);
        }
        Err(error.into())
    }

    /// Fallibly admit all native descriptor allocations before converting this array.
    /// This retains original input ownership; it does not admit original backing.
    /// Custom implementations under required ownership must provide their own seam.
    fn try_to_data(&self) -> Result<ArrayData, arrow_schema::ArrowError> {
        crate::native_resource::custom_to_data(self)
    }

    /// Preflight for recursive native ArrayData conversion. Successful admission
    /// must precede every new allocation in `to_data`, including child descriptors.
    #[doc(hidden)]
    fn try_admit_to_data(&self) -> Result<(), arrow_schema::ArrowError> {
        crate::native_resource::custom_prepare(self)
    }
...
unsafe impl Array for ArrayRef {
    ...
    fn try_visit_buffers(
        ...
        self.as_ref().try_visit_buffers(visitor)
    }

    fn try_to_data(&self) -> Result<ArrayData, arrow_schema::ArrowError> {
        self.as_ref().try_to_data()
    }
    fn try_admit_to_data(&self) -> Result<(), arrow_schema::ArrowError> {
        self.as_ref().try_admit_to_data()
    }
    fn to_data(&self) -> ArrayData {
        self.as_ref().to_data()
    }
...
unsafe impl<T: Array> Array for &T {
    ...
    fn try_visit_buffers(
        ...
        T::try_visit_buffers(self, visitor)
    }
...
/// Pre-admit native concrete Arc and descriptor allocations, preserving the
/// already-owned input buffers and metadata, then use the native array factory.
pub fn try_make_array(data: ArrayData) -> Result<ArrayRef, arrow_schema::ArrowError> {
    crate::native_resource::make(data)
}

/// Admits the native factory's descriptor allocations without allocating.
///
/// Selected native callers must invoke the existing factory within the same
/// required resource scope. This does not admit the original input backing.
pub fn try_admit_make_array(data: &ArrayData) -> Result<(), arrow_schema::ArrowError> {
    crate::native_resource::prepare_make(data)
}
    ...
    fn test_memory_size_null() {
        ...
        assert_eq!(
            std::mem::size_of::<usize>()
                + std::mem::size_of::<arrow_schema::resource::ResourceOwnerHandle>(),
