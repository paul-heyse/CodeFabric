...
pub struct RunArray<R: RunEndIndexType> {
    ...
    run_end_resource_owner: arrow_schema::resource::ResourceOwnerHandle,
}
...
impl<R: RunEndIndexType> Clone for RunArray<R> {
    fn clone(&self) -> Self {
        Self {
            ...
            run_end_resource_owner: self.run_end_resource_owner.clone(),
    ...
    pub unsafe fn new_unchecked(
        ...
        if cfg!(feature = "force_validate") {
            ...
            let run_array = Self {
                ...
                run_end_resource_owner: arrow_schema::resource::ResourceOwnerHandle::capture(),
                data_type,
                run_ends,
                values,
            };
...
impl<R: RunEndIndexType> From<ArrayData> for RunArray<R> {
    ...
    fn from(data: ArrayData) -> Self {
        let (data_type, len, _nulls, offset, _buffers, child_data, mut resource_owner) =
        ...
        let (
            ...
            _run_end_child_data,
            run_end_owner,
        ) = run_end_child.into_parts_with_resource_owner();
        ...
        Self {
            resource_owner,
            run_end_resource_owner: run_end_owner,
            data_type,
            run_ends,
            values,
        }
...
impl<R: RunEndIndexType> From<RunArray<R>> for ArrayData {
    fn from(array: RunArray<R>) -> Self {
        ...
        let run_ends =
            ArrayDataBuilder::new_with_resource_owner(R::DATA_TYPE, array.run_end_resource_owner)
...
unsafe impl<T: RunEndIndexType> Array for RunArray<T> {
    ...
    fn try_visit_buffers(
        ...
        visitor(self.run_ends.inner().inner())?;
...
unsafe impl<R: RunEndIndexType, V: Sync> Array for TypedRunArray<'_, R, V> {
    ...
    fn try_visit_buffers(
        ...
        self.run_array.try_visit_buffers(visitor)
    }

    fn try_to_data(&self) -> Result<ArrayData, arrow_schema::ArrowError> {
        self.run_array.try_to_data()
    }
    fn try_admit_to_data(&self) -> Result<(), arrow_schema::ArrowError> {
        self.run_array.try_admit_to_data()
    }
    fn to_data(&self) -> ArrayData {
