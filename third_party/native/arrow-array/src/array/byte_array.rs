...
impl<T: ByteArrayType> GenericByteArray<T> {
    ...
    pub fn into_builder(self) -> Result<GenericByteBuilder<T>, Self> {
        // Mutable builders do not carry immutable resource roots. Keep an owned
        // input intact and let callers use their ordinary copy fallback.
...
unsafe impl<T: ByteArrayType> Array for GenericByteArray<T> {
    ...
    fn try_visit_buffers(
        ...
        visitor(self.value_offsets.inner().inner())?;
        visitor(&self.value_data)?;
