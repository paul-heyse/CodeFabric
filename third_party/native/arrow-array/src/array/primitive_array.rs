...
pub struct PrimitiveArray<T: ArrowPrimitiveType> {
    ...
    pub(crate) resource_owner: arrow_schema::resource::ResourceOwnerHandle,
}
...
impl<T: ArrowPrimitiveType> PrimitiveArray<T> {
    ...
    {
        {
            let mut output = PrimitiveArray::new(self.values.clone(), self.nulls.clone());
            output.resource_owner = self.resource_owner.clone();
            output.resource_owner.capture_current_or_latch();
            output
        }
    }
    ...
    pub fn into_builder(self) -> Result<PrimitiveBuilder<T>, Self> {
        // Mutable builders cannot retain this immutable allocation owner.
...
impl<T: ArrowPrimitiveType> From<ArrayData> for PrimitiveArray<T> {
    fn from(data: ArrayData) -> Self {
        let (data_type, len, nulls, offset, mut buffers, _child_data, mut resource_owner) =
