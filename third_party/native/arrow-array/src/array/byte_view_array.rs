...
impl<T: ByteViewType + ?Sized> From<ArrayData> for GenericByteViewArray<T> {
    fn from(data: ArrayData) -> Self {
        let (data_type, len, nulls, offset, buffers, _child_data, mut resource_owner) =
    ...
    V: ByteViewType<Native = FROM::Native>,
{
    fn from(byte_array: &GenericByteArray<FROM>) -> Self {
        ...
        let mut output = if can_reuse_buffer {
        ...
        output.resource_owner = byte_array.resource_owner().clone();
        output.resource_owner.capture_current_or_latch();
        output
    }
}
...
impl<T: ByteViewType + ?Sized> From<GenericByteViewArray<T>> for ArrayData {
    fn from(array: GenericByteViewArray<T>) -> Self {
        ...
        let builder = ArrayDataBuilder::new_with_resource_owner(T::DATA_TYPE, array.resource_owner)
            .len(len)
            .buffers(buffers)
            .nulls(array.nulls);
    ...
    pub unsafe fn to_string_view_unchecked(self) -> StringViewArray {
        {
            ...
            output.resource_owner = self.resource_owner;
            output.resource_owner.capture_current_or_latch();
            output
        }
    }
