...
impl UnionArray {
    ...
    pub fn into_parts(
        ...
        match data_type {
            DataType::Union(mut union_fields, _) => {
                if let Err(error) = union_fields.try_inherit_resource_owner(&resource_owner) {
                    if let Some(owner) = resource_owner.owner() {
...
impl From<ArrayData> for UnionArray {
    fn from(data: ArrayData) -> Self {
        let (data_type, len, _nulls, offset, buffers, child_data, mut resource_owner) =
...
unsafe impl Array for UnionArray {
    ...
    fn try_visit_buffers(
        ...
        visitor(self.type_ids.inner())?;
        if let Some(offsets) = &self.offsets {
            visitor(offsets.inner())?;
        }
