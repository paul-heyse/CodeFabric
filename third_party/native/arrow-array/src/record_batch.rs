...
impl RecordBatch {
    /// Original native allocation owner, shared independently of Arrow semantics.
    ...
    pub unsafe fn new_unchecked(
        ...
        let mut resource_owner = schema.fields.resource_owner().clone();
...
mod tests {
    ...
    fn byte_size_should_not_regress() {
        ...
        let record_batch =
            RecordBatch::try_new(Arc::new(schema), vec![Arc::new(a), Arc::new(b)]).unwrap();
        // The native array and schema field headers now retain resource owners.
        assert_eq!(record_batch.get_array_memory_size(), 428);
    }
