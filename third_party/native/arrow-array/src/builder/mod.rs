...
pub trait ArrayBuilder: Any + Send + Sync {
    ...
    /// Finishes selected native builders after admitting finalization allocations.
    ///
    /// Existing builder construction and appended payload must already be admitted
    /// by the required owner. This covers final immutable owners, descriptors and
    /// recursive child finalization. Custom builders reject under a required owner.
    fn try_finish(&mut self) -> Result<ArrayRef, arrow_schema::ArrowError> {
        crate::native_resource::custom_finish(self)
    }
