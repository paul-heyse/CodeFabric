...
impl ArrayBuilder for StructBuilder {
    ...
    fn try_finish(&mut self) -> Result<ArrayRef, arrow_schema::ArrowError> {
        ...
        resource::vector::<ArrayRef>(
            &owner,
            self.field_builders.len(),
            "native finished struct columns",
        )?;
        resource::array_arc::<StructArray>(&owner)?;
        ...
        for field in &mut self.field_builders {
            arrays.push(field.try_finish()?);
        }
