...
impl<T: ByteArrayType> GenericByteBuilder<T> {
    ...
    pub fn finish(&mut self) -> GenericByteArray<T> {
        ...
        let array_builder = ArrayDataBuilder::new(array_type)
            .len(self.len())
            .buffers(vec![
                std::mem::take(&mut self.offsets_builder).into(),
                std::mem::take(&mut self.value_builder).into(),
            ])
...
impl<T: ByteArrayType> ArrayBuilder for GenericByteBuilder<T> {
    ...
    fn try_finish(&mut self) -> Result<ArrayRef, arrow_schema::ArrowError> {
        ...
        resource::vector::<Buffer>(&owner, 2, "native finished byte descriptors")?;
        resource::vector::<T::Offset>(&owner, 1, "native reset byte offsets")?;
        resource::array_arc::<GenericByteArray<T>>(&owner)?;
