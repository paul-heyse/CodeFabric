...
impl<K: ArrayBuilder, V: ArrayBuilder> ArrayBuilder for MapBuilder<K, V> {
    ...
    fn try_finish(&mut self) -> Result<ArrayRef, arrow_schema::ArrowError> {
        let Some(owner) = arrow_schema::resource::current_resource_owner() else {
            return Ok(Arc::new(self.finish()));
        };
        use crate::native_resource as resource;
        resource::finish_buffer(&owner)?;
        resource::finish_nulls(&owner, self.null_buffer_builder.as_slice().is_some())?;
        resource::vector::<i32>(&owner, 1, "native reset map offsets")?;
        let len = self.len();
        let keys = self.key_builder.try_finish()?;
        let values = self.value_builder.try_finish()?;
        if self.key_field.is_none() {
            resource::field(&owner, &self.field_names.key, keys.data_type())?;
        }
        if self.value_field.is_none() {
            resource::field(&owner, &self.field_names.value, values.data_type())?;
        }
        resource::vector::<arrow_schema::FieldRef>(&owner, 2, "native map entry fields")?;
        resource::arc_slice::<arrow_schema::FieldRef>(&owner, 2, "native map entry Fields Arc")?;
        resource::vector::<ArrayRef>(&owner, 2, "native map entry columns")?;
        let key_field = match &self.key_field {
            Some(field) => field.clone(),
            None => Arc::new(Field::new(
                &self.field_names.key,
                keys.data_type().clone(),
                false,
            )),
        };
        let value_field = match &self.value_field {
            Some(field) => field.clone(),
            None => Arc::new(Field::new(
        ...
        resource::field(&owner, &self.field_names.entry, entries.data_type())?;
        resource::vector::<Buffer>(&owner, 1, "native map ArrayData offsets")?;
        resource::vector::<ArrayData>(&owner, 1, "native map ArrayData entries")?;
        let map_field = Arc::new(Field::new(
            &self.field_names.entry,
            entries.data_type().clone(),
            false,
        ));
        let entry_data = entries.try_to_data()?;
        let offsets = Buffer::from_vec(std::mem::take(&mut self.offsets_builder));
        self.offsets_builder = Vec::with_capacity(1);
        self.offsets_builder.push(0);
        let data = ArrayData::builder(DataType::Map(map_field, false))
            .len(len)
            .buffers(vec![offsets])
            .child_data(vec![entry_data])
            .nulls(self.null_buffer_builder.finish());
        // Safety: the native map builder maintains the offsets and child layout.
        let data = unsafe { data.build_unchecked() };
        crate::try_make_array(data)
    }
