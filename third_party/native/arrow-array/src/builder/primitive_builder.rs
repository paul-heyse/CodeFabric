...
impl<T: ArrowPrimitiveType> ArrayBuilder for PrimitiveBuilder<T> {
    ...
    fn try_finish(&mut self) -> Result<ArrayRef, arrow_schema::ArrowError> {
        ...
        resource::vector::<Buffer>(&owner, 1, "native finished primitive descriptors")?;
        resource::data_type_clone(&owner, &self.data_type)?;
        resource::array_arc::<PrimitiveArray<T>>(&owner)?;
...
impl<T: ArrowPrimitiveType> PrimitiveBuilder<T> {
    ...
    pub fn finish(&mut self) -> PrimitiveArray<T> {
        ...
        let builder = ArrayData::builder(self.data_type.clone())
            .len(len)
            .buffers(vec![std::mem::take(&mut self.values_builder).into()])
            .nulls(nulls);
