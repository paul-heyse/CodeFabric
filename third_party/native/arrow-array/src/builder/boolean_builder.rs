...
impl BooleanBuilder {
    ...
    pub fn finish(&mut self) -> BooleanArray {
        ...
        let builder = ArrayData::builder(DataType::Boolean)
            .len(len)
            .buffers(vec![self.values_builder.finish().into_inner()])
...
impl ArrayBuilder for BooleanBuilder {
    ...
    fn try_finish(&mut self) -> Result<ArrayRef, arrow_schema::ArrowError> {
        ...
        resource::vector::<Buffer>(&owner, 1, "native finished boolean descriptors")?;
        resource::array_arc::<BooleanArray>(&owner)?;
