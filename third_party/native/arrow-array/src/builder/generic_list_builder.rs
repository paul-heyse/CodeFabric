    ...
    T: 'static,
{
    ...
    fn try_finish(&mut self) -> Result<ArrayRef, arrow_schema::ArrowError> {
        ...
        resource::vector::<OffsetSize>(&owner, 1, "native reset list offsets")?;
        resource::array_arc::<GenericListArray<OffsetSize>>(&owner)?;
        let values = self.values_builder.try_finish()?;
        if self.field.is_none() {
            resource::field(&owner, "item", values.data_type())?;
        }
        let nulls = self.null_buffer_builder.finish();
        let offsets = Buffer::from_vec(std::mem::take(&mut self.offsets_builder));
        // Safety: these offsets are maintained by the native list builder.
        let offsets = unsafe { OffsetBuffer::<OffsetSize>::new_unchecked(offsets.into()) };
