...
fn concat_primitives<T: ArrowPrimitiveType>(arrays: &[&dyn Array]) -> Result<ArrayRef, ArrowError> {
    if !crate::resource::current().is_empty() {
        use crate::resource;
        let len = resource::total(arrays)?;
        for array in arrays {
            resource::adopt(*array)?;
        }
        resource::arc::<PrimitiveArray<T>>()?;
        if arrays.len() == 1 {
            let array = arrays[0].as_primitive::<T>();
            return Ok(Arc::new(
                PrimitiveArray::<T>::new(array.values().clone(), array.nulls().cloned())
                    .with_data_type(array.data_type().clone()),
            ));
        }
        resource::array::<T::Native>(len, "concat primitive values")?;
        resource::owner()?;
        if arrays.iter().any(|a| a.null_count() != 0) {
            resource::bitmap(len, "concat primitive validity")?;
            resource::owner()?;
        }
        // PrimitiveBuilder::finish pushes one Buffer into an initially empty
        // native Vec. Rust's pinned RawVec minimum for this element is four.
        resource::array::<arrow_buffer::Buffer>(4, "concat primitive finish descriptors")?;
        let mut builder =
            PrimitiveBuilder::<T>::with_capacity(len).with_data_type(arrays[0].data_type().clone());
        for array in arrays {
            builder.append_array(array.as_primitive());
        }
        let output = builder.finish();
        resource::retain(output.values().inner())?;
        if let Some(nulls) = output.nulls() {
...
fn concat_boolean(arrays: &[&dyn Array]) -> Result<ArrayRef, ArrowError> {
    if !crate::resource::current().is_empty() {
        ...
        if arrays.len() == 1 {
            let array = arrays[0].as_boolean();
            return Ok(Arc::new(BooleanArray::new(
                array.values().clone(),
                array.nulls().cloned(),
            )));
        }
        resource::bitmap(len, "concat boolean values")?;
        resource::owner()?;
        if arrays.iter().any(|a| a.null_count() != 0) {
            resource::bitmap(len, "concat boolean validity")?;
            resource::owner()?;
        }
        resource::array::<arrow_buffer::Buffer>(4, "concat boolean finish descriptors")?;
...
pub fn concat(arrays: &[&dyn Array]) -> Result<ArrayRef, ArrowError> {
    ...
    } else if arrays.len() == 1 && crate::resource::current().is_empty() {
        let array = arrays[0];
    ...
    if !crate::resource::current().is_empty() {
        downcast_primitive! {
            ...
            DataType::Boolean => { return concat_boolean(arrays); },
            _ => { return concat_fallback(arrays, Capacities::Array(crate::resource::total(arrays)?)); }
        }
    }
...
fn concat_fallback(arrays: &[&dyn Array], capacity: Capacities) -> Result<ArrayRef, ArrowError> {
    crate::resource::array::<arrow_data::ArrayData>(arrays.len(), "concat input data descriptors")?;
    let mut array_data = Vec::new();
    array_data
        .try_reserve_exact(arrays.len())
        .map_err(|_| crate::resource::overflow("concat input data descriptors"))?;
    for array in arrays {
        array_data.push(array.try_to_data()?);
    }
    crate::resource::array::<&arrow_data::ArrayData>(arrays.len(), "concat input data references")?;
    let mut references = Vec::new();
    references
        .try_reserve_exact(arrays.len())
        .map_err(|_| crate::resource::overflow("concat input data references"))?;
    references.extend(array_data.iter());
    let mut mutable = MutableArrayData::try_with_capacities(&references, false, capacity)?;
