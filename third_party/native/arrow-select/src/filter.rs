...
pub(crate) fn try_null_mask_buffer(filter: &BooleanArray) -> Result<BooleanBuffer, ArrowError> {
    Ok(FilterBuilder::try_new(filter)?.filter.values().clone())
}
...
pub fn filter(values: &dyn Array, predicate: &BooleanArray) -> Result<ArrayRef, ArrowError> {
    let mut filter_builder = FilterBuilder::try_new(predicate)?;
    ...
    if FilterBuilder::is_optimize_beneficial(values.data_type()) {
        ...
        filter_builder = filter_builder.try_optimize()?;
    }
...
impl FilterBuilder {
    /// Fallible native mask preparation with allocation admission when a native
    /// resource owner is installed. The original infallible constructor remains
    /// available for callers outside the governed profile.
    pub fn try_new(filter: &BooleanArray) -> Result<Self, ArrowError> {
        if crate::resource::current().is_empty() {
            return Ok(Self::new(filter));
        }
        crate::resource::adopt(filter)?;
        let count = filter.true_count();
        let filter = if filter.null_count() == 0 {
            filter.clone()
        } else {
            // Native collect_bool uses one exact Vec<u64> word allocation.
            crate::resource::array::<u64>(filter.len().div_ceil(64), "filter predicate mask")?;
            crate::resource::owner()?;
            let nulls = filter.nulls().unwrap();
            let mask =
                BooleanBuffer::collect_bool(filter.len(), |i| filter.value(i) && nulls.is_valid(i));
            crate::resource::retain(mask.inner())?;
            BooleanArray::new(mask, None)
        };
        ...
        Ok(Self {
            ...
            count,
            strategy,
            resource_owner: crate::resource::current(),
        })
    }

    /// Fallibly materialize the native optimization with exact iterator counts.
    pub fn try_optimize(mut self) -> Result<Self, ArrowError> {
        if crate::resource::current().is_empty() {
            return Ok(self.optimize());
        }
        self.resource_owner.try_capture_current()?;
        match self.strategy {
            IterationStrategy::SlicesIterator => {
                let count = SlicesIterator::new(&self.filter).count();
                crate::resource::array::<(usize, usize)>(count, "filter slice descriptors")?;
                let mut slices = Vec::new();
                slices
                    .try_reserve_exact(count)
                    .map_err(|_| crate::resource::overflow("filter slice descriptors"))?;
                slices.extend(SlicesIterator::new(&self.filter));
                self.strategy = IterationStrategy::Slices(slices);
            }
            IterationStrategy::IndexIterator => {
                crate::resource::array::<usize>(self.count, "filter index descriptors")?;
                let mut indices = Vec::new();
                indices
                    .try_reserve_exact(self.count)
                    .map_err(|_| crate::resource::overflow("filter index descriptors"))?;
                indices.extend(IndexIterator::new(&self.filter, self.count));
...
impl FilterPredicate {
    ...
    pub fn filter_record_batch(
        ...
        let filtered_arrays = if crate::resource::current().is_empty() {
            record_batch
                .columns()
                .iter()
            ...
            let mut owner = crate::resource::current();
            owner.try_inherit(record_batch.resource_owner())?;
            owner.try_inherit(record_batch.schema_ref().fields().resource_owner())?;
            owner.try_inherit(&self.resource_owner)?;
            crate::resource::array::<ArrayRef>(
                record_batch.num_columns(),
                "filter batch column descriptors",
            )?;
            let mut columns = Vec::new();
            columns
                .try_reserve_exact(record_batch.num_columns())
                .map_err(|_| crate::resource::overflow("filter batch column descriptors"))?;
            for array in record_batch.columns() {
                columns.push(filter_array(array, self)?);
            }
            columns
        };
...
fn filter_array(values: &dyn Array, predicate: &FilterPredicate) -> Result<ArrayRef, ArrowError> {
    ...
    if !crate::resource::current().is_empty() {
        downcast_primitive_array! {
            values => { return try_filter_primitive(values, predicate); },
            DataType::Boolean => { return try_filter_boolean(values.as_boolean(), predicate); },
            _ => { return filter_fallback(values, predicate); }
        }
    }
    ...
    match predicate.strategy {
        ...
        _ => downcast_primitive_array! {
            ...
            _ => filter_fallback(values, predicate)
        },
    }
}
...
fn filter_fallback(
    ...
    crate::resource::adopt(values)?;
    crate::resource::current().try_inherit(&predicate.resource_owner)?;
    let data = values.try_to_data()?;
    let mut mutable = MutableArrayData::try_new(&[&data], false, predicate.count)?;
    match &predicate.strategy {
        IterationStrategy::Slices(slices) => {
...
fn try_filter_boolean(
    array: &BooleanArray,
    predicate: &FilterPredicate,
) -> Result<ArrayRef, ArrowError> {
    use crate::resource;
    resource::adopt(array)?;
    resource::current().try_inherit(&predicate.resource_owner)?;
    resource::arc::<BooleanArray>()?;
    let (values, nulls) = match &predicate.strategy {
        IterationStrategy::All => (
            array.values().slice(0, predicate.count),
            array.nulls().map(|n| n.slice(0, predicate.count)),
        ),
        IterationStrategy::None => {
            resource::owner()?;
            let buffer: Buffer = Vec::<u64>::new().into();
            resource::retain(&buffer)?;
            (BooleanBuffer::new(buffer, 0, 0), None)
        }
        strategy => {
            let preflight = |kind| -> Result<(), ArrowError> {
                match strategy {
                    IterationStrategy::IndexIterator | IterationStrategy::Indices(_) => {
                        resource::array::<u64>(predicate.count.div_ceil(64), kind)?
                    }
                    _ => resource::bitmap(predicate.count, kind)?,
                }
                resource::owner()
            };
            preflight("filter boolean values")?;
            let buffer = filter_bits(array.values(), predicate);
            resource::retain(&buffer)?;
            let nulls = if array.null_count() != 0 {
                preflight("filter boolean validity")?;
                let nulls = predicate.filter_nulls(array.nulls());
                if let Some(nulls) = &nulls {
                    resource::retain(nulls.inner().inner())?;
                }
                nulls
            } else {
                None
            };
            (BooleanBuffer::new(buffer, 0, predicate.count), nulls)
        }
    };
    Ok(Arc::new(BooleanArray::new(values, nulls)))
}

fn try_filter_primitive<T: ArrowPrimitiveType>(
    ...
    let (values, nulls) = match &predicate.strategy {
        ...
        IterationStrategy::None => {
            resource::owner()?;
            let buffer: Buffer = Vec::<T::Native>::new().into();
            resource::retain(&buffer)?;
            (ScalarBuffer::new(buffer, 0, 0), None)
        }
        strategy => {
            let bytes = predicate
                .count
                .checked_mul(std::mem::size_of::<T::Native>())
                .ok_or_else(|| resource::overflow("filter primitive values"))?;
            match strategy {
                IterationStrategy::IndexIterator => {
                    resource::mutable(bytes, "filter primitive values")?
                }
                _ => resource::array::<T::Native>(predicate.count, "filter primitive values")?,
            }
            resource::owner()?;
            ...
            let nulls = if array.null_count() != 0 {
                match strategy {
                    IterationStrategy::IndexIterator | IterationStrategy::Indices(_) => {
                        resource::array::<u64>(
                            predicate.count.div_ceil(64),
                            "filter primitive validity",
                        )?
                    }
                    _ => resource::bitmap(predicate.count, "filter primitive validity")?,
                }
                resource::owner()?;
                let nulls = predicate.filter_nulls(array.nulls());
                if let Some(nulls) = &nulls {
            ...
            (ScalarBuffer::new(buffer, 0, predicate.count), nulls)
        }
    };
    Ok(Arc::new(
        PrimitiveArray::<T>::new(values, nulls).with_data_type(array.data_type().clone()),
    ))
}
