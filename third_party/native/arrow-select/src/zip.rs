...
pub fn zip(
    ...
    let falsy = falsy.try_to_data()?;
    let truthy = truthy.try_to_data()?;
...
fn zip_impl(
    ...
    let mut mutable = MutableArrayData::try_new(&[truthy, falsy], false, mask.len())?;
    ...
    let mask_buffer = crate::filter::try_null_mask_buffer(mask)?;
...
impl ScalarZipper {
    ...
    pub fn try_new(truthy: &dyn Datum, falsy: &dyn Datum) -> Result<Self, ArrowError> {
        ...
        if !crate::resource::current().is_empty() {
            // The native generic transform provides a common fallible allocation
            // path for scalar expansion as well as array selection.
            crate::resource::arc::<FallbackImpl>()?;
            let zip_impl = Arc::new(FallbackImpl {
                truthy: truthy.try_to_data()?,
                falsy: falsy.try_to_data()?,
            });
            return Ok(Self { zip_impl });
        }
