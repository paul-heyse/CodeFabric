...
impl ModelDriver for RegistryCbefDriver {
    ...
    fn describe(&self) -> Result<DriverDescriptor, DriverProtocolError> {
        let descriptor = DriverDescriptor {
            ...
            outputs: vec![
                ...
                Self::output(
                    "output:model-digest-frames-rust",
                    RUST_DIGEST_FRAMES_PATH,
                    DriverOutputRole::RustBinding,
                )?,
                Self::output(
                    "output:model-digest-frames-extractor-rust",
                    EXTRACTOR_DIGEST_FRAMES_PATH,
                    DriverOutputRole::RustBinding,
                )?,
                Self::output(
    ...
    fn render(
        ...
        let mut outputs: Vec<(String, Vec<u8>)> = vec![
            ...
            (RUST_DIGEST_FRAMES_PATH.to_owned(), render_digest_frames()),
            (
                EXTRACTOR_DIGEST_FRAMES_PATH.to_owned(),
                render_digest_frames(),
            ),
            (
...
fn render_rust_runtime_registries(
    ...
    let public_errors: Vec<governed::PublicError> =
        registry_records(values, "codefabric.registry.error-registry")?;
    let mut output = String::from(
    ...
    for domain in &flags.records {
        let name = runtime_pascal(&domain.domain);
        writeln!(
            output,
            "#[derive(Clone, Copy, Debug, Eq, PartialEq)]\npub struct {name}(u64);\nimpl {name} {{"
        )
    ...
    output.push_str(
        "#[derive(Clone, Copy, Debug, Eq, PartialEq)]\n\
         pub struct PublicErrorEntry { pub code: u16, pub name: &'static str, pub owning_layer: &'static str, pub severity: &'static str, pub retryability: &'static str, pub scope: &'static str, pub public_message_template: &'static str, pub diagnostic_linkage: &'static str, pub grpc_status: &'static str, pub mcp_mapping: &'static str }\n\n\
         pub const PUBLIC_ERROR_ENTRIES: &[PublicErrorEntry] = &[\n",
    );
    for error in &public_errors {
        writeln!(
            output,
            "    PublicErrorEntry {{ code: {}, name: {:?}, owning_layer: {:?}, severity: {:?}, retryability: {:?}, scope: {:?}, public_message_template: {:?}, diagnostic_linkage: {:?}, grpc_status: {:?}, mcp_mapping: {:?} }},",
            error.code,
            error.name,
            error.owning_layer,
            error.severity,
            error.retryability,
            error.scope,
            error.public_message_template,
            error.diagnostic_linkage,
            error.grpc_status,
            error.mcp_mapping,
        )
        .unwrap();
    ...
    for (constant, artifact_id, field) in [
        ...
        output.push_str("];\n\n");
        if constant == "CAPABILITY_IDS" {
            writeln!(output, "pub const CAPABILITY_CODES: &[u16] = &[").unwrap();
            for (index, _) in records.iter().enumerate() {
                let code = u16::try_from(index + 1)
                    .map_err(|_| RegistryCbefError::RegistryModel("capability count".to_owned()))?
                    .checked_mul(10)
                    .ok_or_else(|| {
                        RegistryCbefError::RegistryModel("capability allocation".to_owned())
                    })?;
...
fn render_digest_frames() -> Vec<u8> {
    b"// @generated by codefabric-model; edit registry authorities instead.\n\
pub(crate) fn digest_frames(\n\
    domain: &[u8],\n\
    fields: impl IntoIterator<Item = Vec<u8>>,\n\
) -> String {\n\
...
from types import MappingProxyType\n\n\n",
    ...
    for type_code in &cbef.type_codes {
        writeln!(output, "    {} = {}", type_code.name, type_code.code).unwrap();
    }
    output.push_str("\n\n");
    for domain in &enums.records {
        writeln!(output, "class {}(IntEnum):", runtime_pascal(&domain.domain)).unwrap();
        for value in &domain.values {
    ...
    for machine in &machines {
        if !enum_type_names.contains(&machine.machine_id) {
            ...
            for state in &machine.states {
                writeln!(output, "    {} = {}", state.name, state.code).unwrap();
            }
            output.push_str("\n\n");
        }
    }
    for domain in &flags.records {
        writeln!(output, "class {}(IntFlag):", runtime_pascal(&domain.domain)).unwrap();
        output.push_str("    NONE = 0\n");
        for value in &domain.values {
    ...
    for machine in &machines {
        let constant = screaming_snake_from_pascal(&machine.machine_id);
        if !enums.records.iter().any(|domain| domain.domain == constant) {
    ...
    for phrase in &phrases {
        writeln!(
            output,
            "    {:?}: MappingProxyType({{\"owner_section\": {}, \"canonical_text\": {:?}, \"accepted_aliases\": tuple({:?}), \"plan_node_kind\": {:?}, \"output_role\": {:?}}}),",
            phrase.phrase_id,
            phrase.owner_section,
            phrase.canonical_text,
            phrase.accepted_aliases,
    ...
    for (category, artifact_id, field) in [
        (
            "entity_kinds",
            "codefabric.registry.ontology-entity-registry",
            "canonical_name",
...
mod tests {
    ...
    fn model_registry_generated_consumers_compile_and_typecheck() {
        ...
        let registries = String::from_utf8(
            render_rust_runtime_registries(&plan.registry_values, &plan.enums, &plan.flags)
                .unwrap(),
        )
        .unwrap();
        let python = String::from_utf8(
            render_python_registries(&plan.cbef, &plan.registry_values, &plan.enums, &plan.flags)
                .unwrap(),
        )
        .unwrap();
