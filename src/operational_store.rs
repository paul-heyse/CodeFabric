//! Application-owned `SQLite` operational state with one logical writer.
...
type OperationalColumnShapes = BTreeMap<String, Vec<(String, String, bool)>>;

fn active_operational_ddl() -> String {
    crate::schema_registry::operational_table_specs()
        .iter()
        .map(|spec| spec.sqlite_ddl)
        .collect::<Vec<_>>()
        .join("\n")
}
...
/// One bounded provider-run lifecycle projection for the operational `provider_run` table.
#[derive(Clone, Debug, Eq, PartialEq)]
...
pub enum OperationalStoreError {
    ...
    #[error("operational DDL lineage is invalid: {0}")]
    DdlLineage(String),
...
impl OperationalStore {
    ...
    fn open_with_fault(
        ...
        store.assert_operational_table_census()?;
        Ok(store)
    }
        ...
        self.write_transaction(|transaction| {
            let terminal_waves = {
                let mut statement = transaction.prepare(
                    "SELECT wave_id FROM update_wave
            ...
            let terminal_provider_runs = {
                let mut statement = transaction.prepare(
                    "SELECT provider_run_id FROM provider_run
            ...
            for run_id in terminal_provider_runs {
    ...
    fn migrate_from(
        ...
        let transaction = self
            ...
            .transaction_with_behavior(TransactionBehavior::Immediate)?;
        match version {
            0 => transaction.execute_batch(&active_operational_ddl())?,
            1..=14 => {
                return Err(OperationalStoreError::DdlLineage(format!(
                    "schema {version} contains retired mutable-publication state and cannot be reopened"
                )));
            }
            _ => {
                return Err(OperationalStoreError::DdlLineage(format!(
    ...
    fn assert_operational_table_census(&self) -> Result<(), OperationalStoreError> {
        let expected = operational_table_names();
        let mut statement = self.connection.prepare(
        ...
        let expected_columns = operational_column_shapes()?;
...
/// Digest of the exact operational DDL bytes compiled into the daemon.
#[must_use]
pub fn operational_ddl_digest() -> String {
    crate::integrity::framed_digest(active_operational_ddl().as_bytes())
}

fn verify_ddl_lineage() -> Result<(), OperationalStoreError> {
    for spec in crate::schema_registry::operational_table_specs() {
        if operational_table_ddl(spec.name)? != spec.sqlite_ddl {
            return Err(OperationalStoreError::DdlLineage(format!(
                "active operational DDL for {} differs from its typed table spec",
                spec.name
            )));
        }
    }
...
fn operational_table_names() -> BTreeSet<String> {
    crate::schema_registry::operational_table_specs()
        .iter()
...
fn operational_table_ddl(table: &str) -> Result<String, OperationalStoreError> {
    let spec = crate::schema_registry::operational_table_spec(table).ok_or_else(|| {
        OperationalStoreError::DdlLineage(format!("operational specs have no table {table}"))
    })?;
    Ok(spec.sqlite_ddl.to_owned())
}

fn operational_column_shapes() -> Result<OperationalColumnShapes, OperationalStoreError> {
    ...
    for table in crate::schema_registry::operational_table_specs() {
        ...
        if columns.is_empty() {
            return Err(OperationalStoreError::DdlLineage(format!(
                "operational table spec {} has no columns",
                table.name
            )));
        }
...
mod tests {
    ...
    fn wp69_operational_acceptance() {
        ...
        let names = operational_table_names();
        let shapes = operational_column_shapes().unwrap();
        assert_eq!(names.len(), specs.len());
        assert_eq!(shapes.len(), specs.len());
        for spec in specs {
            assert!(names.contains(spec.name));
            let ddl = operational_table_ddl(spec.name).unwrap();
            assert!(ddl.starts_with(&format!("CREATE TABLE {} (", spec.name)));
            assert!(ddl.ends_with(") STRICT;\n"));
            let expected_column_count = spec.arrow_schema.fields().len();
            assert_eq!(shapes[spec.name].len(), expected_column_count);
        }
    }
    ...
    fn wp13_structural_acceptance() {
        ...
        assert_eq!(
            operational_table_names().len(),
            active_operational_ddl()
                .lines()
    ...
    fn wp13_negative_zero_state() {
        ...
        let tables = operational_table_names();
        for prohibited in [
            "source_bytes",
            "arrow_rows",
            "query_result_bytes",
            ...
            assert!(!active_operational_ddl().contains(&format!("CREATE TABLE {prohibited}")));
        }
    }
    ...
    #[test]
    fn retired_mutable_publication_schema_fails_closed() {
        let (_directory, path) = database();
        let store = OperationalStore::open(&path).unwrap();
        drop(store);
        ...
        assert!(matches!(
            OperationalStore::open(&path),
            Err(OperationalStoreError::DdlLineage(detail))
                if detail.contains("retired mutable-publication state")
        ));
    }
}
