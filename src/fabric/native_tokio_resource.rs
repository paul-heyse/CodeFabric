...
//! Uses the tracked native Tokio implementation selected by the root Cargo graph.
use buoyant_kernel::resource::{AllocationRequest, NativeResourceScope, ResourceExhausted};
use std::alloc::Layout;
use std::sync::Arc;
use tokio::runtime::resource::{
    ResourceLayoutError, RuntimeAllocationAdmission, RuntimeAllocationReceipt,
...
fn arc_bytes<T>() -> Result<usize, ResourceLayoutError> {
    Layout::new::<[usize; 2]>()
        ...
        .map_err(|_| ResourceLayoutError {
            kind: "native Tokio receipt layout",
            requested: usize::MAX,
...
fn reserve(
    ...
    scope
        .reserve(AllocationRequest { kind, bytes })
        .map_err(|_| {
            convert(scope.failure().unwrap_or(ResourceExhausted {
                kind,
                requested: bytes,
                limit: 0,
            }))
...
#[derive(Debug)]
pub(crate) struct NativeTokioAdmission {
    scope: Arc<NativeResourceScope>,
}
impl NativeTokioAdmission {
    pub(crate) fn try_new(
        scope: Arc<NativeResourceScope>,
    ) -> Result<Arc<Self>, ResourceLayoutError> {
...
struct BankReceipt {
    bytes: usize,
    _bank: Arc<NativeResourceScope>,
}
impl RuntimeAllocationReceipt for BankReceipt {
    fn bytes(&self) -> usize {
        self.bytes
    }
}
impl RuntimeAllocationAdmission for NativeTokioAdmission {
    fn try_reserve(
        ...
        let bytes = request
            .bytes
            .checked_add(arc_bytes::<BankReceipt>()?)
            .filter(|bytes| *bytes <= isize::MAX as usize)
            .ok_or(ResourceLayoutError {
                kind: request.kind,
                requested: usize::MAX,
                limit: isize::MAX as usize,
            })?;
        reserve(&self.scope, request.kind, bytes)?;
        Ok(Arc::new(BankReceipt {
            bytes: request.bytes,
            _bank: self.scope.clone(),
        }))
    }
