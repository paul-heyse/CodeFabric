...
use crate::fabric::native_execution_lane::{
    NativeLaneJoined, NativeLaneOutput, NativeLaneResourcePolicy, NativeResourceFailure,
    output_seal,
};
...
impl NativeOperationResourcePolicy {
    pub(crate) fn try_new(
        owner: Arc<NativeResourceOwner>,
    ) -> Result<Arc<Self>, NativeResourceFailure> {
        if owner.scope().allocations_sealed() {
            return Err(NativeResourceFailure {
                kind: "native resource owner already joined",
                requested: 1,
                limit: 0,
            });
        }
        let layout = std::alloc::Layout::new::<(usize, usize)>()
            .extend(std::alloc::Layout::new::<Self>())
            .map_err(|_| NativeResourceFailure {
                kind: "native resource bridge layout",
                requested: usize::MAX,
        ...
        owner
            ...
            .reserve(AllocationRequest {
                kind: "native resource bridge",
                bytes: layout.size(),
            })
            .map_err(|_| {
                failure(owner.scope().failure().unwrap_or(ResourceExhausted {
                    kind: "native resource bridge",
        ...
        let runtime_admission =
            crate::fabric::native_tokio_resource::NativeTokioAdmission::try_new(
                owner.scope().clone(),
            )
            .map_err(|error| NativeResourceFailure {
                kind: error.kind,
                requested: error.requested,
                limit: error.limit,
            })?;
...
impl NativeLaneResourcePolicy for NativeOperationResourcePolicy {
    fn runtime_admission(
        ...
        Some(self.runtime_admission.clone())
    }
    ...
    fn enter_thread(&self) {
        ...
        let guard = self
            .owner
            .enter_thread()
            .expect("native worker policy lifecycle invariant");
        NATIVE_POLICIES.with(|slot| {
            let mut slot = slot.borrow_mut();
            assert!(
                slot.is_none(),
                "native worker has duplicate resource policies"
            );
            *slot = Some(guard);
        });
    }
    ...
    fn check_available(&self) -> Result<(), NativeResourceFailure> {
        self.owner.check_available().map_err(failure)
    }
    fn joined(&self, proof: NativeLaneJoined) -> Result<(), NativeResourceFailure> {
        *self
            .joined_runtime
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner) = Some(proof.runtime_id());
...
impl NativeLaneOutput for NativeOwnedRecordBatch {
    fn validate_retained_owner(&self, budget: &ResourceBudget) -> Result<(), ResourceBudgetError> {
        let owner = self
            ...
            .owner()
            .and_then(|owner| {
                (owner.as_ref() as &dyn std::any::Any).downcast_ref::<NativeResourceOwner>()
            })
            .ok_or(ResourceBudgetError::ForeignOwner)?;
        if !owner.belongs_to(budget)
            || !self
                .owners()
                .native_scope()
                .is_some_and(|scope| Arc::ptr_eq(scope, owner.scope()))
        {
