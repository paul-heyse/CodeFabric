...
4. **A failing check is feedback, not termination.** Diagnose, correct, rerun,
   and continue.
5. **Do not use a tool to certify its own modification.** When editing
   `smartref-code-intel` or native substrate behavior exposed through it, use
   independent source, compiler, test, and text/AST fallbacks.
...
- Accepting a partial structural query as global decommission proof.
