...
reconciled_head: 7184b86dc80adedc8a2b8d081179fa52d3dfee20
...
supersedes:
  - docs/designs/codefabric_execution_proved_relational_data_fabric_design_v1_2026-08-29.md
...
# CodeFabric execution-proved relational data fabric — target design v2
...
This v2 successor integrates every finding from
...
It preserves the relational target while correcting provider authority, derived-analysis
ownership, logical/physical schema adaptation, relation-scoped IPC, authorized view binding,
exact Delta semantics, activation ordering, compiler trust, and cross-version writer
revocation. These are target corrections, not compatibility layers for hypothetical future
library changes.
...
This v2 integration was reconciled at cleanup HEAD
`7184b86dc80adedc8a2b8d081179fa52d3dfee20`, with the bounded dirty-tree identity recorded in
frontmatter. Existing user changes were treated as inputs and were not overwritten. Read-only
current-tree reconciliation found no implementation drift under `src/`, `rustc-extractor/`,
`pyrefly-sidecar/`, `contracts/`, or `tooling/ci/` relative to that HEAD while this dossier was
integrated. The material current-state evidence remains:
...
- the independent implementation review records unresolved causality, activation,
  ...
  realization; and
- the v1 plan audit verified that exact Pyrefly `Query` does not supply the previously claimed
  import/definition/structured-diagnostic surface, exact `rustc_public` does not supply the
  previously claimed stable compiler keys/borrowck/derived dataflow, the pinned Delta provider
  chooses a supplied snapshot over a simultaneous version selector, and the pinned
  `OptimizeBuilder` owns internal retries.

The user declared cleanup complete before this integration. Broad validation was therefore not
rerun merely to restate a green baseline; the audit's focused design/API findings and current
repository facts are the evidence relevant to this successor. The implementation plan owns
packet-local and final repository gates after the corrected mechanisms exist.
...
only at public JSON, human-authored fixture, or vendor boundary seams. One model-derived
`SchemaContract` owns the logical Arrow schema, qualified `DFSchema`, storage schema,
logical-to-physical casts, projection/filter/statistics remapping, output restoration, and
validation at every plan/stream/batch/write boundary. Delta `BINARY` storage never silently
changes a logical `FixedSizeBinary` domain.

#### I-23 — Raw fidelity before normalization

Every fact genuinely emitted by a provider is available in a typed provider-native relation
with exact provider version, provider-local identity, coordinates, and run provenance.
Application-built CFG, dataflow, alias, effect, summary, and graph facts are never labeled
provider-native: they live in typed `derived` relations with algorithm, precision, input,
completeness, and provenance identities. Normalized facts are a separate relational
projection. A new provider API shape is handled by an explicit adapter/schema migration,
not silently dropped into an opaque field.
...
proof receipt. New query admission closes and the predecessor/fence is revalidated before
that durable selection; it reopens only after epoch swap and temporal-cache agreement. One
OS-released workspace lease plus the daemon's single command actor permits exactly one
target writer; concurrent multi-daemon mutation of one workspace is not a supported
deployment mode. Crossing `NEW_MUTATING` also invokes an external or bridge-release
revocation boundary that the frozen legacy binary cannot bypass after process restart.
The current epoch is derived as the unique valid activation-chain head. SQLite and
in-memory state may cache that answer but cannot name a different epoch.
...
filters, joins, aggregations, bounded recursive queries, and derivations use native
DataFusion nodes. A custom `TableProvider`, table function, analyzer rule, or physical
operator is permitted only when the semantic operation cannot be expressed correctly at a
higher level. Every provider truthfully forwards all supplied `ScanArgs` and reports
pushdown, constraints, partitioning, ordering, and ordinary plan statistics. DataFusion 55
does not itself produce or consume `StatisticsRequest`; this target does not claim a
query-aware statistics feature until a separately proved application producer, response
mapping, consumer, and plan-identity rule exist.
...
language grammars, `rustc_public` on nightly-2026-08-18, and the smallest required
version-pinned `rustc_private` enrichment seam. Pyrefly integration selects Query, TSP/
module resolution, optional Glean/internal indexing, and LSP fallback per accepted fact
family; it never assigns all families to Query. The architecture does not weaken present
functionality to hide hypothetical future changes. An upgrade changes the exact adapter,
provider-native schema, model migration, fixtures, and proof together.
...
`AccessScopeId`. A public view is recompiled against child-owned catalogs/functions or its
complete bound provider/function/subquery/view/extension/variable/object-store dependency
closure is recursively validated and sealed. The child installs a fresh allowlisted
object-store registry; it does not inherit one through `RuntimeEnvBuilder::from_runtime_env`.
No request-context filtering is delegated to DataFusion catalog methods that do not carry
request context, and no raw provider/session/plan handle crosses the query port.

#### I-33 — Derived analysis has one explicit application authority

Every accepted derived fact family has exactly one model-selected producer: native
DataFusion composition, a versioned CPG analysis, or a bounded petgraph-backed extension.
The producer relation records inputs, algorithm and precision release, invalidation scope,
materialization policy, output completeness, and provenance. A relational coverage query
maps every accepted ontology/query family to one producer or an explicit unsupported
remainder; provider rows and generic plan compilers cannot stand in for missing algorithms.

#### I-34 — Semantic compilation is an explicit trust boundary

Pyrefly and rustc semantic work execute only through policy-bearing launchers. Rust
compilation treats build scripts and procedural macros as untrusted code: inputs are
immutable/read-only, network and credentials are absent, outputs and target directories are
private and bounded, process groups and resources are constrained, and unsupported platform
containment fails closed. Any separately authorized `TRUSTED_LOCAL` posture is an observed
capability/provenance state, never the silent default.
...
`ViewTable` is not an authorization boundary: it retains a prebuilt logical plan whose table
sources and function expressions may already contain bound authorities. The epoch builder
therefore stores model-level public view definitions, not trusted full-session `ViewTable`
instances. `AccessSessionFactory` recompiles those definitions against the reduced child
registries, or accepts a precompiled view only after recursively proving its complete bound
dependency closure. A child runtime receives a fresh allowlisted object-store registry even
when it shares the epoch memory pool and spill manager.
...
| `raw_ruff` | Ruff tokens, AST, trivia, scopes, bindings, imports, references, and diagnostics actually exposed by the pinned crates | provider-native Arrow/Delta |
...
| `raw_rustc` | compilation, public items/types/instances, raw MIR bodies/blocks/locals/places/operands/statements/terminators/access events, narrow private stable/source/borrowck enrichment, and diagnostics | Arrow IPC to exact Delta/Arrow schema |
...
| `derived` | application-owned Python CFG/dataflow/alias/effect/resource facts, Rust ownership/dataflow/alias/drop facts, graph/dominance/reachability, summaries, and metrics | native plans or a bounded logical extension/table function selected per operation; materialized only by policy |
...
relations use `ViewTable` after their plans have been type/analyzer validated and their
bound dependency closure has been authorized for the session that will execute them. The
`ViewTable` constructor is not treated as a validator. A custom provider is reserved for
remote, policy-sensitive, streaming, logical/physical schema adaptation, or otherwise
irreducible sources and must remain optimizer-visible wherever native projection can carry
the contract.
...
“Exact API” means an explicit fact-family authority matrix, not one convenient facade per
library. For Pyrefly, Query owns bulk inferred types, callees, members, and subtype; TSP or
the pinned module resolver owns imports and declared/computed/expected distinctions;
optional Glean/internal indexing owns accepted bulk declarations/xrefs; LSP is only a
named navigation fallback. One long-lived workspace state applies the actual semantic
environment and records actual affected modules or conservatively refreshes reverse
importers. For rustc, `rustc_public` owns raw items/types/instances/MIR/access events, a
minimal version-pinned `rustc_private` seam owns required stable keys, exact source/hygiene,
borrowck, and selected vtable/mono facts, and CodeFabric owns derived dataflow. A provider
cannot claim another surface's fact family.
...
The data plane is relation-scoped. One Arrow IPC stream has exactly one schema and its own
dictionary scope. An outer versioned Protobuf control envelope identifies relation ID,
stream ID, schema fingerprint, sequence, source/context pins, and terminal status. It may
multiplex independently framed relation streams, but cannot concatenate heterogeneous
`RecordBatch` schemas into one raw stream. End-of-stream, coverage/remainder trailers,
partial failure, truncation, cancellation, and backpressure ordering are explicit and
fail closed.

Every provider run emits its actual observed schema, API revision, semantic environment or
compiler configuration, source/context pins, requested families, completed and affected
families/modules, diagnostics, and coverage. Capability starts unknown. A proof plan joins
demanded model families to completed provider relations and only then emits supported/
partial/unknown.
...
expressions/children; recompute properties after child replacement; reset mutable execution
state for repeated/recursive use; participate honestly in child statistics requests and
statistics propagation; and preserve physical invariants after optimizer rewrites.

DataFusion 55's `StatisticsRequest` is only vocabulary transported through `ScanArgs`.
The initial target forwards any supplied requests without loss but relies on ordinary
provider/plan statistics and native pruning. A later query-aware statistics feature requires
a model-selected application producer, provider response mapping, optimizer consumer,
precision semantics, observable plan oracle, and request-sensitive plan/cache identity; no
nonempty request is generated merely to claim API use.
...
DataFusion plans, including `RecursiveQuery` only when the compiler supplies explicit
depth, cycle, and resource bounds.
...
opaque and is evaluated as such. The rung is selected per compiled operation, never once for
an entire graph family. Each compiled graph use emits a derived
...
command + predecessor epoch + explicit inputs + policy
  ...
  -> close new admissions and revalidate predecessor/writer fence
  -> atomically append epoch + activation event and read it back
  -> swap Arc<FabricEpoch> and reconcile temporal cache
  -> reopen admissions
  -> acknowledge
```
...
The daemon lease/generation fences target processes, not an old binary that predates the
generation protocol. The deployment transition to `NEW_MUTATING` therefore also consumes a
durable revocation proof enforced by a bridge legacy release or an external platform
authority that can deny the frozen binary's serving and write capabilities after restart.
The proof is bound to the selected activation event, release, workspace, and writer
generation. A controller journal records and reconciles this transition but is not itself
...
registered object stores; canonical tables are never opened as raw Parquet listings. One
selector is authoritative: either a previously loaded snapshot whose root/version is
validated against the epoch pin, or a log store plus `with_table_version` with no supplied
snapshot. Supplying both is forbidden because the pinned builder ignores `table_version`
when a snapshot exists. Writes from logical plans receive the epoch's `SessionState` and
require it rather than silently constructing a fallback session. This preserves the same
functions, runtime, object stores, and policy context across read, derivation, validation,
and write.

The model-derived `SchemaContract` separates logical Arrow meaning from Delta/Parquet
physical representation. It validates fixed-width IDs, nullability, nesting, extension
metadata, column mapping, and logical-to-storage casts before writes; reconstructs and
validates the logical schema after scans; and keeps projection/filter/statistics mappings
visible. Empty streams and every emitted batch are checked, not inferred from the provider's
declared schema.
...
forbidden. The pinned `OptimizeBuilder` is not used by the command-owned compaction route
because it performs internal retries. Compaction uses controlled write primitives with zero
commit retries; any conflict returns to `FabricCommand` reconciliation before another legal
attempt is selected.
...
### 3.11A D-30 — Explicit derived-analysis authority and feature closure

Provider extraction and derived analysis are separate compiler phases. Provider adapters
emit only exact native observations. Model-selected application analyses consume those rows
and emit typed `derived` relations; generic plan compilers lower an already defined
calculation but do not substitute for the calculation's semantic implementation.

The initial accepted analysis program has three dependency-closed layers:

1. Python owner-local CFG, evaluation order, reaching definitions/liveness, value flow,
   conservative memory/alias/points-to, exceptional flow, effects, resources, async, and
   explicit unknowns over Ruff structure enriched by Pyrefly semantics;
2. Rust MIR normalization into access events followed by owner-local ownership state,
   reaching definitions/liveness, alias/points-to, drop/resource, async/lowering, unsafe/
   FFI, and explicit unknowns, with exact private borrowck facts kept distinct; and
3. common relational/petgraph graph algorithms and interprocedural effect/resource/callable
   summaries.

Every analysis implementation emits a runtime producer row. A closure query joins the
accepted ontology/query families to producer rows and requires exactly one authority or an
explicit unsupported remainder. Precision, algorithm release, input epoch/projection,
invalidation scope, completeness, and clean-recomputation oracle are part of each output's
meaning. This advances P2, P3, P9–P10, P17–P20, P27, P29, and P36.

### 3.11B D-31 — One model-derived schema contract across logical and physical phases

`SchemaContract` is compiled from model field/type/key/representation rows and the exact
physical binding. It owns:

- Arrow `SchemaRef` and qualified `DFSchema` construction;
- logical-to-storage and storage-to-logical type/cast rules;
- projection, filter, column, and statistics index mapping;
- nullability, nested-child, dictionary, extension-metadata, and column-mapping rules;
- validation after analysis, optimized logical planning, initial/optimized physical
  planning, stream construction, every batch, and every write sink; and
- an explicit empty-stream schema result.

DataFusion/Delta providers may perform file-level adaptation, but only this application
contract restores and proves domain meaning. Adaptation uses native projections/casts when
possible and one generic transparent provider only where the storage seam is irreducible.
This advances P7–P8, P12, P14–P15, P27, P31–P32, and P36.

### 3.11C D-32 — Relation-scoped Arrow IPC multiplexing

Each provider relation travels in an independent Arrow IPC stream with one schema and one
dictionary scope. The Protobuf control plane multiplexes streams by relation/stream identity
and carries schema fingerprint, sequence, source/context pins, flow-control acknowledgments,
and terminal status. A coverage/remainder trailer follows successful IPC end-of-stream for
that relation; truncation, cancellation, corruption, out-of-order frames, duplicate IDs, or
missing trailers yield typed partial/unknown coverage. Control frames never carry semantic
rows. This advances P7, P9, P12, P16, P20, P22, P31, and P36.

### 3.11D D-33 — Rust semantic compilation trust boundary

The Rust semantic lane has a policy-bearing launcher distinct from the compiler adapter.
For untrusted workspaces it provides immutable/read-only source and dependency views,
private target/output directories, no inherited credentials or network, bounded environment,
CPU/memory/time/process/output limits, process-group cancellation, and descriptor-relative
ingress/egress validation. Build scripts and procedural macros execute only inside that
containment. If the platform cannot establish the selected profile, the run fails closed or
emits the explicitly authorized degraded capability; it never silently executes on the host.
This advances P5, P13, P16, P20–P24, P32–P36.

### 3.11E D-34 — Cross-version legacy authority revocation

The transition controller journal records deployment intent and recovery state but cannot
revoke a binary that predates it. Before `NEW_MUTATING`, deployment must install and prove one
of two revocation mechanisms:

- a bridge legacy release that checks a monotonic retirement generation at every serving and
  durable-write ingress; or
- an external platform boundary that revokes the frozen binary's service entrypoint and
  storage/write credential or namespace.

The selected mechanism persists across target crash, process restart, host reboot, and
controller restart. Its receipt binds workspace, old/new release, selected Delta activation
event, and writer generation. The exact frozen legacy executable is then restarted in a
negative oracle and must be unable to serve or write. This advances P3, P11, P13, P16,
P18–P20, P23, P25, P31, P34, and P36.

### 3.11F D-35 — Authorized bound-plan dependency closure

Catalog name filtering is necessary but insufficient because DataFusion logical plans may
already contain bound providers and function implementations. Public views are therefore
compiled from model-level definitions inside the reduced child session. A precompiled view is
accepted only when a recursive verifier proves every table source, nested/subquery view,
scalar/aggregate/window/table function, extension node, variable, and object-store URL belongs
to the `AccessScopeId`. The child has its own allowlisted object-store registry and function/
planner registries. Any unknown bound node fails closed before physical planning. This advances
P3, P5, P13–P16, P21, P23, P31–P32, P35, and P36.
...
DataFusion 55 `scan_with_args`; it forwards any `StatisticsRequest` supplied by a caller
without claiming that DataFusion produces or consumes query-aware requests. The initial
release advertises only ordinary honest provider/plan statistics. A query-aware feature
does not exist until an application producer, mapping, consumer, cache identity, and
observable plan decision are implemented and proved together.
...
**Risk:** extension metadata and fixed-width logical domains can be lost through expressions or Delta/Parquet mappings; the model-derived `SchemaContract` restores and validates physical meaning rather than trusting metadata or a provider declaration
**Validation:** `just relational-arrow-boundary-check` and `just relational-schema-lifecycle-check` must prove relation-scoped IPC, Parquet/Delta, expression, qualified-schema, nested-type, nullability, fixed-width, column-mapping, deletion-vector, empty-stream, and extension-metadata behavior
...
**Decision:** wrap
**Version basis:** DataFusion 55.0.0 `TableProvider`, `ScanArgs`, ordinary plan/provider statistics, constraints, functional dependencies, partitioning, ordering, and pushdown contracts
**Displaces:** domain-specific provider wrappers after one model-derived transparent schema adapter proves equivalent coverage; hand-maintained statistics posture constants
**Risk:** incorrect exact pushdown, stale metadata, or lossy logical/physical remapping changes results; unsupported/inexact is the default until epoch proof establishes stronger claims, and `StatisticsRequest` is transport-only in the initial target
**Validation:** `just table-provider-contract-check`, `just provider-statistics-contract-check`, and `just relational-schema-lifecycle-check` with residual-filter, projection, limit, ordinary statistics, constraint, schema-remapping, and `EXPLAIN` oracles
...
**Risk:** any extension can hide semantics or bypass policy; the compiler selects and proves the narrowest valid rung per operation, with native and bounded `RecursiveQuery` plans ahead of custom nodes, while every surviving physical extension satisfies the complete DataFusion 55 expression/child/rewrite/property/reset/statistics/resource/invariant contract
**Validation:** `just function-runtime-closure-check`, `just graph-extension-conformance-check`, `just graph-execution-contract-check`, and plan-node policy mutation tests
...
**Risk:** Delta has no cross-table transaction or writer election; the exact provider builder also treats a supplied snapshot as authoritative and ignores a simultaneous version selector, while the pinned `OptimizeBuilder` retries internally
**Validation:** `just delta-exact-version-reconstruction-check`, `just fabric-transaction-contract-check`, and `just fabric-activation-recovery-check` must prove mutually exclusive selectors, observed root/version, controlled zero-retry command writes, every commit/acknowledgment fault, and rejection of a second/stale writer before domain writes
...
**Risk:** the petgraph boundary is opaque to DataFusion; it is selected only after native relational/recursive and function rungs are rejected for the compiled operation, relational selection happens in an extension child before materialization, and parameterized providers admit their opacity explicitly
**Validation:** `just graph-extension-conformance-check`, `just graph-execution-contract-check`, rung-selection causal probes, differential reference fixtures, and forced resource/cancellation cases

### LD-25 — Exact current code-fact APIs without defensive semantic abstraction

**Decision:** adopt
**Version basis:** Ruff crates 0.0.7; Pyrefly 1.2.0 at `1933169a...` using exact Query/TSP/module-resolver and deliberately selected Glean/LSP surfaces; Tree-sitter 0.26.12 with Python 0.25.0 and Rust 0.24.2 grammars; `rustc_public` plus a narrow exact-nightly `rustc_private` enrichment seam on nightly-2026-08-18
...
**Risk:** upstream upgrades require deliberate migrations; each fact family must stay on its real exact surface, private compiler facts cannot masquerade as public output, and application analyses cannot masquerade as provider-native output
**Validation:** `just exact-provider-api-check`, `just pyrefly-exact-surface-matrix-check`, `just rustc-public-private-authority-check`, and provider-native Arrow fixture conformance for every modeled family
...
**Risk:** schema/wire skew, heterogeneous relation confusion, and oversized messages; handshake negotiates released control features while one-schema-per-stream Arrow validation and outer relation/stream framing gate every data stream
**Validation:** `just provider-protocol-check`, `just relational-arrow-boundary-check`, `just adapter-test`, and cross-language multiplexing, truncation, corruption, backpressure, cancellation, trailer-order, and partial-coverage tests
...
5. Before shared model/provider/query implementation begins, freeze the exact legacy executable
   or isolated worktree and its decoded comparison contract, and independently accept the model,
   provider, query, public, security, activation, and causal expectation inputs. Later release
   work may re-execute them but cannot first author them.
...
6. Once that acceptance and the explicitly bounded importer rollback decision close, seal the
   migration/review evidence, delete the executable importer and every build/runtime/tool route
   to it, and remove all live reads of the static migration inputs. If exact predecessor bytes
   remain necessary for frozen comparison or old-binary rollback, move only those bytes into a
   non-live archive with no build, runtime, test-generator, or tooling reader; delete that
   archive when its final retention commitment expires.

The importer is `encapsulate-temporarily`: it is never linked into the daemon, its input
tree is frozen, and its executable/live-input path is deleted immediately after the accepted
migration and importer-specific rollback decision. It cannot become the ongoing authoring
path. A retained comparator archive is evidence, never an importable authority.
...
2. Replace Pyrefly's module-level JSON payload with typed context/module/type/member/call/
   import/definition/navigation/diagnostic/remainder relations through the exact Query,
   TSP/module-resolver, selected Glean/internal, and narrowly accepted LSP surfaces. Pin the
   semantic environment and affected-module/reverse-importer invalidation in run meaning.
3. Replace rustc's MIR summary payload with `rustc_public` item/body/block/local/place/
   operand/rvalue/statement/terminator/CFG/call/instance/access relations, a minimal
   `rustc_private` stable-key/source-hygiene/borrowck/selected mono-vtable enrichment, and
   explicit unsupported remainders. Keep application dataflow out of raw relations.
4. Execute the Rust lane through the policy-bearing untrusted-compilation launcher; hostile
   build scripts/proc macros must prove containment before target capability is accepted.
5. Replace Ruff's opaque semantic fields and bespoke normalized DTO chain with direct
   provider-native syntax/semantic relations; retain raw AST/token/trivia distinctions and do
   not label CodeFabric CFG/dataflow as Ruff output.
6. Emit Tree-sitter grammar-native nodes/fields/errors and query captures without losing raw
   kinds.
7. Version the relation-scoped Arrow IPC provider protocol and prove flow control,
   cancellation, schema, source/context, terminal trailers, and completeness semantics end to
   end.
8. Run the separately versioned Python owner-local, Rust MIR-derived, common graph, effect/
   resource, and interprocedural analysis program. Prove one producer or an explicit
   unsupported remainder for every accepted fact/query family.
...
2. Implement the single command reducer, durable overlay segment staging, exact-version
   epoch construction, proof, fenced single-writer activation selection, admission barrier,
   readback, and `ArcSwap` publication. The order is: build and seal the candidate; close new
   admissions; revalidate predecessor and writer fence; append and read back activation; swap;
   reconcile the temporal cache; reopen admissions; acknowledge. Existing leases drain on
   their already pinned predecessor epoch.
...
4. Execute the durable cutover state machine:
   ...
   swaps it, and serves read-only before mutation is enabled. Before `NEW_MUTATING`, the
   controller installs and reads back either the bridge-release monotonic retirement fence or
   the external service/storage authority revocation from D-34, then restarts the exact frozen
   binary and proves that it cannot serve or write. The controller journal records the state
   but is not the enforcement boundary. Every transition is durable, idempotent,
   predecessor-checked, reboot-recoverable, and bound to the exact activation event and writer
   generation.
5. Do not retain a runtime fallback to the old model. Old-binary rollback is allowed only
   ...
   `NEW_MUTATING` is permitted only after the cross-version revocation proof makes the legacy
   writer mechanically unable to regain authority after process or host restart; subsequent
   recovery is forward through the new command path.
...
|---|---|---|---|
| L-20 | `src/bin/codefabric_model/**` and the `model-compiler` feature/bin | **delete** | Replace artifact generation with model replay and generic runtime compilers. Extract only foreign Protobuf build generation and the one-time importer. Delete the importer executable, build/runtime route, feature edge, and live-input readers immediately after Stage 1 acceptance and its bounded importer rollback decision; final comparator-archive bytes are separate L-52 evidence. |
| L-21 | `tooling/model/**`, `scripts/model_*`, DesiredTree sync/plan/repro/family/release tooling | **delete** | Relational replay, catalog closure, causal proof, and package-time foreign generation replace it. Remove the live importer/tooling path at Stage 1 acceptance; later decommission removes unrelated predecessor tooling only after its consumers move. |
| L-22 | `contracts/registry/*.yaml`, `contracts/schema/schema-contract-ir.json`, schema fragments, current policy/comparison/fault/config registries | **encapsulate-temporarily** | Read once as migration evidence with row-level disposition. Never link into the daemon. Exit invariant for live use: accepted migration bijection plus the bounded importer rollback decision, followed by zero build/runtime/tool readers. Only explicitly committed frozen-comparison/old-binary bytes may move to a non-live archive, deleted at final retention expiry. |
...
| L-32 | `pyrefly-sidecar/**`, `src/pyrefly_service.rs` | **reshape** | Preserve process/revision isolation, source/context validation, backpressure, cancellation, and relation-scoped Arrow IPC. Replace one-row opaque payload with exact Query/TSP/module-resolver/selected Glean/LSP relation authorities, semantic-environment identity, affected-module invalidation, remainders, and derived capability proof. |
| L-33 | `rustc-extractor/**`, `src/rustc_service.rs` | **reshape** | Preserve dated-nightly process isolation and control protocol. Replace `OwnedMirItem` summaries with `rustc_public` raw relations, the smallest exact-nightly `rustc_private` stable-key/source/borrowck enrichment, and separately versioned application MIR analyses. No borrowed compiler value crosses the process boundary. |
| L-34 | `src/provider_runtime*`, `provider_types.rs`, `provider_sandbox.rs` | **reshape** | Preserve job/sandbox/transport lifecycle; add the fail-closed Rust build-script/proc-macro trust launcher and derive registrations/capabilities from installed exact adapters and proof, not hard-coded inventories or a claimed sandbox digest. |
| L-35 | `src/fabric.rs`, `fabric/{mutation,publication,snapshot_catalog,serving}.rs` | **reshape** | Preserve exact Delta-version providers, one mutation spine, bounded sessions, and Arrow streams. Rebase around `FabricEpoch`, relational model, activation event, mutually exclusive Delta selector authority, controlled zero-retry writes, and the model-derived logical/physical `SchemaContract`. |
...
| L-41 | petgraph projection DTOs/registries and persisted `NodeIndex`-like identities | **delete** | Select the highest DataFusion rung per compiled operation. Retain petgraph only as a transient bounded kernel for a proved irreducible algorithm behind a fully conforming typed logical extension, or an explicitly opaque planning-time provider for wholly scalar-named inputs, with canonical external IDs and Arrow output. |
...
- no non-live predecessor comparator/archive remains after its explicit retention and rollback
  commitments expire, and no archive reader is reachable before then;
...
| I-22 one Arrow universe | `just relational-arrow-boundary-check`, `just relational-schema-lifecycle-check` | type/nullability/metadata/nested/fixed-width mismatch fails at every logical, physical, stream, batch, and sink boundary |
| I-23 raw fidelity | `just exact-provider-api-check`, `just provider-native-arrow-conformance-check` | every requested native family is rows, explicit remainder, diagnostic, or unknown; application derivations cannot claim provider provenance |
...
| I-26 fenced writer/activation event/current query | `just single-writer-fence-check`, `just fabric-transaction-contract-check`, `just activation-fault-matrix-check`, `just legacy-writer-fence-check` | admission closes before durable selection; second/stale and exact frozen legacy writers fail before serving/domain writes after process and host restart |
...
| I-29 visibility/honest providers | `just table-provider-contract-check`, `just provider-statistics-contract-check`, `just graph-execution-contract-check`, `just plan-visibility-check` | residual filters, ordinary statistics, constraints, rewrites, reset/repeated execution, and nested plans are exercised; no inert request channel is claimed as a feature |
| I-30 exact APIs | `just exact-provider-api-check`, `just pyrefly-exact-surface-matrix-check`, `just rustc-public-private-authority-check` plus extractor/sidecar gates | compile/API/schema/authority drift is explicit migration work, never silently accepted |
...
| I-32 catalog-construction authorization | `just access-catalog-isolation-check`, `just authorized-view-bound-authority-check`, `just public-query-port-check` | forbidden catalog/schema/table/column names, bound providers/functions/stores, and raw provider/session/`DataFrame`/serialized-plan handles are unreachable, not merely row-filtered |
| I-33 explicit derived-analysis authority | `just derived-analysis-authority-coverage-check`, owner-local and clean-recompute semantic fixtures | every accepted analysis family has exactly one runtime producer with algorithm/precision/provenance or an explicit unsupported remainder |
| I-34 semantic compilation trust boundary | `just rustc-untrusted-compilation-sandbox-check` | hostile build scripts/proc macros cannot reach network, credentials, mutable inputs, or unbounded host resources; unavailable containment fails closed |
...
The provider authority matrix is executable. Pyrefly fixtures distinguish Query, TSP/module
resolution, selected Glean/internal indexing, and accepted LSP fallback behavior while varying
the semantic environment and reverse-importer closure. Rust fixtures separately compile-probe
`rustc_public`, the narrow `rustc_private` enrichment seam, and application analyses. A family
cannot pass by appearing under a different authority, and borrowed vendor/compiler values never
cross an adapter callback or process boundary.

Schema proof crosses Rust Arrow, one-schema-per-relation IPC streams, Delta/Parquet,
qualified DataFusion logical schemas, analyzed/optimized logical and physical plans,
streams/batches/sinks, and PyArrow where applicable. It tests IDs, wrong-width values,
decimals, timestamps/timezones, dictionaries, nested lists/structs/maps, nullability,
ordering, metadata, extension registration, projection/filter/statistics remapping, joins,
empty streams, column mapping, deletion vectors, and round-trip failure. Extension metadata
is annotation until an enforcement consumer and fault prove otherwise.

Normalization proof joins raw observations to canonical results and explicit
unknown/conflict rows. Separately versioned Python owner-local, Rust MIR-derived, common graph,
effect/resource, and interprocedural calculations prove their input/output relations,
algorithm/precision release, invalidation, completeness, provenance, clean recomputation, and
independent semantics. Authority mutants swap precedence, relabel a derived row as native,
delete evidence, introduce conflicts, or remove coverage. Raw rows remain available to show why
a normalized or derived result exists.
...
filters, zero and nonzero limits, ordinary statistics, supplied request forwarding without a
fabricated producer, constraints, multiple partitions, cancellation, and schema mismatch.
`EXPLAIN` structure and result equivalence prove that native views expose overlay/derivation
plans without snapshotting fragile plan strings.
...
Each graph operation first proves its selected native relational/`RecursiveQuery`, function,
table-function, or custom-extension rung. Surviving custom nodes expose/rewrite expressions,
replace children, recompute properties, reset mutable state, request/propagate child statistics,
preserve physical invariants, and support repeated execution. Results are compared with small
hand-authored graphs and an independent reference implementation, plus property tests over
permutation, duplicate edges, cycles, disconnected components, empty graphs, bounded depth, and
external-ID round-trip. Resource and cancellation faults terminate actual graph work. No test
compares persisted `NodeIndex` values.
...
Faults cover lease loss, daemon death, timeout after successful commit, timeout before commit,
stale predecessor, forked activation history, duplicate operation ID, corrupt manifest, missing
component/compiler version, readback mismatch, and concurrent admissions before and after every
barrier/selection/readback/swap/cache/reopen/acknowledgment step. The exact frozen legacy binary
is restarted after `NEW_MUTATING` and must be denied by the external or bridge enforcement
boundary. No retry may guess or silently rebase.
...
arbitrary SQL/physical identifiers. Hostile Rust build-script/proc-macro fixtures probe network,
credential, mutable-source, output-path, process, time, CPU, and memory escape under the actual
launcher; lack of platform containment fails closed unless an explicit separately authorized
`TRUSTED_LOCAL` capability is present. An unauthorized catalog name must be as invisible as its
rows. Bound views are recompiled in the child or recursively verified through providers,
functions, extensions, variables, nested views, and a fresh allowlisted object-store registry.
Negative compile- and runtime-path tests must also prove that the public query port cannot receive
or return an internal `TableProvider`, `SessionState`, `DataFrame`, logical/physical plan,
serialized plan, or unvalidated table identifier.
...
| a reduced catalog retains pre-bound authority | child-side view recompilation or recursive bound-plan closure plus fresh object-store/function registries | an allowed view carries a hidden provider, function, extension, variable, nested view, or store URL |
...
| logical meaning is lost at Delta/Parquet storage | one model-derived `SchemaContract` validates/remaps/restores every logical and physical phase | fixed-width IDs, nested/nullability/metadata, column mapping, or deletion vectors cannot be restored without a domain-specific bypass |
| heterogeneous IPC is ambiguous | one schema and dictionary scope per relation stream under an explicit outer control frame | a provider requires unframed mixed-schema batches or cannot make partial coverage explicit |
...
| derived facts are mislabeled or missing | one application-analysis authority and accepted-family-to-producer closure | an accepted analysis family lacks exactly one producer/remainder or needs provider provenance it cannot possess |
| semantic compilation executes repository code unsafely | policy-bearing launcher, fail-closed untrusted profile, hostile fixtures | the supported platform cannot contain build scripts/proc macros and the product requires untrusted extraction there |
...
| cutover revives legacy authority | durable cutover states, admission-before-selection, and bridge/external revocation enforced against the exact frozen binary | old binary can serve or write after `NEW_MUTATING`, process restart, or host reboot, or both writer generations exist |
...
- DataFusion 55 `SessionStateBuilder`, in-memory catalog providers, `ViewTable` and its
  pre-bound logical plan, fresh runtime/object-store registries,
  ...
  functional dependencies, memory pools, spill, task metrics, `RecursiveQuery`, and the full
  logical/physical extension rewrite/reset/properties/statistics contract;
...
- delta-rs revision `43a0cf10...` exact-version table providers, including the rule that a
  supplied snapshot bypasses `table_version`, DataFusion-plan writes with session state,
  atomic log creation through `PutMode::Create`, object-store conditional-create requirements,
  and the pinned `OptimizeBuilder`'s internal retry behavior. These are table-commit
  capabilities, not distributed writer election, cross-table CAS, or command-owned zero-retry
  compaction; and
- exact Pyrefly Query/TSP/module-resolution/Glean/LSP roles, exact `rustc_public` versus narrow
  `rustc_private` roles, and application-owned analyses described by the pinned code-fact
  references and locally resolved current sources.

This integration used the complete v1 audit and refreshed its load-bearing current-source
claims at HEAD. It did not run broad repository validation because no implementation was changed
and the user explicitly said those tests already pass. Focused revalidation commands from the
audit are recorded in the plan v2 integration ledger; missing future recipes remain packet-owned
proof obligations rather than evidence that the design is implemented.

### 7.3 Audit integration log

Source audit:
`docs/reviews/plan_audit_codefabric_execution_proved_relational_data_fabric_implementation_plan_v1_2026-08-29_2026-08-29_v1.md`
(`v1`, verdict `needs-redesign`). Source design/plan: design v1 and implementation plan v1.
Revised artifacts: this design v2 and implementation plan v2. Stable design IDs D-20–D-29 and
I-20–I-32 remain; corrections are amendments plus D-30–D-35 and I-33–I-34.

| Finding | Design treatment in v2 | Plan realization |
|---|---|---|
| F-001 | Successor cannot bootstrap its own governance authority. | External governed candidate-readiness is an entry prerequisite; WP01 self-activation work is removed. |
| F-002 | I-26/D-26 close admission before durable selection and reopen after swap/cache agreement. | WP18 owns the corrected order and concurrent admission fault matrix. |
| F-003 | D-34 requires bridge or external revocation that the frozen binary cannot bypass. | WP21 owns installation, reboot recovery, and exact old-binary restart proof. |
| F-004 | D-22/D-23 distinguish raw from derived; D-30 defines complete analysis authority. | New dependency-closed Python, Rust, common-analysis, and producer-closure packets precede semantic queries. |
| F-005 | D-23/LD-25 select exact Pyrefly Query/TSP/module-resolver/Glean/LSP roles and semantic invalidation. | Pyrefly packet and lifecycle proof use the exact surface matrix and environment identity. |
| F-006 | D-23/LD-25 split `rustc_public`, narrow `rustc_private`, and application analyses. | Rust provider and derived-analysis packets independently prove each seam. |
| F-007 | I-34/D-33 establish a policy-bearing Rust compilation trust boundary. | A dedicated launcher packet precedes the rustc provider and owns hostile fixtures. |
| F-008 | D-28 preserves independent expectations as inputs, never post-hoc products. | An early evidence packet precedes provider, query, security, activation, and release comparison consumers. |
| F-009 | I-22/D-31 define one model-derived logical/physical `SchemaContract`. | A dedicated schema-lifecycle packet precedes catalog/provider, compilation, and Delta replacement. |
| F-010 | I-29/D-24/D-25 select the highest rung per operation and define full custom-plan duties. | Graph packet proves native/recursive/function/extension selection and exact physical conformance. |
| F-011 | I-32/D-35 close bound provider/function/view/extension/store authority. | Authorized child-catalog packet recompiles views or verifies recursive closure with a fresh store registry. |
| F-012 | D-27 uses one exact Delta selector and excludes retrying `OptimizeBuilder`. | Delta packet proves observed root/version and controlled zero-retry command reconciliation. |
| F-013 | D-24/LD-20 remove query-aware `StatisticsRequest` as an initial feature. | Providers forward supplied vocabulary and prove ordinary statistics without an inert producer/consumer claim. |
| F-014 | I-22/D-32 define one schema/dictionary scope per relation stream under an outer frame. | Arrow-boundary and provider packets prove multiplexing, trailers, partial failure, cancellation, and corruption. |
| F-015 | Stage 1 and L-20–L-22 split immediate importer/live-input removal from final archive expiry. | Early DB01 follows M01; final DB08 removes non-live comparator/archive bytes. |
| F-016 | Baseline and current-tree evidence are reconciled to cleanup HEAD `7184b86...`. | Plan v2 records the same baseline and a bounded dirty-tree digest. |

## 8. Acceptance

This dossier selects the corrected clean-sheet target and is paired with a versioned
implementation plan v2. It remains `draft` until focused independent re-audit confirms the
integrated design/plan pair, current declared-input digests, externally governed
candidate-readiness prerequisite, and dependency closure. Concurrent multi-host writers for one
workspace remain outside the target; adding them requires a new design with a proved distributed
fencing protocol.

not-ready — focused re-audit required
