...
working_tree_digest: 41cc717779eba0f1e3ebc253dabec4e8f622f8e2f1c7916569e8935e046a4978
supersedes: docs/designs/codefabric_ontology_compiled_data_fabric_design_v3_2026-08-27.md
...
# CodeFabric ontology-compiled data fabric — target design v4
...
The principal reversals from v3 are explicit:

| v3 decision | v4 decision |
|---|---|
...
- At this pin the public `Expr` and `LogicalPlan` enums can be matched exhaustively. The
  semantic analyzer must not contain a catch-all accepting arm; an added variant at a future
  pin therefore becomes a compile-time upgrade obligation.
...
- **TI-10 — Arrow-native compiled authority.** One content-addressed
  ...
  that carry its digest; none reparses owner authorities or redeclares semantic rows.
- **TI-11 — DataFusion causal execution.** Every mechanically relational validation, closure,
  ...
  contract. Removing or changing the program record changes execution and its receipt.
- **TI-12 — Fail-closed semantic planning.** No executable DataFusion session escapes the
  adapter. Every plan passes one generated policy and domain analyzer. Any unclassified plan
  node, expression, function, domain effect, metadata erasure, or entry path is rejected.
- **TI-13 — Semantic self-description.** A single stable bootstrap relation discovers every
  ontology relation and program. A closure receipt proves authority, schema, content,
...
- **TI-14 — Candidate-bound proof.** Every receipt binds the canonical candidate manifest,
  ...
  plan text, filenames, and WP labels are not proof.
...
- **TI-18 — Bounded shared execution.** Candidate validation and serving use explicitly built
  ...
  scope. A validation candidate uses one session, not one default context per rule.
...
The compiler emits a canonical Arrow IPC bundle under `contracts/generated/` for runtime and
reproducibility use. Delta ontology tables and any generated Rust façade are derived from those
same batches. Generated Rust may contain schema-safe adapters and the one bootstrap address; it
must not duplicate program rows, rule operands, phrase semantics, or function contracts.

The bundle digest covers canonical schema fingerprints, ordered relation identities, and
order-independent content digests. A rebuild from identical inputs is byte-identical. This
...
**Decision.** DataFusion's analyzer becomes the unavoidable runtime enforcement point for the
bundle's generated domain policy, while the bundle remains the single semantic authority.
`SessionContext` and `SessionState` are private to the adapter; callers can execute only an
opaque `GovernedPlan` returned by `OntologyProgramCompiler` or the semantic query compiler.

The installed `FabricSemanticAnalyzer` performs an exhaustive, recursive semantic-effect
analysis after resolution:

1. `TreeNode` walks every nested plan and expression, including subqueries and lambdas.
2. `ExprSchemable::to_field` and full `FieldRef` metadata recover resolved semantic type and ID
   domain wherever DataFusion can represent them.
3. A match over every DataFusion 55 `Expr` variant computes a generated `DomainEffect`:
   `None`, `Preserve`, `ConsumeSameDomain`, `Produce`, or `ExplicitErase`.
4. A match over every `LogicalPlan` variant checks join keys, projections, grouping, windows,
   unions/set alignment, unnest/list elements, subqueries, extensions, and forbidden mutation/
   statement forms.
5. Function calls resolve through the generated calculation catalog. An unknown function that
   consumes a CodeFabric logical domain fails closed.
6. Any expression consuming a logical ID and losing its domain without a declared consuming
   operation is rejected. Casts between domains or to untyped storage fail unless a named
   boundary adapter owns the cast.
...
There is no accepting wildcard arm. Because the pinned `Expr` and `LogicalPlan` enums are
exhaustively matchable, a future DataFusion variant becomes a compile failure that forces an
explicit upgrade decision. `AnalyzerRule` may run more than once; a valid plan is unchanged and
double analysis is an oracle.

Early semantic binding may report the same error before lowering, but it consumes the identical
generated `DomainOperationPolicy`. It is a diagnostic projection, not a second authority.
Direct SQL, arbitrary DataFrames, DDL, DML, `COPY`, and caller-created logical plans are denied
at the adapter boundary and again by plan policy (`LOG-07`, `GOV-03`, `TST-12`).
...
Its address, Arrow schema, and compatibility rules are daemon bootstrap knowledge. Its rows
identify every ontology/program relation by stable relation ID, catalog address, semantic role,
authority, required/optional status, schema contract/digest, content digest, bundle ID, and exact
candidate table identity. No other ontology table name or count is compiled into discovery code.
...
programs. DataFusion executes those programs over the catalog and produces normalized
violations for:

- missing, duplicate, undeclared, or wrong-version relations;
- authority and bundle-digest mismatch;
...
digests, fixture or production-input identity, optimized-result semantic checksum, output
schema, violation digest/count, execution status, executable/code identity, and reproducibility
status. Logical/physical plan artifacts, metrics, and `EXPLAIN` output are retained as
diagnostics but are not semantic identity (`OBS-01`–`OBS-12`; `df-plan` §§55–56).
...
Memory pool, spill directory/factory, task identity, timeout, cancellation, batch size, and
metrics are explicit inputs whose digest enters the receipt. A rule or FK check may not call
`SessionContext::new()` or install its own default runtime. Resource exhaustion returns a stable
resource error and leaves candidate/activation state unchanged. Spill cleanup and cancellation
are operational state, not semantic authority.
...
|---|---|---|---|
| semantic compilation | malformed registry, invalid program graph, unknown calculation | no candidate | never until inputs change |
| semantic analysis | domain mismatch, metadata erasure, unsupported expression/plan form | no execution | never for same program |
...
| resource | memory/time limit, cancellation, spill failure | no semantic mutation | bounded safe retry |
...
**Risk:** treating metadata or IPC bytes as authored policy, or allowing generated projections
to drift. Mitigation: authored authorities remain explicit, every projection carries the bundle
digest, the canonical bundle rebuild is byte-identical, and runtime equality is checked against
the Arrow artifact.

**Validation:** `just ontology-program-compiler-check` and `just model-repro-check`.
...
authority, diagnostics, or receipt identity. Mitigation: one typed lowerer and common violation
schema own those application semantics; built-ins remain visible to the optimizer.
...
phrase or governance authority. Mitigation: built-in-first placement, an explicit function
family/semantics contract, stable function IDs, truthful volatility/null/resource hooks, and a
...
**Risk:** analyzer registration alone does not create domain semantics, and function/type
coercion ordering can erase metadata. Mitigation: the Arrow program supplies policy, all ingress
is sealed, the match is exhaustive and fail-closed, custom functions return full fields, output
schemas are independently validated, and the policy runs before execution.
...
quietly implementing v4 under v2 wording. After this dossier, the planning workflow must create
...
artifact, not a second active authority. Exit invariant: byte-reproducible bundle, complete
authority/input coverage, and no downstream direct registry parsing.
...
- Produce the common violation schema and program execution ledger.
...
result matches independently authored semantics, and changing a record changes behavior.

#### Stage 3 — Sealed ingress and exhaustive DataFusion analysis

- Build the exhaustive `Expr`/`LogicalPlan` semantic-effect analyzer without accepting wildcard
  arms.
...
- Make direct session/plan execution private and add structural governance against new bypasses.
...
Exit invariant: the analyzer ingress matrix covers every authorized route, the compiler proves
...
- Add durable candidate, receipt, owner-decision, activation, and idempotency records.
...
- Publish one non-active candidate containing the Arrow program-backed ontology plane.
- Reopen every exact version and construct the frozen candidate session.
- Execute the semantic closure, validation, compatibility, and resource oracles.
...
| `DomainConformanceRule` partial recursion | **replace** | exhaustive generated-policy analyzer and sealed ingress pass |
...
| `semantic_query.rs` phrase match arms and literal semantic branches | **delete** | registry-only mutation causality and structural zero-state pass |
...
| upfront FAB/LIFE/QRY/SUITE text | **reshape** | v4 target and contracts replace the v3 Stage-2b wording |
...
|---|---|
| TI-10 Arrow-native compiled authority | `just ontology-program-compiler-check`; dual isolated `just model-repro-check` |
| TI-11 DataFusion causal execution | `just ontology-program-causality-check`; mutate every rule/phrase/calculation binding and prove the expected plan/result/diagnostic changes |
| TI-12 fail-closed semantic planning | `just id-domain-plan-enforcement-check`; compile-time DataFusion variant census plus SQL/DataFrame/direct-plan bypass negatives |
| TI-13 semantic self-description | `just ontology-self-description-check`; relation-family corruption matrix and additive-domain fixture with no resolver code change |
| TI-14 candidate-bound proof | `just ontology-candidate-receipt-check`; mutate every manifest/program/session/proof field and require rejection |
...
| TI-18 bounded shared execution | `just ontology-runtime-resource-check`; deterministic memory/time/cancel/spill failures leave durable state unchanged |
...
- For each rule, mutate/remove its program binding and prove exactly that rule's behavior and
  receipt change. This distinguishes causal authority from metadata presence.
...
- Every authorized plan constructor is executed through the installed session analyzer.
...
### 6.4 Closure, activation, and recovery proof
...
- Closure tests corrupt each manifest and relation family separately: address, exact version,
  ...
  result, publication, snapshot, and plan links.
...
### 6.5 Structural governance and zero-state proof

Promote these recurring invariants to tested `rules/` entries:

- no direct DataFusion execution outside the sealed adapter;
...
### 6.6 Final validation posture

The final plan gate includes the new named semantic/activation checks, `just artifacts-check`,
...
**accepted for implementation planning.** The pinned load-bearing APIs have been verified in
the exact local references and resolved source; no target invariant depends on an unselected
probe branch. A new implementation plan must supersede plan v2, preserve the old state as
history, and implement the transition and proof obligations in this dossier without restoring
the rejected duplicate authorities.
