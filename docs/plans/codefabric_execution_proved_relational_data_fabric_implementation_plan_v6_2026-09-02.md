...
working_tree_digest: f85cd5351c74527732996c7892defc6832c46cd041c0a83663a567b98510fbf3
state_path: docs/plans/state/codefabric-execution-proved-relational-data-fabric_v6_state.json
cutover: true
...
# CodeFabric execution-proved relational data fabric -- implementation plan v6

This plan pivots the unfinished v5 program to the accepted compiled-release, provider, fabric, and
runtime boundary design. It assigns new packet IDs because v5 WP49--WP52 describe a different
architecture and cannot be reinterpreted as target completion. V5 WP43--WP48 proving commits remain
historical evidence for functional outcomes that the final v6 candidate must re-execute; their
completion labels are not copied into v6 state. Target-neutral deletions already present in the
...
cut-over production path. Audit, approval, and the confirm-gated activation transaction remain
separate operations.
...
`working_tree_digest` identifies the task-relevant pre-plan binary diff, porcelain status, and the
accepted design digest while excluding the unrelated untracked `Untitled` file. It is identity
only, not correctness or ownership proof.
...
- V5 WP43--WP48 have recorded proving commits. V6 treats them as historical evidence for v2.3,
  released wire, FastMCP 4, and independent expectation outcomes, not as v6 packet completion.
...
Execution records these baseline failures in v6 state when the plan is activated. A later failure
...
The plan depends on the accepted pivot design, the superseded v5 execution specification, the sole
current doctrine, and the synchronized v2.3 authoritative suite. Digests are computed once at plan
authoring time and thereafter only recomputed by artifact tooling.
...
The implementation sequence intentionally migrates provider APIs to application-owned jobs before
the production release compiler becomes authoritative. This avoids a noncompiling intermediate
`semantic-release` feature. Only tests construct fixture jobs during that interval; the old route
remains the sole production path until WP61, and no dual production execution is introduced.
...
WP53 application contracts and feature substrate
  +--> WP56 cancellation/task ownership -----------------------------------+                |
                                                       +--> WP54 state split --> WP55 fabric +
                                                       +--> WP57 Tree-sitter/Ruff jobs -----+
                                                       +--> WP58 rustc boundary ------------+
                                                       +--> WP59 Pyrefly lifecycle ---------+
                                                                                           v
              WP54 + WP55 + WP56 + WP57 + WP58 + WP59 --> WP60 compiled release
...
WP56 follows WP53 because it owns the direct cancellation dependency and runtime task contract.
After WP56, WP54--WP55 and the three provider packets may proceed as disjoint workstreams. WP57,
WP58, and WP59 require WP56 because their terminal lifecycle proof exercises that cancellation
contract. WP53 owns the shared provider-neutral admission/recipe seam; later provider packets do
not edit it, so their known-touch sets are genuinely disjoint.
WP60 is the first milestone that composes every inward product; WP61 is the only production
cutover. FreshActivation and DB23 closure precede performance so the measured candidate is the
actual final topology. Any repair or tuning returns to the owning implementation packet and
invalidates all dependent WP63--WP65 evidence before WP66.
...
provenance, resource bounds, and synchronous cancellation probes. It imports minimal Arrow
array/schema types but no provider library, DataFusion, Delta, gix, SQLite, Tonic, daemon, or
supervisor. Contract construction and tamper faults are executable. No production provider path is
...
3. Define immutable lane jobs that bind requested families/scopes, exact identities, effective
...
4. Define one terminal result shape for owned relation batches/streams, requested/completed/
...
5. Separate release semantics, compiled policy ceilings, and per-run effective values. Establish
   ...
   shared seam.
6. Add `provider-job-contract-check` and the provider-contracts slice of
   `feature-architecture-check`. Both include negative fixtures that would fail on a provider-native
   field, generated-message field, forbidden dependency, widened bound, wrong pin, or false complete
   terminal.
7. Derive contract observations from constructed values. Do not add a generated registry, static
...
transport authority; operational SQLite state cannot select semantic table state. Existing exact
Delta publication, readback, reconstruction, activation, and unknown-outcome behavior remains
causally green.

**Dependencies.** WP53 and WP56.

**Target invariants.** I-60, I-63, I-65, I-66, I-69, and I-71.
...
- `just feature-architecture-check state` proves independent useful feature graphs and forbidden
  dependency-family absence.
...
- `just delta-exact-reconstruction-v4-check` and
  `just delta-durability-protocol-integrity-check` prove exact restart and protocol behavior.
...
**Packet-Local Gates.** `just feature-architecture-check state`; `just delta-publication-contract-check`;
...
irreducible semantic relation truth, delta-rs lacks the exact readback/reconciliation seam, or the
feature split requires a new package rather than same-package boundaries.
...
**Integration Milestone.** Completes M13 with WP53--WP54.
...
**Dependencies.** WP53. The state/fabric workstream may execute in parallel, but shared Cargo and daemon
...
3. Replace unowned spawn and abort/drop-without-join behavior in production paths with registered
   task scopes. Terminal and expiry cannot merely remove a `JoinHandle`.
4. Drain in order: stop admission, signal the parent, reserve cleanup time, join children, release
   reservations/leases/provider/result resources, persist required terminal state, and close
   services.
5. Keep `CancelQuery` idempotent and durable across reconnect. Process escalation occurs only after
   a bounded cooperative-cancellation timeout.
6. Add `cancellation-tree-check` with parent/child propagation, sibling isolation, replay,
...
1. Introduce application-owned accepted compilation/owner headers and terminals containing only the
   validated categorical pins, stable application identities, counts, coverage, diagnostics, and
...
3. Route source generations through provider jobs and preserve the sidecar's provider-reported
   ...
   and bound.
4. Bridge the structured cancellation tree to cooperative run cancel. Escalate to process-group
   termination only after the bounded acknowledgment deadline; implement a real shutdown signal so
   service drain stops and joins the serving loop.
...
`just provider-ipc-contract-integrity-check`; `just relation-ipc-provider-operations-check`;
`just provider-trust-coverage-remainder-check`; `just cancellation-tree-check`;
`just sidecar-ci-fast`.

**Integration Milestone.** Completes M14 with WP56--WP58.
...
generic engine in this same packet. The isolated `semantic-release` feature performs useful
provider-to-proof behavior without repository input, operational state, RPC, or supervisor.

**Dependencies.** WP53, WP54, WP55, WP56, WP57, WP58, and WP59.
...
5. Make release-owned preparation the only production-capable constructor for provider jobs and
   release-owned admission the only conversion into candidate epoch inputs. Fixture release
   definitions remain test-only and explicit.
6. Add the target `semantic-release` feature with the accepted fact-generation + data-fabric
   dependency edges and no repository/operational/RPC/supervisor edge.
...
- `just feature-architecture-check semantic-release` proves the isolated useful feature graph and
  one closed release definition.
...
- `just semantic-request-program-check` executes all eight query forms through the compiled program
  and isolated fabric with bounded streaming behavior.
...
`just provider-job-contract-check`; `just data-fabric-core-check`;
`just semantic-request-program-check`; `just feature-architecture-check semantic-release`;
`just stable-graph-check`.
...
**Dependencies.** WP54, WP55, WP56, WP57, WP58, WP59, and WP60.
...
**Integration Milestone.** Completes M15 with WP60.
...
5. Replace `RFV5_*`, v5-plan-bound selectors, gate-census entries, and current-tooling imports with
   target v6 behavior or remove them. Immutable v5 plans/states/proving commits remain history but
...
6. Preserve independently authored FastMCP expectations only when their semantic clauses remain
   valid; bind and execute them anew against v6. Do not treat a v5 evidence result as v6 proof.
...
clauses executed anew against v6.
...
**Outcome.** At one committed candidate, every v6 packet oracle, milestone, decommission batch, retained v2.3
...
no semantic code. Only then may governed v6 state mark the plan complete.
...
git status --short --untracked-files=all
just plan-status
just plan-dependency-check docs/plans/codefabric_execution_proved_relational_data_fabric_implementation_plan_v6_2026-09-02.md
...
4. Re-execute valid v5 WP43--WP48 functional expectations against v6 where still applicable. Do not
   import their completion statuses, old production evidence, or proving results as v6 proof.
...
6. Add `relational-fabric-v6-certification` as a transparent aggregate that lists every child,
...
- `just relational-fabric-v6-certification` includes and preserves every behavioral child in §7.
...
Executable oracle: `v6_certification_artifact_and_ancestry_integrity`
Governed criterion: `PC-WP66-INT`

Executable oracle: `v6_complete_successor_behavior_matrix`
Governed criterion: `PC-WP66-BEH`

Executable oracle: `v6_legacy_and_failure_masking_zero_state`
Governed criterion: `PC-WP66-NEG`

Executable oracle: `v6_terminal_release_certification`
...
**Packet-Local Gates.** `just relational-fabric-v6-certification` and every command in §7, followed by an independent
implementation review on the unchanged candidate.

**Integration Milestone.** Completes M17 and the v6 implementation program.
...
**Rollback or Recovery.** A failed terminal gate leaves v6 executing. Repair through the owning packet, produce a new
...
### M13 — Inward contracts, state separation, and generic fabric are coherent

**Entry:** v6 is activated with a fresh state file and WP53 is ready.

**Packets:** WP53, WP54, and WP55.

**Completion:** Provider contracts are application-owned and useful; fabric/repository/operational
features are separated; the generic Arrow/DataFusion/Delta core executes synthetic behavior and
losslessly preserves structured scans.

**Gates:** All packet-local gates for WP53--WP55, especially `feature-architecture-check`,
`data-fabric-core-check`, `datafusion-scan-contract-check`, and
`delta-publication-contract-check`.

### M14 — Provider and async lifecycles are owned and independently useful

**Entry:** M13 complete for shared contracts; WP56 may have proceeded in parallel.

**Packets:** WP56, WP57, WP58, and WP59.

**Completion:** Every async task is owned/joined; Tree-sitter/Ruff use job-driven Arrow; rustc
generated types terminate at ingress; Pyrefly retains bounded compatible state and reconstructs
explicitly.

**Gates:** `cancellation-tree-check`, `inprocess-provider-lifecycle-check`,
`rustc-provider-lifecycle-check`, `pyrefly-incremental-lifecycle-check`, provider/type/IPC gates,
and extractor/sidecar domain gates.

### M15 — One behavior-bearing release is the sole live production route

**Entry:** M13 and M14 complete.

**Packets:** WP60 and WP61.

**Completion:** The compiled release owns actual programs and categorical identity; one injected
`Arc` reaches all application services; thin Tonic adapters preserve the released wire; no
production selector reaches the old route.

**Gates:** `release-program-contract-check`, `compiled-suite-identity-check`,
...
- `just governance`
- `just ci-pr`
- `just relational-fabric-v6-certification` **new, WP66**
...
1. Independently audit this draft. Apply material corrections in a successor plan artifact, approve
   ...
   schema-v2 v6 state and atomically switch the active pointer. Preserve v5 plan/state as history.
2. V6 state contains only WP53--WP66, M13--M17, and DB19--DB23. It copies no v5 completion status,
   milestone status, batch status, proving commit, or check result. V5 proving commits remain
   discoverable historical evidence.
3. Execute WP53 first and freeze feature/contract names needed by parallel work. Attribute every
   dirty overlap; preserve target-neutral WP49 deletions without claiming them complete.
4. Execute WP56 after WP53. Then execute WP54 and the three provider packets as disjoint
   workstreams; execute WP55 after WP54. Serialize only shared Cargo, `src/lib.rs`, rule, test, and
   `justfile` integration.
5. Execute WP57, WP58, and WP59 after WP53/WP56. Parallel implementation uses isolated worktrees;
   the provider-neutral contract/admission/recipe seam established by WP53 is immutable during
   these packets, while shared test and gate-tooling integration merges serially.
6. Execute WP60 only after M13/M14 inward products are coherent. Compile the release and move the
   release-specific proof/query seam in one transaction; no inert program product is accepted.
...
This draft remains inactive. Activation must validate this exact file and declared inputs, verify
baseline ancestry and the dirty-tree inventory, create a new schema-v2 state at the declared v6
path with only WP53--WP66/M13--M17/DB19--DB23, atomically replace the active-plan pointer, and leave
v5 artifacts immutable. A failed activation leaves v5 selected and creates no partial v6 authority.
...
transaction may then mark v6 complete.
