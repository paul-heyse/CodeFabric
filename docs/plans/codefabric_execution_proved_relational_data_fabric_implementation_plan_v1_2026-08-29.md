...
date: 2026-08-29
status: draft
design_path: docs/designs/codefabric_execution_proved_relational_data_fabric_design_v1_2026-08-29.md
design_version: v1
baseline_commit: bcee3f0ae0618231357bfef91ae59403acf61fed
state_path: docs/plans/state/codefabric-execution-proved-relational-data-fabric_v1_state.json
cutover: true
supersedes_on_activation: docs/plans/codefabric_ontology_compiled_data_fabric_implementation_plan_v3_2026-08-28.md
---

# CodeFabric execution-proved relational data fabric — implementation plan v1

This plan realizes the accepted clean-sheet design as one governed replacement program.
...
schema-version-2 state file and does not change `docs/plans/active-plan.json`. After independent
plan audit and explicit approval, `just plan-activate` must create and validate the state before
atomically replacing the active pointer. Until that transaction, the current v3 plan and runtime
remain the only active execution authority.
...
3. Tree-sitter, Ruff, Pyrefly, and `rustc_public` expose every accepted current API family as
   typed provider-native Arrow relations with requested/completed coverage, remainders,
   diagnostics, provenance, and explicit unknowns. Pyrefly and rustc retain their justified
   process/toolchain isolation; semantic payloads use Arrow IPC rather than opaque JSON or
   row-per-message Protobuf.
...
8. The cutover reaches `NEW_MUTATING` only after independent comparison and then proceeds
   forward-only. Completion requires all L-20 through L-55 dispositions and DB01 through DB07
   exit invariants: the former model compiler, generated registries, opaque provider payloads,
   old serving/activation routes, static governance, packaging residue, and compatibility
   fallbacks are unselectable and absent outside immutable history.
...
The planning baseline is `bcee3f0ae0618231357bfef91ae59403acf61fed`. The accepted design was
reconciled at `4e94c22ad28e6b5a01b9eb3f55db74fc0d81d7fb`; three intervening commits changed
the predecessor ontology generator and compiled products (`b40b087`) and then bound serving
epochs to durable predecessor authority across fabric, snapshot, operational-store, ontology,
query, and integration-test code (`292ad66`). The final cleanup commit `bcee3f0` then corrected
the governed activation-count expectation in
`tests/integration/ontology_datafabric_cutover.rs`. These changes enlarge and clarify the frozen
migration, comparison, test, and purge surface but do not change the target decisions. They are
therefore planned against, not silently inherited as target architecture. At this baseline there
are no modified tracked implementation files; the design, v2 principles, this plan, and the
predecessor review are untracked planning inputs/artifacts. WP01 still reconciles the actual
commit and working tree before any implementation edit because cleanup may continue after this
draft.

The earlier pre-plan `just ci-fast` probe at the design-reconciled tree exited 1 at `root-fmt` on
then-pre-existing ontology/schema edits. Those edits have since been reconciled outside this
planning artifact, so that exact failure is historical evidence rather than a claimed current
failure. The user reported ongoing cleanup that can temporarily make tests unreliable. WP01
must capture a fresh activation-time baseline and execution state must distinguish its failure
fingerprints from packet regressions; no eventual acceptance gate is weakened or waived.

The accepted design also records that `just stable-graph-check` did not reach graph validation
because the dated extractor nightly failed to resolve. WP01 preflight must run `just doctor`
and `just stable-graph-check`; exact-provider packets cannot be completed while that environment
...
| path | sha256 |
|---|---|
| docs/designs/codefabric_execution_proved_relational_data_fabric_design_v1_2026-08-29.md | f1d7a451b3dfd103b61aabb9d0a6395d870690f85a8013d2c84a27e4fb85de27 |
...
The planning session used `git diff --stat <design-reconciled-head>..HEAD -- <design
scope>`, targeted `ast-grep outline` over all four build domains, and hidden-aware `rg`
candidate searches over source, contracts, tests, tooling, CI, rules, packaging, and live agent
instructions. The intervening committed drift is confined to predecessor machinery already
selected for migration or removal and does not invalidate the design. Current-tree evidence
confirms these known-touch clusters; they are navigation evidence, not an exhaustive must-touch
manifest:
...
- **I-22 — One Arrow universe.** Arrow 59.2.0 typed schemas and IPC carry semantic data across
  Rust domains; opaque JSON is limited to true external or human-authored seams. Advances P7,
  P12, P16, and P32.
- **I-23 — Raw fidelity before normalization.** Exact provider-native typed observations,
  remainders, diagnostics, and coverage remain queryable beside canonical facts. Maintains the
  suite's raw/normalized and absence-is-not-proof doctrine; advances P2, P9, and P20.
...
- **I-25 — One mutation path.** Every durable fact/model/publication/maintenance change is an
  ...
  P33–P34, and P36.
- **I-26 — Fenced writer and event-derived current.** One local daemon writer appends proved
  activation events; Delta history is semantic authority and SQLite/ArcSwap are reconstructible
  caches. Advances P3, P11, P18–P19, and P34; risk of multi-host demand is design-reopened.
...
- **I-29 — Optimizer and validator visibility.** Native nodes are preferred; every unavoidable
  provider/extension reports DataFusion 55 scan, expression, child, statistics, resource, and
  cancellation contracts honestly. Advances P14–P16 and P25.
- **I-30 — Exact current APIs.** Ruff 0.0.7, Pyrefly 1.2.0 at the pinned revision,
  Tree-sitter 0.26.12 with pinned grammars, and nightly-2026-08-18 `rustc_public` are direct
...
- **I-32 — Authorization by catalog construction.** Public requests see only an epoch-pinned
  reduced child catalog; no internal provider, session, DataFrame, plan, or identifier handle
  crosses the query port. Advances P5, P13, P21, P32, and P35.
...
None. A bounded preactivation substep may change only plan-governance validation and the overlap
ledger needed to validate this inactive candidate; it produces a reviewed commit but no runtime,
design-authority, state-file, or active-pointer change. The remainder begins only after this plan
is audited, approved, and activated through the confirm-gated state transaction.
...
git diff --stat bcee3f0ae0618231357bfef91ae59403acf61fed..HEAD -- AGENTS.md README.md CLAUDE.md .claude docs/authoritative_design docs/spec_index contracts tooling/ci scripts justfile .github rules rule-tests src tests fuzz Cargo.toml codefabric-cpg-mcp
```
...
authority shape. `docs/plans/active-plan.json` currently selects the approved v3 plan, whose state
is `executing` with WP19 in progress and later packets stale at the planning baseline.
`contracts/governance/plan-overlap-dispositions.yaml` and
`tooling/ci/{plan_assurance,artifact_contracts}.py` currently key overlap decisions only by reused
packet IDs, allowing one plan's record to alias another plan's packet semantics.
...
- Before activating this successor plan, require an accountable owner decision that stops the v3
  plan at a coherent boundary and explicitly preserves, migrates, supersedes, or rejects its
  in-progress WP19 work and every stale downstream obligation. Activation may create new state
  and replace the pointer only after that decision; it cannot silently adjudicate predecessor
  work. Add `predecessor-plan-transition-check` to validate the durable disposition.
- In the preactivation substep, migrate the overlap-decision contract to a plan-qualified key:
  `plan_id + plan_version + resource + unordered packet pair`. Backfill existing decisions to
  their actual owning plans; do not delete historical phase rationale merely because its packet
  IDs recur. Update parser/schema/tests so only the selected plan's records participate in missing
  and stale calculations, and reject cross-plan aliases.
- Reshape `plan-dependency-check` to accept an inactive candidate plan path while retaining active
  default behavior. Make `plan-activate` run candidate structure, acyclic dependency, known-touch
  overlap, oracle-catalog, input, and predecessor-transition assurance before it creates state or
  replaces the pointer. Add `plan-overlap-ledger-check` for schema/backfill/alias proof. The
  preactivation substep is complete only when the inactive draft passes the same dependency
  algorithm and the existing active plan still passes under its qualified records.
- Replace the current state-then-pointer activation sequence with an idempotent, journaled
  `PlanActivationTransaction`, persisted as a versioned execution record under
  `docs/plans/state/activation-transactions/<transaction-id>.json`. Its immutable key is
  successor plan ID/version/plan digest plus
  the expected predecessor pointer digest, predecessor plan/state identity, and predecessor state
  digest. Under one OS-backed activation lease, durably write and read back `PREPARED`, create or
  reuse only an exactly matching successor state, atomically compare-and-swap the active pointer,
  durably link the predecessor state to its accepted supersession disposition, then mark the
  journal `COMPLETE`; fsync every file and containing directory at its publication boundary.
  Recovery must converge from every prefix, including an existing matching successor state,
  refuse any mismatched state or changed predecessor, and preserve the active pointer as the one
  execution authority throughout. Inject crashes after journal preparation, successor-state
  publication, pointer replacement, predecessor-state linkage, and completion readback. Add
  `plan-activation-recovery-check`; a retry may resume the exact transaction but may never treat
  an orphan state file as a second active plan or require its manual deletion.
  Evolve execution-state schema with an optional typed `supersession` record containing successor
  plan/state identities, activation transaction ID, accepted predecessor-decision reference, and
  pointer-switch evidence. Only after the pointer CAS does recovery set the predecessor overall
  status to `superseded`; it preserves every packet judgment as historical evidence rather than
  relabeling unfinished work complete.
...
- `just predecessor-plan-transition-check` — new; proves the active v3 packet/state disposition
  was accepted before successor activation.
...
- `just plan-overlap-ledger-check` — new; proves plan-qualified keys, historical backfill, no
  cross-plan alias, and exact missing/stale closure for active and candidate plans.
- `just plan-activation-recovery-check` — new; fault-injects every durable activation prefix,
  proves idempotent recovery, predecessor CAS, exact successor-state reuse, durable predecessor
...
- `just plan-dependency-check docs/plans/codefabric_execution_proved_relational_data_fabric_implementation_plan_v1_2026-08-29.md`
  — reshaped candidate-path form and mandatory activation preflight.
...
- `just plan-overlap-cross-plan-alias-zero-state-check` — new.
...
- Executable oracle: `just plan-activation-recovery-check`
...
before approval if the platform cannot supply the required atomic replace, durable directory
sync, or single-controller lease; never paper over a partially published successor state.
...
git diff --name-status 4e94c22ad28e6b5a01b9eb3f55db74fc0d81d7fb..HEAD -- contracts src tooling/model tooling/proto rules tests
```
...
`tooling/model/**`, and the committed predecessor changes through `bcee3f0` supply current
migration evidence. The dirty L-54 bytes recorded by the design were reconciled before this plan's
baseline; execution must determine which associated semantic decisions/artifacts remain in HEAD or
accepted history rather than pretending those working-tree edits still exist.
...
Encapsulates L-22 and L-54; preserves L-44 and L-52; begins L-55 replacement. DB01 deletes the
importer and frozen current authorities only after the target is `NEW_MUTATING` and the old
...
released commitments require a longer compatibility window. Reopen the design if a retained
...
Design D-23, LD-17, LD-20, LD-25, LD-26, §§6.4–6.5; Arrow 59, DataFusion 55,
...
- Define provider run/schema/coverage/remainder/provenance relations and a versioned Arrow IPC
  stream envelope with source/context pins, bounded batches, flow control, cancellation, and
  typed corruption/compatibility failures.
- Establish the separately owned `ProviderBoundaryContract` port, review transaction, and
  ownership rule before any exact adapter packet is ready. Contract rows are human-reviewed,
  typed inputs; an adapter or generated adapter output cannot author its own expected surface.
...
```text
Protobuf control frame + Arrow schema/batch stream + terminal coverage/diagnostic frame
```
...
Design D-21–D-22, LD-18, LD-20; DataFusion 55 catalog, `SessionStateBuilder`, `ViewTable`,
`TableProvider`, `ScanArgs`, statistics, constraints, and runtime APIs.
...
- Implement one structured `plan_scan(ScanArgs)` path for every non-native provider. Forward
  projection, filters, limit, and `statistics_requests`; report pushdown, constraints,
  functional dependencies, ordering, partitioning, and statistics only after proof.
...
references, CFG/dataflow observations, diagnostics, raw kinds, and remainders.

**Dependencies.**

WP04 and WP06. This exact-provider lane may proceed in parallel with the other independently
...
- Map current Ruff parse/tokens/trivia/typed AST/indexer/semantic surfaces to typed raw relations
  without copying them through opaque semantic JSON or a lowest-common-denominator DTO.
...
The pinned Pyrefly sidecar emits typed context, module, definition, type, member, import,
call-target, diagnostic, unresolved, and remainder relations through bounded Arrow stream IPC.
The current module-level JSON payload no longer participates in the target route.
...
- Bind directly to the pinned `Query` APIs for type tables, callees with location, members,
  imports, cross-module context, and diagnostics; map every accepted family to typed Arrow.
...
- Consume independently authored and accepted boundary rows/fixtures; the sidecar implementer
  ...
  `pyrefly-provider-native-check` and `pyrefly-exact-api-check` aggregate recipes.
...
- Executable oracle: `just pyrefly-exact-api-check`
- Executable oracle: `just pyrefly-opaque-payload-zero-state-check`
- Executable oracle: `just sidecar-ci-fast`
...
The dated-nightly extractor emits typed compilation, item, stable-identity input, MIR body,
block, local, place, operand, rvalue, statement, terminator, CFG, call, instance, span,
ownership/dataflow, diagnostic, and remainder relations. Names/counts/debug strings no longer
stand in for available compiler facts on the target route.

**Dependencies.**

WP04 and WP06, plus a resolved dated-nightly environment. This exact-provider lane may proceed
in parallel with the other independently owned exact-provider lanes in isolated worktrees.

**Target invariants.**

I-22, I-23, I-29, and I-30. Advances P2, P4, P7, P9, P20, and P24; maintains compiler-private
process isolation and application-owned canonical identity.
...
- Bind directly to pinned `rustc_public` item/MIR/instance/dataflow surfaces. Retain native
...
- Derive canonical Rust identity only from application-owned stable inputs such as
  `StableCrateId + DefPathHash`; keep MIR locals/blocks and provider handles run-local.
...
- Consume independently authored and accepted boundary rows/fixtures; the extractor implementer
  ...
  `rustc-provider-native-check` and `rustc-exact-api-check` aggregate recipes.
...
- Executable oracle: `just rustc-exact-api-check`
...
I-20, I-23, I-24, I-27, I-29, and I-30. Advances P2, P4, P9–P10, P14–P16, P20, P24, P27,
P29, and P36; maintains raw/normalized coexistence, provider authority, and explicit unknowns.
...
- Compile normalization, canonical identity, per-family authority, conflict, explicit-unknown,
  ...
  facts and retain conflicting evidence.
- Implement every target `TableProvider::scan_with_args(ScanArgs)` path so projections, filters,
  limits, and optimizer-requested statistics are accepted or honestly reported. Forward
  statistics rather than using a default method that drops the channel; prove a production
  optimizer request reaches each custom provider.
...
- `just provider-statistics-contract-check` — reshaped here to exercise production optimizer
  requests and `ScanArgs` forwarding across every exact target provider.
...
### WP12 — Compile bounded graph analyses as a DataFusion logical extension

**Outcome.**

Model-selected graph analyses participate in DataFusion logical and physical planning as bounded,
typed operators over relational inputs and Arrow outputs. petgraph is a transient algorithm
kernel; canonical IDs, resource accounting, cancellation, statistics, and deterministic results
remain owned by the fabric.
...
I-21, I-24, I-27, I-29, and I-32. Advances P2, P14–P16, P25, P27, P29, and P35; maintains
fact-substrate doctrine by emitting derived facts rather than evaluative judgments.
...
- Define a typed `LogicalPlan::Extension` using `UserDefinedLogicalNodeCore` for each accepted
  bounded graph-analysis family, with model-derived operation selection, relational inputs,
  canonical external IDs, exhaustive expressions/children, output schema, stable equality/hash,
  and explain formatting.
- Register the matching `ExtensionPlanner` in every internal physical-planner construction path.
  Whenever it lowers node-owned expressions, it must forward the received
  `PhysicalPlanningContext` unchanged into DataFusion's physical-expression construction; a
  default context is forbidden in production because it loses scalar-subquery and planner-owned
  state. The execution node must reserve memory, honor cancellation, declare
  partitioning/order/statistics honestly, bound input/output, and return Arrow batches
  deterministically.
...
- `just graph-extension-conformance-check` — new; compares extension output with independently
  expected facts for representative algorithms.
...
- Executable oracle: `just graph-extension-planner-registration-check`
...
`just graph-extension-context-forwarding-check`; `just graph-resource-contract-check`;
`just root-clippy`; `just root-test`.
...
WP11 and WP12, with the generic compilers from WP06.

**Target invariants.**

I-21, I-24, I-28, I-29, and I-32. Advances P5–P8, P13–P16, P21, P27, P31–P32, and P35;
...
Design D-21, D-22, D-24, D-29, LD-18–LD-21, §§3.12 and 6.7–6.8; DataFusion 55 catalog,
...
- Construct a new reduced `MemoryCatalogProviderList`/catalog/schema graph and explicitly install
  only allowed functions, planners, variables, object stores, runtime options, and metadata.
...
- Add the design-named `access-catalog-isolation-check` and the focused
  `epoch-session-seal-check` recipe here.
...
- Executable oracle: `just epoch-session-seal-check`
...
WP11, WP12, WP14, and WP16, using WP05's catalog foundation. WP14 first establishes the sealed
...
Design D-21, D-22, D-26, D-27, LD-17–LD-22, §§3.12–3.13 and 6.6; exact Delta revision,
...
- Define the new physical relation/partition contract independently of legacy table layouts.
  ...
  no selected write, DML, compaction, or optimize path enables delta-rs internal retries: a
  ...
  legal. Reject an incompatible or missing session rather than silently constructing a fallback.
  Record exact committed versions; never discover latest during epoch construction.
- Load exact snapshots with `DeltaTableBuilder::with_version(...).load()` or
  `DeltaTable::load_version(...)` as appropriate, then use `DeltaTable::table_provider()` and
  `TableProviderBuilder::{with_snapshot, with_session, with_table_version}` to register the pinned
  revision's kernel-backed provider/`DeltaScanExec` path and model-generated `ViewTable`
  definitions. The
...
markers, zero internal retries, or `RequireSessionState` writes as required. Revise physical
...
set. The single writer stages, proves, commits, reads back, selects, crosses an admission barrier,
and swaps one `Arc<FabricEpoch>`; crash recovery derives the unique valid head and reconciles every
unknown outcome.

**Dependencies.**

WP15, WP16, and WP17. Public query/result delivery, the exclusive command path, and exact durable
relations must all exist before activation/pinning can be proved dependency-closed.
...
- Implement the ordered publish protocol: stage data; execute proof; append activation event;
  read back exact event and versions; close admission; drain old admissions as required; swap one
  epoch; reopen admission; acknowledge. No query observes a partial step.
...
- `just activation-fault-matrix-check` — new; injects every named crash point and reconciles to
  one valid outcome.
...
- Executable oracle: `just activation-route-exclusivity-check`
...
```text
stage -> prove -> append activation -> read back -> admission barrier -> ArcSwap -> acknowledge
```
...
I-21–I-27, I-29, and I-31–I-32. Advances P3–P5, P11–P12, P16, P18–P22, P25, P31,
...
- Lower invalidation/update work into `FabricCommand`s pinned to source generations and provider
  coverage. Coalesce safely, cancel superseded work, and activate only a fully proved epoch.
...
### WP20 — Produce independent semantic, causal, security, and performance release evidence

**Outcome.**

An independently owned corpus proves model meaning, all exact provider families, all semantic
request forms, public delivery, authorization, failure handling, and intentional old/new deltas.
Expectations are decoded and reviewable; causal interventions show that model/proof rows affect
behavior; benchmarks and security probes validate the accepted operational envelope.

**Dependencies.**

WP15 and WP19, which close M03 and M04 respectively. Independent expectation/causal ownership
must be established before implementation authors populate this packet's accepted evidence.

**Target invariants.**

I-20–I-32. Advances P4, P9–P10, P20–P21, P27, P29, and P36; maintains independent proof,
...
- Establish independent owners and reviewable typed expectation relations for bootstrap/model
  semantics, each provider contract, normalization/authority/unknown behavior, every public query
  form/composition role, authorization/redaction, errors, status/reference content, and result
  lifecycle. Store decoded expected rows and rationale, not producer digests.
...
- Preserve a frozen read-only legacy executable or isolated old worktree before shared build/source
  changes destroy it. Run old and new against identical frozen inputs; use independent expectations
  as the oracle and classify every delta. Legacy output is comparison evidence only.
...
- Establish measured latency, memory, spill, throughput, and update-amplification budgets before
  tuning. Add the M05 readiness aggregate and its independent-semantic, causal, comparison,
  security, and performance recipes here.
...
- `just expectation-independence-check` — new; rejects producer-generated expected rows and
  production-to-oracle dependency edges.
- `just comparison-engine-isolation-check` — new; proves the legacy engine is frozen/read-only.
...
`just relational-fabric-performance-check`; `just package-interop-check`.
...
serves read-only before mutation; `NEW_MUTATING` irreversibly fences the legacy writer; no runtime
...
I-20–I-32, especially I-21, I-25–I-27, and I-31. Advances P3, P11, P18–P22, P31,
P33–P36; maintains exactly one mutation and serving authority through every transition.

**Design and library references.**

Design §§5.1–5.3 and 6.6–6.9, D-21, D-26–D-29, LD-22–LD-23 and LD-26; all L-20–L-55
exit dependencies.
...
- Make one external Rust `CutoverController` in the existing stable package the sole writer of a
  ...
  cross-version crash-recovery wire contract, not semantic model authority.
...
- Make `NEW_MUTATING` a durable irreversible fence for the legacy writer. Thereafter recover only
  through compatible prior/new corrective epochs on the target command path.
...
Establishes when every L-20–L-55 deletion becomes safe. It does not itself delete history or
predecessor bytes; DB01–DB07 perform consumer-first removal after `LEGACY_RETIRED`.
...
- `just legacy-writer-fence-check` — new; proves old binaries/generations cannot mutate after
  `NEW_MUTATING`.
...
- Executable oracle: `just deployment-transition-semantic-authority-zero-state-check`
...
Reopen D-26 if authority cannot remain singular in any transition, an external controller must
write fabric facts, or rollback after new mutation is required. Revise the plan if deployment
...
bootstrap/release replay; independently reviewed row-level import disposition; Arrow schema/IPC
contracts; model-only catalog closure; native compiler causality; relational proof and provenance.

**Gate:** WP07 adds `just relational-model-foundation-check`, an aggregate that runs the WP01–WP07
packet gates without mutating artifacts. No target provider or production mutation is claimed.

### M02 — Exact provider fabric is complete and honest

**Packets:** WP08–WP11.

**Evidence:** all accepted current API families have independent boundary contracts, typed native
relations, IPC integrity, requested/completed coverage, normalized/canonical coexistence,
provenance, explicit unknowns, capability proof, and optimizer-visible statistics.

**Gate:** `just exact-provider-fabric-check`.

### M03 — Authorized semantic delivery is end-to-end

**Packets:** WP12–WP15.

**Evidence:** bounded graph extensions, all semantic forms/compositions, sealed reduced child
catalogs, policy/redaction, dynamic catalog reference/status, Arrow result resources, and a real
FastMCP-to-daemon vertical with no public physical surface or packaged semantic authority.

**Gate:** `just semantic-delivery-vertical-check`.

### M04 — Durable epoch reconstruction and recovery are proved

**Packets:** WP16–WP19.

**Evidence:** exclusive command ingress, writer generation fencing, exact Delta versions, native
overlay views, immutable activation events, crash reconciliation, resource governance, lifecycle
integration, and clean/incremental reconstruction with legacy inputs unavailable.
...
authority, and irreversible legacy fencing at `NEW_MUTATING`.
...
**Batches:** DB01–DB07 after `LEGACY_RETIRED`.
...
pass; no legacy loader/generator/import/activation/query/package/fallback path remains; all four
build domains and feature graphs are green.

**Gate:** DB07 adds `just relational-fabric-final-certification`.

## 6. Cross-packet decommission batches

### DB01 — Retire the one-time importer and frozen migration inputs

**Prerequisites:** M01 accepted; the row-level migration and independent expectation evidence are
immutable; the cutover rollback window no longer requires rerunning the importer; retained epochs
reconstruct from migrations and compiler releases. L-22 and L-54 selectors must be fresh at the
batch proving commit.

**Disposition:** delete the importer executable/feature/tests and its live access to frozen YAML,
schema IR, fragments, policy/comparison/fault/config registries, predecessor bundle artifacts, and
the semantic evidence reconciled between the design baseline and `bcee3f0`. Preserve accepted
`ModelMigration`/`ModelDecision` rows, independent review, released allocations, and historical
commits. If a released artifact remains retained, move only its immutable evidence to the accepted
archive and remove all build/runtime/tool reads.
...
`just retained-epoch-compatibility-check`;
...
migration under a new compiler release, not reactivation of predecessor inputs.
...
and every target-route compatibility decoder. Preserve short-lived exact vendor bindings,
...
authorized catalogs, result lifecycle, FastMCP topology, and only a foreign-build cache whose
...
### DB07 — Detach history from live reads and certify total purge
...
legacy daemon is retired and physically unavailable to clean-rebuild proof.
...
history. Remove every active runtime/build/test-oracle/generator/package/status/pointer read from
that archive. Delete obsolete retained data only under accepted tombstone/retention policy. Add
the aggregate `relational-fabric-final-certification` recipe and remove transition-only checks
whose forbidden state is now structurally impossible, while retaining permanent residue guards.
...
- Environment and artifact integrity: `just doctor`, `just artifacts-check`,
  ...
  `just v2-authority-cutover-check`, `just plan-overlap-ledger-check`,
...
- Exact providers and Arrow: `just exact-provider-fabric-check`,
  ...
  `just relational-arrow-boundary-check`, `just extractor-ci-fast`, `just sidecar-ci-fast`.
- Query, policy, and delivery: `just semantic-query-relational-conformance-check`,
  ...
  `just graph-extension-context-forwarding-check`,
  `just semantic-delivery-vertical-check`,
...
Activation prerequisites are outside the immutable packet DAG: independently audit this draft,
integrate any blockers into a new plan version or this still-draft version, obtain explicit
approval, adjudicate the active v3 plan's in-progress WP19 work, and complete WP01's bounded
preactivation governance substep. That substep plan-qualifies existing overlap decisions, proves
the active and inactive candidate DAGs independently, and makes candidate dependency assurance a
hard precondition of `plan-activate`; it cannot alter runtime or design authority. Only then may
the confirm-gated, journaled activation transaction create or exactly reuse schema-version-2
state, CAS the predecessor pointer, durably link the predecessor disposition, and recover every
write prefix while the pointer remains the sole active execution authority.
  ...
  -> WP02
       ...
       -> WP04
            -> WP05 -> WP06 -> WP07 ------------------------------+
                       ...
                       |       +-> WP10 --+          |              |
                       |                             v              |
                       |                        WP12 -> WP13 -> WP14 -> WP15 -> M03
                       |                          |                  |        |
                       +-> WP16 -> WP17 ---------+------------------+-> WP18 -> WP19 -> M04
                                                                               |
                                                                M03 + M04 -> WP20 -> WP21 -> M05
                                                                                 |
                                        DB01 -> DB02 -> DB03 -> DB04 -> DB05 -> DB06 -> DB07 -> M06
```

WP08–WP10 may run in parallel only in isolated worktrees with independent provider-contract
ownership. WP16 may begin after M01 while provider/query work proceeds, but WP17 cannot accept
provider data before M02 and WP19 cannot complete before M03. No decommission batch starts before
`LEGACY_RETIRED`; within decommission, consumers precede authorities and dependency removal.
...
- **Current cleanup and baseline drift.** `b40b087` materially expanded the legacy generated
  surface, `292ad66` added durable predecessor serving-epoch authority, and `bcee3f0` corrected
  the governed activation-count oracle; the user reports continued cleanup. Every packet reruns
  current-tree preflight; baseline failures are fingerprinted, never waived. Drift into a frozen
  legacy authority is a discovered obligation and may require plan revision before edits.
- **Partial plan activation.** Successor state, pointer, and predecessor disposition are separate
  durable writes. The journaled activation transaction, expected-predecessor CAS, fsync/readback,
  and prefix fault matrix are approval prerequisites; an orphan state is recoverable evidence,
  not permission to delete or activate it manually.
...
- **DataFusion extension/provider honesty.** Default provider methods, copied sessions, or hidden
  custom nodes can discard optimizer, security, statistics, resource, or cancellation contracts.
  Production-path requests and reduced-catalog negative probes are mandatory.
- **Independent proof contamination.** Importers, producer output, legacy results, and generated
  Gate B artifacts cannot author expected semantics. Ownership, decoded review, dependency-edge
  checks, and causal controls are cutover prerequisites.
- **Legacy comparison availability.** Shared-source changes can destroy the old reference engine.
  WP20 needs a frozen read-only executable or isolated old worktree before that occurs; it may
  compare but never serve as a runtime fallback or truth oracle.
...
- **Deployment-journal ambiguity.** The controller journal governs only deployment phase while
  Delta governs semantic current. A mismatched head/record, writer generation, selected epoch,
  activation event, owner/mode, or process observation fails closed; neither authority may repair
  the other by guessing.
...
- **Cross-plan packet-ID aliasing.** Current overlap governance keys records by resource and
  packet IDs only. WP01 plan-qualifies and backfills the ledger before activation, proves both
  active and candidate dependency closure, and later replaces the transition ledger with
  relational plan/phase decision facts. A stale or missing record blocks activation.
- **Cutover irreversibility.** `NEW_MUTATING` forbids old-binary rollback. The state machine,
  writer-generation fence, independent semantics, crash matrix, and forward recovery must be
  accepted before that transition.
...
- **Implementation adaptation:** exact helper/module names, native plan shapes, batch sizing,
  ...
  through D-29 and LD-17 through LD-26. Record the adaptation and evidence in execution state.
...
is uncovered/overlapping/unparsed, old/new comparison lacks independent truth, retained data
cannot be classified, or authority would be dual at any point. Test instability caused by the
current cleanup is a recorded baseline/blocker, never grounds for marking work complete.
