...
design_version: v1.0
predecessor_plan_path: docs/plans/codefabric_waves_4-7_core_facts_implementation_plan_v3_2026-08-22.md
baseline_commit: b3cb347e6e7fdc4bca13851ee8775385f11630d6
state_path: docs/plans/state/codefabric-waves-4-7-core-facts_v4_state.json
audit_path: docs/reviews/plan_audit_codefabric_waves_4-7_core_facts_implementation_plan_v1_2026-08-21_2026-08-21_v1.md
cutover: true
---

# CodeFabric Waves 4–7 Core Facts — Implementation Plan v4

Version 4 is the user-approved execution successor to v3. It preserves the audited
WP27–WP53 outcomes, dependencies, milestone boundaries, decommission obligations, and
proof matrix and the v3 Rust-floor correction. Its additional correction closes the
WP29 replan trigger by adding the design-required `resource_profile_id` to the released
`ProviderJobSpec` as field 15. The change is additive on the Protobuf wire, flows through
the single-FDS compilation unit, and gives the runtime a closed registry identity to
resolve before admission; it introduces no second profile authority or provider-specific
wire model.
...
2. **A proven vertical architecture**: Readiness Gate B passes exactly as `SUITE` Part V
   ...
   accepted immutable `gate-b-v1` coverage profile inside the candidate corpus
   `tests/golden/codefabric-golden-v1/` (Wave 5, M06). The corpus itself is not
   described as fully released until all AC-G-78 profiles are complete.
...
tables, generated provider raw-kind catalogs and normalization mappings, the provider
resource-profile registry, a Rust `AnalysisContext` runtime type, any provider
executor/adapter, the golden
...
| Path | SHA-256 |
|---|---|
| docs/upfront_design/codefabric_1.3_implementation_roadmap_v1.0.md | b9c8bf769d40960645fc924ca2e60894e674ea7c5309a54f9ac85213cc3f5f51 |
| docs/upfront_design/codefabric_present_state_cpg_suite_governance_and_release_manifest_v1.3.md | 5143896a5da6f0b913318586ce10169c32200d99ea675a8f1742458dc3078c92 |
| docs/upfront_design/code_property_graph_present_state_fact_ontology_specification_v1.3.md | b7cda018600da4a434c5f3f211cc171040f73f12b55fc0ee8ad3fb6ffafebb89 |
| docs/upfront_design/present_state_cpg_fact_generation_specification_python_rust_v1.3.md | bd818f37454f2397be4852286d8053c49fb4b2a532e7cfe82b764d3cc812972c |
| docs/upfront_design/present_state_cpg_data_fabric_specification_rust_arrow_datafusion_deltalake_v1.3.md | cf739c36e118af8b13506057b870a6a037152256e4b457dd86ca804f46765169 |
...
| docs/upfront_design/code_property_graph_semantic_query_specification_v1.3.md | 014ce032045736f108a74a12c2f6c95798c80332ed11b9064ab7c79b5317e412 |
...
| docs/plans/codefabric_waves_4-7_core_facts_implementation_plan_v3_2026-08-22.md | c80220676086b3b39e1ccc1438c825194cdddb903222fdf8524315bef0369efc |
...
| docs/reviews/plan_audit_codefabric_waves_4-7_core_facts_implementation_plan_v1_2026-08-21_2026-08-21_v1.md | a6a7d8f3445375033812ca9a6ea43731db910364ab488dfca6831d4ce4f48670 |

### 2.1 Precedence

1. Normative owner in the current 1.3 design suite (`SUITE AC-G-01` ownership map).
2. Accepted audit-integrated implementation decisions in this plan (§5).
...
## 3. Baseline, adoption, and implementation disposition

Baseline HEAD is b3cb347e6e7fdc4bca13851ee8775385f11630d6 with a clean tree. It contains
the proven WP27 and WP28 outcomes and the v3 state record that stopped WP29 on the
protocol-shape replan trigger. The executor re-runs `just ci-fast` at execution
preflight and records pre-existing failures in state before any product edit
(repo-spec §59.1; `validation-policy.md §3`).
...
schema-2 state at the frontmatter path with exactly WP27–WP53, M05–M08, and
DB07–DB09. Because WP27 and WP28 were proven under v3, v4 state imports those judgments
and proving commits and keeps WP29 as the current packet. Before WP29 resumes, the
executor SHALL make this v4 artifact active in every generic plan/status/validation
entry point and prove that both explicit validation of this plan/state pair and the
argument-free defaults resolve v4. This adoption prerequisite is execution
infrastructure, not a product work packet; if it is not complete, WP29 is blocked.
...
exercise most are restated with the additions I-29–I-40:
...
- I-35 (new): provider-native raw-kind inventories are generated from pinned library
  surfaces; only normalization mappings are authored. Every upstream key is mapped,
  explicitly ignored, or rejected by an exhaustive generated boundary.
- I-36 (new): every provider job resolves one closed resource-profile record before
  admission. Work, memory/output, concurrency, retained-revision, deadline, and
  cancellation limits are enforced by the owning provider and reflected in terminal
  capability evidence.
- I-37 (new): Cargo launches the rustc extractor through
  `RUSTC_WORKSPACE_WRAPPER`; the wrapper receives the real rustc path/arguments and
  reports to the daemon-hosted protocol. Compiler caching cannot suppress extraction.
- I-38 (new): Wave-5 rustc execution is limited to the committed golden fixture under
  an explicit `TRUSTED_LOCAL` grant with network and credentials removed. Arbitrary or
  untrusted repositories fail closed until Waves 9–11.
- I-39 (new): accepted golden coverage profiles are immutable even while the complete
  AC-G-78 corpus remains candidate and append-only.
- I-40 (new): deprecated `UpdateWaveState` codes remain decode-only; new writers and
  generated transitions emit only the accepted 17-state vocabulary.
...
- **D-23 — Integrated plan, wave-segmented execution.** One plan covers Waves 4–7 by
  ...
  `RM §2.1` and the accepted integrated-program rule in `RM §28`; each wave exits
  through its milestone before the next wave's packets start.
...
- **D-25 — `fact-generation` build capability.** The parser/provider stack enters the
  ...
  `daemon`, hence `local-workstation`), keeping narrow graphs parser-free exactly as
  the corrected `FAB §2.1` feature map requires.
- **D-26 — `git_state` is a module boundary, not a crate.** The subsystem remains the
  isolated application module behind `repository-state`, enforced by the existing
  `gix-boundary-only`/`gix-read-only` rules and the corrected `LIFE §37`/`§155` one-
...
- **D-28 — Parse-error/missing-syntax dual surface.** One adapter fact derives all
  ...
  (`GEN §16.5`, `FAB §19`–`§20`). `MISSING_SYNTAX`
...
- **D-29 — Enum-registry authority for `FreshnessState`.** The generated enum registry
  (`ONT §62.7` via `SUITE AC-G-06`) is authoritative and includes
  `AWAITING_CURRENT`; WP45 generates one vocabulary rather than forking states.
...
- **D-31 — Periodic generic audit.** Per corrected `LIFE §55.1`, audit after every
  bulk reconcile and on a configurable idle interval; divergence forces an
  authoritative rescan and sets `GIT_DEGRADED` until reconverged.
- **D-32 — Stabilization tuple hardening.** Use the inclusion-policy and attributes
  fingerprints in the corrected `LIFE §62` tuple; a mid-switch
  `.gitignore` or attributes change destabilizes the bulk wave.
- **D-33 — Git DTO naming and path forms.** `GitCandidateDelta` (LIFE §156/App F) is
  canonical in `LIFE §70`/Appendix F. The byte-preserving `LIFE §43` path
  representation is used consistently. Supporting DTOs are application-owned inside
  the `git_state` boundary.
...
- **D-35 — Watcher pin.** `notify-debouncer-full =0.7.0` and `notify =8.2.0` are
  exact `LIFE §2.2` pins; 0.8.0 release candidates are not targeted.
- **D-36 — Rust grammar pin.** `tree-sitter-rust =0.24.2` with grammar ABI 15 is the
  exact `GEN §2`/`FAB §2.1` pin and is recorded in the toolchain bundle. Later
  movement is a provider upgrade with generated-catalog diffs and fixture gates.
...
- **D-39 — Derived raw-kind catalogs.** Provider-native inventories are generated by
  catalog derivation units into `contracts/generated/provider-raw-kinds/*.json` from
  Tree-sitter `NODE_TYPES`/`Language` and exhaustive Ruff enum adapters. Only
  `contracts/registry/provider-normalization/*.yaml` is authored (`ONT AC-G-70`,
  `SUITE AC-G-05`).
...
- **D-42 — Active-plan adoption is a stop gate.** The first executor action creates
  schema-2 state, points generic plan tooling at the current successor, and proves
  explicit and default validation/status resolution before product execution.
  Integration does not manufacture state.
- **D-43 — Cargo-native extractor topology.** The daemon launches Cargo with
  `RUSTC_WORKSPACE_WRAPPER`; the wrapper receives the real rustc invocation, invokes
  `rustc_public`, and streams owned events to a daemon-hosted bidirectional
  `RustcExtractor.Observe` route generated from the shared FDS. Provider analysis
  bypasses sccache so extraction cannot disappear behind a compiler-cache hit.
- **D-44 — Wave-5 trust staging.** WP35 is golden-fixture-only under explicit
  `TRUSTED_LOCAL`, with network/credentials removed and a pinned resource profile.
  Arbitrary and untrusted compiler execution remains unavailable until Waves 9–11.
- **D-45 — Provider resource profiles.** The generated provider registry references
  one closed resource profile governing source/work/deadline/output/depth/concurrency/
  retained-tree/cancellation limits and bounded retry policy. Every provider enforces
  it before terminal output.
- **D-46 — Golden profile lifecycle.** WP34 creates a candidate corpus and WP40
  accepts the immutable `gate-b-v1` profile. Later waves append profiles; they do not
  mutate Gate-B bytes or claim full AC-G-78 release.
- **D-47 — Update-wave registry migration.** WP42 retains exact matching codes,
  appends the new 17-state vocabulary, makes old `RUNNING`/`PUBLISHING`/`COMPLETE`
  decode-only, inventories persisted rows, and closes DB09 before scheduler activation.
- **D-48 — Ruff-compatible stable Rust floor.** The stable-domain compatibility floor
  is `1.95.0`. This is the maximum of the pinned delta-rs minimum (`1.94.1`) and the
  exact Ruff `0.0.7` provider-train minimum (`1.95`). Cargo metadata, the toolchain
  contract, generated identities, documentation, MSRV proof, and graph governance must
  agree. This user-approved correction supersedes only v2's inherited `1.94.1` floor;
  it changes no provider, packet boundary, or runtime contract.
- **D-49 — Provider job resource-profile identity.** `ProviderJobSpec` carries the
  design-required `string resource_profile_id = 15`. Application ingress requires a
  non-empty value that resolves exactly once in the generated provider-resource-profile
  registry before admission. The additive field preserves existing Protobuf field
  numbers and unknown-field compatibility, is generated by the sole `grpcio-tools` FDS
  compilation unit and consumed by Rust through `compile_fds`, and does not create a
  second profile authority. This correction realizes `GEN §90` and `AC-G-32`, advances
  doctrine P10/P11/P12/P16/P29/P31, and closes v3's WP29 protocol-shape replan trigger.
...
- **LD-21 — tree-sitter core + Python grammar: adopt.**
  ...
  mitigated by exact pins, generated raw-kind catalogs (D-39), and fixture corpora.
...
- **LD-22 — tree-sitter-rust: adopt `=0.24.2` (D-36).**
...
- **LD-23 — Ruff component crates: adopt.** Version basis: `=0.0.7` component crates
  of Ruff 0.16.1 (`GEN §2`): `ruff_python_parser =0.0.7`,
  `ruff_python_ast =0.0.7`, `ruff_python_trivia =0.0.7`,
  `ruff_python_index =0.0.7`, `ruff_source_file =0.0.7`, and
  `ruff_text_size =0.0.7`. No moving sibling selection remains.
...
- **LD-25 — gix profile retain-and-verify.** Retain the already-complete exact
  `=0.86.0`, `default-features = false` profile (`sha1`, `sha256`, `index`, `status`,
  `attributes`, `excludes`, `dirwalk`, `blob-diff`, `interrupt`, `parallel`,
  `auto-chain-error`, `tracing`). WP49 verifies the resolved graph and API probes; it
  does not reopen the manifest absent a concrete failed capability probe.
...
- **LD-27 — Rayon: adopt `=1.12.0`.** Activate only through `fact-generation` and use
  dedicated fixed-width provider pools, never an implicit global-pool contract.
  Stable-graph and narrow-feature gates prove the boundary.
- **LD-28 — Extractor transport families: reuse exact repository pins.** The separate
  extractor package uses `prost =0.14.4`, `prost-types =0.14.4`, `tonic =0.14.6`,
  `tonic-prost =0.14.6`, and Arrow/Arrow-array `=58.4.0`, matching the stable root,
  with bindings from the one shared FDS. It introduces no second protoc, descriptor
  authority, or independently versioned wire family.
...
### 6.2 Accepted specification corrections

Every behavior- or authority-changing issue found during the v1 audit is corrected in
its normative owner before its consumer packet begins. Editorial observations may be
recorded without blocking; implementation decisions do not override contradictory
design text.

| ID | Corrected owner | Accepted resolution |
|---|---|---|
| SI-01 | GEN §2 / SUITE AC-G-07 / FAB §2.1 | exact `tree-sitter-rust =0.24.2` ABI-15 pin and toolchain recording |
| SI-02 | FAB §2.1 | closed `fact-generation` feature with exact parser/Ruff/Rayon dependencies |
| SI-03 | SUITE AC-G-05/Part IV; ONT AC-G-70 | generated raw-kind catalogs plus authored normalization mappings |
| SI-04 | FAB §18/§19 | explicit `token_id` and `annotation_id` primary keys |
| SI-05 | RM §10 | `CompilationBegin`/`OwnerBegin`/`OwnerEnd`/`CompilationEnd` vocabulary |
| SI-06 | ONT §62.7 | `AWAITING_CURRENT` included in the sole freshness vocabulary |
| SI-07 | LIFE §30 | `RECONCILE_REQUESTED` defined as coordinator flag, not lifecycle state |
| SI-08 | LIFE §2.2/§37/§155 | exact gix pin and one-package `git_state` module architecture |
| SI-09 | LIFE §9.1/§62 | inclusion-policy and attributes fingerprints join stabilization tuples |
| SI-10 | LIFE §70/App F | `GitCandidateDelta` and byte-preserving path DTOs used consistently |
| SI-11 | LIFE §55.1 | mandatory post-bulk/configurable-idle audit and divergence recovery |
| SI-12 | LIFE §2.2 | exact watcher/debouncer pins |
| SI-13 | QRY §47 / SRV AC-G-65 | overlapping public errors use the AC-G-65 registry names |
| SI-14 | GEN §15 | `.py`/`.pyi` core-source scope; notebooks emit explicit `UNSUPPORTED` evidence |

### 6.3 Audit Integration Log

Audit source: `docs/reviews/plan_audit_codefabric_waves_4-7_core_facts_implementation_plan_v1_2026-08-21_2026-08-21_v1.md`.
The exact revalidation commands are the fenced commands under each cited audit finding;
integration runs them verbatim after all edits and records their exit status here.

| Finding | Disposition | Affected artifacts/sections | Rationale and evidence | Revalidation |
|---|---|---|---|---|
| F-001 | applied; adoption-gated | Plan §3, §14, §16 | successor-plan adoption is an explicit stop gate; integration never fabricates execution state | `just artifacts-check`; `just plan-status` |
| F-002 | applied | GEN AC-G-31; D-43; WP35 | Cargo-native wrapper, daemon-hosted bidi transport, one FDS, and sccache separation close the topology | exact command exit 0 |
| F-003 | applied | GEN AC-G-35; D-44; WP35 | Wave-5 compiler execution is golden-only, explicitly trusted, credentialless, offline, resource-pinned, and otherwise fail-closed | exact command exit 0 |
| F-004 | applied | all eight design owners; §2, §5, §6.2 | behavior-changing SIs moved into normative owners; RM §28 now permits explicitly authorized wave-segmented program plans | exact command exit 0 |
| F-005 | applied | GEN/FAB/SUITE; LD-22/23/27/28; WP27/29/31/35 | exact grammar, Ruff, Rayon, and shared transport families replace moving or conditional choices | exact command exit 0 |
| F-006 | applied | ONT AC-G-70; SUITE AC-G-05; D-39; WP28/30/31 | provider inventory is generated from library-native surfaces; only normalization is authored | exact command exit 0 |
| F-007 | applied | GEN AC-G-32; D-45; WP29–WP31 | one typed resource-profile authority drives native parser/query/cancellation limits and no-partial publication | exact command exit 0 |
| F-008 | applied | SUITE AC-G-78; D-46; WP34/WP40 | candidate corpus plus immutable accepted `gate-b-v1` profile avoids a partial corpus masquerading as released | exact command exit 0 |
| F-009 | applied | WP35/WP38/WP46 dependencies; §15 | shared golden and coordinator writes are serialized explicitly | exact command exit 0 |
| F-010 | applied | LIFE §21; D-47; WP42; DB09 | append-only state migration, persisted-row census, deprecated-write zero state, and guard are explicit | exact command exit 0 |
| F-011 | applied | LD-25; WP49 | existing complete gix feature graph is retained and verified without speculative manifest churn | exact command exit 0 |
| F-012 | applied; audit-command typo noted | packet gates; §14 | mandatory mutation obligations use named argument-free recipes; no prose pseudo-command remains | verbatim nested-quote command malformed; equivalent fixed-string zero-state command exit 0 |

Integration evidence: plan frontmatter/digest/ID/oracle validation found 27 packets,
four milestones, three decommission batches, and four unique oracles per packet;
`just contracts-verify`, `just contracts-verify-released`, and
`just contracts-repro-check` exit 0 after regenerating the authority-derived artifact
index and requirement provenance. Full plan/state validation intentionally remains the
§3 adoption gate because the state artifact does not exist during integration.
...
`tree-sitter-rust =0.24.2`, the six exact Ruff analysis crates `=0.0.7`, and
`rayon =1.12.0` (LD-21/22/23/27), composed into `daemon` →
...
The §3 active-plan/state adoption prerequisite (first product packet; M04 substrate).

#### Target Invariants

I-08, I-10, I-16 (v5), I-36, D-25, D-36. Doctrine P6, P8 maintained.
...
retain the pinned crates' default feature sets and enable no additional optional
features; do not add Ruff semantic/formatter/codegen crates;
record pins in the toolchain bundle through the contracts machinery (mutating recipe,
diff-reviewed); prove the pinned Python and Rust grammars report ABI 15 and remain
compatible with `tree_sitter::LANGUAGE_VERSION`; assert Rayon appears only in intended
feature graphs.
...
`Parser::set_language` accepts each pinned `LANGUAGE` and grammar ABI versions match
the pinned core.
...
(asserted inside `stable_graph_check.sh`); Ruff formatter, codegen, and semantic crates
remain absent from the Wave-4 graph.
...
The exact 0.24.2 grammar fails the accepted ABI-15 probe, or duplicate-family conflicts
...
`owner_bucket` partitioning (`FAB §95.2`), mutation axes
...
field-role codes (`GEN §16.2` names), a closed provider resource-profile registry,
authored provider-normalization mappings, and provider-raw-catalog derivation units
whose outputs live under `contracts/generated/provider-raw-kinds/*.json` (D-39);
a Rust `AnalysisContext` runtime model
...
I-08, I-11, I-13, I-29, I-35, I-36; `ONT AC-G-70`; `SUITE AC-G-05`.
Doctrine P10, P12, P29 advance.

#### Design and Library References

`FAB §7`, `§9`–`§11` (TableSpec axes + round-trip gate §11.1), `§17`–`§20`, `§92`,
`§95.2`; `ONT §5`–`§6`, `§62`–`§64`, `AC-G-70`; `GEN §16.2`, AC-G-32;
`SUITE AC-G-05`; D-28, D-39, D-45.
...
Author the IR/registry, resource-profile, and normalization source edits; extend the
single typed compilation-unit model with a provider-raw-catalog derivation kind whose
provenance binds exact provider/package version, grammar ABI, `NODE_TYPES` digest,
runtime inventory/fingerprint, query-bundle digest, input identities, and outputs.
Generate each Tree-sitter catalog from `NODE_TYPES` plus `Language::node_kind_count`,
named/visible/supertype/subtype and `field_count` inventories; define the exhaustive
Ruff mapping-generator contract consumed in WP31. Bind the resource-profile registry
and generated raw catalogs into the provider bundle so provider runs, metrics, and
snapshot pins identify the same accepted profile/catalog digests. Run `just contracts-gen` and
...
maxima; generated raw-kind catalogs contain zero canonical ontology codes; every raw
key has exactly one normalization/ignore/unsupported disposition; new TableSpecs
declare PKs and all four mutation axes. A grammar fingerprint change emits a stable,
reviewable full-inventory diff.
...
closed `resource_profile_id` from every `ProviderJobSpec` resolved and enforced before
admission; the
...
I-04, I-05, I-14, I-36; `LIFE §22` provider-run states; `GEN §86` generation output
...
First add `resource_profile_id = 15` to `contracts/rpc/provider_control.proto`, regenerate
the single FDS and all Rust/Python bindings through `just --yes proto-gen`, and update
the reviewed compatibility baseline through the repository's explicit Proto governance
workflow. Prove descriptor parity, prior-wire compatibility, and rejection of a missing,
empty, unknown, or provider-incompatible profile at application ingress. Then implement
executor/admission/supersession/cancellation; bind generated DTOs to an
...
observation ingress) with negative fixtures. Resolve the typed resource profile and
construct a dedicated fixed-width Rayon pool per configured provider class; reject
unknown/mismatched profiles and resource estimates above hard bounds before `RUNNING`.
...
deadline; stale-generation output rejected with `STALE_RESULT`; a descriptor-driven
round trip preserves field 15 while bytes produced before field 15 still decode.
...
with tested fixtures); no handwritten provider DTO or second profile lookup authority;
missing, empty, unknown, and mismatched profile identities are rejected before permits.
...
emitted; bounded-channel backpressure observable under a slow consumer; profile ID and
every limit outcome are recorded without unbounded diagnostic payload.

#### Edit-Local Gates

`just proto-check`; `just proto-repro-check`; targeted nextest slice on the new module;
`just governance-scan`.

#### Packet-Local Gates

`just ci-fast`; `just mutants-wp29` (argument-free recipe added by this packet for the
post-discovery supersession/admission source file).
...
Another required provider-job field or a non-additive protocol/package change is found;
the single-FDS compatibility baseline cannot accept field 15 without weakening an
existing invariant; or application ingress cannot resolve the closed profile identity
without a second authority → stop and revise the plan/design.
...
or an explicit capability outcome. Parser/query work, live tree revisions, and output
are bounded by the accepted provider resource profile; cancellation cannot publish a
partial tree/capture set as complete.

#### Dependencies

WP27, WP28 (normalization mappings and generated raw-kind catalogs). From WP29 only the accepted
job-contract surface (DTO/event shapes); the live executor integration lands in
WP32/WP33, so WP30 may run parallel to WP29.

#### Target Invariants

I-10, I-29, I-30, I-35, I-36; `GEN §96` ("raw provider kind remains recoverable",
...
Grammar registration uses `Parser::set_language` as the authoritative runtime ABI
check, then proves startup equality against the full generated `NODE_TYPES` +
`Language` catalog and grammar fingerprint; parse-tree walk producing `RawSyntaxFact`
rows with raw kinds resolved through generated hot-path ID lookups; normalized-kind
mapping from authored records; error/missing/extra flags; byte-range validation against
`SourceImage` bounds; incremental `InputEdit` application + changed-range surfacing.
Use worker-owned parser/query-cursor pools; `ParseOptions::progress_callback` enforces
work/deadline/cancellation and `parser.reset()` follows every cancelled parse. Query
execution uses `QueryCursorOptions`, `match_limit`, bounded byte ranges and explicit
range/depth/node/output caps. Retain only the configured number of tree revisions and
atomically commit a complete new revision after all validation. Add repetitive, deep,
large, cancellation, valid, partially invalid, and malformed fixture families.
...
with the runtime inventory exactly equal to its generated catalog. An unrecognized key
is a provider-version mismatch, never silently dropped or string-fallback mapped.
...
Resource fixtures hit parse/query/node/depth/output/retained-tree bounds, acknowledge
cancellation, discard partial captures, and leave the last complete revision active.
...
`just ci-fast`; `just mutants-wp30` (argument-free recipe added by this packet for the
post-discovery kind-normalization source file).
...
The in-process Ruff provider adapter (`GEN §7.2`) retains exactly one complete Ruff
`Parsed` value per source revision and constructs `TriviaRanges`, `Indexer`, and
`LineIndex` once for reuse across all Wave-4 projections. It emits the Wave-4 members of
...
(governance rule `ruff-boundary-only`). Whole-source Ruff parsing is non-incremental:
source/work/depth/output limits are checked before and during bounded visitors; if the
underlying parse cannot be interrupted, cancellation is acknowledged and the completed
stale parse is discarded rather than published.
...
I-10, I-29, I-30, I-35, I-36; `GEN §96`; `ONT §4.2`–`§4.4`. Doctrine P6, P11 advance.
...
Parse once via the exact Ruff crates on `ProviderText`; retain one complete `Parsed`
snapshot and build `TriviaRanges`, `Indexer`, and `LineIndex` once per source revision;
Ruff whole-file non-incremental parsing enforces deadline, work/output budget, and
cancellation/drop policy at the provider boundary;
generate exhaustive enum matches from the authored normalization mapping so any new
provider variant fails compilation; token/trivia/index extraction;
...
fixtures with expected rows for each `GEN §15` fact rule. Enforce whole-file byte,
deadline, traversal depth/node, diagnostic, and output row/byte budgets; cancellation
after a non-interruptible parse discards the stale snapshot and all partial output.
...
Resource/cancellation fixtures prove repetitive/deep/large inputs terminate within the
profile and no acknowledged/stale whole-file parse becomes current.
...
`just ci-fast`; `just mutants-wp31` (argument-free recipe added by this packet for the
post-discovery coordinate-mapping source file).
...
`just mutants-wp32` (argument-free targeted recipe added by WP32 for the §80 ladder
and §81 merge-key source).
...
(`workspace/`, `scenarios/`, `expected/`) as a closed typed `CANDIDATE` corpus whose
Wave-5 `gate-b-v1` coverage profile contains: one
...
holds (an accepted profile is immutable; later work appends profiles and members);
unsupported
...
I-08, I-24, I-39; `SUITE AC-G-78 §78.4–§78.10`. Doctrine P25, P30 advance.

#### Design and Library References

`SUITE AC-G-78` candidate/profile lifecycle and §§78.4–78.10, `AC-G-05`;
`RM §10` WP1; D-30 (state-digest check staging), D-46.
...
edit, Rust compile failure/recovery); declare `corpus_status: CANDIDATE`, a closed
`gate-b-v1` member/requirement list, and no accepted profile digest yet; expected-output directories populated per
...
Fixture tree is additive; `gate-b-v1` exact bytes become immutable when WP40 accepts
its digest. The overall corpus remains candidate until all AC-G-78 profiles complete.

### WP35 — Thin rustc extractor slice (GEN AC-G-31 subset)

#### Outcome

The extractor's `--serve` no-op is replaced by the real Cargo-native protocol for the
committed golden crate. The daemon hosts
`RustcExtractor.Observe(stream ExtractionEvent) returns (stream ExtractorControl)`;
the short-lived wrapper connects, receives acceptance/credit/cancel controls, and sends
`CompilationBegin → (OwnerBegin → OwnerObservationChunk* → OwnerEnd)* →
CompilationEnd` over the private endpoint with Arrow IPC chunk payloads, sequence
numbers, digests, and credit control (≤4 outstanding chunks, ≤16 MiB unacknowledged);
daemon-side launcher constructs the immutable provider view, invokes pinned Cargo with
`RUSTC_WORKSPACE_WRAPPER` set to the extractor, and provides the
`CODEFABRIC_EXTRACTOR_*` environment/resource-profile contract,
...
touches rustc stdout/stderr. This slice is selectable only for the committed fixture
under explicit `TRUSTED_LOCAL`; network and credentials stay removed and every other
workspace/trust combination fails closed with compiler-semantic capability unavailable.
The provider run pins `trust_profile` and `sandbox_profile_digest`; either changing
invalidates the analysis context.

#### Dependencies

WP29 (job runtime), M05, WP34 (the exact authorized golden fixture).

#### Target Invariants

I-06, I-10 (no compiler type escapes the callback), I-30, I-36, I-37, I-38;
`GEN §96`
...
`GEN AC-G-31` (Cargo wrapper and bidi protocol), `AC-G-32` (compiler placement),
`AC-G-35` (staged trust), `AC-G-14`, `§7.4`, `§38`, `§39.1`, `§41`, `§11`;
`SUITE §80.6`; D-27, D-43, D-44, D-45; LD-12 (dated nightly), LD-28. Library ref:
...
rg -n 'rustc-wrapper|RUSTC_WORKSPACE_WRAPPER' .cargo/config.toml scripts/ src/ rustc-extractor/
...
`rustc-extractor/Cargo.toml` (exact root-family prost/tonic/Arrow transport deps),
`contracts/rpc/rustc_extractor.proto`, the single FDS/generator outputs, daemon-side
launcher module behind `fact-generation`+`rpc`, `scripts/run_rustc_extractor.sh`.

#### Required Changes

Correct the pre-production proto direction to the single bidi `Observe` route; extend
the single descriptor-set derivation unit, invoke
the repository's one `grpc_tools` compiler root and feed its committed FDS to Rust
`compile_fds`, generating extractor and daemon bindings from the same descriptor IR.
Delete the old `Extract` route rather than retaining dual protocol authority.
Extractor-side: accept Cargo's real rustc path as argument zero and preserve all real
arguments/cwd/stdout/stderr/exit status; pass probes through; use the `rustc_public`
callback to extract D-27 families into owned rows and Arrow IPC. Daemon-side: build the
immutable `ProviderWorkspaceView`, launch Cargo with `RUSTC_WORKSPACE_WRAPPER`, bypass
the repository's sccache `RUSTC_WRAPPER` for analysis and use a sandbox-private target,
host the private transport, and enforce env/cancel/credit/profile/trust handling as a
`COMPILER_GROUP` provider. Add a fixture allowlist bound to exact source/context digest,
require explicit `TRUSTED_LOCAL`, remove network/credential environment, and reject
ordinary/untrusted registrations before launch. The dated-nightly root and target dirs
remain isolated from the stable root.
...
syntax intact. Focused `wp35_wrapper_topology_acceptance` proves real-rustc argv/cwd/
status preservation, probe passthrough, one callback execution despite an ordinary
sccache-populated build, and the daemon-hosted bidi direction.
...
with `PROTOCOL_ERROR`; credit-control bounds enforced under a slow consumer; descriptor
census proves one `Observe` route and zero legacy `Extract` route.
...
assertion); no `rustc_*` type crosses the callback boundary; non-golden, absent-grant,
`UNTRUSTED_SANDBOXED`, credential-bearing, or network-requesting launches fail before
Cargo starts; golden build-script fixtures attempting home, mutable-workspace,
credential, network, or undeclared-file access are denied; no second
protoc/FDS/bindings authority exists.
...
private and offline; trust/profile/fixture identity recorded. Exact prost/tonic/Arrow
families match the repository stable-graph assertions.
...
`just proto-repro-check`; `just extractor-ci-fast`; `just ci-fast`;
daemon↔extractor interop and trust-negative tests in `tests/integration/`.
...
`just ci-fast`; `just mutants-wp36` (argument-free targeted correspondence/precedence recipe);
...
phrases fail with registry error codes under the synchronized `QRY §47` /
`SRV AC-G-65` registry (`SEMANTIC_PHRASE_AMBIGUOUS` from its explicit list;
`SEMANTIC_PHRASE_UNRECOGNIZED` from `QRY AC-G-44`); typed `PlanSpec`
...
M05 substrate and WP37's accepted projection/registry output.
...
`just ci-fast`; `just mutants-wp38` (argument-free targeted ordering/phrase recipe).
...
passes exactly as `SUITE` Part V defines**; the independently reviewed exact
`gate-b-v1` member/output digest is appended to `accepted_profile_digests`, making
that profile immutable while `corpus_status` remains `CANDIDATE` — M06.
...
I-02, I-09 (scenario-scoped), I-24, I-33, I-39; `LIFE §125` pin order; `FAB §91`
...
operations to the real daemon components; expected-output pinning + independent review;
typed profile digest acceptance without releasing the incomplete corpus; recipes.
...
publication/snapshot manifests per `§78.7`–`§78.8`); recomputation matches the accepted
`gate-b-v1` digest and a post-acceptance byte mutation fails.
...
hook); no gate artifact satisfied by a test double; corpus remains candidate and no
non-Gate-B profile is marked accepted or released.
...
append-only discipline; exact-name states retain codes (`COLLECTING=10`,
`SNAPSHOTTING=20`, `FAILED=60`, `SUPERSEDED=70`); `RUNNING=30`, `PUBLISHING=40`,
and `COMPLETE=50` become decode-only deprecated values with no new emission;
new states use codes 80–200 as `LIFE §21` fixes; `registries.rs` and transition tables regenerated, and the `AC-G-25`
...
I-03, I-14, I-31, I-40; `LIFE §157` inv. 8, 11, 16–17; `LIFE §158` inv. 3–4, 7;
...
+ `contracts-repro-check` before completion per §4.1). Before scheduler activation,
inventory persisted rows and record the census: historical terminal coarse `COMPLETE`
may remain readable; persisted nonterminal `RUNNING`/`PUBLISHING` rows are rejected
with a migration diagnostic and restarted as a new `COLLECTING` generation. Generate
no writer or transition target for deprecated codes and close DB09. Implement the scheduler actor with command
...
DB09 owns the coarse-state cutover and reintroduction guard; it closes with this packet.
...
resets `source_generation` (persistence fixture); persisted-row migration fixtures
cover terminal/deprecated nonterminal cases; deprecated emission zero state proves no
writer or generated transition emits codes 30/40/50.
...
`just ci-fast`; `just mutants-wp42` (argument-free targeted classification/supersession recipe).
...
`just ci-fast`; `just mutants-wp43` (argument-free targeted propagation recipe).
...
`just ci-fast`; `just mutants-wp44` (argument-free targeted reuse-condition recipe).
...
`just ci-fast`; `just mutants-wp45` (argument-free targeted policy-condition recipe).
...
WP42 (waves feed overlays), WP44, WP45 (the sole freshness/barrier coordinator writes);
the Wave-3 kernel and WP40's scenario-scoped
rebase.
...
`just ci-fast`; `just wave3-integration-check`; `just mutants-wp46`
(argument-free targeted trigger/rebase-drop recipe).
...
`just mutants-wp48` (argument-free targeted canonical-row-encoding recipe).
...
boundary; `LIFE §43` and Appendix F use the same byte-preserving path forms.
...
`src/git_state.rs` (+submodules), `rules/` (rule fixtures extended for new DTO surface),
...
fingerprint; retain the already-complete Appendix E feature manifest and verify it with
the resolved-graph/API probes; author the missing DTOs. Touch `Cargo.toml` only if a
concrete accepted capability probe fails and the plan is revised to explain why.
...
`just ci-fast`; `just git-parity-check`; `just mutants-wp51` (argument-free targeted
reconcile retry/broaden recipe).
...
branch-switch scenarios; `just mutants-wp52` (argument-free targeted stabilization-
tuple recipe).
...
artifacts end-to-end over the accepted immutable `gate-b-v1` profile inside the
candidate corpus; `just wave5-integration-check`; all IDs, rows, response bytes, and
checksums match its independently reviewed expected outputs
(`RM §10` exit evidence); D-30 state-digest equivalence on every Wave-5 scenario;
`just ci-fast` + `just extractor-ci-fast` green. The overall AC-G-78 corpus remains
candidate. The gate is the suite's, not this plan's: any missing gate contract stops
and returns to its normative owner.
...
Prerequisite: WP42. Exit invariant: the persisted-row migration census is recorded;
exact-name codes 10/20/60/70 remain stable; historical terminal code 50 is decode-only;
nonterminal codes 30/40 are rejected/restarted with explicit diagnostics; no production
writer, generated transition, SQL default/constraint, fixture, or RPC encoder emits
deprecated `RUNNING`, `PUBLISHING`, or `COMPLETE`. The 17 accepted states and their
append-only codes round-trip. Standing oracle: `wp42_negative_zero_state`, rechecked by
M07/M08 and a governance reintroduction guard.
...
- **A-01** — The generated contract machinery (Contract IR, registries, bundles) can
  express every Wave-4 extension (new tables, kinds, provider-raw-catalog derivations,
  normalization mappings, and resource profiles). Owner:
...
- **A-05** — The current exact gix 0.86 resolved feature graph already supplies the
  accepted Appendix E capabilities under `default-features = false`; WP49 retains and
  verifies this current fact. A failed concrete probe triggers LD/design review rather
  than speculative manifest edits.
- **A-06** — Normative corrections in §6.2 are accepted design inputs. Any future
  behavior- or authority-changing ambiguity blocks its consumer packet until the owning
  specification is corrected; the plan cannot serve as the permanent override.
...
Each owning packet adds its argument-free mutation recipe after discovering the final
source file. `just fuzz` remains scoped to a packet-named parser/protocol target. A
performance recipe becomes a completion gate only at W19; Waves 4–7 record
measurements without storing claims in execution state (v5 D-21).
...
Wave 5
  M05 -> WP34 -> WP35 -> WP36 -> WP37 -> WP38 -> WP39 -> WP40 -> M06
         (golden-corpus, extractor, and query/derivation authorities serialize)

Wave 6
  M06 -> WP41 -> WP42 -> WP43 -> WP44 -> WP45 -> WP46 -> WP47 -> WP48 -> M07
         (strictly sequential; WP45 and WP46 share coordinator/freshness writes;
...
Decommission
  DB07 exits at WP36; DB08 exits at WP45; DB09 exits at WP42; all are re-checked
  at M07 and M08.
~~~

Additional direct edges: WP32 → {WP28, WP30, WP31}; WP33 → {WP29, WP32};
WP35 → WP34; WP36 → {WP32, WP35}; WP38 → WP37; WP39 → WP38;
WP40 → {WP34–WP39}; WP43 → WP42; WP44 → {WP42,
WP43, WP30}; WP45 → {WP42, WP43, WP44}; WP46 → {WP42, WP44, WP45}; WP50 → {WP49, WP41};
...
- [ ] §3 adoption — schema-2 state exists; explicit/default artifact validation,
      status, and active-plan tests resolve this v4 plan
...
- [ ] DB09 — deprecated update-wave emission zero state and persisted-row migration green
...
- [ ] §6.2 normative design corrections and §6.3 audit dispositions revalidated
...
- **R-03 Grammar/registry mismatch.** Tree-sitter raw-kind inventories may exceed
  generated-catalog assumptions. Mitigation: derive the complete catalog from
  `NODE_TYPES`/`Language`, compare startup inventory and grammar fingerprint, and fail
  upgrades until every new key has an authored mapping (D-39, I-35).
...
- **R-09 Compiler wrapper/caching composition.** An incorrect Cargo wrapper stack can
  skip extraction or change compiler behavior. Mitigation: D-43 single wrapper
  topology, analysis-specific sccache bypass, exact argv/status probes, and one-FDS
  descriptor census in WP35.
- **R-10 Partial resource-limit output.** Native parser/query cancellation can leave
  partial observations. Mitigation: D-45 profiles, parser reset, atomic revision
  commit, stale/drop policy, and adversarial limits in WP30/WP31.
