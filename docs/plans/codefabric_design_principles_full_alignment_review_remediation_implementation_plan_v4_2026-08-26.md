...
design_version: v1
audit_path: docs/reviews/plan_audit_codefabric_design_principles_full_alignment_review_remediation_v2_2026-08-26_v1.md
audit_version: v1
reaudit_path: docs/reviews/plan_audit_codefabric_design_principles_full_alignment_review_remediation_v4_2026-08-26_v1.md
reaudit_version: v1
...
state_path: docs/plans/state/codefabric-design-principles-full-alignment-review-remediation_v4_state.json
cutover: true
supersedes_plan_path: docs/plans/codefabric_design_principles_full_alignment_review_remediation_implementation_plan_v3_2026-08-26.md
activation_requires: codefabric-design-principles-full-alignment-review-remediation-v1/M04+WP06
---

# CodeFabric design-principles full-alignment review remediation — implementation plan v4

## 1. Outcome and non-goals

### 1.1 Outcome

This v4 preserves the accepted v3 target design and packet scope while correcting two
execution-governance ambiguities discovered during WP09: the strict dependency parser could read
an explanatory packet-name repetition as a self-edge, and a supplemental Python projection test
shared the Rust packet oracle's name. Neither correction changes a dependency, outcome, semantic
contract, acceptance check, or accountable boundary. The underlying v3 is a targeted successor
to v1 and audited v2 after the accountable repository owner rejected
...
`412af14566393c2379ba4e174387361cea5370e8`. The v3 replan was made at
...
| docs/reviews/plan_audit_codefabric_design_principles_full_alignment_review_remediation_v2_2026-08-26_v1.md | 1d10ff8253af98b706edd6fd282cf5bab643f730007706b4849d064c78b18044 |
| docs/reviews/plan_audit_codefabric_design_principles_full_alignment_review_remediation_v3_2026-08-26_v1.md | c55a118f7ec2fd99a3a91ec333e81686a986fd91264afd4e1e562e87e1342496 |
| docs/plans/codefabric_design_principles_full_alignment_review_remediation_implementation_plan_v2_2026-08-26.md | c71b97ebcc2337fc9b315448a7973ede67c3718a7ba9b8ae30a6cafbee09c242 |
...
| contracts/schema/schema-contract-ir.json | 322b557e295897a18a7eadbf6e419068514de1bca72e3a10829c8e7815b52ba5 |
...
| docs/library_ref/serde_json_rust_advanced_reference_1.0.151.md | 6849a8acaa6561fad97680173b7f845231986ff6531cd0d9cdcf8fab340a03c0 |
...
**Audit:**
`docs/reviews/plan_audit_codefabric_design_principles_full_alignment_review_remediation_v2_2026-08-26_v1.md`
(v1). **Source design:** behavioral-conformance design v1. **Source plan:** remediation v2.
**Revised plan:** remediation v3. The revision makes planning inputs current, removes the
closeout cycle, grounds strict JSON ingress in duplicate detection, and closes the new functional
invariants without changing the selected behavior-first architecture.

- `F-001` — `applied-plan`
  - Finding: baseline and declared inputs were stale/incomplete.
  - Resolution: frontmatter now uses planning commit `a3efb30d...`; §2 declares current SUITE,
    GEN, FAB, LIFE, schema-IR bytes plus serde_json, FastMCP, grpcio, Protobuf, the audited v2
    plan, and its audit.
  - Revalidation: the audit's `validate_plan(..., verify_declared_inputs=True,
    _allow_missing_state=True)` command exited 0.
  - Rationale: activation will start from a verifiable current input boundary rather than the
    copied v1 snapshot.
- `F-002` — `applied-plan`
  - Finding: WP08, M04/M05, and DB02/DB04/DB05 formed a closeout cycle.
  - Resolution: WP08 no longer depends on decommission batches; M04 no longer claims DB02;
    DB prerequisites no longer require M05; §8 separates candidate-side and release-side exits
    and places M05 after WP08 plus all DB exits.
  - Revalidation: the audit's Python negative dependency assertion exited 0.
  - Rationale: the normative and linearized sequences now admit a topological order while keeping
    the human WP07 checkpoint load-bearing.
- `F-003` — `applied-plan`
  - Finding: strict serde DTO wording did not guarantee duplicate-key rejection.
  - Resolution: WP09 now requires Visitor-based duplicate detection before typed
    deserialization, nested duplicate fixtures, and the named
    `functional_golden_duplicate_key_rejected` oracle; §2 declares the serde_json reference.
  - Revalidation: `just functional-golden-contract-check` exited 1 because WP09 has not yet
    introduced the planned recipe; plan structure/input validation exited 0 and execution may
    not complete WP09 until this exact future gate passes.
  - Rationale: the plan now names the library-correct mechanism and executable proof instead of
    relying on ordinary `from_slice` behavior.
- `F-004` — `applied-plan`
  - Finding: WP08 omitted GI-15–GI-18 and DB05 from final closure text.
  - Resolution: WP08 targets GI-01–GI-18 and its legacy disposition jointly names DB04/DB05.
  - Revalidation: the audit's covered `rg` query exited 0 with the expected WP08 and closure
    matches.
  - Rationale: independent meaning, public equivalence, causal sensitivity, and self-referential
    legacy removal are now explicit certification obligations.
...
rg -n 'Visitor|visit_map|deny_unknown_fields|from_slice|duplicate.*key|canonical' src tests contracts
```
...
3. Define strict application-owned DTOs for claim IDs, normative references, proof universes,
   ...
   transitions, and negative claims without a complete universe. Ordinary
   `serde_json::from_slice` plus `deny_unknown_fields` is insufficient for duplicate object
   members: run a Visitor-based duplicate-detecting ingress before typed deserialization and
   cover top-level and nested claim, selector, universe, operation, and query objects.
...
- **Negative/Zero-State — Executable oracle:** `functional_golden_negative_claim_requires_complete_universe`
  ...
  currentness boundary is missing; `functional_golden_duplicate_key_rejected` rejects every
  top-level and nested duplicate-key fixture before typed DTO construction; and
  `gate_b_rejected_candidate_zero_state` proves v3 immutable and unreleasable.
...
**Dependencies.** WP04, WP05, WP07, WP09–WP12. DB01–DB05 are milestone exit
conditions verified by this packet, not prerequisites of the certification packet.

**Target invariants.** GI-01–GI-18; all Data-Fabric Principles 1–25 and applicable HOL
Principles 1–31.
...
status/review artifacts rather than editing history. DB04/DB05 ensure only the corrected corpus,
functional golden authority, and certification are current while predecessors remain immutable.
...
WP05 and WP06 are complete and the rebuild half of DB03 exits. The full corpus runs
...
**Prerequisites.** WP09–WP12 and WP07.
...
WP04 + WP05 + WP07 + WP09–WP12 ────────────────→ WP08
WP08 + DB01–DB05 ───────────────────────────────→ M05

DB01 after WP03 / M02
DB03 after WP04 + WP05 / M03 + M04
DB02 candidate-side after WP12; release-side after WP07
DB04 after WP07 + WP08
DB05 candidate-side after WP12; release-side after WP07
```
...
WP01 (M01)
  ...
  → DB02/DB05 release-side exit → WP08 → DB04 → verify DB01–DB05 (M05)
```
