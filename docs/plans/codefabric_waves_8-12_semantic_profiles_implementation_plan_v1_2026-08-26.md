...
state_path: docs/plans/state/codefabric-waves-8-12-semantic-profiles_v1_state.json
activation_requires: codefabric-design-principles-full-alignment-review-remediation/M05
cutover: true
---

# CodeFabric Waves 8–12 semantic profiles — implementation plan v1
...
- **No movement from the FAB §2.1 dependency baseline**: DataFusion 55.0.0,
  ...
  (`std` only), gix 0.86.0. New direct dependencies are limited to the pinned Ruff
  0.0.7-train semantic crates (LD-RF-01).
...
`ruff_python_semantic` (and, if the traversal layer requires it,
`ruff_python_codegen`-adjacent helper crates) at the same pinned versions.
...
### LD-PY-01 — Pyrefly 1.2.0 pinned-source query surface

**Decision:** retain-current (deepen use)
**Version basis:** the exact pinned Pyrefly source/tag already built by
`pyrefly-sidecar/` (1.2.0 reference basis).
...
**Version basis:** pinned grammars per the generated raw-kind catalogs.
...
The load-bearing bindings for this plan, from `full_data_fabric_design_principles.md`
via the two alignment manuals (`align §23`–`§24`, `delta-align §34`–`§35`):

- **P1/P2 model-first, executable models** — fact families, observation schemas,
...
- **P3 one authoritative owner** — provider observations are never canonical; the
...
`docs/spec_index/invariants-and-doctrine.md` remain the cross-reference.

## 3. Global target invariants

GI-01 through GI-10 are the suite-wide invariants of `RM §1`, restated here as the
plan's binding set; GI-11 through GI-14 are the cross-cutting invariants this
...
at its proving commit and at HEAD (`just packet-oracle-check <WP>`); oracle names
...
two cross-cutting substrate packets WP01/WP36). WP36 and WP37 were added during
the independent plan challenge; packet IDs are stable labels — the §8 dependency
graph, not ID order, governs sequence. The two language lanes may execute in
parallel per `RM §4.1`, with two deliberate cross-lane serializations: the
language-neutral fact tables and the canonical type interner are defined once by
the Python-lane packets that reach them first, so the Rust fact packets (WP18
onward) sequence behind WP11's closure of that shared authority; and both lanes'
provider integration consumes the WP36 substrate. Wave 12 (WP29–WP35, WP37) is
the integration barrier.

Two group-wide rules: every packet that introduces a table registers its
property records in `contracts/registry/ontology-property-registry.yaml` at
landing — WP30 adds generated enforcement, never late registration; and each
`waveN-integration-check` recipe's oracle selection excludes the closure
...
packet establishes the contract substrate they build on.

**Dependencies.** None.

**Target invariants.** GI-03, GI-04, GI-09, GI-11, GI-12; P1–P3, P7, P9.
...
schema driver, and the generated registries. Exact symbols are re-derived at
...
lanes to consume. (This packet was split out of WP01 by the plan challenge; its
ID is late, its position is early.)

**Dependencies.** WP01.

**Target invariants.** GI-03, GI-04, GI-11, GI-12, GI-13; P3, P9, P23.
...
`src/operational_store.rs` (run journaling), and the provider and
resource-profile registries.
...
4. Build the sandbox substrate: trust-profile model
   (`UNTRUSTED_SANDBOXED`/`TRUSTED_LOCAL`/`PARSING_ONLY`), platform containment
   probe (Seatbelt on darwin, namespaces on Linux CI), sandbox-profile digests
   pinned into runs and snapshots, and the fail-closed unavailable path (GI-13).
   Lane-specific enforcement lands in WP08/WP16.
...
- **Negative/Zero-State — Executable oracle:**
  ...
  registered reason; no code path launches a provider unsandboxed under the
  untrusted profile.
...
`just governance-scan`, `just packet-oracle-check WP36`.

**Integration Milestone.** M01.

**Replan Triggers.** Provider-runtime integration needs a different boundary
than the accepted-handle model provides (would touch the AC-G-32 contract); or
the lane-neutral view/bundle split proves impossible without lane-specific
...
**Target invariants.** GI-05, GI-09; P1, P3, P18.
...
Known current touch includes `src/analysis_context.rs`, `src/continuous.rs`
(`replace_analysis_context`), `src/operational_store.rs`, the context tables
(codes 3/4), and the enum/flag registries for the new diagnostics.
...
**Target invariants.** GI-04, GI-05, GI-06, GI-11; P1, P3, P7, P12.
...
jq '.tables | keys' contracts/generated/model/schema/table-specs.json | rg 'scope|binding|reference'
...
Known current touch includes `Cargo.toml` (new pinned `ruff_python_semantic`),
`src/ruff_adapter.rs`, `contracts/schema/schema-contract-ir.json` (three tables +
observation schema), the ontology entity/relation registries, the capability
registry (`SCOPES_BINDINGS`), generated encoders, and `src/core_facts.rs` ingest
wiring.

**Required changes.**

1. Add the pinned `ruff_python_semantic` dependency and build the adapter
...
**Target invariants.** GI-03, GI-06; P1, P3.
...
relation kinds), and `src/core_facts.rs` ingest.
...
**Target invariants.** GI-04, GI-06; P1, P3, P12; doctrine: call syntax ≠
...
**Target invariants.** GI-04, GI-06, GI-10, GI-12; P2, P20, P21.
...
`src/core_facts.rs` capability aggregation, the capability registry, and the
...
4. Complete the `GEN §33` unknown table for Wave 8: `getattr`/`exec`/dynamic
   ...
   producer.
5. Advertise `PYTHON_SEMANTIC_V1` as `PARTIAL` via `aggregate_capability`: the
   ...
   (`LIFE §7.2`).
6. Populate `just wave8-integration-check` with the Wave 8 oracle selection plus
...
- **Structural — Executable oracle:** `py_semantic_profile_partial_parity` — the
  ...
  reason code, and the profile can never report `COMPLETE` while one exists.
- **Negative/Zero-State — Executable oracle:**
  ...
  dynamic construct without its unknown remainder fails.
...
**Target invariants.** GI-04, GI-09, GI-11, GI-12, GI-13; P9, P20, P23.
...
3. Apply the WP36 sandbox substrate to the sidecar per `GEN AC-G-35` (default `UNTRUSTED_SANDBOXED`: no
   network, read-only leased blobs, quotaed private tmp, resource limits,
   process-group termination; macOS Seatbelt / Linux namespaces); when
   containment cannot be established, Python semantic execution reports
   unavailable — never unsandboxed (GI-13). Sandbox profile digest and trust
   profile pin into runs and snapshots.
...
5. Route all invocation through the WP01 `ProviderAdapter`; the Gate B vertical
   harness's direct spawn of the sidecar reaches zero (DB01 Python half).
...
DB01. Last-good sidecar state is a declared operational cache (P23), never
...
`just provider-protocol-check`, `just packet-oracle-check WP08`.
...
is a plan revision. Sandbox containment primitives unavailable on the host
platform → the fail-closed posture stands and the packet still completes with
`TRUSTED_LOCAL` fixtures only, recorded as a bounded scope exclusion.
...
**Target invariants.** GI-03, GI-09, GI-13; P9, P11.
...
**Target invariants.** GI-04, GI-05, GI-06; P3.
...
Known current touch includes `pyrefly-sidecar/src/pyrefly_link.rs` (module/xref
extraction), the observation schema (new columns/streams via the Contract IR),
`src/core_facts.rs` reconciliation, and the WP04 import facts (resolution-class
upgrade in place, no schema change).

**Required changes.**

1. Extract module-resolution and declaration/xref observations from the sidecar
   (module graph, import targets, exported names, star-import export sets) and
   register their observation schema through the Contract IR.
...
**Replan Triggers.** The pinned Pyrefly source exposes no usable module-graph /
xref surface beyond the two wired queries (the reference routes bulk xrefs to
optional Glean) — extracting via the type-table/callee machinery plus
application derivation is the planned shape; needing a Pyrefly source patch is a
plan revision.
...
type output is never `Any`. The `type_detail`/`type_fact_detail` tables land.

**Dependencies.** WP10.

**Target invariants.** GI-04, GI-05, GI-06; P1, P3, P18.
...
a new type-interning module in the stable root, the Contract IR (`type_detail`,
`type_fact_detail`), ontology registries, and `src/core_facts.rs`.
...
2. Intern types into the canonical algebra: identity from structured normalized
...
**Target invariants.** GI-04, GI-06; P3, P20.
...
**Target invariants.** GI-04, GI-06; P3, P20; doctrine: resolved target set ≠
unknown target.
...
replacing `callees_json` — DB03), the observation schema, `src/core_facts.rs`
reconciliation, the Contract IR (`call_target_detail`), and the WP05 call-site
...
**Target invariants.** GI-04, GI-05, GI-06, GI-10, GI-12; P3, P10, P19, P20,
P25.
...
Known current touch includes `src/core_facts.rs` (`reconcile_pyrefly_run`
generalization), `src/fact_ingest.rs` (Python authority tables), `src/lifecycle.rs`
/ `src/continuous.rs` (lane ordering, dependency-driven invalidation), the
comparator harness in `src/fabric/serving.rs`, the capability registry, and the
`justfile` (`wave9-integration-check`).
...
Known current touch includes `src/analysis_context.rs`, a new Rust discovery
module, `src/operational_store.rs` (configuration dependency records), the
context registries, and `rustc-extractor/src/wrapper.rs` (digest consumers —
today the wrapper echoes daemon-derived digests; discovery must produce them).
...
**Target invariants.** GI-03, GI-09, GI-11, GI-13; P9, P20, P23.
...
4. Prove platform containment per `GEN AC-G-35` (macOS Seatbelt here; the
   Linux namespace path is CI-verified) and pin sandbox profile digest + trust
   profile into runs and snapshots; when containment cannot be established,
   untrusted compiler execution is unavailable (GI-13).
5. Route execution through `ProviderRuntime` with supersession keys and
   registry-resolved resource profiles; retire the Gate B direct spawn (DB01).
...
`just provider-protocol-check`, `just packet-oracle-check WP16`.
...
**Target invariants.** GI-03, GI-04, GI-09, GI-12; P9, P16.
...
if any field is missing, and `src/core_facts.rs`
(`reconcile_rustc_compilation` — accepts only closed manifests).
...
reconstruction. The WP11 dependency is deliberate: the canonical type
interner and the language-neutral fact tables are defined once by the
Python-lane packets that reach them first, and the Rust fact packets consume
that single authority rather than minting a second one.

**Dependencies.** WP11, WP17.

**Target invariants.** GI-04, GI-06; P1, P3; identity: application-owned keys,
...
IR), `src/core_facts.rs` reconciliation, ontology registries, and the shared
type-interning module from WP11 (language-neutral algebra).
...
**Target invariants.** GI-04; P1, P7; `ONT §48.3` raw+normalized coexistence.
...
jq '.tables | keys' contracts/generated/model/schema/table-specs.json | rg 'mir'
...
**Target invariants.** GI-04, GI-06; P20; doctrine: unwind never collapses into
normal flow.
...
**Target invariants.** GI-03, GI-06, GI-12; P20, P23.
...
Known current touch includes `src/continuous.rs`/`src/lifecycle.rs` (failure
wave handling), `src/core_facts.rs` (capability aggregation),
`src/semantic_query.rs` (degradation fields), the capability registry, and the
`justfile`.
...
**Target invariants.** GI-04; P1, P7; `ONT §50` distinctions.
...
**Target invariants.** GI-04, GI-06; P20; `ONT §51.1` program-point identity.
...
**Target invariants.** GI-06, GI-09, GI-13; P18, P20; repo invariant: no
...
**Dependencies.** WP20, WP24.

**Target invariants.** GI-04, GI-06; P20; `ONT §52` definition-vs-instance
separation.
...
**Target invariants.** GI-03, GI-09; P3, P18; `ONT §27` representation
classes.
...
**Dependencies.** WP22, WP25.

**Target invariants.** GI-04, GI-06; P20; `ONT §55.2`, `§57`–`§58`.
...
**Dependencies.** WP23, WP25, WP26, WP27.

**Target invariants.** GI-05, GI-09, GI-10, GI-12; P18, P19, P25.
...
Known current touch includes `src/lifecycle.rs`/`src/continuous.rs` (Rust lane
ordering, reverse dependency propagation), `src/core_facts.rs`, the comparator
corpus, the capability registry, and the `justfile`.
...
**Target invariants.** GI-04, GI-11; P2, P3, P14, P15; `RM §17` exit: no
...
**Target invariants.** GI-06, GI-12; P12; `ONT AC-G-71` rules 1–8.
...
1. Verify registry coverage: every property emitted by Waves 8–11 was
   ...
   occurrence-specificity, owner family — the §4 group rule); close any gap
   found, then have the schema generator emit Arrow fields, Delta constraints,
...
**Legacy Disposition and Decommission.** Any Wave 8–11 property that landed
without a registry entry is regularized here — the registry becomes the
closed authority and model compilation rejects unregistered property IDs.
...
**Target invariants.** GI-06, GI-12; P20, P21.
...
**Dependencies.** WP29, WP30.

**Target invariants.** GI-06; P20; `ONT §65` separations.
...
**Dependencies.** WP31, WP32.

**Target invariants.** GI-02, GI-06; P20, P24; `RM §17` exit: `PROVEN_EMPTY`
...
**Dependencies.** WP29, WP33.

**Target invariants.** GI-05, GI-06; P3, P13; `RM §17` exit: multi-context
...
**Dependencies.** WP29, WP32.

**Target invariants.** GI-05, GI-06; P3, P20.
...
**Dependencies.** WP30, WP33, WP34, WP37.

**Target invariants.** GI-10, GI-12, GI-14; P3, P25; `RM §17` exit evidence
complete.
...
5. Populate `just wave12-integration-check` (reconciliation, cardinality,
   ...
   `just wave-acceptance-check`'s successor surface.
...
**Packet-Local Gates.** `just root-ci-fast`, `just model-repro-check`,
`just wave12-integration-check`, `just packet-oracle-check WP35`.
...
DB02 negative checks green; `PYTHON_SEMANTIC_V1 = PARTIAL` with named
...
DB01 and DB03 **Python-half** negative checks green.
...
retirement proven (`rustc_unsandboxed_execution_falsification`); DB03
...
`rust_toolchain_mismatch_activation_falsification` green; DB01 **Rust-half**
negative checks green.
...
production path.
**Negative proof.** Split by lane. Python half (verified at M02): zero-hit
`rg -n 'run_pyrefly'` outside production adapter modules plus
`pyrefly_provider_runtime_parity` green. Rust half (verified at M04):
zero-hit `rg -n 'run_rustc'` outside production adapter modules plus
`rustc_provider_runtime_parity` green. Both halves plus a zero-hit `ast-grep`
scan for `Command::new` targeting the sidecar/extractor binaries outside
`ProviderRuntime`-owned launchers, plus a clean build (tier 1), re-verified
at M05.
...
**Negative proof.** `handwritten_observation_schema_falsification` green;
zero-hit `rg -n 'include_str!' pyrefly-sidecar/src rustc-extractor/src`
matching schema files outside `src/generated/`; `just model-repro-check`
green.
...
**Negative proof.** Split by lane. Python half (verified at M02): zero-hit
`rg -n 'type_table_json|callees_json'` outside generated/historical paths
plus `py_type_enrichment_conformance` green. Rust half (verified at M03):
the MIR summary blob shape absent from ingest inputs plus
`rust_mir_body_fixture_conformance` green. Both halves plus a structural
scan proving no reconciliation path parses free-form JSON into canonical
rows, re-verified at M05.
...
- `just packet-oracle-check <WP01…WP37>` — per-packet, at each proving commit
  and re-derivable at HEAD
...
Python lane: WP02 → WP03 → { WP04, WP05 } → WP06 → WP07  [M01]
             WP08 → WP09 → WP10 → WP11 → WP12 → WP13 → WP14  [M02]
             (WP08 requires WP07 + WP36)
Rust lane:   WP15 → WP16 → WP17 → WP18 → WP19 → WP20 → WP21  [M03]
             (WP15 requires WP02; WP16 requires WP36; WP18 requires WP11 —
              the shared type-interner/table authority crosses lanes here)
             WP22 → WP23 ─┐
             WP24 ────────┼→ WP25 → WP27 ─┐
             WP26 ────────┘               ├→ WP28  [M04]
             (WP22/WP24 require WP21; WP23, WP25, WP26, WP27 → WP28)
Integration barrier (requires M02 + M04; W7 already complete):
  WP29 → WP30 → WP32 ─┐
         WP31 ────────┼→ WP33 → WP34 ─┐
                      │   WP37 ───────┼→ WP35  [M05]
                      └───────────────┘
  (WP32 requires WP29 + WP30; WP33 requires WP31 + WP32; WP34 requires
   WP29 + WP33; WP37 requires WP29 + WP32; WP35 requires WP30 + WP33 +
   WP34 + WP37)
...
The lanes overlap in wall-clock terms only up to WP18: Rust infrastructure
(WP15–WP17) parallelizes with the Python lane, and the Rust fact packets
sequence behind WP11's shared-authority closure by design.

Execution is wave-segmented: only the current wave group and accepted
predecessor interfaces load as active context. Parallel packets coordinate
through serialized integration edges — a packet that must touch one of these
while a parallel packet is in flight coordinates through the lead
(`RM §28`): `Cargo.toml`, `contracts/schema/schema-contract-ir.json` and all
generated registries, `justfile`, `src/core_facts.rs`, `src/fact_ingest.rs`,
`src/analysis_context.rs`, `src/operational_store.rs`,
`src/provider_runtime.rs`, `src/continuous.rs`, `src/lifecycle.rs`, the
comparator harness in `src/fabric/serving.rs`,
`contracts/registry/capability-registry.yaml`, the provider/resource-profile
registries, `src/ruff_adapter.rs` (WP04∥WP05), and
`rustc-extractor/src/rustc_link.rs` (the Rust lane's core seam — its packets
execute effectively sequentially).
...
3. **Pyrefly surface gaps.** Declared/expected types, structured member
   types, and xrefs have no stable Query surface; the plan already assigns
   application-side derivations, and WP11/WP12 triggers govern the rest.
   The experimental query module may panic; supervision is WP08's contract.
...
5. **Observation volume.** Full MIR and Python semantic streams are orders
   ...
   sizing with benchmark gates in the ingest oracles (WP19/WP22/WP29);
   `perf` recipes (`just data-fabric-upgrade-bench`) on regression suspicion
   — performance claims require the §8 evidence order of AGENTS.md.
6. **Sandbox platform variance.** Seatbelt (darwin) vs namespaces (CI
   Linux). Mitigation: GI-13 fail-closed posture is itself the contract;
   WP16's oracle proves unavailable-not-unsandboxed on both.
7. **Registry/contract regeneration cadence.** Nearly every packet touches
   the Contract IR and generated registries — a serialized edge that could
   bottleneck parallel lanes. Mitigation: batch registry additions at packet
   boundaries; `just model-repro-check` in every packet-local gate keeps the
   two-root reproduction honest.
...
`docs/plans/state/codefabric-waves-8-12-semantic-profiles_v1_state.json`
