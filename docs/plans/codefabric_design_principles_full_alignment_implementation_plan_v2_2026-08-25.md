...
working_tree_digest: 296830a59078d10b0edc14717706face007b981012c4efb39dcefe32c68abbe3
state_path: docs/plans/state/codefabric-design-principles-full-alignment_v2_state.json
cutover: true
---

# CodeFabric design-principles full alignment — implementation plan v2

This plan executes remediation moves R1–R11 of the audited design v2. It
supersedes plan v1 and integrates all 16 findings from the 2026-08-25 plan
audit. Full alignment is certified only after the normative design corpus is
amended and accepted, every DP-001–DP-124 detector is re-derived against the
current tree, and the resulting active worklist is implemented and proved.
...
`working_tree_digest` is derived by `git diff HEAD | shasum -a 256` at v2
planning time. It covers the complete tracked diff, not an asserted two-file
subset; untracked paths are outside that digest and remain user-owned until
an explicit owner disposition. WP73 re-derives `git status --porcelain=v1`
and classifies every changed/untracked/deleted path as plan-owned,
separately-owned, or excluded before any proving commit. No untracked tree is
deleted without an owner-approved recoverable archive or manifest.

The pass-3 register is a declared historical input, not the current worklist.
DP-022 is contradicted at `d89cc90` and HEAD by production wave-table writes.
WP73 re-runs every detector and publishes the superseding current-tree
register before downstream packet scope is finalized.

A fresh planning-session `just ci-fast` ran: 265/265 Rust tests and doctests
passed before `typos` failed on the v1 plan/proposal notation. WP73/WP54 own
the baseline disposition; no proving commit may relabel that run green.

No execution state is created during audit integration. The current validator
rejects any inactive draft whose declared state path is absent, so this v2
plan is not activatable until WP54's pre-activation governance contract is
landed through a separately authorized governance change: inactive
draft/audited plans may declare a future state path, while activation must
atomically create and validate schema-2 state before switching
`active-plan.json`. The active pointer may never reference an unresolved
state path.
...
| docs/upfront_design/present_state_cpg_data_fabric_specification_rust_arrow_datafusion_deltalake_v1.3.md | 84c156c6915eae789b99327b502870bd5ad6387cacdd7d3de0710428f1236c55 |
...
| docs/upfront_design/present_state_cpg_fact_generation_specification_python_rust_v1.3.md | f6e62bb19e6f801767c9166aa74896a71b9e089459c1eb79b30f505fb69e4f51 |
...
implementation plan v1. Revised design/plan: remediation proposal v2 and this
implementation plan v2. Revision reason: replace invalid query, provider,
...
Linearized default order: WP73 → WP54 → WP55 → WP56 → WP69 (M09, DB12) →
WP57 → WP58 → WP59 → WP60 → WP74 (M10, DB11) → WP61 → WP62 → WP75 →
WP63 → WP64 (M11, DB10) → WP65 → WP66 (M12) → WP67 → WP68 (M13) →
WP70 → WP71 → WP76 → WP72 (M14).

Execution cannot start until F-003's separately authorized pre-activation
governance change is landed. Activation then atomically creates and validates
the schema-2 state file before switching `docs/plans/active-plan.json`; failure
leaves the prior pointer unchanged. Every packet completion requires its four
criterion-mapped oracles, packet gate, proving commit, and current-HEAD rerun.
...
6. **Human checkpoints (WP73/WP76).** Normative amendments and golden answers
   require owner decisions. Lack of an accountable acceptance is expected
   blocking state, not permission for agent self-approval.
...
`..._implementation_plan_v3_<date>.md`; triggers include: any declared-input
