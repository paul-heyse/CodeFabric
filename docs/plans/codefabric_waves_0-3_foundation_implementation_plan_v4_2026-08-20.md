...
baseline_working_tree_digest: 96caca12cf29ecce3781f0255d2650b2ccf8b8a434301af3d9fc609c8ba3a001
state_path: docs/plans/state/codefabric-waves-0-3-foundation_v4_state.json
cutover: true
---

# CodeFabric Waves 0–3 Foundation — Implementation Plan v4
...
Execution began under v3 and reached WP01 before exact graph evidence reopened
the default-storage invariant. This successor preserves every stable packet ID
and all unaffected v3 obligations, integrates the accepted local-storage
dependency-isolation correction, and resumes from the reconciled WP01 working
tree. The seed packaging surface remains subject to deliberate decommission.
...
- Object-store (S3) runtime backends: spec §2.1 keeps `deltalake/s3` **out**
  of the default CodeFabric feature set and exposes it only through
  `s3-storage`. All Wave 3 storage runs on the local filesystem. The default
  graph must omit `deltalake-aws` and the AWS SDK, while graph evidence must
  honestly report the latent `object_store` cloud features forced by the
  pinned kernel. Nothing in Waves 0–3 authorizes cloud provider construction.
...
| `present_state_cpg_data_fabric_specification_rust_arrow_datafusion_deltalake_v1.3.md` | `f2958ac17636c20aff79ad55fd42cd8ae8ea1c9692d14e74bac19e434adfc527` |
...
The eight suite digests are the post-v3-integration values except Data Fabric,
whose §2.1 digest changed for the accepted v4 correction. The ninth digest is
the versioned correction dossier that records evidence, alternatives, and the
WP19 review boundary.
...
**In-flight spec revisions captured at planning and execution time.** The
data-fabric spec was revised in place four times before v4 execution resumed.
...
*Fourth revision — v4 exact-graph correction.* WP01 resolution proved that
the released `buoyant_kernel` 0.25.x `arrow-58` feature unconditionally
enables `object_store` 0.13.2's `aws`, `azure`, `gcp`, and `http` features.
Data Fabric §2.1 now defines local-profile isolation as an application-owned
provider/configuration boundary and requires honest reporting of that latent
compiled surface. The default graph must omit `deltalake-aws` and the AWS SDK;
only `s3-storage` may activate them. Exact advisory exceptions are registry-
backed and reviewed at WP19. The accepted evidence and alternatives are in
`docs/designs/codefabric_local_storage_dependency_isolation_design_v1_2026-08-20.md`.
...
- **D-11 — Local storage isolation is an operational authority boundary.**
  Preserve the exact FAB §2.1 dependency baseline. The default feature set
  omits `deltalake/s3`; the default graph must contain neither
  `deltalake-aws` nor `aws-sdk-*`; and `local-workstation-v1` accepts only
  local filesystem provider configuration. The graph validator must also
  report, not deny, the `object_store` cloud features forced by the pinned
  kernel's `arrow-58` feature. Exact advisory exceptions live in a committed
  machine-readable registry and receive mandatory review at WP19. Doctrine:
  P8 least privilege, P27 provenance, and P31 executable governance
  (Advances). Design correction v1 owns the alternatives and evidence.
...
- Baseline is a **commit-plus-working-tree identity**: commit
  `a689f1ddf712c0f8fe5cf93d9a50a559f84e4b91` (branch `master`) plus working-
  tree digest `96caca12cf29ecce3781f0255d2650b2ccf8b8a434301af3d9fc609c8ba3a001`
  captured after the accepted design correction and before v4 plan edits.
  The reconciled v4 state preserves the per-file inventory of user edits and
  v3 execution changes.
- The original v3 pre-edit `just ci-fast` run on 2026-08-20 was **not clean**.
  ...
  failures. WP01's reshaped fmt/check/clippy/nextest/doctest/typos/machete/
  shear/graph/ast-grep gates are green at v4 issue time.
- WP01 is partially implemented in the v4 baseline: the stable exact graph,
  compatibility probes, seed deletion, root command/CI reshape, documentation,
  and graph/governance harness are present. The old root Python project and
  its lockfile are absent. V4 resumes rather than redoing that work.
- Environment (SessionStart report): uv 0.12.5, Python 3.14.7, stable Rust
  ...
  sccache/just/nextest/typos/rg/ast-grep/cargo-deny/audit/shear/
  machete present, direnv allowed.
- License evaluation is excluded from the active policy recipe and CI by the
  user's 2026-08-20 direction. Advisory, ban, source, and `cargo audit` checks
  remain active; dormant license configuration is not completion evidence.
- Exact policy resolution found four advisory IDs. Their constrained registry
  and deny reconciliation are remaining WP01 work under D-11; no broad or
  undocumented suppression is accepted.
...
- **I-16** `local-workstation-v1` constructs only local filesystem storage;
  the default graph contains no `deltalake-aws` or AWS SDK package even though
  the pinned kernel forces latent `object_store` cloud features. The actual
  feature surface and every advisory exception remain machine-visible and are
  reviewed at WP19 (Data Fabric §2.1; D-11).
...
### v3 → v4 execution-discovered design correction (current)

- **Trigger:** exact WP01 graph resolution contradicted FAB §2.1 and v3's
  literal no-cloud-compilation premise.
- **Owner correction:** Data Fabric §2.1 now distinguishes the latent compiled
  `object_store` feature surface from application-owned runtime provider
  authority. The accepted design dossier is
  `docs/designs/codefabric_local_storage_dependency_isolation_design_v1_2026-08-20.md`.
- **Plan correction:** stable IDs and dependency order are unchanged. WP01,
  WP05, M01, WP19, the gate matrix, and risk policy now carry truthful graph
  evidence plus exact advisory-registry ownership. V3 remains immutable.
- **User-directed scope:** license evaluation is excluded from active gates;
  advisories, bans, sources, and audit remain enforced.
- **Revalidation:** default graph lacks `deltalake-aws`/AWS SDK; S3 graph adds
  them; kernel-forced cloud features are reported; registry, deny ignores, and
  Cargo.lock agree; WP19 refreshes or reopens every exception.
...
**Target invariants.** I-10, I-12 (preparation), I-16, L-01–L-04. Doctrine
P6, P8, P27, P31.
...
rows).
**Target invariants.** I-05, I-08, I-12, I-16. Doctrine P8, P16, P29, P31.
...
**Gates.** Root gates + refreshed advisory-registry gate + fabric suite
...
- the exact local/S3 feature table is present; the default graph contains no
  `deltalake-aws` or AWS SDK package, the kernel-forced `object_store` cloud
  features are reported, and local profile configuration cannot construct a
  cloud provider; gix resolves both SHA algorithms; rusqlite resolves
...
- **R-11 kernel-forced cloud surface and advisories (accepted, D-11).** The
  exact Delta kernel activates latent `object_store` cloud features in the
  default graph, selecting four current RustSec advisory IDs. Mitigation:
  runtime local-only provider authority, no default `deltalake-aws`/AWS SDK,
  exact registry-to-deny-to-lock equality, and mandatory WP19 refresh.
  Escalation: local-profile reachability, a new vulnerability, or inability
  to remove or deliberately renew an exception at WP19 → design reopening;
  no silent suppression or automatic renewal.
...
*End of plan v4 — execution-corrected successor to the audit-integrated v3 plan; see the Audit Integration Log.*
