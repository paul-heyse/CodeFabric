...
date: 2026-09-04
status: draft
design_path: docs/designs/codefabric_real_time_cpg_planning_contracts_design_v1_2026-09-04.md
design_version: v1
...
state_path: docs/plans/state/codefabric-real-time-cpg_v1_state.json
cutover: true
supersedes_on_activation: docs/plans/codefabric_execution_proved_relational_data_fabric_implementation_plan_v7_2026-09-02.md
---

# Complete real-time Python/Rust CPG — implementation plan v1
...
This is the realization plan for the **entire** comprehensive review, not only its first production vertical or fourteen defect fixes. Review §§2–6, LD-01–LD-07, all 18 native-capability selections and the full GEN/ONT selected profiles remain in scope. The planning-contract addendum resolves its draft architectural blockers and adds the necessary LD-RT08 native-maintenance correction so full completion cannot be reduced to permanent deletion denial.
...
- No indiscriminate feature enablement, speculative distributed/GPU/Flight/ADBC adoption or automatic dependency upgrade. LD-RT08 is the narrow required exception: native maintenance must acquire a certified command-compatible capability.
...
The frontmatter records HEAD and the same dirty implementation identity used by the review. Re-derive with:
...
The digest excludes unrelated untracked files and documentation; it is identity, not correctness or ownership. Attribute overlapping dirty edits before execution and preserve concurrent work. The planning session reran `just ci-fast`: exit 101 at `root-clippy`, after root formatting/type checking. The existing [v7 status assessment](../reviews/implementation_status_codefabric_execution_proved_relational_data_fabric_implementation_plan_v7_2026-09-02_2026-09-04_v2.md) additionally records stale WP65 measurements and refusal of dirty-candidate certification; these were not silently converted to passes. WP105 must close residual quality failures. A packet cannot claim completion while one of its required local checks is red; baseline attribution is not a waiver of the terminal matrix.

This task writes a plan and its additive planning design only. It creates no state file, changes no active pointer, and does not start execution. The future state follows schema version 2. Draft validation must not manufacture state merely to satisfy an active-plan checker. Approval and confirm-gated activation are later separate operations.
...
The selected planning target is accepted-with-named-assumptions in the addendum. The source review remains immutable draft review evidence; its unresolved details are not silently ignored. The following is the only declared-input digest table, computed once during authoring. Recompute through tooling for validation; never restamp stale rows.
...
- Proposed test `rt_cpg_wp77_faults`, invoked by the same packet recipe: mutate a complete claim, producer operand, required expectation, feature edge or test selector; the owning checker rejects it. Applicable DB checks also prove old-route absence and target-positive behavior.
...
**Design and library references.** Review F13/F14, §3.9; D-RT05; v7 WP59/WP61; LIFE §6 and cancellation/lease obligations; LD-02/07.
...
- Proposed test `rt_cpg_wp79_behavior`, invoked by `just real-time-cpg-packet-check WP79`: concurrent candidates, queries and retained epochs share one enforceable envelope and release charges once.
...
- Proposed test `rt_cpg_wp79_operations`, invoked by the same packet recipe: live cancellation under full data admission joins work and leaves reserved control operations responsive.
...
**Packet-Local Gates.** `just real-time-cpg-packet-check WP79`; `just cancellation-tree-check`; `just grpc-flow-control-contract-check`; `just grpc-slow-consumer-check`; `just datafusion-cache-resource-operations-check`; `just root-check`. Proposed recipes are identified in §7; they must exist and select substantive cases before completion.

**Integration Milestone.** M18.

**Replan Triggers.** Reopen if any native blocking operation has neither a safe bounded cost envelope nor controllable process/task ownership, or a claimed aggregate budget excludes a material allocation path. Also apply §9's current-tree, dependency-closure, security and operational triggers.
...
Future state path: `docs/plans/state/codefabric-real-time-cpg_v1_state.json`; schema version 2. No file is created by this planning request.

After explicit plan approval and execution authorization, use the repository's confirm-gated activation workflow. It creates validated state before atomically selecting this plan and never overwrites existing state. The prior active v7 plan/state remain history; its judgments are not imported as target completion. A failed activation leaves the prior pointer unchanged.
...
### 8.3 Draft validation without execution

The existing active-plan aggregate requires state, so validate this inactive draft through the repository's current nonmutating validator functions:
...
plan = ROOT / "docs/plans/codefabric_real_time_cpg_implementation_plan_v1_2026-09-04.md"
...
PY
just plan-dependency-check docs/plans/codefabric_real_time_cpg_implementation_plan_v1_2026-09-04.md
```
...
This artifact is a **draft execution specification**, not an activated plan or implementation result. The next safe action is independent plan audit/approval, followed by explicit execution authorization and activation. No state file, active pointer, production code, dependency pin, measurement artifact or authoritative master is changed by creating it.

The authoritative scope is the review's full design plus the accepted planning-contract addendum, including LD-RT08. Existing v7 boundaries and valid expectations remain required; no inherited red gate, permanent provider gap, disabled optimizer or denied maintenance operation is disguised as the completed product.
