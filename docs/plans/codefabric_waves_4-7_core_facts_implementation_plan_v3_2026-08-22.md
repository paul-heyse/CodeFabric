...
design_version: v1.0
predecessor_plan_path: docs/plans/codefabric_waves_4-7_core_facts_implementation_plan_v2_2026-08-21.md
baseline_commit: 1d1207c3f8fc0ed25a02fae31cc253bb1a35c5bf
state_path: docs/plans/state/codefabric-waves-4-7-core-facts_v3_state.json
audit_path: docs/reviews/plan_audit_codefabric_waves_4-7_core_facts_implementation_plan_v1_2026-08-21_2026-08-21_v1.md
cutover: true
---

# CodeFabric Waves 4–7 Core Facts — Implementation Plan v3

Version 3 is the user-approved execution successor to v2. It preserves the audited
WP27–WP53 outcomes, dependencies, milestone boundaries, decommission obligations, and
proof matrix. Its sole target-design correction raises the stable Rust compatibility
floor from `1.94.1` to `1.95.0`, because every exact Ruff `0.0.7` component required by
LD-23 declares Rust `1.95`. The Ruff provider train remains unchanged.
...
| docs/plans/codefabric_waves_4-7_core_facts_implementation_plan_v2_2026-08-21.md | f41c2e764d01e526cd191e9e99cf89e4c3948c9ab9a797954b5ed2c797062a33 |
...
Baseline HEAD is 1d1207c3f8fc0ed25a02fae31cc253bb1a35c5bf with a clean tree. The cached
...
DB07–DB09. Before WP27 begins it SHALL also make this v3
artifact the active plan in every generic plan/status/validation entry point and add
focused default-plan coverage. Both explicit validation of this plan/state pair and the
argument-free defaults must resolve this plan rather than the Waves 0–3 predecessor.
This adoption prerequisite is execution infrastructure, not a product work packet; if
it is not complete, WP27 is blocked.
...
- **D-42 — Active-plan adoption is a stop gate.** The first executor action creates
  schema-2 state, points generic plan tooling at v3, and proves explicit and default
  validation/status resolution before WP27. Integration does not manufacture state.
...
|---|---|---|---|---|
| F-001 | applied; adoption-gated | Plan §3, §14, §16 | v3 adoption is an explicit pre-WP27 stop gate; integration never fabricates execution state | exact command exit 1 at predecessor-default `artifacts-check`, as expected before state/active-plan adoption |
...
- [ ] §3 adoption — schema-2 state exists; explicit/default artifact validation,
      status, and active-plan tests resolve this v3 plan
