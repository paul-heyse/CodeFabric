...
# CodeFabric Waves 0–3 Foundation — Implementation Plan v1
...
- Reconciliation authority tables and derivations — Waves 5/12/13+.
...
contract fingerprints inside `contracts/` use BLAKE3 `b3:` digests per the
manifest's AC-G-02, including the manifest-carried digests of the prose
...
1. Each spec's `AC-G-*` completion contracts override less-specific prose in
   ...
   Fact Gen §7.3 (six wire operations); Lifecycle §43 over its Appendix F
   (path structs).
...
- **D-01 — Four build domains as three Cargo roots plus one uv project.**
  ...
  **standalone Cargo roots outside any shared workspace**, each with its own
  ...
  verbatim by roadmap Wave 0 WP2/WP3). They cannot be workspace members: a
  workspace shares one lockfile and one toolchain resolution, which would
  violate both mandates. The Python adapter is `codefabric-cpg-mcp/` exactly
...
- **D-06 — One persisted workspace state machine with projections.** The
  three overlapping machines (Lifecycle §18 runtime lifecycle, AC-G-10
  registry machine, AC-G-28 public startup states) are realized as: the
  AC-G-10 registry machine persisted in the operational store; the §18
  lifecycle as the coordinator's runtime state (persisted in
  `worktree_state.lifecycle_state_code`); AC-G-28 startup states as a derived
  projection. Consistent with §130.2's single lifecycle column. All are
  generated from the AC-G-25 registry YAML (Ambiguity A-14).
- **D-07 — Wave 3 synthetic ingest is a bounded transition.** §72 makes the
  `ReconciliationEngine` the sole canonicalization authority, but real
  reconciliation is Waves 5/12. Wave 3 implements the §63 observation
  boundary (validated Arrow streams, manifests, generation fences) plus a
  `SyntheticCanonicalIngest` that is the *only* ingress and is explicitly
  marked as the pre-reconciliation stub. Exit condition: replaced by Wave 5's
  minimal reconciliation packet. Owner: WP20. Doctrine: bounded transition
  recorded per doctrine policy.
- **D-08 — Operational vs control-plane authority split.** The SQLite
  ...
  operational registry at enable/publication time and never edited through
  query SQL. `cpg_control` exposes both with disjoint names (`workspace` =
...
- **D-09 — Deep-integration toolchain posture (user decision, 2026-08-20).**
  ...
  `rust-src`, `llvm-tools-preview`) with `rustc_public` plus narrowly scoped
  ...
  Repo-spec §76's adoption conditions are satisfied deliberately: exact date
  pin and matching components (WP02), the semantic golden corpus (registered
  as an obligation now; its first fixtures land with the extractor's first
  real facts in Wave 5), and the managed update procedure below. The same
  ...
  **Managed update procedure** (both domains; deliberate per update, never
  ...
  (4) update the toolchain-bundle record and digests (AC-G-02/AC-G-14, §4.1
  item 6); (5) daemon↔extractor/sidecar version negotiation keeps a stale
...
- **I-06** Absence is never proof of absence: unknown kinds, capability gaps,
  ...
  never means "unknown value" (suite inv. 6; AC-G-71 rule 2, AC-G-73).
...
| LD-12 | Nightly extractor toolchain — **committed baseline (D-09)** | `nightly-2026-08-18` + `rustc-dev`,`rust-src`,`llvm-tools-preview`; `rustc_public 1.100.0-nightly`; exact rustc commit hash recorded at WP02 | spec-pin (Fact Gen §2); adopted as settled architecture per D-09 (repo-spec §76 conditions satisfied deliberately); the root toolchain stays stable. Consumption mode is `rustc-dev` components; the WP02 probe confirms mechanics only and records the exact commit hash (A-22 → decided). Updates follow the D-09 managed procedure. |
...
- *Likely touch:* `_typos.toml` (path excludes), `bacon.toml` (drop
  ...
  policy), `.gitignore` (dist/ no longer produced).
...
`nightly-2026-08-18` (components `rustc-dev`, `rust-src`,
`llvm-tools-preview`), building executable `codefabric-rustc-extractor` that
prints its exact toolchain identity (rustc version + commit hash, per AC-G-14)
...
- *Must touch:* new `rustc-extractor/{Cargo.toml,Cargo.lock,rust-toolchain.toml,src/main.rs}`;
  ...
  toolchain-posture text (D-09 ratification).
...
§60.2, §68.6, §79 (Phase-1 subset), §0.6; LD-11.
...
yet (Serving §79 phase discipline). All logging to STDERR.
...
3. CI jobs per domain with pinned actions; `uv sync --frozen` for the
   ...
   now); duplicate-family check active immediately.
...
this is transport mechanics, not a design change (Serving §8 names gRPC+UDS,
not a crate).
...
requirements.jsonl,traceability.jsonl}`, `registry/` — the fourteen named
registry files, `identity/{cbef-v1.yaml,type-algebra-v1.yaml,
...
ascending-field-tag emission with verifier rejection of out-of-order records,
...
(AC-G-15 constructor set, normalization/interning rules, de Bruijn binders,
version field) with a canonical type-algebra encoder and its own KAT vectors
...
§43 (PlatformPath/GitRepoPath), AC-G-09 preimages; Data Fabric §7.1–7.3.
...
`SEMANTIC_PHRASE_UNRECOGNIZED`…), enum/flag registries with the §62 verbatim
code tables (62.1–62.9), provider registry (record authored per A-19),
`model-pack.schema.json` (AC-G-38 format schema; no packs ship in 1.x
baseline), phrase registry + `english-controlled-v1` grammar artifact
(AC-G-44 EBNF + registry record schema), and AC-G-25 state-machine YAML for
the Wave-2 machines (`WorkspaceLifecycle`, the AC-G-10 registry machine,
`SourceTrustState`, `EventStreamHealth`, `GitAccelerationStatus`) with
`from/event/guard/to/actions/idempotency_key/error_on_illegal` rows and
model-checked reachability.
...
all 9 §62 code tables; 12 unknown kinds + 9 reason classes + 4 negative
families; 13 projections; 37 effect + 10 resource codes; every error code
named anywhere in the 1.3 suite; and a phrase-registry entry set covering
**every** Query §50–§102 catalog section — the verifier counts sections and
fails on any gap. If the phrase harvest stalls M02, the split is a plan
revision creating WP08b (R-07); there is no in-packet deferral.
...
- *Preflight:* enumerate Query §50–§102 section list (spec-outline) as the
  phrase-harvest checklist the verifier will count against.
...
`capability_status`, `diagnostic`, `enum_catalog`) and universal facts
...
`cpg-semantic-query-response.schema.json` (per Query §103–104: envelope
fields, public-ID patterns), plus `query/planspec.schema.json` (AC-G-46
...
- *Negative:* a committed drift fixture (schema changed, version unchanged)
  ...
  run; `Utf8View`/JSON-blob/EAV shapes rejected by generator tests (§5.1
  prohibitions).
...
enum verbatim, message field sets per AC-G-58, 4 MiB/1 MiB caps, `identity|zstd`
...
run-state enum from the §85 registry, supersession keys, credit-control
...
suite defines those protocols in prose. The feature registry backs the
handshake feature bits under the AC-G-03 per-family compatibility matrix.
...
AC-G-08 field set verbatim (sqlite-wal operational store, delta-local
fact store, network listeners disabled, overlay journal disabled, symlink
...
zero-orphan rules per AC-G-04 (orphaned mandatory kinds, unowned schema
columns, untested requirements without an explicit `verification_deferred`
record fail).
...
- *Preflight:* none material — state/runtime/config roots are fixed by
  AC-G-08's platform table (macOS: `~/Library/Application Support/CodeFabric`
  state+config, private short-path directory under `$TMPDIR` for runtime;
  Linux: XDG roots). Verify the macOS `$TMPDIR` path stays under the UDS
...
refuse-to-open-newer-schema; the Wave-2 table set from §130 (workspace
registration + `worktree_state` keyed by `workspace_id`, `git_state_vector`,
`git_operation_run`, nested-root exclusion records (A-15), credentials
metadata placeholders, recovery/generation counters) generated from WP09 DDL;
...
**Outcome.** Source-image capture per Lifecycle §33 + AC-G-33: the 7-step
stable-read algorithm with metadata fencing and retry/defer (default retry 3),
BLAKE3-256 digests, content-addressed immutable blob store (temp-write,
...
`GitInventoryResult`); exact-path open with the trust policy (config sources
restricted, env overrides off, hooks/filters/credentials/network disabled —
recommended defaults §76); repository kind/bare detection, work/git/common
...
§78–§80, §156; roadmap §7 WP7. LD-06 (all caveats).
...
`source_trust_state = CURRENT` after an authoritative inventory; queries —
...
- *Must touch:* root `src/` fabric modules (schema-registry runtime, delta
  ...
  toolchain bundle update per §4.1 item 6.
- *Preflight:* compile probe for `CreateBuilder` configuration keys
  ...
  checks at the pinned rev.

**Acceptance evidence.**
- *Behavioral:* bootstrap creates the full namespace on a fresh workspace;
  reopen validates digests; round-trip gate green per table.
...
consuming synthetic observation fixtures from `contracts/fixtures/synthetic/`
(authored in this packet). Scope note: this is the **fabric-side** ingest
...
- *Behavioral:* synthetic fixture families ingest to exact expected rows;
  §112.2 batch matrix green (empty batch, one row, all-nullable-null, max
...
subset: PK uniqueness, 16-byte IDs, relation endpoints exist, owners exist,
span sanity, row counts) → VALIDATING→VALIDATED→COMMITTING→COMPLETE → AC-G-26
...
handle (whole-table OCC makes any concurrent commit conflict) → post-commit
...
pointer-table commits as failures (probe contradicts §9.22's model) → plan
...
partial replacement of a FULL_TABLE_REPLACE table rejected); AC-G-22
...
WP22's publication + pointer CAS).
...
**Dependencies.** WP22, WP23; WP13 (SQLite).
...
`cpg_control` (§13.12 minimum set that exists in Waves 2–3); **the pinned-
...
**Change surface / preflight.** Programmatic `ViewTable`/logical-plan view
construction compile probe (the reference documents only SQL `CREATE VIEW`);
`MemTable::try_new` partition shape (`Vec<Vec<RecordBatch>>`).
...
**Replan triggers.** Neither programmatic view nor thin provider composes
the anti-join plan with acceptable correctness → plan revision to a custom
...
### M02 — Wave 1 exit: Readiness Gate A (after WP06–WP11)
...
| A-06 | `UNKNOWN_MEMORY` vs `UNKNOWN_MEMORY_LOCATION` | Ontology §66 vs AC-G-73 | PREC: AC-G-73's 12 names win (WP08) |
...
| A-11 | `TableSpec` lacks overlay policy; two policy enums | Data Fabric §11 vs §91/AC-G-21 | AUTHOR: TableSpec gains `overlay_policy: OverlayMutationPolicy`; `OwnerReplacementPolicy` retained for the durable §68 mutation class; relationship documented (WP09) |
...
| A-14 | Three overlapping workspace state machines | Lifecycle §18 / AC-G-10 / AC-G-28 | DEV: D-06 one persisted machine + projections, generated from AC-G-25 YAML (WP08/WP14) |
...
| A-24 | gix pin omits `revision` feature | Lifecycle §39 | PROBE: WP17; add feature only if HEAD resolution requires it (DEV recorded if so) |
...
| Extractor | compile + tests on pinned nightly | `extractor-check` / `extractor-test` (runs in `rustc-extractor/`) | M01 |
| Sidecar | compile + tests + policy | `sidecar-check` / `sidecar-test` / `sidecar-policy` (runs in `pyrefly-sidecar/`) | M01 |
...
Wave 3:   M03 ──► WP19 ──► WP20 ──► WP21 ──► WP22 ──► WP23 ──► WP24 ──► WP25 ──► M04
          (linear; WP23's rebase consumes WP22's pointer CAS — hard edge)
```
...
- [ ] WP08 — registries + state machines
...
- [ ] WP23 — hot overlay + consolidation + rebase
...
- **R-07 Wave 1 registry breadth.** The ontology registry spans ~50 fact
  domains; authoring is large but mechanical. Mitigation: registry schema +
  verifier land first (WP08 acceptance is invariant enforcement, not domain
  completeness prose); phrase-registry long tail explicitly tracked inside
  WP08. Escalation: if Wave-1 breadth stalls M02, split WP08 by registry
  family — plan revision, sequence unchanged.
