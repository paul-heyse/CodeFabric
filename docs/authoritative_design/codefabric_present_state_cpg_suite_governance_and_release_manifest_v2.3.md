...
`codefabric-relational-data-fabric-suite`. It governs the versioned v2.3
authoritative design suite. The v2.2, v2.1, v2.0, and v1.3 suites remain immutable
...
`codefabric-relational-data-fabric` version `2.3.0`; no generated manifest,
...
## 1. Product boundary retained by v2.3
...
- one `WorkspaceSupervisor` and one Rust daemon per workspace, plus one
  attach-only `AgentStdioLauncher` and modern-only FastMCP 4 presentation cell per agent;
...
V2.3 carries the v2.2 ontology, provider, analysis, data-fabric, lifecycle, and eight-form query
product forward. Its substantive successor boundary is the presentation and daemon contract:
protocol era `2026-07-28` only, one atomic query-start outcome, daemon-authored guarded input,
daemon-minted public resource handles, authorized reference completion, explicit cancellation,
and a reconstructible Python cell with no independent semantic or durable authority.
...
22. `codefabric.cpgd.v2` starts ordinary queries through one closed
    `Accepted | InputRequired | Rejected` outcome; explicit validation is pure dry-run data;
23. every input challenge and continuation is daemon-authored, bounded, expiring, replay-aware,
    and reauthorized on every modern request leg before accepted work exists;
24. public resource handles are daemon-minted only after authoritative lease/package state exists,
    contain no secret lease token or path, and are reauthorized for every read and release;
25. one FastMCP 4 cell serves only protocol `2026-07-28`, publishes exactly four tools, two
    resource families, one narrow authorized reference-completion surface, and no prompts;
26. host cancellation after acceptance invokes daemon cancellation exactly once under a bounded
    cleanup budget, while observation loss never resubmits query start;
27. Python owns no CPG, semantic/session/task/cache/resource-lease/auth registry, generated
    attempt identity, duplicate freshness, or static coequal schema authority; and
28. CodeFabric registers no application extension or UI component. The only allowed extension
    advertisement is the exact empty framework-owned `io.modelcontextprotocol/ui: {}` emitted by
    the pinned FastMCP 4.0.0 modern server; it is never authorization, component registration,
    semantic capability, or behavioral proof.
...
The following are Class 1 commitments once issued and cannot be rewritten:
...
The pre-release v2 serving draft is not a deployed compatibility authority. Its atomic-start,
challenge, public-handle, completion, and cancellation revision lands once as a coherent source,
descriptor, generated Rust/Python, service/client, and fixture transaction; only that issued target
then becomes immutable Class 1 authority. No compatibility v3 service is created for the draft.

The live presentation dependency profile is exactly FastMCP 4.0.0, MCP SDK 2.1.1, and Pydantic
2.13.4 on the repository Python authority. The camel-case compatibility bridge is disabled.
FastMCP application sessions, tasks, auth, caches, providers, transforms, proxy/gateway surfaces,
Apps, Tool Search, Code Mode, sampling, and root listing are outside the local product profile.
...
- atomic start, guarded-input binding, public-handle authorization, reference completion,
  modern-protocol admission, cancellation acknowledgement, reconnect, and presentation zero state;
...
The v2.3 expectation release independently distinguishes live modern discovery containing exactly
the empty framework-owned UI advertisement, the empty application extension registry, and resolved
component catalogs containing no UI/App tool, resource, provider, transform, prompt, or handler.
Production inspection cannot author those expected values.
...
The edge is one factory-constructed, lifespan-owned, sessionless FastMCP 4 presentation cell with
one authenticated `grpc.aio` channel and a hidden application-owned daemon port. Its catalog is
exactly `query_code_graph`, `validate_code_graph_query`, `get_code_graph_status`, and
`get_code_graph_reference`; two bounded result/reference resource families; one authorized
reference-template completion handler; and no prompts or application extensions. Every older MCP
era is rejected before daemon business dispatch.

Ordinary query calls issue one atomic daemon start. An incomplete request returns a typed daemon
challenge which FastMCP maps through native guarded input and sealed request state; every leg
re-enters admission and daemon authorization. A valid continuation may accept once. Pure validation
never reserves capacity or creates a challenge, task, query, package, or lease. The daemon alone
mints public resource handles and completion candidates and owns cancellation and reconnect/resume.
...
The v2.3 serving cutover additionally removes FastMCP 3 pins/APIs, old MCP SDK use, camel-case
compatibility, import-time global-server assumptions, normal-path Validate-then-Start, duplicate
freshness and attempt IDs, Python resource-lease maps, static adapter schema authority, phantom
prompts, stale serving/evidence recipes, and any selectable v1 runtime service. Target consumers
and independent causal evidence precede physical purge; performance is measured after purge.
...
| QRY | compositional requests, preparation/input requirements, atomic-start meaning, compilation, evidence, results, and semantic errors |
| LIFE | source change, commands, update waves, publication, activation, policy, supervisor/daemon lifecycle, leases, and recovery |
| SRV | daemon RPC, launch-grant handshake, modern FastMCP/STDIO presentation, guarded input, public handles, completion, cancellation, authorization, streaming, and operations |
...
The v2.3 successor advances through five readiness milestones after retaining the ancestrally
proved v2.2 relational substrate:

1. synchronized v2.3 authority and independently authored modern expectations;
2. exact startup reconciliation and target-coherent daemon contract;
3. real installed modern FastMCP 4 delivery;
4. independent evidence, complete serving purge, and measured release candidate; and
5. FreshActivation, sole-target authority, and terminal certification.
...
- `fastmcp4-successor-authority-integrity-check` and
...
- modern-protocol, atomic-start, guarded-input, public-resource, completion, cancellation,
  reconnect, UI-advertisement, application-extension-zero-state, and real-STDIO checks;
...
The v2.3 suite is realized only when one reconstructible FabricEpoch supplies the
complete present-state product behavior; every required proof is green; the
modern FastMCP 4 vertical is the only serving route; the target runtime is the only mutation
authority; and predecessor
registries, generators, bundles, mutable pointers, opaque payloads, duplicate
query paths, FastMCP 3/duplicate Python authority, package residue, and selectable fallbacks are
absent outside immutable history. The inert empty framework-owned UI advertisement remains an
observed library capability only while application extension and UI component zero state hold.
