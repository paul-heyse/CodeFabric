...
successor_path: docs/authoritative_design/codefabric_present_state_cpg_suite_governance_and_release_manifest_v2.1.md
predecessor_path: docs/authoritative_design/codefabric_present_state_cpg_suite_governance_and_release_manifest_v1.3.md
---
...
`codefabric-relational-data-fabric-suite`. It governs the versioned v2.0
authoritative design suite. The v1.3 suite remains immutable historical evidence
...
`codefabric-relational-data-fabric` version `2.0.0`; no generated manifest,
...
## 1. Product boundary retained by v2
...
## 2. Governing relational model

Governance is data executed by the fabric, not a family of hand-maintained
registries. The model compiler replays immutable migrations into typed Arrow
relations including:

| Relation | Governing meaning |
|---|---|
| `model_migration` | ordered authored change to the semantic model |
...
The closed Rust intrinsic algebra defines the only executable primitive
vocabulary. Relations parameterize those primitives. A row that does not alter
compiled schemas, plans, policies, proof, or lifecycle behavior is inert and is
rejected by causal conformance.

Historical AC-G-01 through AC-G-84 identities are preserved in immutable
history and imported into contract/migration relations with an accountable
...
1. one current model authority and one current suite;
...
7. conflicting evidence is retained and resolved only by modeled authority;
...
- suite, model, compiler-release, schema-contract, provider-adapter, analysis,
...
Compatibility is evaluated from modeled rules. Unknown compatibility fails
...
Each proof result names the exact model, input, implementation, and oracle
...
- replayed model relations and compiler release;
- model-derived logical and physical SchemaContracts;
...
1. replayed model foundation and independent expectations;
...
- `model-replay-check` and `model-causality-check`;
...
The v2 suite is realized only when one reconstructible FabricEpoch supplies the
