...
predecessor_path: docs/authoritative_design/codefabric_continuous_cpg_update_lifecycle_management_specification_v2.2.md
---
...
The v2.2 predecessor is immutable release history. V2.3 preserves its current-source semantics,
...
and executed relational proof. The modern MCP guarded-input rounds introduced by QRY/SRV are
pre-acceptance presentation legs and create no lifecycle task, query lease, or alternate update
path. Once accepted, query cancellation, observation loss, restart, retention, and recovery remain
owned by this lifecycle and the daemon command/query authorities.
