...
artifact_id: CF-ONT-2.0.0
suite_id: codefabric-relational-data-fabric
suite_version: 2.0.0
artifact_tag: ONT
artifact_version: 2.0.0
authority_status: historical
successor_path: docs/authoritative_design/code_property_graph_present_state_fact_ontology_specification_v2.1.md
predecessor_path: docs/authoritative_design/code_property_graph_present_state_fact_ontology_specification_v1.3.md
---

# Code Property Graph present-state fact ontology specification v2.0

## 0. Governing contract and transition status

The stable artifact identity is `CF-ONT-2.0.0`. Together with the other current
`codefabric-relational-data-fabric` v2 masters, this artifact is the sole current ontology
authority. Its v1.3 predecessor is immutable historical evidence, not a coequal runtime or
authoring authority.

This successor preserves the released CodeFabric fact, identity, unknown, provenance, query,
and wire behavior while replacing generated registries, bundles, censuses, fingerprints, and
hand-maintained traceability as semantic authorities. Until the v2 runtime cutover proves and
activates a `FabricEpoch`, the deployed runtime may remain legacy; publication of this master
does not claim runtime conformance.

The authoritative ontology for a model epoch is the result of replaying accepted immutable
`ModelMigration` events with the exact `FabricCompilerRelease` into typed model relations.
Execution reads those relations. This prose fixes their semantics and acceptance obligations;
it does not establish a parallel current catalog.
...
them. A current live catalog, model relation, or exact library API beats a cached navigation
page or derived documentation.

### 0.2 Relational authority

At minimum the replayed model exposes typed relations equivalent to:

```text
model.fact_family
model.entity_kind
model.property_kind
model.relationship_kind
model.type_algebra_variant
model.identity_recipe
model.authority_rule
model.derivation
model.provider_boundary
model.query_requirement
model.unknown_rule
model.schema_contract
model.proof_obligation
```

Names above identify semantic roles, not a frozen physical table-name API. Their stable model
IDs, keys, dependencies, and `SchemaContract` links are produced by replay. A generated file,
...
contracts, the minimal bootstrap metamodel and closed compiler algebra, immutable migrations,
independent accepted expectations, and historical records. Current schemas, provider surfaces,
capabilities, traceability, validation status, and coverage are derived by executing the model
and inspecting the epoch.

### 0.4 Released compatibility

This version does not remove the eight public semantic request forms, public entity/fact ID
encodings, released error/result meanings, Protobuf field identities, accepted tombstones, or
one-snapshot response semantics. Internal provider payloads and generated catalogs are not wire
commitments. Every released ID receives an explicit preserve, migrate, supersede, or tombstone
decision before physical deletion.
...
link to an executable `SchemaContract` derived from the model; Arrow metadata alone never makes
...
Optionality is model-defined per fact family. Ranges are valid only as
...
The dimensions remain orthogonal model rows, not one confidence score. Released names retain
...
Accepted numeric allocations remain immutable model-migration data. A generated enum or prose
table may render them but cannot become current authority.
...
model-derived recipe-specific builders; arbitrary tagged-field vectors and delimiter strings are
non-conforming.
...
The epoch registers model, provider-native, canonical, derived, capability/proof, and authorized
public-projection layers. Dependency direction is model/source/provider → canonical → derived →
query/proof. A higher layer cannot rewrite lower-layer evidence.
...
Core conformance requires typed model replay, `SchemaContract` closure, CBEF identity, complete
...
No model row, provider adapter, normalizer, derivation, query compiler, or proof compiler may ask
...
the model or schema is too narrow. If execution does not read a declaration, that declaration
...
behavior, and exact decode rejection rules remain wire commitments. Identity recipes are active
model relations produced by migrations; generated accessors are derived build products only.

## AC-G-15 — Canonical type algebra

The type algebra in §§10, 35, and 47 is a closed bootstrap discriminant set extended only by an
accepted model/compiler migration. Every type row has a canonical structural key, provider raw
...
The former machine-registry contract is superseded. The current ontology is the replayed model
relations plus their exact compiler release. Conformance joins those relations to live catalog,
provider, producer, query, and proof relations; it MUST fail on missing, duplicate, dangling,
inert, or uncovered rows. No new static registry or suite census may replace this contract.

## AC-G-71 — Property schema, value types, cardinality, null, and storage mapping

Each property/fact family links to one model-derived `SchemaContract` that owns logical Arrow
...
Profiles are selected model relations with dependency closure, not hand-maintained lists. Core,
...
Graph projections are model rows compiled into DataFusion-native plans or the highest bounded
...
The generation layer emits typed provider-native and derived relations conforming to this model.
The fabric registers them in one epoch and compiles normalization/authority/proof from model
rows. The query layer exposes only authorized semantic projections while preserving provenance,
...
1. isolated model replay and bootstrap closure;
2. complete relation/field/key/reference/`SchemaContract` closure;
3. released ID and wire compatibility, including old supported clients;
...
The compiler emits typed Arrow model relations and `SchemaContract` links from accepted
`ModelMigration` events. Rendering source, Markdown, JSON, Rust, Python, or diagrams from those
relations is allowed for navigation or tooling, but those artifacts are disposable and cannot
be imported as production semantic authority. Replaying the same migrations with the same exact
compiler release must reproduce complete schemas and logical rows; digests locate differences
but do not prove semantic equality.
