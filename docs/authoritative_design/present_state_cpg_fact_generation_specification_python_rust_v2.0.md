...
artifact_id: CF-GEN-2.0.0
suite_id: codefabric-relational-data-fabric
suite_version: 2.0.0
artifact_tag: GEN
artifact_version: 2.0.0
authority_status: historical
successor_path: docs/authoritative_design/present_state_cpg_fact_generation_specification_python_rust_v2.1.md
predecessor_path: docs/authoritative_design/present_state_cpg_fact_generation_specification_python_rust_v1.3.md
---

# Present-state CPG fact generation specification for Python and Rust v2.0

## 0. Governing contract and transition status

The stable artifact identity is `CF-GEN-2.0.0`. This artifact is the current generation
authority for the `codefabric-relational-data-fabric` v2 suite. The v1.3 predecessor remains
immutable history and a compatibility reference; its generated registries, provider DTOs,
...
unknowns, incremental behavior, semantic query capabilities, and released wire commitments. It
replaces the realization with exact provider-native Arrow relations, explicit application-owned
analysis producers, model-compiled normalization/authority/proof, relation-scoped Arrow IPC,
...
Accepted `ModelMigration` events replayed by the exact `FabricCompilerRelease` produce the
provider boundary, authority, normalization, derivation, query-requirement, coverage, unknown,
and proof relations that execution reads. Generated catalogs, YAML registries, censuses,
fingerprints, bundle manifests, and cached capability lists MAY diagnose or render the model but
...
Authority selection is a model-compiled relational plan. It retains all conflicting raw
...
```text
immutable source images + analysis context + model epoch
  ...
  -> model-compiled identity, normalization, authority, conflict and unknown plans
...
release/revision/toolchain/grammar, model epoch, analysis context, semantic-environment identity,
...
IPC batches. Control messages name protocol version, run/job, relation/model/schema IDs, source
...
The receiver validates the model-derived `SchemaContract` before accepting a batch: logical
...
renderings where a typed field does not exist. Normalized relations are separate model-compiled
projections linked through provenance. New provider variants require an explicit schema/model
migration or a typed raw escape/remainder already authorized for that family; they are never
...
lane, Pyrefly's semantic lane, model-compiled reconciliation, then CodeFabric owner-local and
...
Graph projections are compiled from model rows specifying relation inputs, node/edge identity,
...
Structural edges are emitted from provider-native roles/ranges and normalized through model
rules without deleting raw evidence.
...
model/source/provider input closure, completeness, and proof identity.
...
allowed only by a model rule that emits its method and ambiguity. Conflicting or unmappable
...
same model-derived schema. It accepts no provider object, borrowed tree/compiler value, semantic
...
for semantic data, DataFusion 55 for model-compiled relational work, and petgraph 0.8.3 only for
...
Jobs are application-owned control DTOs containing job/run ID, provider release, model epoch,
...
Graph execution receives a typed, model-compiled projection descriptor or logical plan with
...
The predecessor declarative model-pack registry is superseded. Model semantics enter only as
accepted immutable migrations replayed through the minimal metamodel. A packaged rendering may
be transported for diagnostics or review but cannot be loaded as parallel semantic authority.
...
The extractor handshake binds exact nightly/compiler/target/sysroot/model/protocol/schema/trust
...
Capabilities are derived by joining model demand, boundary contract, installed handler/schema,
...
## AC-G-38 — Relational model migration, matching, and trust

The former model-pack format is not authority. Accepted model changes are immutable migrations
with predecessor, compiler release, author/review provenance, and independently accepted
expectations. Replay produces typed relations; structural matching and plan compilation consume
those relations. Untrusted external model input is not executable without an accepted migration
and proof transaction.
...
File admission policy is typed model data and produces one of accepted, excluded-not-applicable,
...
links. The fabric registers provider-native/canonical/derived relations in one epoch and compiles
normalization, proof, and semantic query plans from model rows. Lifecycle owns immutable input,
...
Each check executes against the exact current adapter, model relations, and production path;
...
## Provider boundary for executed relational models
