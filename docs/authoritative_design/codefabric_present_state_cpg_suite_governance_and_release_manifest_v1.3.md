...
contracts/
  manifests/
    ...
    traceability.jsonl
  registry/
    design-principle-registry.yaml
    design-principle-detector-registry.yaml
    enum-registry.yaml
    flag-registry.yaml
...
Every governed semantic authority with more than one consumer SHALL declare at
least two derived operations that consume its typed model rather than its
rendered prose or generated bytes. Each generated projection SHALL carry the
stable authority identity and source digest required by the drift oracle. A
consumer that privately re-encodes a governed vocabulary, identity recipe,
schema, state machine, or query meaning violates AC-G-05 even when its bytes
happen to compare equal. The two design-principle registries above are the
machine authorities for principle ownership and detector execution; their
generated traceability and conformance views are projections, never second
owners.
...
Equality SHALL first require the exact versioned canonical schema fingerprint,
including field order, nullability, extension metadata, and governed schema
metadata. For a table whose governed primary key is unique, the comparator
SHALL prove that uniqueness and then compare every full projected row and row
count. Every other table SHALL compare the canonical projected rows as a bag,
including duplicate multiplicity. Bidirectional set difference, distinct
`EXCEPT`, and `EXCEPT ALL` without an independently proved multiplicity
contract are not accepted equality oracles.
...
The synchronized 1.3 suite adopts one ontology-compiled DataFusion/Arrow execution model.
Authored Schema Contract IR and registries remain the source authority. Their generated
`OntologyProgramBundle` is a deterministic Arrow-native projection and SHALL NOT become a
second handwritten authority. The bundle has distinct bootstrap, authored-content, logical-
program, IPC-member, and package identities; its package is one canonical manifest over ordered,
schema-homogeneous IPC members.

Build proof, runtime proof, observation, decision, and activation are separate authorities:

- compiler/package reproduction proves generated bytes only;
- `GateResultChecksum` records semantic gate output;
- `GateExecutionArtifact` records diagnostic plan, metric, and resource observations;
- opaque candidate receipts bind those results to the complete sealed candidate;
- an accountable owner decision authorizes one durable administrative activation transaction.

No digest-shaped string, filename, plan text, metric, test name, or caller-constructed dossier is
acceptance evidence. The active predecessor's trusted policy selects the required candidate gates;
the candidate cannot authorize itself. The suite manifest fingerprints the eight masters under
`docs/authoritative_design/`, generated program/package contracts, receipt and decision contracts,
and every versioned serving-epoch authority. Cross-document conformance SHALL reject a missing,
extra, or mixed-root master, a stale generated digest, or a mismatch among FAB, QRY, LIFE, SRV,
ONT, GEN, RM, and this manifest.
