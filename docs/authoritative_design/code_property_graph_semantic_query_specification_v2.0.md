...
successor_path: docs/authoritative_design/code_property_graph_semantic_query_specification_v2.1.md
predecessor_path: docs/authoritative_design/code_property_graph_semantic_query_specification_v1.3.md
---

# Composable Semantic Query Specification v2.0
...
The v1.3 predecessor is immutable release history. V2.0 preserves its eight request-form names,
public ID grammar, request/result roles, orthogonal status dimensions, canonical error names,
one-logical-response behavior, and inline/resource semantic equivalence. Released v1.3 clients
remain supported through explicit handshake negotiation; a v2.0 implementation MUST NOT silently
reinterpret a v1.3 field. Additive v2 epoch fields appear only in a negotiated response profile.

The realization changes: request semantics, phrases, bindings, projections, policies, and
capabilities are replayed model relations compiled by Rust into authorized DataFusion plans.
...
binding. `semantic_request_id`, MCP call ID, RPC attempt ID, and daemon query ID are distinct.
...
The semantic compiler joins request relations to replayed `model.query_form`, phrase, binding,
projection, policy, producer-closure, and live capability relations. It constructs native typed
...
Phrase resolution is deterministic against the pinned epoch's model and facts. Code identifiers,
...
is computed to source images, model decisions, provider runs, table versions, compiler release,
and independent expectations; it is not a maintained boolean.
...
and resources. No detached computation may continue under a completed/cancelled response.
...
The negotiated v2 profile additionally exposes `fabric_epoch_id`, compiler release, model epoch,
...
of the pinned epoch. Changing accepted model or provider/proof rows changes these surfaces after
activation without editing a package bundle. A form is advertised only when its required model
bindings, producer closure, functions/extensions, policies, and executable proof are present.
...
## 13. Executable acceptance obligations
...
| model-to-plan causality | `just model-plan-causality-check` |
...
Acceptance fixtures are independently authored and decoded. Production model/compiler output,
