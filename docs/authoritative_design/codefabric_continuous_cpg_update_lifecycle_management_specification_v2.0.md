...
successor_path: docs/authoritative_design/codefabric_continuous_cpg_update_lifecycle_management_specification_v2.1.md
predecessor_path: docs/authoritative_design/codefabric_continuous_cpg_update_lifecycle_management_specification_v1.3.md
---

# CodeFabric Continuous CPG Update and Lifecycle Specification v2.0
...
The v1.3 predecessor is immutable release history. V2.0 preserves its current-source semantics,
...
invalidation planner joins changed source facts with model relation ownership, semantic
...
- environment, manifest, import, macro, build, policy, model, or compiler-release changes may
...
The model selects each algorithm and its native DataFusion/recursive/function/extension rung.
...
- complete activation pins under one compiler release.
...
workspace, authorization, writer generation, model/compiler/source/provider pins, resource
...
naming the complete model/source/provider/table/policy/proof/compiler set. Current is the unique
...
Recovery runs before new query or mutation work. It rederives source/Git state, command outcomes,
the unique activation head, exact Delta/segment providers, proof/compatibility, and the sealed
epoch. It may discard reconstructible SQLite/in-memory caches. It never guesses, silently rebases,
or labels incomplete work healthy.

### 11.2 Clean reconstruction

A clean rebuild runs with predecessor generated registries/bundles/inputs physically unavailable.
It replays accepted model migrations under the exact compiler release, inventories and captures
current source, reruns the real admitted providers/analyses, reconstructs exact durable state and
activation, and compares complete canonical/public rows, unknowns, diagnostics, capability, and
provenance with incremental state. Digest equality or replaying an accepted wave alone is not the
oracle.
...
swap, cache, reopen, acknowledgement, lease, journal, and process side-effect boundary. A v2.0
