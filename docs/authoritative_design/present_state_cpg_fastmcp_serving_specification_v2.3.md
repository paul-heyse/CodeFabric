...
predecessor_path: docs/authoritative_design/present_state_cpg_fastmcp_serving_specification_v2.2.md
---
...
resources and completion, strict public adapter models, guarded-input/cancellation/progress
translation, public
status/error projection, immutable result resources, and one-logical-response delivery.

The v2.2 predecessor is immutable release history. V2.3 preserves the four-tool MCP catalog, one
...
V2.3 replaces the FastMCP 3 presentation profile with one modern-only FastMCP 4 cell, replaces
ordinary Validate-then-Start with one atomic closed start outcome, maps daemon-authored typed input
requirements through native guarded input, replaces Python lease/handle authority with daemon-minted
public handles, adds authorized reference completion and explicit cancellation, and removes prompts.
...
programming agent
  <-> attach-only AgentStdioLauncher
       <-> one FastMCP 4.0.0 modern-only STDIO process
...
queries, challenges/continuations, public handles, internal leases, completion authority,
capabilities, errors, and all durable mutation. The adapter owns MCP framing, one
...
part of the local v2.3 profile. The only supported MCP protocol era is `2026-07-28`; earlier eras
fail before daemon business dispatch and have no alternate result path.
...
| presentation | strict Pydantic/FastMCP tool, resource, completion, guard, and metadata models | SRV |
...
serialize/log. `FASTMCP_MCP_CAMELCASE_COMPAT=false` is mandatory in every lint, type, test,
inspection, wheel, STDIO, and release gate. Python API fields are snake case while wire aliases
remain protocol-defined. Strict/frozen/extra-forbid Pydantic models and module-scoped adapters are
the presentation schema authority; static adapter schemas are not coequal authority.

The server is created by a pure application factory only after the fd3 launch descriptor is
strictly validated. One lifespan-owned `grpc.aio` channel implements a hidden application-owned
`DaemonPort`; generated Protobuf types terminate there. FastMCP is constructed fail-closed with
`tasks=False` and no FastMCP session, task, auth, cache, provider, transform, proxy, gateway,
App, Tool Search, Code Mode, sampling, root-listing, or custom extension authority.

The v2 package and previously issued field/method allocations are authority. Its undeployed draft
response may be revised once for the closed start/challenge/public-handle/completion/cancel target,
but source, descriptor IR, generated Rust/Python bindings, service/client, census/history policy,
and fixtures must change as one coherent transaction. Generated bindings and descriptors are
derivable caches and MUST regenerate/compare and interoperate. A
...
`StartQuery` authenticates, normalizes, validates, applies freshness and capacity policy, and
returns one closed Protobuf `oneof`: `Accepted | InputRequired | Rejected`. Absence or an unknown
variant fails closed. `Accepted` reserves enforceable coordinator, journal, idempotency, query,
result, and retention capacity and returns daemon-owned accepted identity. `InputRequired` returns
a bounded typed daemon challenge without accepted work. `Rejected` returns public-safe validation
failure without accepted work. Same idempotency key and normalized immutable fields returns the
original outcome; any changed bound field is a typed conflict.
...
or projection through a daemon-minted public handle with transport backpressure.
`ReleaseResource` idempotently releases the authoritative object and stabilizes races with a bounded tombstone.
...
coverage, bounds, authorization, input requirements, and cost class without executing retrieval.
It is pure preparation data: no challenge, acceptance, capacity reservation, task, query, package,
handle, or lease is created.

An input challenge carries stable semantic field IDs, allowlisted input kinds and strict
constraints, safe presentation keys, optional authorized choices, opaque daemon continuation,
expiry, challenge/round identity, remaining rounds, and a public explanation code. The continuation
binds principal, workspace, semantic request and immutable normalized fields, session/daemon
generation, policy/revocation generation, answer shape, expiry, and start idempotency. Tamper,
replay, changed arguments, expiry, excess rounds, or cross-context use creates no work.

`GetReference` also exposes one bounded completion projection for approved reference-template
variables. It accepts prefix/selector/cap and returns only authorized safe candidates plus bounded
`total`/`has_more`; the eventual reference read reauthorizes independently.
...
-> establish semantic_request_id and request-local transport correlation
-> normalize idempotency inputs and compute one closed start outcome
-> when incomplete, issue or verify one bounded daemon continuation and create no accepted work
-> when complete, reserve all accepted-work capacity atomically
-> validate the released request with canonical freshness exactly once
...
The adapter forwards daemon public handles, maps progress, applies host delivery policy, and propagates
cancellation. It does not select or inspect a `FabricEpoch`, compile a plan, or decide semantic
errors, challenge content, completion candidates, resource authority, or acceptance.
...
FastMCP maps `InputRequired` to native `InputRequiredResult`. Each round is a new modern request
that re-enters protocol admission, redaction, tracing, deadline/cancellation projection, and daemon
authorization. FastMCP seals only the opaque daemon token plus safe presentation metadata with a
process-ephemeral `RequestStateSecurity` key and TTL no longer than the daemon challenge. On
re-entry the adapter passes unsealed state and `ctx.input_responses` to the daemon without semantic
interpretation. A valid continuation may accept exactly once.
...
strict result exposes effective semantic and daemon query IDs plus request-local correlation;
execution, availability, completeness,
...
### 7.4 Resources, completion, and absent prompts

The product exposes exactly two bounded resource families: immutable result manifest/page
resources and authorized reference resources. URIs carry only a daemon public handle and bounded
selector/page index; Python stores no handle-to-secret map, lease registry, filesystem path,
package registry, or Arrow decoder.

One FastMCP completion handler serves approved variables of the authorized reference resource
template. It calls the daemon completion projection, applies only the protocol cap, and returns
advisory values with bounded `total`/`has_more`. It never completes tool arguments, result handles,
paths, source/entity inventories, hidden capabilities, or principal data.

The base catalog contains no prompts. Query guidance is a live authorized reference projection and
missing structured input is handled by daemon-authored guarded input. No dormant prompt,
provider, transform, App, UI tool/resource, or custom extension may appear in resolved catalogs.
...
Before acceptance, host cancellation ends the attempt and leaves no query. After acceptance, the
adapter sends idempotent `CancelQuery` once under a separate bounded cleanup budget, awaits the
configured acknowledgement or terminal observation, records only a redacted outcome, and
re-raises host cancellation. Dropping a `WatchQuery` cancels observation only. Adapter/launcher
exit revokes its session
...
```text
codefabric-result://<public-handle>/<bounded-selector>
```

Selectors such as manifest/page/reference are typed fragments or resource parameters, never
filesystem paths. The opaque public handle is distinct from internal package, artifact, lease,
session, challenge, query, or semantic identity and contains no secret lease token. Its daemon
record binds package/resource identity, workspace, owner or sharing class, policy/revocation and
daemon generation, expiry, allowed selectors/ranges, and release state. Handle creation occurs
only after authoritative package/resource/lease state exists.

Public metadata includes only allowed agent/workspace projection, epoch/snapshot, semantic request,
content type/encoding/length/checksum, source-bearing flag, expiry, and subresource identity. It
never exposes lease IDs, storage paths, object-store URLs, tokens, or internal durable coordinates.
Files remain private, immutable, and stored under mode-`0700` directories with mode-`0600` files
...
Every full/range read and release reauthorizes credential, agent/workspace ownership or sharing
class, policy/revocation and daemon generation, expiry, selector/range, release state, and current
source-disclosure policy. Offset/checksum/range are validated before bytes. Cross-agent access is
denied even when the handle is known; a narrowed/revoked ACL denies new bytes. Daemon restart
invalidates stale-generation handles; policy may mint a replacement only from an authorized
retained package.
...
After strict fd3 launch-descriptor validation, a pure factory constructs exactly one FastMCP 4
application, one immutable settings object, one lifespan-owned authenticated daemon channel/port,
the fixed product components, and middleware. Hidden dependency injection supplies the typed
`DaemonPort`; `CurrentContext` is request-local presentation state only. There is no import-time
global server and no global mutable CPG/session/task/cache/resource/catalog object in Python.

Middleware order is explicit, operation-aware, and tested: modern protocol admission, safe
request/correlation span enrichment, bounded deadline/cancellation projection, and public-safe
error redaction. The `2026-07-28` era is checked before business dispatch on every leg. Initialize,
cancellation, completion, and notifications do not enter tool counters or semantic policy. Retry is
limited to proven idempotent transport operations; response caching and background tasks are absent.

CodeFabric registers no `ServerExtension` or UI/App component. On the exact pinned FastMCP 4.0.0
stack, live modern discovery may contain only the inert empty framework-owned
`io.modelcontextprotocol/ui: {}` advertisement. The application extension registry and resolved
UI/App tool, resource, provider, transform, prompt, and handler catalogs remain empty. No policy,
authorization, semantic branch, result shape, or daemon request consumes that advertisement. Any
other identifier, non-empty settings, component registration, or disappearance that changes the
pinned observation is dependency drift and fails closed.
...
Trace identity keeps framework MCP request/leg ID, semantic request ID, daemon query ID, challenge
ID, agent/workspace, epoch/snapshot, package/public-resource identity, and daemon session distinct
and correlated. No public/random MCP-call or RPC-attempt identity is generated. Metrics cover
