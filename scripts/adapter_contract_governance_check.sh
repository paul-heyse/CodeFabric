...
if [[ "$output_count" != "6" ]]; then
  echo "adapter model compiler must own exactly six catalog outputs" >&2
  exit 1
fi
...
for schema in \
  ...
  contracts/adapter/fastmcp-public-meta.schema.json; do
  owner_count="$(jq --arg path "$schema" '[.derivations[] |
    select(.derivation_id == "codefabric.derivation.adapter-models") |
    .outputs[] | select(.path == $path and .output_kind == "adapter-public-schema")] |
    length' "$repository_root/contracts/manifests/suite-manifest.json")"
  if [[ "$owner_count" != "1" || ! -f "$repository_root/$schema" ]]; then
    echo "adapter public schema lacks its sole cataloged compiler owner: $schema" >&2
    exit 1
  fi
done
