...
**Spec anchors.** `present_state_cpg_fact_generation_specification_python_rust_v1.2.md` §2 names exactly these four as its source basis; **§5.1/§5.2 give the authority order per fact family** (mirrored in REFERENCE.md §2) and **§7.1-§7.4 give the per-provider isolation requirements**. Cite specs by **section number, not line number** — the spec filenames and line numbers move, section numbers hold.
