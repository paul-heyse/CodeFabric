...
**Spec anchors.** `present_state_cpg_fact_generation_specification_python_rust_v2.0.md` §2 names the exact provider references; §§4–5 own the fact-family/API authority split and §7 owns provider isolation. Cite the current spec by section number and title, not line number. `docs/spec_index/library-routing.md` maps current provider boundaries to these references; the v1.3 master is historical only.
