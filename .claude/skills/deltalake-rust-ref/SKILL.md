...
description: "Reference navigator for CodeFabric's Delta state layer — Rust `deltalake` 1.0.0 at exact revision `43a0cf10` on DataFusion 55.0.0 and Arrow/Parquet 59.2.0, bound to the data-fabric design constitution. SKILL.md maps three documents at `docs/library_ref/`: `deltalake_rust_1.0.0_43a0cf10_datafusion55_arrow59_advanced_reference_2026-08-23.md` (version/feature baseline, deployment and storage backends, loading/snapshots/time travel, schema and Arrow type mapping, writes, DataFusion reading and the exhaustive integration track, create-table, DML, Change Data Feed, constraints/properties/protocol, layout and pruning, optimize/vacuum/restore; §0 and §2–§13 — there is no §1), `full_data_fabric_design_principles.md` (Principles 1–25 + agent constitution), and `deltalake_1.0.0_43a0cf10_design_principle_alignment_manual_2026-08-26.md` (P1–P25 alignment, 156 utilization patterns in 16 families MOD/STA/SCH/GOV/TXN/WRT/DML/CDF/LAY/MNT/STO/QRY/OBS/INT/EXT/TST, 13 requirement flows, 13 design artifacts, a Delta authority matrix and a release gate). REFERENCE.md (same folder) holds the chapter index with line numbers, a 183-row symbol index, the pattern→section binding matrix, the P1–P25 binding table, decision trees, and operating rules. Use when Rust or Cargo touches `deltalake`, authors `DeltaTable`/`DeltaTableBuilder`/`DeltaOps`/`open_table`/`load_version`/`update_incremental`/`without_files`/`BlindDeltaTable`/`CommitProperties`/`SaveMode`/`SchemaMode`/`DeltaScanConfig`/`scan_cdf`/`OptimizeBuilder`/`VacuumBuilder`/`RestoreBuilder`/`create_checkpoint`/`LogStore`/`FileSelection`, or when a durable table-state outcome must map to principles P1–P25 or MOD-/STA-/TXN-style utilization patterns. Non-Delta DataFusion and Arrow APIs → sibling `datafusion-pyarrow-rust-ref`; fact providers → `code-facts-lib-ref`; graph analytics → `petgraph-ref`."
