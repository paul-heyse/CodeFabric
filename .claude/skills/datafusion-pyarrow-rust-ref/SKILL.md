...
description: "Reference navigator for CodeFabric's Rust data fabric — DataFusion 55.0.0 + Arrow/Parquet 59.2.0 APIs bound to the v2 data-fabric constitution. SKILL.md maps four documents at `docs/library_ref/`: the exact DataFusion 55 and Arrow 59 references, `full_data_fabric_design_principles_v2.md` (staticness test, P1–P36, agent constitution), and the DataFusion/Arrow alignment manual (its P1–P25 capability mapping remains library evidence; v2 P26–P36 apply directly). Use when Rust or Cargo touches `datafusion`/`arrow`/`arrow_*`/`parquet`/`object_store`, authors catalog/session/schema/expression/plan/provider/function APIs, or maps a data-fabric outcome to the v2 principles and MOD-/ARR-/SCH-style utilization patterns. Delta tables → sibling `deltalake-rust-ref`; fact providers → `code-facts-lib-ref`; graph analytics → `petgraph-ref`."
