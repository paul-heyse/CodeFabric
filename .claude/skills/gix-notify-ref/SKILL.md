...
**Cite the lifecycle spec by section number, not line number** — its filename carries `_v1.2` and its line numbers drift, while section numbers have held. Note it has **no Part IV**, so §26-§36 (including every watcher section) sit under Part III; cite sections, never Parts.
