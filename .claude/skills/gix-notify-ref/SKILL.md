...
**Cite the current lifecycle spec by section number and title, not line number** — `codefabric_continuous_cpg_update_lifecycle_management_specification_v2.0.md` owns repository/source truth in §2, event reconciliation in §4, source images/invalidation in §5, and the update pipeline in §6. The v1.3 section/Part map is historical and no longer drives new implementation.
